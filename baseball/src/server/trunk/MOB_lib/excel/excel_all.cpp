...
void Load_Common_CSV()
{
	...
	#define jLOAD_CSV(Table,Name,ns1,ns2) jt_sprintf(buf , _T("%s/../etc/csv/file_%s.csv"),currDir.c_str() ,jS(Table) ); g_##Table.Load(buf);
...
#	define FUNC_print_CSV(Table,Name,ns1,ns2) \
	printf("<< %s >> \n",#Table);\
	for(i=0 ;i<jCSV(Table).size() ; ++i)\
	{\
