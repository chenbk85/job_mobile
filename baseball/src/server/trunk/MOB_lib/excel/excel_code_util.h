...
#define jEXCEL_ENUM_StringToVal(enum_tbl,help,root_ns,ns)\
	...
	inline enum_tbl FromString(std::tstring& sz,enum_tbl eVal){return FromString(sz.c_str(),eVal);}\
	...
	inline bool IsValid(enum_tbl eVal){tcstr sz = ToString(eVal); return jt_strcmp(sz ,jS(NULL)) ;}\
...
#define jEXCEL_STRUCT_FIELD_DebugPrint(eng_name,name,data_type,field_order,getter_type,setter_type,data_type_simple)\
 	ToString(Get_##eng_name(),szBuf);\
	jt_printf(_T(",%s"), szBuf);

//	jLOG(_T("\t") jS(eng_name) _T(" = %s"), szBuf);
...
#define jEXCEL_TABLE_COMMON_CODE_CPP_Sys(sys_table,help,ns1,ns2)\
	...
	if(iArrayMax && (out.size() > (size_t)iArrayMax))\
		{\
		fname_t szBuf;\
		jt_sprintf(szBuf,"%s> " jS(sys_table)  _T(" data= %s , size(%d) > iArrayMax(%d)!!! )"),jFUNC,jT(sz), (int32)out.size() ,(int32)iArrayMax);\
		throw tstring(szBuf);\
}\
	(*v).resize(out.size());\
	try\
