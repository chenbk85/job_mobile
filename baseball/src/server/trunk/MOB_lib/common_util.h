...
// 	}
	...
	inline T get_bit_count(T bit_array)
	{
		...
		const size_t iTotBitCount = sizeof(T)*8;
		for(size_t i=1; i< iTotBitCount; ++i)
		{
	...
	//# --------------------------------------------------------
	//# @brief :현재 주자 상태와 타자의 히트상태로 새로운 주자상태와 추가된 점수를 리턴.
...
// 		if(!(eHitType>eBEGIN_EBatterHitType && eHitType<eEND_EBatterHitType))
// 		{
// 			jWARN("if(!(eHitType>eBEGIN_EBatterHitType && eHitType<eEND_EBatterHitType))");
// 			return ret;
// 		}
		...
		case eEBHT_HOMERUN:
			{
				...
				curr_brc >>= 1; //curr_brc의 하위 3bit에는 치고난 결과 주자상태가 저장 되있다.
