...
#else
#define jWARN(...) GetjILog()->Warning(0,jDEBUG_LINE_INFO "%s>",jFUNC);GetjILog()->Warning(0, __VA_ARGS__);GetjILog()->Warning(0,"\n");
#define jERROR(...) GetjILog()->Warning(0,jDEBUG_LINE_INFO  "%S>",jFUNC); GetjILog()->Error(0,__VA_ARGS__);GetjILog()->Warning(0,"\n");
#endif
...
#define jLOG_DEBUG (...)
#else 
#define jLOG_DEBUG jLOG //디버깅 모드에서만 사용.
#endif
...
#define _BF(X) endl<< boost::format(_T(X))
#define __BF(X) boost::format(_T(X))
