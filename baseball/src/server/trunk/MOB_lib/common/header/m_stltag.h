...
jLIST_TYPEDEF_map(string,jUser,m_UserList);

는 아래와 같은 의미입니다.

typedef std::map<string,jUser> m_UserList_t;
typedef m_UserList_t::value_type m_UserList_vt;
m_UserList_t m_UserList;
...
#define jLIST_TYPEDEF_map_header(key,val,var) typedef val var##_value_t; typedef std::map<key,val> 	\
	...
	typedef var##_t::value_type var##_vt; typedef std::pair<var##_it, bool>	var##_pt;
#define jLIST_TYPEDEF_map_typedef(key,val,var) \
	typedef val var##_value_t; \
	typedef std::map<key,val> 	var##_t; \
	typedef var##_t::iterator var##_it;\
	typedef var##_t::value_type var##_vt; \
	typedef std::pair<var##_it, bool>	var##_pt;
...
#define jLIST_for2(X,it,ie) for( X##_it it = X.begin(), ie = X.end();  it!=ie ; ++it)
	...
	inline acstr get_string_ptr(acstr sz){ return sz;}
	inline acstr get_string_ptr(astring sz){ return sz.c_str() ;}
	inline acstr get_string_ptr(wcstr sz){ return "get_string_ptr error";}
	inline acstr get_string_ptr(wstring sz){ return "get_string_ptr error2";}

	struct scbconvert
	{
		tstring szA;
		scbconvert(acstr z):szA(z){}
		scbconvert(astring z)
		{
			szA=z;
		}
		tcstr getA()
