...
{\
	...
	LOG_WARNING( _T("jvRETURN_ERROR(") #__ERROR_NAME " : " _T(#exp) _T(") : user db_id=")  << pUser->db_id() );\
	jMAKE_MSG_BS2X(sendMsg,sd,ans_bs_error);\ sd->set_sys_error_id(jMAKE_ERR(__ERROR_NAME));\
...
{\
	/*jWARN( _T("jvRETURN_ERROR(") #__ERROR_NAME " : " _T(#exp) _T(") : user=%lld") ,pUser->db_id() ) ;*/\
	LOG_WARNING( _T("jvRETURN_ERROR(") #__ERROR_NAME " : " _T(#exp) _T(") : user db_id=")  << pUser->db_id() );\
...
#define jRETURN_ERROR_ROOM(exp , __ERROR_NAME, ...) if(!(exp)) \
{\
	/*jWARN( _T("jvRETURN_ERROR(") #__ERROR_NAME " : " _T(#exp) _T(") : room=%lld") ,pRoom->id() ) ;*/\
	LOG_WARNING( _T("jvRETURN_ERROR(") #__ERROR_NAME " : " _T(#exp) _T(") : room=")  << pRoom->id() );\
	jMAKE_MSG_BS2X(sendMsg,sd,ans_bs_error);\
	sd->set_sys_error_id(jMAKE_ERR(__ERROR_NAME));\
