...
namespace nMech {
	...
	EQuestConditionType Sys_Quest::Get_condition_type3()	{		return (EQuestConditionType)m_condition_type3;}
	void Sys_Quest::Set_condition_type3(EQuestConditionType v)
	{
		if(v<eBEGIN_EQuestConditionType || v>=eEND_EQuestConditionType)
		{
			jERROR(_T("Sys_Quest::Set_condition_type3(enum minmax(%d,%d)error, val=%d")
				,eBEGIN_EQuestConditionType,eEND_EQuestConditionType,v);
		}
		m_condition_type3=v;
	}

	acstr Sys_Quest::Get_condition3()	{		return (acstr)m_condition3;}
	void Sys_Quest::Set_condition3(acstr v)
	{
		if(!v){ throw _T("Sys_Quest::Set_condition3( val = NULL )");}
		nExcelUtil::jSetter(m_condition3 , v, 1024);
	}

	int32 Sys_Quest::Get_condition_value3()	{		return (int32)m_condition_value3;}
	void Sys_Quest::Set_condition_value3(int32 v)
	{
		m_condition_value3=v;
	}
...
tcstr Sys_Quest::ReadCSV(IN size_t iRecord,IN EXCEL_CSV_STRING_VECTOR &out,IN OUT size_t& index,IN bool isCheck)
{
	...
	Set_condition_type3(FromString(out[index++],m_condition_type3) );
	Set_condition3(nExcelUtil_StringConverter(out[index++]).getA());
	Set_condition_value3(FromString(out[index++],m_condition_value3) );
