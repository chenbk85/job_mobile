...
namespace nMech {
	...
	acstr Sys_Error::Get_name()	{		return (acstr)m_name;}
	void Sys_Error::Set_name(acstr v)
	{
		if(!v){ throw _T("Sys_Error::Set_name( val = NULL )");}
		nExcelUtil::jSetter(m_name , v, 256);
	}

	acstr Sys_Error::Get_help()	{		return (acstr)m_help;}
	void Sys_Error::Set_help(acstr v)
	{
