...
#define for_each_nEXCEL_excel_enum_ENUM(ENUM_TABLE)\
	...
	#define for_each_nEXCEL_EQuestConditionType_ENUM_FIELD(ENUM_FIELD)\
		...
		ENUM_FIELD(QCT_SINGLEGAMEWINCOUNT			,2,싱글경기승수조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_MATCHGAMEWINCOUNT			,3,대전경기승수조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_FRIENDGAMEWINCOUNT			,4,친구경기승수조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_SINGELGAMECOUNT			,5,싱글경기횟수조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_MATCHGAMECOUNT			,6,대전경기횟수조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_FRIENDGAMECOUNT			,7,친구경기횟수조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_PLAYERLEVELCOUNT			,8,레벨달성조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_PLAYERCHARACTERCOUNT			,9,캐릭터생성갯수,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_MATCHHOMERUNCOUNT			,10,대전홈런횟수조건,nMech,nEXCEL,EQuestConditionType)\
		ENUM_FIELD(QCT_MATCHSTRIKEOUTCOUNT			,11,대전탈삼진횟수조건,nMech,nEXCEL,EQuestConditionType)\
		...
		ENUM_FIELD(QCT_MANUAL_FINISH			,16,임의완료,nMech,nEXCEL,EQuestConditionType)\
...
namespace nMech { namespace nEXCEL 
{
	...
	enum EQuestConditionType // 설명없음
	{
		...
		/* 없음 */
		eQCT_NULL = 1, // 없음
		/* Sys_Quest의 condition_value에 있는 값만큼 이겼다면 퀘스트 보상 */
		eQCT_SINGLEGAMEWINCOUNT = 2, // 싱글경기승수조건
		/* 대전경기에서 condition_value만큼이겼다면 보상 */
		eQCT_MATCHGAMEWINCOUNT = 3, // 대전경기승수조건
		/* 등록된 친구랑 경기후 condition_value만큼 이겼다면 보상 */
		eQCT_FRIENDGAMEWINCOUNT = 4, // 친구경기승수조건
		/* 싱글경기횟수가 value만큼 되었을때 보상 */
		eQCT_SINGELGAMECOUNT = 5, // 싱글경기횟수조건
		/* 대전경기횟수가 value만큼 되었을때 보상 */
		eQCT_MATCHGAMECOUNT = 6, // 대전경기횟수조건
		/* 등록된 친구와의 경기횟수가 value만큼 되었을때 보상 */
		eQCT_FRIENDGAMECOUNT = 7, // 친구경기횟수조건
		/* 레벨달성조건 */
		eQCT_PLAYERLEVELCOUNT = 8, // 레벨달성조건
		/* 캐릭터생성갯수 */
		eQCT_PLAYERCHARACTERCOUNT = 9, // 캐릭터생성갯수
		/* 대전홈런횟수조건 */
		eQCT_MATCHHOMERUNCOUNT = 10, // 대전홈런횟수조건
		/* 대전탈삼진횟수조건 */
		eQCT_MATCHSTRIKEOUTCOUNT = 11, // 대전탈삼진횟수조건
