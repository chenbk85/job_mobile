...
{
	EItemGradeType get_bonus_item_grade(level_t level);
	level_t get_player_level(exp_t exp);
	int get_bonus_item_count( jTE* peGAME_RESULT,cstr category );
	void calc_bonus_item_count(IN bool isWin, OUT int& stuff_count , OUT int& consume_count);
