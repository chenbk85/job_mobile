...
namespace nBASE_BALL
{
	struct jUser_GameRoomInfo
	{
		...
		void get_score(OUT score_list_t& score)const;  //현제 스코어.

		void print_score();
		void reset_user_info();//경기재시작시 플레이데이타 초기화.
	
		void delete_team_and_reset_team_info(s_tbl_team* pTeam,s_tbl_user* user);// 팀삭제시 플레이어및 유저정보세팅변경.
...
#	define jLAMDA_find_EUserGPS(X,Z) Z(EUserGPS , eVal)
	...
	inline void jUser_GameRoomInfo::reset_user_info()//경기재시작시 플레이데이타 초기화.
	{
	...
	inline void jUser_GameRoomInfo::get_score(OUT jUser_GameRoomInfo::score_list_t& score)const  //현제 스코어.
	{
