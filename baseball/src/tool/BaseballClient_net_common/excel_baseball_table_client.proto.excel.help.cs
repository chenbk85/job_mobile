...
namespace nEXCEL
{
	public class excel_baseball_table_client_proto_helper
	{
			//------------------------------------------------
			//	s_sys_client_graphic_file_name
			//------------------------------------------------
			public static byte[] Serialize(s_sys_client_graphic_file_name m) 
			{
				byte[] b = null;
			...
			public static void Deserialize(ref s_sys_client_graphic_file_name m, byte[] serializationBytes)
			{
				using (var ms = new MemoryStream(serializationBytes))
				{
					BaseballClient_proto_Serializer psz = new BaseballClient_proto_Serializer();
					m = psz.Deserialize(ms, null, typeof(s_sys_client_graphic_file_name)) as s_sys_client_graphic_file_name;
				}
			}
			public static void Decoding_FromSocketData(ref s_sys_client_graphic_file_name m, byte[] socketData)
			{
				int iSize = 0;
	...
	}//class excel_baseball_table_client_proto_helper
}// namespace nEXCEL
