...
namespace nEXCEL
{
	public class excel_baseball_table_game_proto_helper
	{
			//------------------------------------------------
			//	s_sys_runner_state
			//------------------------------------------------
			public static byte[] Serialize(s_sys_runner_state m) 
			{
				byte[] b = null;
			...
			public static void Deserialize(ref s_sys_runner_state m, byte[] serializationBytes)
			{
				using (var ms = new MemoryStream(serializationBytes))
				{
					BaseballClient_proto_Serializer psz = new BaseballClient_proto_Serializer();
					m = psz.Deserialize(ms, null, typeof(s_sys_runner_state)) as s_sys_runner_state;
				}
			}
			public static void Decoding_FromSocketData(ref s_sys_runner_state m, byte[] socketData)
			{
				int iSize = 0;
	...
	}//class excel_baseball_table_game_proto_helper
}// namespace nEXCEL
