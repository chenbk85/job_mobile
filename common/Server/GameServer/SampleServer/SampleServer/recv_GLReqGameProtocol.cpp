...
jMOB_RECV_PACKET(SAMPLE_Plugin_Connection,req_game_test_packet) //pCon , pRecvData
{
	jRETURN(pRecvData);
...
jMOB_RECV_PACKET_END(SAMPLE_Plugin_Connection,req_game_test_packet);
...
jMOB_RECV_PACKET(SAMPLE_Plugin_Connection,req_asdf) //pCon , pRecvData
{
	jRETURN(pRecvData);
	int test_int = pRecvData->test_int();
	string test_string = pRecvData->test_string();
