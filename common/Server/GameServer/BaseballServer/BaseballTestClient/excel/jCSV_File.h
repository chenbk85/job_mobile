...
namespace nMech
{
	namespace nUtil
	{
		inline  bool ReadLineFromFile(TCHAR* buf, int buf_size, FILE* pF)
		{
...
/*
			...
			*/
			...
			bool Load(tcstr _szFile)
			{
				fs::path p = _szFile;
				tstring sFile = jT(p.make_preferred().string());
				...
				try
				{
					...
					else
					{
						...
						if(!fp) 
						{
							...
							if(jt_strstr(szFile,jS(file_Sys__) ))
							{
								jLOG(_T("file load skipped: %s"), szFile);
								return true;
							}
							if(jt_strstr(szFile,jS(file_Sys_Base_) ))
							{
								jLOG(_T("file load skipped: %s"), szFile);
								return true;
							}
