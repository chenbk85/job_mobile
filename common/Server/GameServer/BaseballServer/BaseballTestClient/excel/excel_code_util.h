...
#include "excel/ToString_baseball.h"
...
#define ENUM_FROMSTRING12(ENUM_NAME,VAL,HELP,root_ns,ns,ENUM_TABLE)	\
	if(jt_strcmp(jS(ENUM_NAME),sz)==0 || jt_strcmp(jS(e##ENUM_NAME),sz)==0) return root_ns::ns::e##ENUM_NAME;
	...
	namespace nEXCEL
	{
		...
		struct Excel_PostLoader
		{
			...
			void Run()
			{
				...
				{
					jERROR(_T("load fail(%s) : %s"),jT(it->help) ,e.c_str() );
				}
				catch (...)
				{
					jERROR(_T("load fail unkown (%s) , data=%s"),it->help.c_str() ,jT(it->data));
				}
			}
