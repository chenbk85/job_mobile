...
void Load_Common_CSV()
{
	...
	//astring currDir = boost::filesystem::current_path().string();
	#define jLOAD_CSV(Table,Name,ns1,ns2) jt_sprintf(buf , _T("csv/baseball/file_%s.csv"),jS(Table) ); g_##Table.Load(buf);
