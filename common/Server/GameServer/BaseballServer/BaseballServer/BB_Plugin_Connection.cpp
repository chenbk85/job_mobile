...
//#--------------------------------------------------------------------------
void BB_Plugin_Connection::MOB_OnConnect(Plugin_INetIOModel* p) 
//#--------------------------------------------------------------------------
// BB_Plugin_Connection에 대한 초기화를 해주자.
{
	jASSERT0(string("BB_Plugin_NetIOModel") == p->MOB_GetRTTI());
	static int s_i=0;
	m_gid = s_i++;
}

//--------------------------------------------------------------------------
void BB_Plugin_Connection::MOB_OnDisconnect(acstr szErrorReason,int iErrorReason)
//--------------------------------------------------------------------------
// BB_Plugin_Connection에 대한 메모리 해제를 해주자.
{
	printf("%s\n",szErrorReason);
...
//#--------------------------------------------------------------------------
void BB_Plugin_Connection::OnReadPacket_Unkown(GReqProtocol *p)
//#--------------------------------------------------------------------------
{
...
//#--------------------------------------------------------------------------
void BB_Plugin_Connection::OnReadPacket_GReqGameEnter(GReqGameEnter* p)
//#--------------------------------------------------------------------------
{
	m_MyInfo.CopyFrom(*p);
	
	BB_Room* pRoom = GetParent()->Room_Find(Get_room_id());
...
//--------------------------------------------------------------------------
void BB_Plugin_Connection::OnReadPacket_GReqGameLeave(GReqGameLeave* pRecvPacket)
//--------------------------------------------------------------------------
{
