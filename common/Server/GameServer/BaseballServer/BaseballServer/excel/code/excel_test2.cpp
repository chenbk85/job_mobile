...
namespace nMech {
	...
	int16 Sys_T_Test4::Get_maxbuild2()	{		return (int16)m_maxbuild2_i16;}
	...
	EBuildTypeTest Sys_T_Test4::Get_enum_val()	{		return (EBuildTypeTest)m_enum_val_e;}
	...
	int16 Sys_T_Test4::Get_populationreturn4()	{		return (int16)m_populationreturn4_i16;}
	void Sys_T_Test4::Set_populationreturn4(int16 v)
	{
		m_populationreturn4_i16=v;
	}

	EBuildTypeTest Sys_T_Test4::Get_test4()	{		return (EBuildTypeTest)m_test4_e;}
	...
	int16 Sys_T_Test4::Get_getgold4()	{		return (int16)m_getgold4_i16;}
	...
	jItemObj1_ref Sys_T_Test4::Get_itemObj1()	{		return (jItemObj1_ref)m_itemObj1_o;}
	void Sys_T_Test4::Set_itemObj1(jItemObj1_ref v)
	{
		m_itemObj1_o=v;
	}

	int32 Sys_T_Test4::Get_val_int()	{		return (int32)m_val_int_i32;}
...
OUT nMech::nUtil::jCSV_File<Sys_T_Test4> g_Sys_T_Test4;
...
tcstr Sys_T_Test4::ReadCSV(IN size_t iRecord,IN EXCEL_CSV_STRING_VECTOR &out,IN OUT size_t& index,IN bool isCheck)
{
	...
	jt_sprintf(szHelp, _T("loading(iRecord=%u) file(excel_test2)Sys_T_Test4::sys_test3_ref"),iRecord);
	Excel_PostLoader_registe(&m_sys_test3_ref_ptr, out[index++].c_str()  , szHelp );
	jt_sprintf(szHelp, _T("loading(iRecord=%u) file(excel_test2)Sys_T_Test4::sys_test2_arrary"),iRecord);
	Excel_PostLoader_registe(&m_sys_test2_arrary_ptr_vec , out[index++].c_str() ,0 , szHelp ) ;
	Set_enum_val(FromString(&m_enum_val_e,out[index++]) );
	Set_populationreturn4(FromString(&m_populationreturn4_i16,out[index++]) );
	Set_test4(FromString(&m_test4_e,out[index++]) );
	Set_getgold4(FromString(&m_getgold4_i16,out[index++]) );
	jt_sprintf(szHelp, _T("loading(iRecord=%u) file(excel_test2)Sys_T_Test4::test3_array_id"),iRecord);
	Excel_PostLoader_registe(&m_test3_array_id_ptr_vec , out[index++].c_str() ,3 , szHelp ) ;
	jt_sprintf(szHelp, _T("loading(iRecord=%u) file(excel_test2)Sys_T_Test4::field_array"),iRecord);
	Excel_PostLoader_registe_int32_vec(&m_field_array_vec, out[index++].c_str(), 3 , szHelp );
	jt_sprintf(szHelp, _T("loading(iRecord=%u) file(excel_test2)Sys_T_Test4::field_vector"),iRecord);
	Excel_PostLoader_registe_int32_vec(&m_field_vector_vec, out[index++].c_str(), 0 , szHelp );
	Set_itemObj1(FromString(&m_itemObj1_o,out[index++]) );
	Set_val_int(FromString(&m_val_int_i32,out[index++]) );
	return szKEY;
}
...
{	tname1024_t szBuf;
	...
	typedef nMech::nUtil::jCSV_File<nMech::nEXCEL::Sys_T_Test4> Sys_T_Test4_csv_file_t;
	DECLARE_INSTANCE_TYPE(Sys_T_Test4_csv_file_t);

	namespace nMech { namespace nEXCEL
	{
		jSQ_REGIST_BIND(nMech_nEXCEL_excel_test2)
		{
			jSQ_Interface(Sys_T_Test4_csv_file_t)
			...
			jSQ_end();
			jSQ_g_var(&g_Sys_T_Test4,g_Sys_T_Test4);
