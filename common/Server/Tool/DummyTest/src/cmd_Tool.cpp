...
namespace nMOB
{
	...
	jCONSOLE_CMD_AUTO(con,unload_net,0,"네트웍 모듈DLL 언로딩","<net.dll>")
	{
		tstring sDLL = arg[0];
		...
		if(it !=g_mapDLL.end())
		{
			...
			it->second.Clear();
			SAFE_DELETE(it->second.m_pConsoleDLL);
			g_mapDLL.erase(it);
		}
		return true;
	}
	...
	jCONSOLE_CMD_AUTO(con,load_net,0,"네트웍 모듈DLL 로딩","<net.dll> <netconfig_node_name> [netconfig_xmlfile]")
	{
		...
		jIVar* g_pvNetConfigXml=jCV(exe,NetConfigXml,_T("xml\\net\\RS_Server_NetConfig.xml"), 0, "(xml\\net\\RS_Server_NetConfig.xml)네트웍 객체정보를 참조할 xml");

		if(arg.size()==3)
		{
			g_pvNetConfigXml->Set_cstr(arg[2].c_str());
		}
		jRETURN_ret(false,g_pvNetConfigXml);
	...
	void Console_window_hide()
	{
		if(!g_isShowedCon) return;

		if(g_pjIConsoleDlg)g_pjIConsoleDlg->console_SaveWinPos();
		g_isShowedCon=false;
		FreeConsole();
	}
	void Console_window_show()
	{
		if(g_isShowedCon) return;
		g_isShowedCon=true;
		AllocConsole();
		...
		if(g_pjIConsoleDlg )
			...
			g_pjIConsoleDlg ->console_LoadWinPos(jS(dummy_test));
	}
	void ConsoleDlg_Show(HINSTANCE hInstance,HWND hWnd )
	{
		...
		bool ConsoleDlg_Close();
		if(!g_pjIConsoleDlg) return;
		g_pjIConsoleDlg->Set_Finalizer(ConsoleDlg_Close);
		g_is_show_cmd_dlg=true;
		g_pjIConsoleDlg->Run_WTL_Dlg(hInstance, SW_NORMAL,g_ExeEnv.m_szAppNic,hWnd,false);
	...
	int ConsoleDlg_Init()
	{
		...
		void set_logfile_name();
		set_logfile_name();
		jMech_Init(true,false);//bool isMulti = true , bool debugException=false
		...
		if(pConDlg)
			...
			hWnd  = pConDlg->console_LoadWinPos(jS(dummy_test));
	...
	jCONSOLE_CMD_AUTO(con,toggle_console,0,"콘솔창 토글","")
	{
		if(g_isShowedCon)
		{
			Console_window_hide();
