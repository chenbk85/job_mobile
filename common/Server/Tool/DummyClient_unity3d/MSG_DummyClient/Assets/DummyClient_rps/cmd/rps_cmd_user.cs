...
public class rps_cmd_user : CmdEvent_base
{
	...
	public void login(nNWM.nConsole.jConsoleArg arg)
	{
		...
		if (plugin == null)
		{
			plugin = new nNWM.nRPS.NetEventPlugin_rps(user_id_t.Parse(arg[eUID]), m_CMD, m_MSG);
	...
	void x2ug_echo(nNWM.nConsole.jConsoleArg arg)
	{
		...
		if (plugin == null) { m_CMD.LogWarning("user not found"); return; }
		//-------------------------------------------------------------------
		var sd = plugin.Make(nProtoUGrps.Client_UserGateServer.Type.x2ug_echo);
		sd.m_x2ug_echo = new nProtoUGrps.s_x2ug_echo
		{
			type = 0,
	...
	void x2gs_echo(nNWM.nConsole.jConsoleArg arg)
	{
		...
		var sd = plugin.Make(nProtoGSrps.Client_GameServer.Type.x2gs_echo);
		sd.m_x2gs_echo = new nProtoGSrps.s_x2gs_echo
		{
			type = 0,
			msg = (arg.Get_string(eMSG) + s_iSerial++),
		};
	...
	void rps(nNWM.nConsole.jConsoleArg arg)
	{
		...
		sd.m_rps = new nProtoGSrps.s_rps
		{
			uid = plugin.db_id(),
