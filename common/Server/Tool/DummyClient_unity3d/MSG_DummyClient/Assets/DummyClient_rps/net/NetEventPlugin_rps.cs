...
namespace nNWM
{
	namespace nRPS
	{
		public class NetEventPlugin_rps : nNWM.nDummy.NetEventPlugin
		{
			public User_rps m_User;

			public NetEventPlugin_rps(user_id_t pp_id, CmdEventManager gui, NetEventManager msg)
			{
				m_User = new User_rps(this);
			...
			~NetEventPlugin_rps()
			{
				NetLog("NetEventPlugin_rps detroy :" + m_UserID);
			}
			...
			// send용 함수.
			//#--------------------------------------------------------------------------
			public nProtoUGrps.Client_UserGateServer Make(nProtoUGrps.Client_UserGateServer.Type eType) { return new nProtoUGrps.Client_UserGateServer { type = eType }; }
			public bool send_to_ug(nProtoUGrps.Client_UserGateServer sd)
			{
			...
			public nProtoGSrps.Client_GameServer Make(nProtoGSrps.Client_GameServer.Type eType) { return new nProtoGSrps.Client_GameServer { type = eType }; }
			public bool send_to_gs(nProtoGSrps.Client_GameServer sd)
			{
				NetLog("[Send:gs]" + nNWM.nUtil.jDumper.NamedDump(sd.type, m_MSG.m_PacketLogType), true);
				var sendData = new MSG.ReqGameProtocol(Serialize<nProtoGSrps.Client_GameServer>(sd));
			...
			public override void OnRecv_FromMSG(MSG.AnsAutoJoin rd)  /**  게임 자동 입장에 대한 응답 메시지 */
			{
				...
				m_User.Set_GameRoomInfo( Deserialize<nProtoGLrps.GameRoomInfo>(rd.GameRoomInfo) );
			...
			{
				/* 샘플 코드
				var rd = Deserialize<nProtoGSrps.GameServer_Client>(recvByte);
		...
		}//public class NetEventPlugin_rps : nDummy.NetEventPlugin
