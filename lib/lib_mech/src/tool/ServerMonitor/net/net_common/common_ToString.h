...
namespace nMech
{
	inline void jDebugPrint(tcstr szName, EUserGPS msg)	{		GetjILog()->LogPlus(_T("%s:%s " ) , szName,ToString(msg));	}
	inline void jDebugPrint(tcstr szName, std::vector<EUserGPS>& msg)	{		GetjILog()->LogPlus(_T("%s.size()=%d ") , szName,msg.size());	}
