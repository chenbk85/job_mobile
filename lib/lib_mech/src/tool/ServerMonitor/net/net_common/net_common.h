...
#pragma comment(lib, "net_common.lib")
#endif
...
#ifndef _DEBUG
#define jNOT_USE_DebugPrint_MonitorServer
#endif
...
// 서버간 공유 시작 해더
//--------------------------------------------------------------------------
#define PT_MonitorServer_LIB_API AQ_SERVER_LIB_API
#define jxDECL_MonitorServer jxDECL_NET
#define jxDEFINE_MonitorServer jxDEFINE_NET
...
extern AQ_SERVER_LIB_API jIE* g_peServerExeList;

template <class _Predicate>
void for_each_ServerExeList(_Predicate &__pred ,bool isOnlyRoot = true)  
{
	...
	jrFOR(it)
	{
		...
		else
		{
			...
			jrFOR(it2)
			{
				jIVar* pV = it2->GetAttrVar(jS(ENABLE));
				if(pV && pV->Get_bool()==false) continue;
				__pred(it2);
			}
		}
...
inline tcstr ToServerTypeHelp(const tstring& s){return ToServerTypeHelp(s.c_str());}

#define jCHECK_ServerType(sz) jCHECK(CheckSeverType(sz));
