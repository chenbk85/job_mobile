...
public:
	typedef function<void (CTreeListCtrl*, const any  &lParam, const any &wParam)> Event_t;
private:
#pragma warning(disable : 4251)
	jLIST_TYPEDEF_map(StringID , Event_t, m_mapEvent);
#pragma warning(default : 4251)
public:
	void SetEvent(StringID id , const Event_t &e){ m_mapEvent[id]=e;}
	Event_t *GetEvent(StringID id)
	{
		m_mapEvent_it it = m_mapEvent.find(id);
		if(it==m_mapEvent.end()) return 0;
		return &it->second;
	}
