...
class jHandler_priority_queue
{
public:
  void add(int priority, boost::function<void()> function)
  {
    handlers_.push(_jQueued_handler(priority, function));
  }
  ...
  void execute_all()
  {
    ...
    {
      _jQueued_handler handler = handlers_.top();
      handler.execute();
      handlers_.pop();
    }
  }
  ...
  {
  public:
    wrapped_handler(jHandler_priority_queue& q, int p, Handler h)
  ...
  //private:
    jHandler_priority_queue& queue_;
    int priority_;
    Handler handler_;
  };
...
private:
  class _jQueued_handler
  {
  public:
    _jQueued_handler(int p, boost::function<void()> f)
    ...
    friend bool operator<(const _jQueued_handler& a,
        const _jQueued_handler& b)
    {
  ...
  std::priority_queue<_jQueued_handler> handlers_;
};

// Custom invocation hook for wrapped handlers.
template <typename Function, typename Handler>
void asio_handler_invoke(Function f,
    jHandler_priority_queue::wrapped_handler<Handler>* h)
{
...
int main()
{
  ...
  jHandler_priority_queue pri_queue;

  // Post a completion handler to be run immediately.
  io_service.post(pri_queue.wrap(0, Row_priority_handler));
