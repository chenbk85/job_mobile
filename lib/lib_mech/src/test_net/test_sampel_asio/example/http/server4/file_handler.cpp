...
jFile_handler::jFile_handler(const std::string& doc_root)
  : doc_root_(doc_root)
{
}

void jFile_handler::operator()(const jRequest& req, jReply& rep)
{
  // Decode url to path.
...
bool jFile_handler::url_decode(const std::string& in, std::string& out)
{
  out.clear();
  out.reserve(in.size());
