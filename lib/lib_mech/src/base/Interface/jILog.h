...
namespace nMech
{
	namespace nLOG
	{
		...
		*/
		...
		{
			eLOG_OFF=jxe(LOG_OFF),
			eLOG_LEVEL=jxe(LOG_LEVEL), // 해당 LEVEL에 해당하는 로깅이 가능하게 한다. 
			// 하부 로깅logName이 OFF이면 로깅안됨
			eLOG_ALWAYS=jxe(LOG_ALWAYS),// 해당 LEVEL에 해당하는 모든 로깅을 강제 출력.
		};
...
#define jINTERFACE_jILog(x) public:	\
	...
	virtual nXML_ORB::jIORB* GetConsoleToolORB() ##x\
	virtual void			SetReportEvent(tcstr szSourceName,struct jIReportEvent*pRE) ##x\
	virtual struct jIReportEvent*	GetReportEvent(tcstr szSourceName) ##x\
