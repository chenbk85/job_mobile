#ifndef _jILua_header____1234dkskljow_
#define _jILua_header____1234dkskljow_
...
namespace nMech
{
	...
	namespace nLUA
	{
		...
		typedef void* THIS_PTR ;
		typedef int HTAG ;
		typedef ULONG_PTR			USER_DATA ;//## AMD Port
		...
		struct jILuaDebugSink;

		////////////////////////////////////////////////////////////////////////////
		////////////////////////////////////////////////////////////////////////////
		/*! jILua callback interface
		...
		*/
		...
		//   System.
		struct _jISystem_PostInit_Class
		{
			virtual void PostInit(ISystem*)=0;
		};

		struct jILuaSink;
		typedef void jILuaSink_OnCollectUserData_t(jILuaSink*,INT_PTR nValue,int nCookie) ;
		typedef void jILuaSink_Update_t( jILuaSink* pSink,bool bNoLuaGC );
		...
		struct jILuaSink
		{
			...
			*/
			...
			virtual void PostInit(ISystem*)=0;
			virtual void SetCollectUserDataFunc(jILuaSink_OnCollectUserData_t*)=0;
			virtual void SetLuaSink_UpdateFunc(jILuaSink_Update_t*)=0;
			virtual void RegistPostInit_Class(_jISystem_PostInit_Class*)=0;
			virtual float& GetLastGCTime()=0;
			virtual int& GetLastGCCount()=0;
			virtual float& GetGCFreq() =0;
			virtual void Update( bool bNoLuaGC=false )=0;
		...
		*/
		struct jILua
		{
			...
			*/
			...
			//!debug functions
			//##@{
			virtual void EnableDebugger(//jILuaDebugSink 
				jIVar*pDebugSink) = 0;
			...
			*/
			...
			//##@}
			virtual void GetMemoryStatistics(jcISizer *pSizer) = 0;
		...
		struct IScriptObject
		{
			...
			*/
			...
			@param pSink pointer to an object that implement jILuaObjectSink

			NOTE: if the parent is registered the script object will notify when is deleted
			*/
			virtual void RegisterParent(jILuaObjectSink *pSink) = 0;
		...
		public:
			...
			explicit _SmartScriptObject(jILua *pSS,IScriptObject *p)
			{
				m_pSO=pSS->CreateEmptyObject();
				m_pSO->Attach(p);
			}
			explicit _SmartScriptObject(jILua *pSS,bool bCreateEmpty=false)
			{
				if(!bCreateEmpty)
				{
	...
	{
		extern JBASE_API nLUA::jILua* g_pjILua;
	}
	...
	namespace nLUA
	{
		...
		public:
			...
			_HScriptFunction(jILua *){m_hFunc=0;}
			_HScriptFunction(jILua *,HSCRIPTFUNCTION hFunc){m_hFunc=0;}
			~_HScriptFunction(){ if(m_hFunc)nInterface::g_pjILua->ReleaseFunc(m_hFunc);m_hFunc=0; }
			void Init(jILua *,HSCRIPTFUNCTION hFunc){if(m_hFunc)nInterface::g_pjILua->ReleaseFunc(m_hFunc);m_hFunc=hFunc;}
			operator HSCRIPTFUNCTION() const
			{
				return m_hFunc;
			}
			_HScriptFunction& operator =(HSCRIPTFUNCTION f){
				if(m_hFunc)nInterface::g_pjILua->ReleaseFunc(m_hFunc);
				m_hFunc=f;
				return *this;
			}
		private:
		...
		enum ELuaSinkDataType
		{
			...
			USER_DATA_jIA,
			USER_DATA_jGuiStage,
			USER_DATA_jGuiDialog,
			USER_DATA_jGuiControl,
			ELuaSinkDataType_MAX
		};
...
#endif //_jILua_header____1234dkskljow_
