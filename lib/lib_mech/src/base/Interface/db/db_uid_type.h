...
inline cstr ToString(IN db_uid_type dbid,OUT fname_t sz)
{
	return ToString(dbid.m_db_id,sz);
}
...
{
	jt_sprintf_s(sz, sizeof(aname256_t),_T(jPRINTF_SYSTEMTIME),jSPREED_SYSTEMTIME(t));
	return sz;
}
