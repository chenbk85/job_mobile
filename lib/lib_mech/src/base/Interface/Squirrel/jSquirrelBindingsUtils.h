...
#define _BEGIN_CLASS(classname)  \
		...
		{ \
			nMech::GetjISQ()->sq_pushstring(v,_T(#classname),-1); \
			return 1; \
		} \
...
#define _INIT_STATIC_NAMESPACE(classname) GetjISQ()->BindUtil_CreateStaticNamespace(GetjISQ()->GetVMPtr(),&__##classname##_decl);
#define _INIT_CLASS(classname)CreateClass(GetjISQ()->GetVMPtr(),&__##classname##_decl);
...
#define _CHECK_SELF(cppclass,scriptclass) \
	...
	if(SQ_FAILED(sq_getinstanceup(v,1,(SQUserPointer*)&self,(SQUserPointer)&__##scriptclass##_decl))) { \
		return GetjISQ()->sq_throwerror(v,_T("invalid instance type"));\
	}
...
{ \
	...
	*newquat = quat; \
	if(!GetjISQ()->BindUtil_CreateNativeClassInstance(GetjISQ()->GetVMPtr(),_T(#classname),newquat,classname##_release_hook)) { \
		delete newquat; \
		return FALSE; \
	} \
	return TRUE; \
...
{ \
	...
	if(push_##classname(quat)) { \
		...
		nMech::GetjISQ()->sq_pop(GetjISQ()->GetVMPtr(),1); \
	} \
	return ret; \
} \
int construct_##classname(cppclass *p) \
{ \
	nMech::GetjISQ()->sq_setinstanceup(GetjISQ()->GetVMPtr(),1,p); \
	nMech::GetjISQ()->sq_setreleasehook(GetjISQ()->GetVMPtr(),1,classname##_release_hook); \
	return 1; \
}
...
BOOL GetjISQ()->BindUtil_CreateNativeClassInstance(HSQUIRRELVM v,const SQChar *classname,SQUserPointer ud,SQRELEASEHOOK hook);
*/
