...
/* *_* by icandoit : 2008-04-09 15:03:36
#include "squirrel.h"

#include "SquirrelObject.h"
#include "SquirrelVM.h"
#include "SquirrelBindingsUtils.h"
*/ //*_* by icandoit : 2008-04-09 15:03:38

#include "jSquirrelObject.h"
	...
	namespace nMech
	{
		namespace nSQ
		{
			...
			struct SquirrelError
			{
				SquirrelError()
				{
					GetjISQ()->init_SquirrelError(this);
				}
	...
	// *_* by icandoit : 2008-02-26 15:18:19
	//#include "SquirrelVM.h"
#include "jSquirrelBindingsUtils.h"
...
struct VarRef {
  ...
  VarRef(void * _offsetOrAddrOrConst, ScriptVarType _type, SQUserPointer _instanceType, CopyVarFunc _copyFunc, int _size,VarAccessType _access,const SQChar * _typeName) :
    ...
    if (typeTable.IsNull()) {
      typeTable = GetjISQ()->VM_CreateTable();
...
/* *_* by icandoit : 2008-04-09 18:21:24
// === BEGIN Helpers ===
inline void createTableSetGetHandlers(SquirrelObject & so) {
  SquirrelObject delegate = so.GetDelegate();
  if (!delegate.Exists(_T("_set"))) {
    delegate = GetjISQ()->VM_CreateTable();
    GetjISQ()->VM_CreateFunction(delegate,setVarFunc,_T("_set"),_T("sn|b|s")); // String var name = number(int or float) or bool or string.
    GetjISQ()->VM_CreateFunction(delegate,getVarFunc,_T("_get"),_T("s"));      // String var name.
    so.SetDelegate(delegate);
  } // if
} // createTableSetGetHandlers
*/ //*_* by icandoit : 2008-04-09 18:21:28
...
/* *_* by icandoit : 2008-04-09 17:32:20 jISquirrel::로 옮겼음
inline void createInstanceSetGetHandlers(SquirrelObject & so) {
  if (!so.Exists(_T("_set"))) {
    GetjISQ()->VM_CreateFunction(so,setInstanceVarFunc,_T("_set"),_T("sn|b|s|x")); // String var name = number(int or float) or bool or string or instance.
    GetjISQ()->VM_CreateFunction(so,getInstanceVarFunc,_T("_get"),_T("s"));      // String var name.
  } // if
} // createInstanceSetGetHandlers
*/ //*_* by icandoit : 2008-04-09 17:32:23
...
void BindVariable(SquirrelObject & so,T * var,const SQChar * scriptVarName,VarAccessType access=VAR_ACCESS_READ_WRITE) {
  ...
  *pvr = VarRef(var,TypeInfo<T>(),0,ClassType<T>::getCopyFunc(),sizeof(*var),access,TypeInfo<T>().typeName);
  GetjISQ()->createTableSetGetHandlers(so);
} // BindVariable
...
void BindVariable(T * var,const SQChar * scriptVarName,VarAccessType access=VAR_ACCESS_READ_WRITE) {
  SquirrelObject so = GetjISQ()->VM_GetRootTable();
...
void RegisterInstanceVariable(SquirrelObject & so,SQUserPointer classType,T * var,const SQChar * scriptVarName,VarAccessType access=VAR_ACCESS_READ_WRITE) {
  ...
  *pvr = VarRef(offsetOrAddrOrConst,TypeInfo<T>(),classType,ClassType<T>::getCopyFunc(),sizeof(*var),access,TypeInfo<T>().typeName);
  GetjISQ()->createInstanceSetGetHandlers(so);
...
/* *_* by icandoit : 2008-04-09 15:08:36 jISquirrel로 옮겼음
...
inline BOOL CreateConstructNativeClassInstance(HSQUIRRELVM v,const SQChar * className) {
  int oldtop = GetjISQ()->sq_gettop(v);
  GetjISQ()->sq_pushroottable(v);
  GetjISQ()->sq_pushstring(v,className,-1);
  if (SQ_FAILED(sq_rawget(v,-2))) { // Get the class (created with sq_newclass()).
    GetjISQ()->sq_settop(v,oldtop);
    return FALSE;
  } // if
#if 0
  GetjISQ()->sq_remove(v,-3); // Remove the root table.
  GetjISQ()->sq_push(v,1);    // Push the 'this'.
#else // Kamaitati's change. 5/28/06 jcs.
  GetjISQ()->sq_remove(v,-2); // Remove the root table.
  GetjISQ()->sq_pushroottable(v); // Push the 'this'.
#endif
  if (SQ_FAILED(GetjISQ()->sq_call(v,1,SQTrue,SQ_CALL_RAISE_ERROR))) { // Call ClassName(): creates new instance and calls constructor (instead of sq_createinstance() where constructor is not called).
    GetjISQ()->sq_settop(v,oldtop);
    return FALSE;
  } // if
  GetjISQ()->sq_remove(v,-2); // Remove the class.
  //  int newtop = GetjISQ()->sq_gettop(v);
  return TRUE;
} // GetjISQ()->CreateConstructNativeClassInstance
*/ //*_* by icandoit : 2008-04-09 15:08:39
...
#ifndef SQPLUS_DISABLE_COPY_INSTANCES
  if (!GetjISQ()->CreateConstructNativeClassInstance(v,className)) {
    return FALSE;
  } // if
  SQUserPointer up=0;
  GetjISQ()->sq_getinstanceup(v,-1,&up,ClassType<T>::type());
  if (!up) return FALSE;
  T * newClass = (T *)up;
...
T * GetInstance(HSQUIRRELVM v,SQInteger idx) {
  SQUserPointer up=0;
  GetjISQ()->sq_getinstanceup(v,idx,&up,ClassType<T>::type());
...
bool Match(TypeWrapper<SquirrelObject>, HSQUIRRELVM v, int idx); // See GetjISQ()->sq_getstackobj(): always returns true.
...
HSQUIRRELVM Get(TypeWrapper<HSQUIRRELVM>, HSQUIRRELVM v, int /*idx*/); // GetjISQ()->sq_poptop(v): remove UserData from stack so GetParamCount() matches normal behavior.
...
inline RT GetRet(TypeWrapper<RT>,HSQUIRRELVM v,int idx) { RT ret = Get(TypeWrapper<RT>(),v,idx); GetjISQ()->sq_pop(v,2); return ret; } // sq_pop(v,2): restore stack after function call.)
...
#define DECLARE_ENUM_TYPE(TYPE) namespace nMech{ namespace nSQ { \
  inline bool Match(TypeWrapper<TYPE>,HSQUIRRELVM v,int idx) { return Match(TypeWrapper<int>(),v,idx); } \
  inline TYPE Get(TypeWrapper<TYPE>,HSQUIRRELVM v,int idx) { return (TYPE)Get(TypeWrapper<int>(),v,idx); } \
  inline void Push(HSQUIRRELVM v,TYPE value) { Push(v,(int)value); } \
  template<> struct TypeInfo<TYPE> : public TypeInfo<int> { }; \
} } // nameSpace jSqPlus

// As above but use when function bodies should not be generated
#define PROTOS_ENUM_TYPE(TYPE) namespace nMech{ namespace nSQ { \
...
#define DECLARE_INSTANCE_TYPE_BASE_(TYPE,NAME) \
  ...
  template<> inline bool Match(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { \
    return (nMech::GetjISQ()->sq_gettype(v,idx)==OT_NULL) || (GetInstance<TYPE,false>(v,idx) != NULL); } \
  ...
  template<> inline TYPE * Get(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { \
    if (nMech::GetjISQ()->sq_gettype(v,idx)==OT_NULL) return NULL; \
...
// Ordinary case
#define DECLARE_INSTANCE_TYPE_NAME_(TYPE,NAME) namespace nMech{ namespace nSQ { \
  DECLARE_INSTANCE_TYPE_BASE_(TYPE,NAME) \
  template<> inline void Push(HSQUIRRELVM v,TYPE * value) { \
    if (!value)  sq_pushnull(v); \
    else if (!GetjISQ()->BindUtil_CreateNativeClassInstance(v,GetTypeName(*value),value,0)) \
      throw SquirrelError(_T("Push(): could not create INSTANCE (check registration name)")); } \
  template<> inline void Push(HSQUIRRELVM v,TYPE & value) { if (!CreateCopyInstance(v,GetTypeName(value),value)) throw SquirrelError(_T("Push(): could not create INSTANCE copy (check registration name)")); } \
} } // nameSpace jSqPlus

// Allows for providing custom Push handlers (protos here, impl must be provided by app)
#define DECLARE_INSTANCE_TYPE_NAME_CUSTOM_(TYPE,NAME) namespace nMech{ namespace nSQ { \
  DECLARE_INSTANCE_TYPE_BASE_(TYPE,NAME) \
...
//////////////////////////////////////////////////////////////////////////
  ...
  #define sq_argassert(arg,_index_) if (!Match(TypeWrapper<P##arg>(),v,_index_)) return GetjISQ()->sq_throwerror(v,_T("Incorrect function argument"))
#else
...
public:
  static inline int Dispatch(HSQUIRRELVM v) {
    ...
    return p.instance && p.func ?
      ...
      GetjISQ()->sq_throwerror(v, _T("Invalid Instance Type"));
  }
};
...
public:
  static inline int Dispatch(HSQUIRRELVM v) {
    DirectCallInstanceFuncPicker<Callee, Func> p(v);
    GetjISQ()->sq_poptop(v); // Remove UserData from stack: so sa.GetParamCount() returns actual param count.
...
inline void sq_pushdirectclosure(HSQUIRRELVM v,Func func,SQUnsignedInteger nupvalues) {
  SQUserPointer up = GetjISQ()->sq_newuserdata(v,sizeof(func)); // Also pushed on stack.
  memcpy(up,&func,sizeof(func));
  GetjISQ()->sq_newclosure(v,DirectCallFunction<Func>::Dispatch,nupvalues+1);
#ifdef SQPLUS_ENABLE_AUTO_TYPEMASK
  GetjISQ()->sq_setparamscheck(v,0,sqTypeMask<Func>::Get());
#endif
...
inline void sq_pushdirectclosure(HSQUIRRELVM v,const Callee & callee,Func func,SQUnsignedInteger nupvalues) {
  unsigned char * up = (unsigned char *)GetjISQ()->sq_newuserdata(v,sizeof(Callee*)+sizeof(func));  // Also pushed on stack.
  const SQUserPointer pCallee = (SQUserPointer)&callee;
  memcpy(up,&pCallee,sizeof(Callee*));
  memcpy(up + sizeof(Callee*),&func,sizeof(func));
  GetjISQ()->sq_newclosure(v,DirectCallMemberFunction<Callee,Func>::Dispatch,nupvalues+1);
#ifdef SQPLUS_ENABLE_AUTO_TYPEMASK
  GetjISQ()->sq_setparamscheck(v,0,sqTypeMask<Func>::Get());
#endif
} // sq_pushdirectclosure
...
inline void sq_pushdirectinstanceclosure(HSQUIRRELVM v,const Callee & callee,Func func,SQUnsignedInteger nupvalues) {
  unsigned char * up = (unsigned char *)GetjISQ()->sq_newuserdata(v,sizeof(func));  // Also pushed on stack.
  memcpy(up,&func,sizeof(func));
  GetjISQ()->sq_newclosure(v,DirectCallInstanceMemberFunction<Callee,Func>::Dispatch,nupvalues+1);
#ifdef SQPLUS_ENABLE_AUTO_TYPEMASK
  GetjISQ()->sq_setparamscheck(v,0,sqTypeMask<Func>::Get());
#endif
...
inline void sq_pushdirectinstanceclosureglobal(HSQUIRRELVM v,const Callee & callee,Func func,SQUnsignedInteger nupvalues) {
  ...
  GetjISQ()->sq_newclosure(v,DirectCallInstanceGlobalFunction<Callee,Func>::Dispatch,nupvalues+1);
#ifdef SQPLUS_ENABLE_AUTO_TYPEMASK
  ...
  GetjISQ()->sq_setparamscheck(v,0,tm?tm:_T(""));  
#endif
} // sq_pushdirectinstanceclosure
...
inline void sq_pushdirectinstanceclosurevarargs(HSQUIRRELVM v,const Callee & callee,int (Callee::*func)(HSQUIRRELVM),SQUnsignedInteger nupvalues) {
  ...
  GetjISQ()->sq_newclosure(v,DirectCallInstanceMemberFunctionVarArgs<Callee, FuncType>::Dispatch,nupvalues+1);
...
inline void Register(HSQUIRRELVM v,Func func,const SQChar * name) {
  ...
  GetjISQ()->sq_createslot(v,-3); // Stack is restored after this call (same state as before Register() call).
} // Register
...
inline void RegisterGlobal(HSQUIRRELVM v,Func func,const SQChar * name) {
  ...
  GetjISQ()->sq_poptop(v); // Remove root table.
} // RegisterGlobal

template<typename Func>
inline void RegisterGlobal(Func func,const SQChar * name) {
  RegisterGlobal(GetjISQ()->VM_GetVMPtr(),func,name);
} // RegisterGlobal
...
inline void RegisterGlobal(Callee & callee,Func func,const SQChar * name) {
  RegisterGlobal(GetjISQ()->VM_GetVMPtr(),callee,func,name);
} // RegisterGlobal
...
inline void Register(HSQUIRRELVM v,HSQOBJECT hso,Func func,const SQChar * name) {
  nMech::GetjISQ()->sq_pushobject(v,hso);
  Register(v,func,name);
  GetjISQ()->sq_poptop(v); // Remove hso.
} // Register
...
inline void RegisterInstance(HSQUIRRELVM v,HSQOBJECT hclass,Callee & callee,Func func,const SQChar * name) {
  nMech::GetjISQ()->sq_pushobject(v,hclass);
  GetjISQ()->sq_pushstring(v,name,-1);
  sq_pushdirectinstanceclosure(v,callee,func,0);
  GetjISQ()->sq_createslot(v,-3);
  GetjISQ()->sq_poptop(v); // Remove hclass.
} // RegisterInstance
...
//      numParams = 0; // Clear SQ_MATCHTYPEMASKSTRING (does not mean match 0 params. See GetjISQ()->sq_setparamscheck()).
    } else {
...
//    _snprintf(tm,sizeof(tm),"x"); // instance.
  ...
  if (ptm) { // If ptm == 0, don't check type.
    GetjISQ()->sq_setparamscheck(v,numParams,ptm); // Determine arg count from type string.
  } // if
#ifdef _DEBUG
  GetjISQ()->sq_setnativeclosurename(v,-1,name); // For debugging only.
#endif
  GetjISQ()->sq_createslot(v,-3);
  GetjISQ()->sq_poptop(v); // Remove hclass.
...
struct SquirrelFunction {
  ...
  SquirrelFunction(const SquirrelObject & _object,const SquirrelObject & _func) : v(GetjISQ()->VM_GetVMPtr()), object(_object), func(_func) {}
  SquirrelFunction(const SquirrelObject & _object,const SQChar * name) {
    v      = GetjISQ()->VM_GetVMPtr();
    object = _object;
    func   = object.GetValue(name);
  }
  SquirrelFunction(const SQChar * name) {
    v      = GetjISQ()->VM_GetVMPtr();
    object = GetjISQ()->VM_GetRootTable();
    func   = object.GetValue(name);
  }
...
#define SQPLUS_CHECK_FNCALL(res) if (!SQ_SUCCEEDED(res)) throw SquirrelError(_T("SquirrelFunction<> call failed"))
  ...
  RT operator()(void) {
    nMech::GetjISQ()->sq_pushobject(v,func.GetObjectHandle());
    nMech::GetjISQ()->sq_pushobject(v,object.GetObjectHandle());
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,1,SQTrue,SQ_CALL_RAISE_ERROR));
  ...
  RT operator()(P1 p1) {
    ...
    nMech::GetjISQ()->sq_pushobject(v,object.GetObjectHandle());
    Push(v,p1);
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,2,SQTrue,SQ_CALL_RAISE_ERROR));
    return GetRet(TypeWrapper<RT>(),v,-1);
  }
  ...
  RT operator()(P1 p1,P2 p2) {
    ...
    Push(v,p1);
    Push(v,p2);
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,3,SQTrue,SQ_CALL_RAISE_ERROR));
    return GetRet(TypeWrapper<RT>(),v,-1);
  }
  ...
  RT operator()(P1 p1,P2 p2,P3 p3) {
    ...
    Push(v,p2);
    Push(v,p3);
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,4,SQTrue,SQ_CALL_RAISE_ERROR));
    return GetRet(TypeWrapper<RT>(),v,-1);
  }
  ...
  RT operator()(P1 p1,P2 p2,P3 p3,P4 p4) {
    ...
    Push(v,p3);
    Push(v,p4);
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,5,SQTrue,SQ_CALL_RAISE_ERROR));
    return GetRet(TypeWrapper<RT>(),v,-1);
  }
  ...
  RT operator()(P1 p1,P2 p2,P3 p3,P4 p4,P5 p5) {
    ...
    Push(v,p4);
    Push(v,p5);
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,6,SQTrue,SQ_CALL_RAISE_ERROR));
    return GetRet(TypeWrapper<RT>(),v,-1);
  }
  ...
  RT operator()(P1 p1,P2 p2,P3 p3,P4 p4,P5 p5,P6 p6) {
    ...
    Push(v,p5);
    Push(v,p6);
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,7,SQTrue,SQ_CALL_RAISE_ERROR));
    return GetRet(TypeWrapper<RT>(),v,-1);
  }
  ...
  RT operator()(P1 p1,P2 p2,P3 p3,P4 p4,P5 p5,P6 p6,P7 p7) {
    ...
    Push(v,p6);
    Push(v,p7);
    SQPLUS_CHECK_FNCALL(GetjISQ()->sq_call(v,8,SQTrue,SQ_CALL_RAISE_ERROR));
    return GetRet(TypeWrapper<RT>(),v,-1);
  }
                             ...
                             T *newClass)
{
  ...
  SquirrelObject newObjectTable = GetjISQ()->VM_CreateTable();
  // <TODO> 64-bit compatible version.
  ...
  if (count > 1) {
    ...
    for (INT i = 0; i < count; i++) {
      ...
      nMech::GetjISQ()->sq_pushobject(v,so.GetObjectHandle());
      SQUserPointer typeTag;
      GetjISQ()->sq_gettypetag(v,-1,&typeTag);
...
inline int PostConstruct(HSQUIRRELVM v, T *newClass, SQRELEASEHOOK hook)
{
  ...
  GetjISQ()->sq_setinstanceup(v, 1, newClass);
  GetjISQ()->sq_setreleasehook(v, 1, hook);
  return TRUE;
} // PostConstruct
...
int sq_typeof(HSQUIRRELVM v) {
    GetjISQ()->sq_pushstring(v,TypeInfo<T>().typeName,-1);
    return 1;
}
# endif
...
inline void setupClassHierarchy(SquirrelObject newClass) {
  ...
  if (!newClass.Exists(SQ_CLASS_OBJECT_TABLE_NAME)) { // Will always get table from most-derived registered class.
		SquirrelObject objectTable = GetjISQ()->VM_CreateTable();
  ...
  if (!newClass.Exists(SQ_CLASS_HIER_ARRAY)) {   // Will always get table from most-derived registered class.
    classHierArray = GetjISQ()->VM_CreateArray(0); // The only constructor called will be the most-derived class: this array contains all classes in the hierarchy to be constructed.
...
inline SquirrelObject RegisterClassType(HSQUIRRELVM v,const SQChar * scriptClassName,const SQChar * baseScriptClassName=0) {
  int top = GetjISQ()->sq_gettop(v);
  SquirrelObject newClass;
  if (GetjISQ()->CreateClass(v,newClass,(SQUserPointer)ClassType<T>::type(),scriptClassName,baseScriptClassName)) {
    GetjISQ()->VM_CreateFunction(newClass,&ConstructReleaseClass<T>::construct,_T("constructor"));
# ifdef SQ_USE_CLASS_INHERITANCE
    setupClassHierarchy(newClass);
# endif
# ifdef SQPLUS_ENABLE_TYPEOF
    GetjISQ()->VM_CreateFunction(newClass,&sq_typeof<T>,_SC("_typeof"));
# endif
  } // if
  GetjISQ()->sq_settop(v,top);
  return newClass;
} // RegisterClassType
...
struct SQClassDefBase {
  ...
  // Optional base arg is the name of a base class to inherit from (must already be defined in the Squirrel VM).
  SQClassDefBase(const SQChar * _name,const SQChar * _base=0) : v(GetjISQ()->VM_GetVMPtr()), name(_name), base(_base) {}
#else
  SQClassDefBase(HSQUIRRELVM _v,const SQChar * _name) : v(_v), name(_name) {}
  SQClassDefBase(const SQChar * _name) : v(GetjISQ()->VM_GetVMPtr()), name(_name) {}
#endif
...
#include "SqPlusSmartPointer.h"
  ...
  SQClassDefBase & staticFuncVarArgs(Func pfunc,const SQChar * name,const SQChar * typeMask=_T("*")) {
    GetjISQ()->VM_PushObject(newClass);
    GetjISQ()->VM_CreateFunction(pfunc,name,typeMask);
    GetjISQ()->VM_Pop(1);
    return *this;
  } // staticFuncVarArgs
...
static int _##CLASSNAME##_constructor(HSQUIRRELVM v) {               \
  ...
  GetjISQ()->sq_setinstanceup(v,1,pc);                                          \
  GetjISQ()->sq_setreleasehook(v,1,_##CLASSNAME##_release);                     \
  return 1;                                                          \
}

#define SQ_REGISTER_CLASS(CLASSNAME)                                 \
  RegisterClassType(GetjISQ()->VM_GetVMPtr(),_T(#CLASSNAME),_##CLASSNAME##_constructor)

#define SQ_REGISTER_INSTANCE(NEWSQCLASS,CCLASS,FUNCNAME)             \
  RegisterInstance(GetjISQ()->VM_GetVMPtr(),NEWSQCLASS.GetObjectHandle(),*(CCLASS *)0,&CCLASS::FUNCNAME,_T(#FUNCNAME));

#define SQ_REGISTER_INSTANCE_VARARGS(NEWSQCLASS,CCLASS,FUNCNAME)     \
  RegisterInstanceVarArgs(GetjISQ()->VM_GetVMPtr(),NEWSQCLASS.GetObjectHandle(),*(CCLASS *)0,&CCLASS::FUNCNAME,_T(#FUNCNAME));
...
#if 0 // jSquirrelObject.h로 옮겼음.
// *_* by icandoit : 2008-04-09 15:14:58
//Y:\script\SQUIRREL2_1_1_sqplus_snapshot_20071014 버젼에서 추가된 코드
...
inline _ty SquirrelObject::Get(void) {
  nMech::GetjISQ()->sq_pushobject(GetjISQ()->GetVMPtr(),GetObjectHandle());
  _ty val = SqPlus::Get(SqPlus::TypeWrapper<_ty>(),GetjISQ()->GetVMPtr(),-1);
  GetjISQ()->sq_poptop(GetjISQ()->GetVMPtr());
  return val;
}
...
inline SquirrelObject SquirrelObject::SetByValue(_ty val) { // classes/structs should be passed by ref (below) to avoid an extra copy.
  SqPlus::Push(GetjISQ()->GetVMPtr(),val);
