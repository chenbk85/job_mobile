...
private:
    ...
    struct FunctionNameEnumerator {
        ...
        FunctionNameEnumerator(void) : names(GetjISQ()->VM_CreateTable()) {}
    ...
    public:
        ...
        static inline void addOperatorFunc(SQClassDefBase<TClassType> *def,
                                           Func ofunc,
                                           const SQChar *name) {
            if (Arg<Func>::num() != 1) {
                //assert(false &&
            ...
            SQChar proxy[256];
            scsprintf(proxy, _T("overloaded%s"), name);
            
            addMemberFunc(def, ofunc, proxy);

            SQChar script[512];
            scsprintf(script, _T("%s.%s<-function(o){return %s.%s(o);}"),
...
public:
    ...
    SQClassDefBase<TClassType> &overloadOperator(Ofunc ofunc, const SQChar *n){
