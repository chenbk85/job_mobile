...
#ifndef _jIPak_HDR_123332iowssfkajfw_
#define _jIPak_HDR_123332iowssfkajfw_
using namespace std;
...
namespace nMech
{
	namespace nPAK
	{
		...
		struct jcIArchive: public _reference_target_t
		{
			// Compression methods
			...
			enum EPakFlags
			{
				...
				// If FLAGS_OPEN_READ_ONLY | FLAGS_SIMPLE_RELATIVE_PATHS are set, jIPak 
				...
				// if this is set, jcPak doesn't try to transform the path according to the MOD subdirectories
		...
		TYPEDEF_AUTOPTR(jcIArchive);

		// this special flag is used for findfirst/findnext routines
		// to mark the files that were actually found in Archive
		enum {_A_IN_JC_PAK = 0x80000000};
		struct jICachedFileData
		{
			virtual void* GetData(bool bRefreshCache = true)=0;
			//ZipDir::FileEntry* GetFileEntry() {return m_pFileEntry;}
			virtual unsigned long GetUnCompFileSize()=0;
			virtual unsigned short GetFileEntryMethod()=0;

			// the memory needs to be allocated out of a MT-safe heap here
			virtual unsigned GetFileDataOffset()=0;
		...
		struct jIPak
		{
			...
			virtual long FileLen(FILE *handle)=0;
			...
			virtual nPAK::jcIArchive* OpenArchive (const char* szPath, unsigned nFlags = FLAGS_PATH_REAL) = 0;
			...
			virtual nJC_stream::IStreamEngine *GetStreamEngine(nLOG::jILog* pLog)=0;
			virtual void SetStreamEngine(nJC_stream::IStreamEngine *)=0;
	...
	{
		extern JBASE_API nPAK::jIPak* g_pjIPak;
	}
