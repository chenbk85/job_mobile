...
// usable:
//
//		CStreamData_WorldPos			nMech::nMath::jVec3(x,y,z) world position with fallback if outside of typical area
//		CStreamData_Normal				normalized nMech::nMath::jVec3(x,y,z) in very well compressed in 16bit (cubemap side+2d pos on cubemap)

// next possible candidates:
//
//		Direction								nMech::nMath::jVec3, components in range -1..1
//		Angle										range: 0..2*PI
...
namespace nMech
{
	namespace nJC_stream
	{
		...
		public:
			...
			{
				IGame *pGame=g_pISystem->GetIGame();													assert(pGame);
				nMech::nJC_stream::IStreamEngine *pStreamE=g_pISystem->GetStreamEngine();				assert(pStreamE);
		...
		{
			nMech::nMath::jVec3 &										m_vData;			//!< reference to the nMech::nMath::jVec3 data

		public:	// --------------------------

			//! constructor
			CStreamData_WorldPos( nMech::nMath::jVec3 &inoutvValue ) :m_vData(inoutvValue)
			{
			}

			//! to reproduce the quality loss form compression and decompression
			nMech::nMath::jVec3 GetCompressed() const
			{
				nMech::nJC_stream::CStream stm;
			...
			virtual bool Read( nMech::nJC_stream::CStream &inStream ) const
			{
				...
				m_vData=nMech::nMath::jVec3((float)x,(float)y,(float)z) * 0.002f;		// 2mm units

				return true;
			}

			virtual bool Write( nMech::nJC_stream::CStream &inStream ) const
			{
				if(!IsActive())																// same mask for read and write
		...
		public:	// --------------------------
			...
			//! constructor
			CStreamData_Normal( nMech::nMath::jVec3 &inoutvValue ) :m_vData(inoutvValue)
			{
			}
