...
// -------------------------------------------------------------------------
//  File name:   ijcAnimation.h
//  Version:     v1.00
//  Created:     28/5/2001 by Vladimir Kajalin
//  Compilers:   Visual Studio.NET
//  Description: jcAnimation interface
// -------------------------------------------------------------------------
//  History:
//
////////////////////////////////////////////////////////////////////////////

#ifndef IJC_ANIMATION
#define IJC_ANIMATION

#ifdef JC_GAMEANIMATION_EXPORTS
	#define JC_GAMEANIMATION_API __declspec(dllexport)
#else
	#define JC_GAMEANIMATION_API __declspec(dllimport)
#endif
...
struct jcCharMorphParams;
struct IMatInfo;

//#include "jcParticleSpawnInfo.h"
...
struct jcIBone
{
	...
	virtual void FixBoneMatrix (const nMech::nMath::Matrix44& mtxBone) = 0;
	...
	//     vCharAngles are the same as in the entity and in the Draw call to jcICharInstance
	// See Also:
	...
	virtual void FixBoneOriginInWorld (const nMech::nMath::jVec3& vCharPos, const nMech::nMath::jVec3& vCharAngles, const nMech::nMath::jVec3& vTargetOrigin) = 0;
	...
	virtual void SetBoneOriginInWorld (const nMech::nMath::jVec3& vCharPos, const nMech::nMath::jVec3& vCharAngles, const nMech::nMath::jVec3& vTargetOrigin) = 0;
	...
	// Summary:
	//     Set the plus-matrix rotation components
	virtual void SetPlusRotation(const nMech::nMath::jcQuat& qRotation) = 0;
	...
	// Summary:
	//     Get the position of the bone
	virtual nMech::nMath::jVec3 GetBonePosition() = 0;
	...
	// Return Value:
	//     Return the vector for the requested axis. If cAxis wasn't set to either x, y or z, nMech::nMath::jVec3d(0,0,0) will be returned.
	// Summary:
	//     Get the bone axis
	virtual nMech::nMath::jVec3 GetBoneAxis(char cAxis) = 0;
	...
	// Summary:
	//     Get the parent bone world coordinate rotation as a quaternion
	virtual nMech::nMath::jcQuat GetParentWQuat () = 0;

	// Description:
	//     Get the parent bone.
	// Return Value:
	//     A pointer to a jcIBone derived class representing the parent bone. If there's none, NULL will be returned.
	// Summary:
	//     Get the parent bone
	virtual jcIBone* GetParent() = 0;
	...
	// Summary:
	//     Get the matrix relative to the parent bone
	virtual const nMech::nMath::Matrix44& GetRelativeMatrix() = 0;
	...
	// Summary:
	//     Get the matrix in object coordinates
	virtual const nMech::nMath::Matrix44& GetAbsoluteMatrix() = 0;
};
...
struct AnimSinkEventData
{
	...
	union 
	{
		struct 
		{
			nMech::uint16	m_iFlag;
			nMech::uchar	m_cEventType;
			nMech::uchar	m_cEtcState;
		};
		nMech::uint m_EventFlag;
	};
	jSET_FLAG(_IsEventLuaTable,jBIT_0);
	jSET_FLAG(_IsEventXML,jBIT_1);
	union
	{
		struct 
		{
	...
	bool operator == (const AnimSinkEventData& that) const {return p == that.p && n == that.n && m_EventFlag==that.m_EventFlag;}
};
...
//DOC-IGNORE-BEGIN
//! This interface is free from concepts specific for jcEngine
//DOC-IGNORE-END
...
//! jc... because it's specific to the needs of the jcEngine
struct jcIAnimationSet: public IAnimationSet
{
...
//! interface to those properties is through the jcICharModel.
//! The instance of the jcICharModel is guaranteed to be alive at least as long as
//! all its jcICharInstance are alive

// Description:
//     This class is used to store in memory the model data usually loaded from
//     a gof file. Should usually be created while creating a new character 
//     instance with jIAniManager::MakeCharacter.
#include "smartptr.h"

struct jcICharModel: public _i_reference_target_t
{
	// Description:
	//     Return a pointer of the instance of a jcIAnimationSet derived class
	//     applicable for the model.
	// Return Value:
	//     A pointer to a jcIAnimationSet derived class
	// Summary:
	//     Get the Animation Set defined for the model
	virtual jcIAnimationSet* GetAnimationSet () = 0;
...
struct IDeformableRenderMesh
{
	...
	virtual void ProcessSkinning(const nMech::nMath::jVec3& t,const nMech::nMath::Matrix44& mtxModel, int nTemplate, int nLod=-1, bool bForceUpdate=false) = 0;
};

struct jcICharVisualElement: public _reference_target_t
{
...
struct jcICharSubmesh: public jcICharVisualElement
{
	// returns the model of the submesh, or NULL in case of failure
	virtual jcICharModel* GetModel () = 0;

	//! Start the specified by parameters morph target
	virtual void StartMorph (const char* szMorphTarget, const jcCharMorphParams& params) = 0;

	//! Start the specified by parameters morph target
	virtual void StartMorph (int nMorphTargetId, const jcCharMorphParams& params) = 0;
	...
	virtual void AddDecal (jcEngineDecalInfo& Decal) = 0;
...
struct jcICharFxTrail: public jcICharVisualElement
{
...
//DOC-IGNORE-BEGIN
//! TODO:
//! Split this interface up into a few logical interfaces, starting with the jcICharModel
//DOC-IGNORE-END
...
struct jcICharInstance: public IBindable
{
	virtual ~jcICharInstance() {};
	...
	// Arguments:
	//     nCommand - The command to execute, as declared in jcAnimationScriptCommandEnum
	...
	// Summary:
	//     Draw the character
	virtual void Draw(const SRendParams & RendParams,const nMech::nMath::jVec3& t)=0;	
	...
	//! jcICharInstance owns this light. This light may be destructed without notice upon next call to 
  ...
  //virtual void DrawForShadow(const nMech::nMath::jVec3 & vTranslationPlus = nMech::nMath::jVec3(0,0,0))=0;
  ...
  //! Return dynamic bbox of object
	...
	virtual void GetBBox(nMech::nMath::jVec3& Mins, nMech::nMath::jVec3& Maxs)=0;
  ...
  virtual const nMech::nMath::jVec3 GetCenter()=0;
  ...
  // Summary:
	...
	//! If bCopyLight is true, then this light will be copied and the copy will be managed by the jcCharInstance
	...
	virtual bool StartAnimation (const char* szAnimName, const struct jcCharAnimationParams& Params) {return false;}
...
//DOC-IGNORE-END
	...
	//! Start the specified by parameters morph target
	virtual void StartMorph (const char* szMorphTarget, const jcCharMorphParams& params) {}

	//! Start the specified by parameters morph target
	virtual void StartMorph (int nMorphTargetId, const jcCharMorphParams& params) {}
  ...
  virtual void ResetAnimations()=0;
	...
	//! All the animations are played with the constant nMech::real-time speed multiplied by this factor.
  ...
  //! Processes skining (call this function every frame to animate character)
	...
	virtual void Update (nMech::nMath::jVec3 vPos = nMech::nMath::jVec3(0,0,0), float fRadius=0, unsigned uFlags = 0) {}
  ...
  virtual void SynchronizeWithPhysicalEntity(IPhysicalEntity *pent
				, const nMech::nMath::jVec3& posMaster=nMech::nMath::jVec3(nMech::nMath::zero)
				,const nMech::nMath::Quat& qMaster=nMech::nMath::Quat(1,0,0,0)) = 0;
  ...
  virtual void SetLimbIKGoal(int limbid
		, nMech::nMath::vectorf ptgoal=nMech::nMath::vectorf(1E10f,0,0)
		, int ik_flags=0
		, float addlen=0
		, nMech::nMath::vectorf goal_normal=nMech::nMath::vectorf(nMech::nMath::zero)) = 0;
  virtual nMech::nMath::vectorf GetLimbEndPos(int limbid) = 0;
  virtual void AddImpact(int partid, nMech::nMath::vectorf point,nMech::nMath::vectorf impact) = 0;
	virtual int TranslatePartIdToDeadBody(int partid) = 0;
	virtual nMech::nMath::vectorf GetOffset() = 0;
	virtual void SetOffset(nMech::nMath::vectorf offset) = 0;

  //! Direct access to the specified  bone
  virtual jcIBone * GetBoneByName(const char * szName) = 0;
  ...
  virtual nMech::nMath::jVec3 GetTPVWeaponHelper(const char * szHelperName, ObjectBindingHandle nHandle) = 0;

	//! Returns position of specified helper ( exported into gof file )
	//! Actually returns the given bone's position
	//! Default implementation: 000
	virtual nMech::nMath::jVec3 GetHelperPos(const char * szHelperName) {return nMech::nMath::jVec3(0,0,0);}
	//! Returns the matrix of the specified helper ( exported into gof file )
	//! Actually returns the given bone's matrix
	virtual const nMech::nMath::Matrix44 * GetHelperMatrixByName(const char * szHelperName) {return NULL;}
	...
	virtual bool GetTPVWeaponHelperMatrix(const char * szHelperName, ObjectBindingHandle nHandle, nMech::nMath::Matrix44& matOut) {return false;}
  ...
  // temporary hack - this is used in XPlayer in jcGame only
	/*
	nMech::nMath::jVec3 m_vAngles;
	void		SetAngles( const nMech::nMath::jVec3& angles ) {m_vAngles = angles;}
	nMech::nMath::jVec3&	GetAngles( ) {return m_vAngles;}
	*/
  ...
  //! Spawn decal on the walls, static objects, terrain and entities
	...
	virtual void CreateDecal(jcEngineDecalInfo& DecalLCS)=0;
	...
	//{
	//	return AddParticleEmitter(rInfo, jcParticleSpawnInfo(fSpawnRate, bFacesUp?jcParticleSpawnInfo::FLAGS_RAIN_MODE:0));
	//}
	...
	virtual int AddParticleEmitter(struct ParticleParams& rInfo, const struct jcParticleSpawnInfo& rSpawnInfo) {return -1;}
	...
	virtual void SetScale (const nMech::nMath::jVec3d& vScale) {}

	//! Pushes the underlying tree of objects into the given Sizer object for statistics gathering
	virtual void GetMemoryUsage(class nMech::jcISizer* pSizer) const = 0;
	...
	virtual jcICharSubmesh* NewSubmesh (jcICharModel* pModel, bool bVisible = false) {return NULL;}
	// adds submesh to the specified slot; replaces submesh if there's some there
	virtual jcICharSubmesh* NewSubmesh (unsigned nSlot, jcICharModel* pModel, bool bVisible = false) {return NULL;}
	...
	virtual void RemoveSubmesh (jcICharSubmesh* pSubmesh){}
	...
	virtual jcICharSubmesh* GetSubmesh(unsigned i){return NULL;}


	virtual jcICharFxTrail* NewFxTrail (unsigned nSlot, const struct jcCharFxTrailParams&) {return NULL;}
	virtual void RemoveFxTrail(unsigned nSlot) {}


	// returns the pointer to this character instance, if it's a character instance
	virtual struct jcICharInstance* GetjcICharInstance() {return this;}
};

// Description:
//     This class is the main access point for any character anymation 
//     required for a program which uses jcEngine.
// See Also:
//     CreateCharManager
struct jIAniManager
{
	...
	// Description:
	//     Contains statistics about jcCharManager.
	struct Statistics
	{
	...
	// Return Value:
	//     A pointer to a jcICharInstance class if the model could be loaded 
	//     properly.
	//     NULL if the model couldn't be loaded.
	// Summary:
	//     Create a new instance of a model
	virtual jcICharInstance * MakeCharacter(const char * szFilename, unsigned nFlags = 0)=0;

	// Description:
	//     Load a model file into a jcICharModel class. Usually, MakeCharacter 
	...
	//                  used. Uses flags defined with EModelPersistence.
	// Return Value:
	//     A pointer to a jcICharModel class if the model could be loaded 
	//     properly.
	//     NULL if the model couldn't be loaded.
	// Summary:
	//     Load model file
	virtual jcICharModel* LoadModel(const char* szFileName, unsigned nFlags=0)=0;
	...
	//     MakeCharacter
	// Arguments:
	//     pjcCharInstance - Pointer to the character instance class to delete
	//     nFlags - Set how the model will be kept in memory after being 
	//                  used. Uses flags defined with EModelPersistence.
	// Summary:
	//     Free a character instance
	virtual void RemoveCharacter(jcICharInstance * pjcCharInstance, unsigned nFlags = 0)=0;  
	...
	//     use it again, with jcICharInstance::StartAnimation for example, it 
	//     will loaded back automatically.
	// See Also:
	//     StartLoadAnimation, jcICharInstance::StartAnimation
	// Arguments:
	...
	// See Also
	//     UnloadAnimation, jcICharInstance::StartAnimation
	// Arguments:
	//     szFileName - Filename of the animation
...
// CreatejcAnimation function type definition
typedef jIAniManager * (*PFNCREATE_JC_ANIMATION)(ISystem* pSys,const char * szInterfaceVersion);

// Description:
//     Create an instance of the Animation System. It should usually be called 
//     by ISystem::InitAnimationSystem().
// See Also:
//     jIAniManager, jIAniManager::Release
// Arguments:
...
JC_GAMEANIMATION_API jIAniManager * CreateCharManager(ISystem* pSys, const char * szInterfaceVersion=gAnimInterfaceVersion);
