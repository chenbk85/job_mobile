...
#ifdef WIN32 
#ifdef JC_GAME_EXPORTS
#define JC_GAMEGAME_API __declspec(dllexport)
#else
#define JC_GAMEGAME_API __declspec(dllimport)
#endif
#else	//WIN32
#define JC_GAMEGAME_API
#endif
...
// flags
#define MOD_NEWGAMEDLL	1L<<1 //tells if the MOD contains a replacement of jcGame.dll
...
struct IGame
{
	...
	//##                      compatibility with the gold version of ES_Client 
	//##                      will be broken.
	...
	//          NULL will be returned if only ES_Client is loaded.
...
extern "C" JC_GAMEGAME_API IGame* CreateGameInstance();
