...
struct pe_params_pos : pe_params { //	Sets postion and orientation of entity
	...
	nMech::nMath::vectorf pos;
	nMech::nMath::quaternionf q;
	float scale;
...
struct pe_params_sensors : pe_params { // Attaches optional sensors to entity (sensors raytrace enviroment around entity)
	...
	const nMech::nMath::vectorf *pOrigins; // pOrigins sensors origins in entity CS
	const nMech::nMath::vectorf *pDirections;	// pDirections sensors directions (dir*ray length) in entity CS
};
...
struct pe_simulation_params : pe_params { // Sets gravity and maximum time step
	...
	float damping;
	nMech::nMath::vectorf gravity;
	float dampingFreefall; // damping and gravity used when there are no collisions,
	nMech::nMath::vectorf gravityFreefall; // NOTE: if left unused, gravity value will be substituted (if provided)
	float mass;
	float density;
...
struct pe_params_joint : pe_params {
	...
	nMech::nMath::vectorf pivot; // joint pivot in entity CS
	nMech::nMath::quaternionf q0;	// orientation of child in parent coordinates that corresponds to angles (0,0,0)
	float *pMtx0; // same as 3x3 row major matrix
	float *pMtx0T; // same as 3x3 column major matrix
	nMech::nMath::vectorf limits[2];	// limits for each angle
	nMech::nMath::vectorf bounciness; // bounciness for each angle (applied when limit is reached)
	nMech::nMath::vectorf ks,kd; // stiffness and damping koefficients for each angle angular spring
	nMech::nMath::vectorf qdashpot; // limit vicinity where joints starts resisting movement
	nMech::nMath::vectorf kdashpot; // when dashpot is activated, this is roughly the angular speed, stopped in 2 sec
	nMech::nMath::vectorf q;	// angles values
	nMech::nMath::vectorf qext; // additional angles values (angle[i] = q[i]+qext[i]; only q[i] is taken into account
...
struct pe_params_articulated_body : pe_params {
	...
	int bCollisionResp;
	nMech::nMath::vectorf pivot; // attachment position for grounded bodies
	nMech::nMath::vectorf a; // acceleration of ground for grounded bodies
	nMech::nMath::vectorf wa; // angular acceleration of ground for grounded bodies
	nMech::nMath::vectorf w; // angular velocity of ground for grounded bodies
	nMech::nMath::vectorf v;
	...
	nMech::nMath::vectorf posHostPivot;
	int bInheritVel;

	int nCollLyingMode;
	nMech::nMath::vectorf gravityLyingMode;
...
struct pe_player_dimensions : pe_params {
	...
	nMech::nMath::vectorf sizeCollider; // collision cylinder dimensions
...
struct pe_params_particle : pe_params {
	...
	nMech::nMath::vectorf heading; // direction of movement
	...
	int surface_idx;
	nMech::nMath::vectorf wspin; // angular velocity
	nMech::nMath::vectorf gravity;
	nMech::nMath::vectorf waterGravity;
	nMech::nMath::vectorf normal;
	nMech::nMath::quaternionf q0;	// initial orientation (zero means x along direction of movement, z up)
	float minBounceVel;
...
struct pe_params_rope : pe_params {
	...
	float friction;
	int nSegments;
	nMech::nMath::vectorf *pPoints;
	nMech::nMath::vectorf *pVelocities;
	int iStride; // used in GetParams only

	IPhysicalEntity *pEntTiedTo[2];
	nMech::nMath::vectorf ptTiedTo[2];
	int idPartTiedTo[2];
};
...
struct pe_action_impulse : pe_action {
	...
	nMech::nMath::vectorf impulse;
	nMech::nMath::vectorf momentum;	// r x impulse, optional
	nMech::nMath::vectorf point; // point of application, in world CS, optional
...
struct pe_action_add_constraint : pe_action {
	...
	int partid[2];
	nMech::nMath::quaternionf qframe[2];
	float xlimits[2];
	float yzlimits[2];
	unsigned int flags;
...
struct pe_action_move : pe_action { // movement request for living entities
	...
	nMech::nMath::vectorf dir; // dir requested velocity vector
...
struct pe_status_pos : pe_status {
	...
	nMech::nMath::vectorf pos; // position of center
	nMech::nMath::vectorf BBox[2]; // bounding box relative to pos (bbox[0]-min, bbox[1]-max)
	nMech::nMath::quaternionf q;
	float scale;
	int iSimClass;
...
struct pe_status_sensors : pe_status { // Requests status of attached to the entity sensors
	...
	nMech::nMath::vectorf *pPoints;	// pointer to array of points where sensors touch environment (assigned by physical entity)
	nMech::nMath::vectorf *pNormals; // pointer to array of surface normals at points where sensors touch environment
	unsigned int flags; // bitmask of flags, bit==1 - sensor touched environment
};

struct pe_status_dynamics : pe_status {
	enum entype { type_id=8 };
	pe_status_dynamics() : v(nMech::nMath::zero),w(nMech::nMath::zero),a(nMech::nMath::zero)
		,wa(nMech::nMath::zero),centerOfMass(nMech::nMath::zero) {
		MARK_UNUSED partid,ipart; type=type_id; time_interval=0; submergedFraction=0; waterResistance=0;
	}

	int partid;
	int ipart;
	nMech::nMath::vectorf v; // velocity
	nMech::nMath::vectorf w; // angular velocity
	nMech::nMath::vectorf a; // linear acceleration
	nMech::nMath::vectorf wa; // angular acceleration
	nMech::nMath::vectorf centerOfMass;
...
struct coll_history_item {
	nMech::nMath::vectorf pt; // collision area center
	nMech::nMath::vectorf n; // collision normal in entity CS
	nMech::nMath::vectorf v[2]; // velocities of contacting bodies at the point of impact
...
struct pe_status_sample_contact_area : pe_status {
	...
	nMech::nMath::vectorf ptTest;
	nMech::nMath::vectorf dirTest;
};
...
struct pe_status_living : pe_status {
	...
	nMech::nMath::vectorf camOffset; // camera offset
	nMech::nMath::vectorf vel; // actual velocity (as rate of position change)
	nMech::nMath::vectorf velUnconstrained; // 'physical' movement velocity
	nMech::nMath::vectorf velRequested;	// velocity requested in the last action
	nMech::nMath::vectorf velGround; // velocity of the object entity is standing on
	float groundHeight; // position where the last contact with the ground occured
	nMech::nMath::vectorf groundSlope;
...
struct pe_status_wheel : pe_status {
	...
	nMech::nMath::vectorf ptContact; // point where wheel touches ground
	float w; // rotation speed
	int bSlip;
	nMech::nMath::vectorf velSlip; // slip velocity
...
struct pe_status_vehicle_abilities : pe_status {
	...
	nMech::nMath::vectorf rotPivot;	// returns turning circle center
...
struct pe_status_joint : pe_status {
	...
	nMech::nMath::vectorf q;	// current joint angles (controlled by physics)
	nMech::nMath::vectorf qext; // external angles (from animation)
	nMech::nMath::vectorf dq; // current joint angular velocities
	nMech::nMath::quaternionf quat0;
};
...
struct pe_status_softvtx : pe_status {
	...
	int nVtx;
	nMech::nMath::strided_pointer<nMech::nMath::vectorf> pVtx;
	nMech::nMath::strided_pointer<nMech::nMath::vectorf> pNormals;
};
...
struct pe_geomparams {
	...
	nMech::nMath::vectorf pos; // offset from object's geometrical pivot
	nMech::nMath::quaternionf q; // orientation relative to object
	float scale;
...
struct pe_cargeomparams : pe_geomparams {
	...
	nMech::nMath::vectorf pivot; // upper suspension point in vehicle CS
...
struct geom_world_data {
	...
	nMech::nMath::vectorf offset;
	nMech::nMath::matrix3x3f R;
	float scale;
	nMech::nMath::vectorf v,w;
	nMech::nMath::vectorf centerOfMass;
	int iStartNode;
...
struct intersection_params {
	intersection_params() {
		...
		maxSurfaceGapAngle=1.0f*float(nMech::nMath::pi/180);
		pGlobalContacts=0;
		minAxisDist=0;
		bSweepTest=false;
	...
	nMech::nMath::vectorf centerOfRotation;
	nMech::nMath::vectorf axisOfRotation;
	float time_interval;
	float vrel_min;
	float maxSurfaceGapAngle;
	float minAxisDist;
	nMech::nMath::vectorf unprojectionPlaneNormal;
	nMech::nMath::vectorf axisContactNormal;
	float maxUnproj;
	nMech::nMath::vectorf ptOutsidePivot[2];
	bool bSweepTest;
...
public:
	...
	virtual int PointInsideStatus(const nMech::nMath::vectorf &pt) = 0;
	...
	virtual int FindClosestPoint(geom_world_data *pgwd, int &iPrim,int &iFeature, const nMech::nMath::vectorf &ptdst0,const nMech::nMath::vectorf &ptdst1,
		nMech::nMath::vectorf *ptres, int nMaxIters=10) = 0;
	virtual void CalcVolumetricPressure(geom_world_data *gwd, const nMech::nMath::vectorf &epicenter,float k,float rmin,
		const nMech::nMath::vectorf &centerOfMass, nMech::nMath::vectorf &P,nMech::nMath::vectorf &L) = 0;
	virtual float CalculateBuoyancy(const primitives::plane *pplane, const geom_world_data *pgwd, nMech::nMath::vectorf &submergedMassCenter) = 0;
	virtual void CalculateMediumResistance(const primitives::plane *pplane, const geom_world_data *pgwd, nMech::nMath::vectorf &dPres,nMech::nMath::vectorf &dLres) = 0;
	...
	virtual int GetFeature(int iPrim,int iFeature, nMech::nMath::vectorf *pt) = 0;
...
public:
	...
	virtual IGeometry *CreateMesh(nMech::nMath::strided_pointer<const nMech::nMath::vectorf> pVertices
		,nMech::index_t *pIndices,const short *pIds,int nTris, int flags,bool bCopyTriangles=true,
...
class IPhysUtils {
public:
	virtual int BreakPolygon(nMech::nMath::vector2df *ptSrc,int nPt, int nCellx,int nCelly, int maxPatchTris, nMech::nMath::vector2df *&ptout,int *&nPtOut,
		float jointhresh=0.5f,int seed=-1) = 0;
	virtual int CoverPolygonWithCircles(nMech::nMath::strided_pointer<nMech::nMath::vector2df> pt,int npt,bool bConsecutive, const nMech::nMath::vector2df &center,
		nMech::nMath::vector2df *&centers,float *&radii, float minCircleRadius) = 0;
...
public:
	...
	*/
	virtual int GetStateSnapshot(class nMech::nJC_stream::CStream &stm, float time_back=0, int flags=0) = 0;
	/*! Reads state from snapshot
	@param stm stream
	@return size of snapshot
	*/
	virtual int SetStateFromSnapshot(class nMech::nJC_stream::CStream &stm, int flags=0) = 0;
...
public:
	...
	*/
	virtual void SetupEntityGrid(int axisz, nMech::nMath::vectorf org, int nx,int ny, float stepx,float stepy) = 0;
	...
	*/
	virtual int GetEntitiesInBox(nMech::nMath::vectorf ptmin,nMech::nMath::vectorf ptmax, IPhysicalEntity **&pList, int objtypes) = 0;
	...
	*/
	virtual int RayWorldIntersection(nMech::nMath::vectorf org,nMech::nMath::vectorf dir, int objtypes, unsigned int flags, ray_hit *hits,int nMaxHits,
	...
	*/
	virtual void SimulateExplosion(nMech::nMath::vectorf epicenter,nMech::nMath::vectorf epicenterImp, float rmin,float rmax, float r,float impulsive_pressure_at_r,
	...
	*/
	...
	virtual int CollideEntityWithBeam(IPhysicalEntity *_pent, nMech::nMath::vectorf org,nMech::nMath::vectorf dir,float r, ray_hit *phit) = 0;
	virtual int RayTraceEntity(IPhysicalEntity *pient, nMech::nMath::vectorf origin,nMech::nMath::vectorf dir, ray_hit *pHit, pe_params_pos *pp=0) = 0;
