/*=============================================================================
  ...
  Copyright (c) 2001-2002 jGame. All Rights Reserved.
...
public:
  COrthoNormalBasis()
  {
    m_vForward = nMech::nMath::jVec3(1,0,0);
    m_vUp = nMech::nMath::jVec3(0,0,1);
    m_vRight = nMech::nMath::jVec3(0,1,0);
  }
  ...
  COrthoNormalBasis(const nMech::nMath::jVec3& vForward, const nMech::nMath::jVec3& vUp, const nMech::nMath::jVec3& vRight)
  {
    m_vForward = vForward;
    m_vUp = vUp;
    m_vRight = vRight;
  ...
  nMech::nMath::Matrix44 matrixBasisToXYZ() const
  {
    nMech::nMath::Matrix44 mat;
    mat.SetIdentity();
  ...
  nMech::nMath::Matrix44 matrixXYZToBasis() const
  {
    nMech::nMath::Matrix44 mat;
    mat.SetIdentity();
  ...
  nMech::nMath::Matrix44 matrixBasisToDestBasis(const COrthoNormalBasis& dest) const
  {
    return (dest.matrixXYZToBasis() * this->matrixBasisToXYZ());
  }

  COrthoNormalBasis& rotate(const nMech::nMath::jVec3& axis, const float degrees)
  {
    //nMech::nMath::jVec3 a = axis;
    //a.Normalize();
    nMech::nMath::Matrix33 m;
  ...
  nMech::nMath::jVec3 m_vForward; //tangent
  nMech::nMath::jVec3 m_vUp;      //binormal
  nMech::nMath::jVec3 m_vRight;   //normal
};
...
typedef TAllocator16<nMech::nMath::Matrix44> MatrixAllocator16;
typedef TGrowArray<nMech::nMath::Matrix44, MatrixAllocator16> MatrixArray16;
...
public:
  ...
  short m_VisId;
  short m_VPMatrixId;
  nMech::uint m_ObjFlags;
  nMech::nMath::Matrix44 m_Matrix;
  ...
  short m_nOcclId;
	short m_nHDRLMId;
  nMech::uint m_Reserved1;

  nMech::uint m_DynLMMask;
  float m_Trans2[3];
  float m_Angs2[3];
  ...
  _inline nMech::nMath::jVec3 GetTranslation() const
  {
    return m_Matrix.GetTranslationOLD();
  }
  _inline float GetScaleX() const
  {
    return nMech::nMath::jc_sqrtf(m_Matrix(0,0)*m_Matrix(0,0) + m_Matrix(0,1)*m_Matrix(0,1) + m_Matrix(0,2)*m_Matrix(0,2));
  }
  _inline float GetScaleZ() const
  {
    return nMech::nMath::jc_sqrtf(m_Matrix(2,0)*m_Matrix(2,0) + m_Matrix(2,1)*m_Matrix(2,1) + m_Matrix(2,2)*m_Matrix(2,2));
  }
  ...
  nMech::nMath::Matrix44 &GetMatrix()
  {
    return m_Matrix;
  }

  nMech::nMath::Matrix44 &GetInvMatrix();
  nMech::nMath::Matrix44 &GetVPMatrix();
...
#define CASE_TEXMODANGLE(var_name)\
	if(!stricmp(#var_name,szParamName))\
{\
	var_name = nMech::nMath::Degr2Word(fValue);\
	return true;\
}\
...
struct SEfTexModificator
{
  ...
  int m_UpdateFlags;
  nMech::nMath::Matrix44 m_TexMatrix;
  nMech::nMath::Matrix44 m_TexGenMatrix;
  float m_LastUTime;
  float m_LastVTime;
...
public:
  ...
  virtual nMech::uint GetPreprocessFlags() = 0;
...
struct CMatInfo : public IMatInfo
{
  ...
  float fAlpha;
	...
	nMech::nJC::MAT_ENTITY * pMatEnt; // used by resource compiler
...
public:
	...
	CDLight( void )
  {
    ...
    m_Orientation.m_vForward = nMech::nMath::jVec3(1,0,0);
    m_Orientation.m_vUp    = nMech::nMath::jVec3(0,1,0);
    m_Orientation.m_vRight = nMech::nMath::jVec3(0,0,1);
    m_NumCM = -1;
		m_nEntityLightId = -1;
//		m_nStaticLightId = -1;
  }
  ...
  int															m_Id;
  nMech::nMath::jVec3														m_Origin;					 //world space position
  nMech::nMath::jVec3														m_BaseOrigin;					 //world space position
  CFColor													m_Color;									//!< clampled diffuse light color
  CFColor													m_BaseColor;									//!< clampled diffuse light color
  CFColor 												m_SpecColor;
  CFColor 												m_BaseSpecColor;
  nMech::nMath::jVec3														m_vObjectSpacePos;		 //Object space position
  float														m_fRadius;
  float														m_fBaseRadius;
...
//	int m_nStaticLightId;
  ...
  COrthoNormalBasis								m_Orientation;
	...
	nMech::nMath::Matrix44												m_TextureMatrix;
  ...
  IShader*												m_pShader;
  nMech::nMath::jVec3														m_ProjAngles;
  nMech::nMath::jVec3														m_BaseProjAngles;

  nMech::uint														m_Flags;									//!< flags from above (prefix DLF_)
...
*/
	...
	jcICharInstance * m_pCharInstance; // pointer to character this source is attached to
};
