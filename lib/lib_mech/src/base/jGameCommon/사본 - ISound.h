...
{
	//! Callback event.
	virtual void OnSoundEvent( ESoundCallbackEvent event,nMech::nJC_SOUND::ISound *pSound ) = 0;
};
...
typedef std::map<jcBasicString,string,stl::less_stricmp<jcBasicString> > TThemeBridgeMap;
typedef TThemeBridgeMap::iterator							TThemeBridgeMapIt;

typedef std::map<jcBasicString,SMusicMood*,stl::less_stricmp<jcBasicString> > TMoodMap;
...
typedef std::map<jcBasicString,SMusicTheme*,stl::less_stricmp<jcBasicString> >	TThemeMap;
...
struct IMusicSystem
{
	...
	virtual void GetMemoryUsage(nMech::jcISizer* pSizer) = 0;
	virtual bool LoadMusicDataFromLUA(nMech::nLUA1::IScriptSystem* pScriptSystem, const char *pszFilename) = 0;
...
extern "C"
{
	JC_GAMESOUND_API ISoundSystem* CreateSoundSystem(struct ISystem*, void *pInitData);
}
