...
namespace nMech
{
	namespace nJC_stream
	{
		...
		{
			eDoNotCompress,			// ...
			e8BitNormal,				// nMech::nMath::jVec3,														low quality normalized vector
			eWorldPos,					// nMech::nMath::jVec3,														absolute world position
			...
			e8BitRGB,						// nMech::nMath::jVec3,														8bit Color
			e4BitRGB,						// nMech::nMath::jVec3,														4bit Color
			eQuaternion,				// nMech::nMath::jVec3,														eQuaternion
			eEulerAnglesHQ,			// nMech::nMath::jVec3,														YAW,PITCH,ROLL cyclic in [0..360[, special compression if PITCH=0 (not float but still quite high quality)
			eSignedUnitValueLQ,	// float,														[-1..1] 8+1+1 bit if not zero, 1 bit of zero
		};


		struct nMech::nJC_stream::IBitStream
		{
			//!
			...
			//!
			virtual bool ReadBitStream( CStream &stm, nMech::uint16 &Value, const eBitStreamHint eHint )=0;
			//!
			...
			//!
			virtual bool ReadBitStream( CStream &stm, nMech::nMath::jVec3 &Value, const eBitStreamHint eHint )=0;
			//!
			//! max 256 characters
			...
			//!
			virtual bool WriteBitStream( CStream &stm, const nMech::uint16 Value, const eBitStreamHint eHint )=0;
			//!
			...
			//!
			virtual bool WriteBitStream( CStream &stm, const nMech::nMath::jVec3 &Value, const eBitStreamHint eHint )=0;
			//!
			//! max 256 characters
			...
			//! to get the compression error
			virtual void SimulateWriteRead( nMech::uint16 &Value, const eBitStreamHint eHint )=0;
			//! to get the compression error
			...
			//! to get the compression error
			virtual void SimulateWriteRead( nMech::nMath::jVec3 &Value, const eBitStreamHint eHint )=0;
			//! to get the compression error
