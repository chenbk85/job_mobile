...
struct IXGame : public IGame
{
	...
	*/
	virtual bool Init(struct ISystem* pSys,bool bDedicatedSrv,bool bInEditor,const char *szGameMod) = 0;
	...
	*/
	...
	/* Returns current MOD
	NULL if jGame, name of MOD otherwise
	*/
	...
	*/
	virtual bool IsSoundPotentiallyHearable(nMech::nMath::jVec3d &SoundPos, float fClipRadius)=0;
	...
	*/
	virtual void SetViewAngles(const nMech::nMath::jVec3 &angles) = 0;
	...
	*/
	virtual ITagPoint *CreateTagPoint(const string &name, const nMech::nMath::jVec3 &pos, const nMech::nMath::jVec3 &angles) = 0;
	...
	*/
	...
	// shape
	virtual IXArea *CreateArea( const nMech::nMath::jVec3 *vPoints, const int count, const std::vector<string>	&names, 
		const int type=0, const int groupId=-1, const float width=0.0f, const float height=0.0f) = 0;
	// box
	virtual IXArea *CreateArea( const nMech::nMath::jVec3& min, const nMech::nMath::jVec3& max
		, const nMech::nMath::Matrix44& TM
		, const std::vector<string>	&names, 
		const int type=0, const int groupId=-1, const float width=0.0f) = 0;
	// sphere
	virtual IXArea *CreateArea( const nMech::nMath::jVec3& center, const float radius, const std::vector<string>	&names, 
		const int type=0, const int groupId=-1, const float width=0.0f) = 0;
	...
	*/
	...
	virtual void OnSetVar(nMech::nCONSOLE::ICVar *pVar)=0;
	...
	*/
	...
	virtual int AddTimer(nMech::nLUA1::IScriptObject *pTable,unsigned int nStartTimer,unsigned int nTimer,nMech::nLUA1::IScriptObject *pUserData,bool bUpdateDuringPause)=0;
	...
	virtual nMech::nJC_stream::IBitStream *GetIBitStream()=0;
...
//DOC-IGNORE-BEGIN
//## Macros used to access jGame specific elements from it's previous IGame interface

#define CheckFarjcIGame_Begin( pGame ) \
{ \
	if ( pGame != NULL && pGame->GetInterfaceVersion() == 1 ) \
	{

#define CheckFarjcIGame_End() \
	}\
}
