...
#ifndef JC_GAME3DENGINEINTERFACE_H
#define JC_GAME3DENGINEINTERFACE_H

#include "platform.h"

#ifdef WIN32
	#ifdef _JC_3DENGINE_EXPORTS
		#define JC_GAME3DENGINEENGINE_API __declspec(dllexport)
	#else
		#define JC_GAME3DENGINEENGINE_API __declspec(dllimport)
	#endif
#else
	#define JC_GAME3DENGINEENGINE_API
#endif

// !!! Do not add any headers here !!!
#include <IProcess.h>
#include <jcEngineDecalInfo.h> 
#include "IStatobj.h"
// !!! Do not add any headers here !!!
...
inline nMech::nMath::jVec3 SVariationValue<nMech::nMath::jVec3>::GetVariantValue() const
{
	nMech::nMath::jVec3 v;
...
typedef SVariationValue<nMech::nMath::jVec3> jVec3Variant;


//! Particle Blend Type
//typedef StringID ParticleBlendType;
#if 1
...
struct ParticleParams
{
	...
	nMech::nMath::jVec3 vPosition; // Spawn position
	nMech::nMath::jVec3 vDirection; // Initial direction  (normalization not important)
	...
	nMech::nMath::jVec3 vColorStart; // Initial color
	nMech::nMath::jVec3 vColorEnd; // Final color
	...
	jVec3Variant vRotation; // Rotation speed (degree/sec)
	jVec3Variant vInitAngles; // Initial rotation
	...
	nMech::nMath::jVec3 vPositionOffset;
	// Random offset of the particle relative position to the spawn position
	nMech::nMath::jVec3 vRandomPositionOffset;
	...
	nMech::nMath::jVec3 vNormal; // Lying normal, used for physicalized particles only
	int iPhysMat; // Material for physicalized particles
	nMech::nMath::jVec3 vGravity; // Gravity vector (wind)
	nMech::nMath::jVec3 vSpaceLoopBoxSize; // if PART_FLAG_SPACELOOP is set, this box will be used to loop particles in it
};
...
struct IParticleEffect : public _i_reference_target_t
{
	...
	*/
	...
	// Summary:
	//     Spawn the particles
	virtual void Spawn( const nMech::nMath::jVec3 &pos,const nMech::nMath::jVec3 &dir,float fScale=1.0f ) = 0;
	...
	//     process - Specify for which process; 0 is the primary process and 1 is 
	//               the the child process
	//     s       - filename of the object file (a gof)
...
struct IParticleEmitter : public _i_reference_target_t
{
	...
	virtual void SetPos( const nMech::nMath::jVec3 &vPos,const nMech::nMath::jVec3 &vDir,float fScale ) = 0;
...
enum EParticleFlags
{
	...
	PART_FLAG_HORIZONTAL                 =    jBIT_0, // Flat horisontal rounds on the water
	PART_FLAG_UNDERWATER                 =    jBIT_1, // Particle will be removed if go out from outdoor water
	PART_FLAG_LINEPARTICLE               =    jBIT_2, // Draw billboarded line from vPosition to vPosition+vDirection
	PART_FLAG_SWAP_XY                    =    jBIT_3, // Alternative order of rotation (zxy)
	PART_FLAG_SIZE_LINEAR                =   jBIT_4, // Change size liner with time
	PART_FLAG_NO_OFFSET                  =   jBIT_5, // Disable centering of static objects
	PART_FLAG_DRAW_NEAR                  =   jBIT_6, // Render particle in near space (weapon)
	PART_FLAG_FOCUS_PLANE                =  jBIT_7, // Focus will spread partices along normal plane instead of direction axis.
	PART_FLAG_NO_DRAW_UNDERWATER         =  jBIT_8, // Particle will be not visible when it position is under water
	PART_FLAG_RIGIDBODY                  =  jBIT_9, // If bRealPhysics set to true, physicalizing particles as rigid bodies
	PART_FLAG_SPACELOOP                  = jBIT_10, // Lock paticles in box around emitter position, use ParticleParams::vSpaceLoopBoxSize to set box size
	PART_FLAG_SPACELIMIT                 = jBIT_11, // Limit paticles by box around emitter position, use ParticleParams::vSpaceLoopBoxSize to set box size
	PART_FLAG_SPEED_IN_GRAVITY_DIRECTION = jBIT_12, // Ignores normal passed to particle effect and always uses gravity direction for initial speed direction.
	PART_FLAG_BIND_POSITION_TO_EMITTER   = jBIT_13, // Always keep particle position binded to position of the emitter (for rockets etc...)
	PART_FLAG_BIND_EMITTER_TO_CAMERA     = jBIT_14, // Attach emitter to camera pos
	PART_FLAG_NO_INDOOR                  = jBIT_15, // Kill particle if it enters indoor
};
...
{
//DOC-IGNORE-BEGIN
	virtual void UpdatePoints(const nMech::nMath::jVec3 * pPoints, int nCount, float fHeight) = 0;
...
//DOC-IGNORE-END
	...
	virtual void SetPositionOffset(const nMech::nMath::jVec3d & vNewOffset) = 0;
};
...
struct I3DEngine : public IProcess
{
	...
	//     cam - ...
	//     bToTheScreen - ...
	virtual void SetCamera(const nMech::nMath::CCamera &cam, bool bToTheScreen=true) = 0;

	// Summary:
	//     Draw the world
	// See Also:
	//     SetCamera
	virtual bool Draw(int iFlag = 0) = 0;
...
//DOC-IGNORE-END
	...
	//     be returned.
	virtual float GetWaterLevel(const nMech::nMath::jVec3 * pvPos = NULL, nMech::nMath::jVec3 * pvFlowDir = NULL) = 0;
	...
	virtual float GetWaterLevel(IEntityRender * pEntityRender, nMech::nMath::jVec3 * pvFlowDir = NULL) = 0;
	...
	virtual void CreateDecal( const jcEngineDecalInfo & Decal )=0;
	...
	virtual void DeleteDecalsInRange( nMech::nMath::jVec3d vBoxMin, nMech::nMath::jVec3d vBoxMax, bool bDeleteBigTerrainDecals = true)=0;
...
//DOC-IGNORE-END
	...
	/*! Load gof and gaf files and creates animated object.
		Returns pointer to already loaded object with same name if found.
		Reference counting used */
	...
	// See Also:
	//     jIAniManager::MakeCharacter
	// Note:
	//     This function simply call jIAniManager::MakeCharacter
	virtual jcICharInstance * MakeCharacter(const char * cid_file_name, unsigned int dwFlags = 0)=0;
	...
	// See Also:
	//     jIAniManager::RemoveCharacter
	// Note:
	//     This function simply call jIAniManager::RemoveCharacter
	virtual void RemoveCharacter(jcICharInstance * pjcCharInstance)=0;  

	// Summary:
	//     Gets the current world color 
	virtual nMech::nMath::jVec3 GetWorldColor(bool bScaled=true)=0; // for example red at evening
  
	// Summary:
	//     Set the world color 
	virtual void SetWorldColor(nMech::nMath::jVec3 vColor)=0;

	// Summary:
	//     Set the current outdoor ambient color 
	virtual void SetOutdoorAmbientColor(nMech::nMath::jVec3d vColor)=0;
	...
	// Summary:
	//     Set the fog color
	virtual void SetFogColor(const nMech::nMath::jVec3& vFogColor)=0;
	...
	// Summary:
	//     Gets the fog color
	virtual nMech::nMath::jVec3 GetFogColor( )=0;
...
//DOC-IGNORE-BEGIN
	...
	virtual bool AddStaticObject(int nObjectID, const nMech::nMath::jVec3 & vPos, const float fScale, unsigned char ucBright=255) = 0;
	//! Removes static object from specified position (for editor)
	virtual bool RemoveStaticObject(int nObjectID, const nMech::nMath::jVec3 & vPos) = 0;
...
//DOC-IGNORE-END
	...
	virtual void OnExplosion(nMech::nMath::jVec3 vPos, nMech::nMath::jVec3 vHitDir, float fRadius, int nTexID, bool bDeformTerrain = true) = 0;

//DOC-IGNORE-BEGIN
    // Not used anymore
	//! Makes 3d waves on the water surface
	virtual void AddWaterSplash (nMech::nMath::jVec3 vPos, enum eSplashType eST, float fForce, int Id=-1) = 0;
//DOC-IGNORE-END
...
//DOC-IGNORE-END
	...
	virtual bool IsPointInWater(nMech::nMath::jVec3 vPos) = 0;

	//! Returns true if point is in the building
	//virtual bool IsPointInsideIndoors(const nMech::nMath::jVec3 & vPos) = 0;
	...
	//   nEntityLightId - ...
	//   pMatrix - ...
	virtual void AddDynamicLightSource(const class CDLight & LSource, IEntityRender * pEnt, int nEntityLightId=-1, const nMech::nMath::Matrix44* pMatrix=NULL) = 0;
	...
	virtual void ApplyForceToEnvironment(nMech::nMath::jVec3 vPos, float fRadius, float fAmountOfForce) = 0;
...
// Internal function used by the 3d engine and renderer
	...
	virtual nMech::nMath::jVec3 GetSunPosition(bool bMoveUp = true) = 0;
 
// Internal function used by the 3d engine and editor
	//! Returns light mask for this point (valid only during rendering stage)
	virtual unsigned int GetLightMaskFromPosition(const nMech::nMath::jVec3 & vPos, float fRadius=1.f) = 0;

// Internal function used by the 3d engine
	//! Returns lighting level for this point
	virtual nMech::nMath::jVec3 GetAmbientColorFromPosition(const nMech::nMath::jVec3 & vPos, float fRadius=1.f) = 0;

// Never used
	//! Returns fog volume id
	virtual int GetFogVolumeIdFromBBox(const nMech::nMath::jVec3 & vBoxMin, const nMech::nMath::jVec3 & vBoxMax) = 0;

	//! Return surface normal at specified position
	//virtual nMech::nMath::jVec3 GetTerrainSurfaceNormal(nMech::nMath::jVec3 vPos) = 0;
...
//DOC-IGNORE-END
	...
	// Return Value:
	//   An rgb value contained in a nMech::nMath::jVec3 object.
	virtual nMech::nMath::jVec3 GetOutdoorAmbientColor() = 0;
	...
	// Return Value:
	//   An rgb value contained in a nMech::nMath::jVec3 object.
	virtual nMech::nMath::jVec3 GetSunColor() = 0;

	//! check object visibility taking into account portals and terrain occlusion test
	//  virtual bool IsBoxVisibleOnTheScreen(const nMech::nMath::jVec3 & vBoxMin, const nMech::nMath::jVec3 & vBoxMax, OcclusionTestClient * pOcclusionTestClient = NULL)=0;
	//! check object visibility taking into account portals and terrain occlusion test
	//  virtual bool IsSphereVisibleOnTheScreen(const nMech::nMath::jVec3 & vPos, const float fRadius, OcclusionTestClient * pOcclusionTestClient = NULL)=0;
...
//DOC-IGNORE-END
	...
	virtual void ActivatePortal(const nMech::nMath::jVec3 &vPos, bool bActivate, IEntityRender *pEntity) = 0;

//DOC-IGNORE-BEGIN
	//! Count memory usage
	virtual void GetMemoryUsage(nMech::jcISizer * pSizer)=0;
//DOC-IGNORE-END
...
//mat: todo
	...
	//   bUseDeepness -
	//   bUseInIndoors -
	virtual void UpdateVisArea(IVisArea * pArea, const nMech::nMath::jVec3 * pPoints, int nCount, const char * szName, float fHeight, const nMech::nMath::jVec3 & vAmbientColor, bool bAfectedByOutLights, bool bSkyOnly, const nMech::nMath::jVec3 & vDynAmbientColor, float fViewDistRatio, bool bDoubleSide, bool bUseDeepness, bool bUseInIndoors) = 0;
...
//mat: todo
	...
	//! set wind direction and force nMech::nMath::jVec3(0,0,0) = no wind
	virtual void SetWindForce( const nMech::nMath::jVec3 & vWindForce ) = 0;
	...
	virtual	IVisArea * GetVisAreaFromPos(const nMech::nMath::jVec3 &vPos) = 0;	
...
//DOC-IGNORE-END
	...
	virtual INT_PTR AddStaticLightSource(const class CDLight & LSource, IEntityRender *pCreator, jcICharInstance * pjcCharInstance=NULL, const char * szBoneName=NULL) = 0;	//## AMD Port
...
//DOC-IGNORE-END
  ...
  //virtual int SetCameraFocus(const nMech::nMath::jVec3 &pPos)=0;
  ...
  virtual void CheckPhysicalized(const nMech::nMath::jVec3 & vBoxMin, const nMech::nMath::jVec3 & vBoxMax) = 0;
...
// Createjc3DEngine function type definition
typedef I3DEngine * (*PFNCREATE_JC_3DENGINE)(ISystem	* pSys,const char * szInterfaceVersion);
...
JC_GAME3DENGINEENGINE_API I3DEngine * Createjc3DEngine(ISystem	* pSys,const char * szInterfaceVersion=g3deInterfaceVersion);
