...
#include <string>
#include "jc_Math.h"								// nMech::nMath::jVec3
#include "ISystem.h"								// ISystem
#include "IStreamEngine.h"					// nMech::nJC_stream::IStreamEngine
#include "IGame.h"									// IGame
#include "INetwork.h"								// INetwork
#include "ICompressionHelper.h"			// nMech::nJC_stream::ICompressionHelper
#include "interface/ILog.h"										// ILOg
...
#define STREAM_VERIFY_TYPE_READ(a)	{																																\
	if((g_pISystem->GetStreamEngine()->GetStreamCompressionMask()&0x80)!=0)													\
		{																																																	\
		...
		public:
			...
			CStream()
			{
				...
				if (m_dwAllocatedBitSize < 1)
					g_pISystem->Error(TEXT("CStream:CStream()"));
			...
			CStream(const CStream& s)
			{
				...
				if(s.m_bStackBuffer)
				{
					...
					if (m_dwAllocatedBitSize < 1)
						g_pISystem->Error(TEXT("CStream:GetAllocatedSize"));				
				}
				else
				{
					//m_dwAllocatedBitSize = s.GetAllocatedSize();
			...
			CStream& operator =(const CStream &s)
			{
				...
				{
					g_pISystem->Error(TEXT("CStream:operator(=)"));			
				}
				Reset();
				if(s.GetSize())
				{
			...
			bool Write(const nMech::nMath::jVec3 &v);
...
#define _GROW() {if(!Resize(m_dwAllocatedBitSize*2))return false;}
		...
		inline bool CStream::WriteNumberInBits(unsigned int n,size_t nSize)
		{
			...
			if(nSize>32)
			{
				g_pISystem->Error(TEXT("CStream:WriteNumberinBits"));		
				return false;
			}
			n=n<<(32-nSize);
			nSwapped=__htonl(n);
		...
		inline bool CStream::ReadNumberInBits(unsigned int &n,size_t nSize)
		{
			...
			if(nSize>32)
			{
				g_pISystem->Error(TEXT("CStream:ReadNumberinBits ulong %d>32"),(int)nSize);		
				return false;
			}
		...
		inline bool CStream::ReadNumberInBits(int &n,size_t nSize)
		{
			...
			if(nSize>32)
			{
				g_pISystem->Error(TEXT("CStream:ReadNumberinBits int %d>32"),(int)nSize);		
				return false;
			}
			if(!ReadBits((BYTE *)&nSwapped,(nSize)))return false;
		...
		inline bool CStream::Read( char *psz, const DWORD indwBufferSize )
		{
			...
			nMech::nJC_stream::ICompressionHelper *pCHelper = Get_INetwork()->GetCompressionHelper();
		...
		inline bool CStream::Read(nMech::nMath::jVec3 &v)
		{
		...
		inline bool CStream::Write(const nMech::nMath::jVec3 &v)
		{
...
#define WRITE_COOKIE_NO(stm,c) {\
	if((g_pISystem->GetStreamEngine()->GetStreamCompressionMask()&0x8)==0  || !g_pISystem->GetIGame()->GetModuleState(EGameMultiplayer))\
	stm.Write((BYTE)(c));\
}
...
#define VERIFY_COOKIE_NO(stm,c) {\
	if((g_pISystem->GetStreamEngine()->GetStreamCompressionMask()&0x8)==0 || !g_pISystem->GetIGame()->GetModuleState(EGameMultiplayer))\
		...
		{	\
		stm.Debug();\
		g_pISystem->Error(TEXT("COOKIE ERROR %x!=%x AT %i, %s, %s"),(DWORD)cCookie,(DWORD)(c), __LINE__, __FUNCTION__,  __FILE__ );\
...
#define VERIFY_ENTITY_COOKIE_NO(stm,c) {\
	if((g_pISystem->GetStreamEngine()->GetStreamCompressionMask()&0x8)==0 || !g_pISystem->GetIGame()->GetModuleState(EGameMultiplayer))\
		...
		{\
		stm.Debug();\
		GetjILog()->Error(TEXT("Entity read cookie error %x!=%x file '%s' @ line %d (%s of class %s)"),\
