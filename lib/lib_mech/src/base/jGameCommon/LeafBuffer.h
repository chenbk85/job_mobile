...
struct TangData
{
	nMech::nMath::jVec3 tangent;
	nMech::nMath::jVec3 binormal;			
  nMech::nMath::jVec3 tnormal;			
};
namespace nMech
{
	namespace nJC
...
struct CLeafBuffer
{
  ...
  nMech::nMath::jVec3 *	      			m_TempNormals;								//!<
  ...
  _inline byte *GetNormalPtr(int& Stride, int Id=0, bool bSys=true)
  {
    ...
    Stride = sizeof(nMech::nMath::jVec3);
  ...
  nMech::uint										m_bMaterialsWasCreatedInRenderer : 1;
  nMech::uint										m_bOnlyVideoBuffer : 1;
  nMech::uint										m_bDynamic : 1;

  nMech::uint										m_UpdateVBufferMask;				//!<
  nMech::uint                    m_UpdateFrame;
  nMech::uint                    m_SortFrame;								//!< to prevent unneccessary sorting during one frame
  ...
  list2<CMatInfo> *				m_pMats;										//!<
	...
	nMech::uint *									m_arrVtxMap;												//!< [Anton] mapping table leaf buffer vertex idx->original vertex idx
  ...
  // compact buffer
  int FindInBuffer(struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F &opt, SPipTangents &origBasis, nMech::uint nMatInfo, nMech::uint *uiInfo, struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F* _vbuff, SPipTangents *_vbasis, int _vcount, list2<unsigned short> * pHash, TArray<nMech::uint>& ShareNewInfo);
  void CompactBuffer(struct_VERTEX_FORMAT_P3F_N_COL4UB_TEX2F *_vbuff, SPipTangents *_tbuff, int * _vcount, TArray<unsigned short> * pindices, bool bShareVerts[128], nMech::uint *uiInfo);

  virtual void Unload();
  virtual void UpdateCustomLighting(float fBackSideLevel, nMech::nMath::jVec3 vStatObjAmbientColor, const nMech::nMath::jVec3 & vLight, bool bCalcLighting);
  ...
  nMech::nMath::jVec3								m_vBoxMin, m_vBoxMax;							//!< used for hw occlusion test
  nMech::nMath::jVec3 *							m_pLoadedColors;									//!<

  virtual void AllocateSystemBuffer( int nVertCount );
  virtual void FreeSystemBuffer( void );
  virtual bool Serialize(int & nPos, nMech::uchar * pSaveBuffer, bool bSaveToBuffer, 
    char * szFolderName, char * szFileName, double & dCIndexedMesh__LoadMaterial);
  void SerializeMemBuffer(nMech::uchar * pSerialBuffer, nMech::uchar * pData, int nSize, bool bSaveToBuffer);
  ...
  bool LoadMaterial(int m, 
    ...
    nMech::nJC::MAT_ENTITY * me, bool bFake);
  static int SetTexType(nMech::nJC::TextureMap3 *tm);
	virtual bool UpdateTangBuffer(SPipTangents *pBasis);
	bool IsEmpty() { return !m_SecVertCount	|| !m_pSecVertBuffer || !m_SecIndices.Num(); }
	virtual void RenderDebugLightPass(const nMech::nMath::Matrix44 & mat, int nLightMask, float fAlpha);
