...
//////////////////////////////////////////////////////////////////////
//
//	n_jGame_c jcENGINE Source code
//	
...
struct SjcRenderInterface
{
  class CMalloc  *igcpMalloc;

  ILog     *ipLog;
  IConsole *ipConsole;
	nMech::nTIME::ITimer   *ipTimer;
  ISystem  *ipSystem;
  int      *ipTest_int;
...
//  nMech::nMath::jVec3 m_vCenter;
  ...
  nMech::nMath::jVec3 m_Vecs[3];

  nMech::uchar m_dwFlags;
  float m_fArea;	
};
...
public:	
  ...
  nMech::uint m_bFenceSet : 1;
  nMech::uint m_bDynamic : 1;
	int		m_vertexformat;
...
/*struct IndexedVertexBuffer
{
  ...
  int strip_step;
  nMech::nMath::jVec3 vBoxMin,vBoxMax;
};*/
...
struct IRenderer//: public IRendererCallbackServer
{
  ...
  virtual void  ReleaseIndexBuffer(SVertexStream *dest)=0;
	...
	virtual	void	Draw3dBBox(const nMech::nMath::jVec3 &mins,const nMech::nMath::jVec3 &maxs, int nPrimType=DPRIM_WHIRE_BOX)=0;
	...
	virtual	void	Draw3dPrim(const nMech::nMath::jVec3 &mins,const nMech::nMath::jVec3 &maxs, int nPrimType=DPRIM_WHIRE_BOX, const float* fRGBA = NULL)
	{
	...
	virtual	const nMech::nMath::CCamera& GetCamera()=0;
  ...
  virtual void	Draw2dImage	(float xpos,float ypos,float w,float h,int texture_id,float s0=0,float t0=0,float s1=1,float t1=1,float angle=0,float r=1,float g=1,float b=1,float a=1,float z=1)=0;
	...
	virtual void GetMemoryUsage(nMech::jcISizer* Sizer)=0;
  ...
  virtual void  GetProjectionMatrix(float *mat)=0;
	...
	//! for editor
  virtual nMech::nMath::jVec3 GetUnProject(const nMech::nMath::jVec3 &WindowCoords,const nMech::nMath::CCamera &cam)=0;

  virtual void RenderToViewport(const nMech::nMath::CCamera &cam, float x, float y, float width, float height)=0;
  ...
  virtual ITexPic			*EF_LoadTexture(const char* nameTex, nMech::uint flags, nMech::uint flags2, byte eTT, float fAmount1=-1.0f, float fAmount2=-1.0f, int Id=-1, int BindId=0)=0;
  // Load lightmap for name (name)
  virtual int			    EF_LoadLightmap (const char *name)=0;
  virtual bool			    EF_ScanEnvironmentCM (const char *name, int size, nMech::nMath::jVec3& Pos)=0;
  ...
  // Create splash
  virtual void			EF_AddSplash (nMech::nMath::jVec3 Pos, eSplashType eST, float fForce, int Id=-1)=0;
  ...
  virtual bool EF_SetLightHole(nMech::nMath::jVec3 vPos, nMech::nMath::jVec3 vNormal, int idTex, float fScale=1.0f, bool bAdditive=true)=0;
  ...
  // Return different common shader parameters (used in ShaderBrowser) jcIndEd.exe
  ...
  virtual void SetClearColor(const nMech::nMath::jVec3 & vColor)=0;
  ...
  virtual void MakeMatrix(const nMech::nMath::jVec3 & pos, const nMech::nMath::jVec3 & angles,const nMech::nMath::jVec3 & scale, nMech::nMath::Matrix44* mat)=0;
...
//////////////////////////////////////////////////////////////////////	
	...
	*/
	virtual void DrawLabelImage(const nMech::nMath::jVec3 &vPos,float fSize,int nTextureId)=0;

  virtual void DrawLabel(nMech::nMath::jVec3 pos, float font_size, const char * label_text, ...)=0;
  virtual void DrawLabelEx(nMech::nMath::jVec3 pos, float font_size, float * pfColor, bool bFixedSize, bool bCenter, const char * label_text, ...)=0;
...
//////////////////////////////////////////////////////////////////////	
  ...
  virtual void	RotateMatrix(const nMech::nMath::jVec3 & angels)=0;
	virtual void	TranslateMatrix(float x,float y,float z)=0;
	virtual void	ScaleMatrix(float x,float y,float z)=0;
	virtual void	TranslateMatrix(const nMech::nMath::jVec3 &pos)=0;
  virtual void  MultMatrix(float * mat)=0;
	virtual	void	LoadMatrix(const nMech::nMath::Matrix44 *src=0)=0;
  ...
  virtual	void UpdateTextureInVideoMemory(nMech::uint tnum, unsigned char *newdata,int posx,int posy,int w,int h,ETEX_Format eTFSrc=eTF_0888)=0;	
...
//  virtual void DrawLine(float * p1, float * p2)=0;
  ...
  virtual void DrawLine(const nMech::nMath::jVec3 & vPos1, const nMech::nMath::jVec3 & vPos2)=0;
  virtual void DrawLineColor(const nMech::nMath::jVec3 & vPos1, const CFColor & vColor1, const nMech::nMath::jVec3 & vPos2, const CFColor & vColor2)=0;
  ...
  virtual void DrawBall(const nMech::nMath::jVec3 & pos, float radius )=0;
  ...
  virtual void DrawQuad(const nMech::nMath::jVec3 &right, const nMech::nMath::jVec3 &up, const nMech::nMath::jVec3 &origin,int nFlipMode=0)=0;
  virtual void DrawQuad(float dy,float dx, float dz, float x, float y, float z)=0;
  virtual void ClearDepthBuffer()=0;  
  virtual void ClearColorBuffer(const nMech::nMath::jVec3 vColor)=0;  
  ...
  virtual unsigned int  MakeSprite(float object_scale, int tex_size, float angle, IStatObj * pStatObj, nMech::uchar * pTmpBuffer, nMech::uint def_tid)=0;
  virtual unsigned int  Make3DSprite(int nTexSize, float fAngleStep, IStatObj * pStatObj)=0;
  virtual ShadowMapFrustum * MakeShadowMapFrustum(ShadowMapFrustum * lof, ShadowMapLightSource * pLs, const nMech::nMath::jVec3 & obj_pos, list2<IStatObj*> * pStatObjects, int shadow_type)=0;
...
// Query types for jcInd editor (used in EF_Query() function)
#define EFQ_NUMEFS    0
...
struct SRendParams
{
  ...
  nMech::nMath::jVec3				vPos;
	//! scale of render elements
  float				fScale;
	//! angles of the object
  nMech::nMath::jVec3				vAngles;
	//! object transformations
  nMech::nMath::Matrix44		*pMatrix;
...
{
	virtual void	Release()=0;	
	virtual nMech::nMath::jVec3	GetPos()=0;
	virtual void	SetPos(const nMech::nMath::jVec3 &vPos)=0;
  ...
  virtual	nMech::nMath::jVec3 * GetSysVertBufer() = 0;
