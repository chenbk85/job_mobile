...
//	
//	File:jcParticleSpawnInfo.h
//	Description: declaration of struct jcParticleSpawnInfo.
//
//	History:
...
#ifndef __JCGAME_COMMON_PARTICLE_SPAWN_INFO_HDR__
#define __JCGAME_COMMON_PARTICLE_SPAWN_INFO_HDR__
...
public:
	...
	*/
	...
	// the bone number, can be determined with the jcICharModel::GetBoneByName(), should be non-negative
	unsigned nBone;
	// valid only with FLAGS_SPAWN_FROM_BONE:
	// the position of the particle in local bone coordinates
	nMech::nMath::jVec3 vBonePos;
	...
	jcParticleSpawnInfo (float _fSpawnRate, unsigned _nFlags = 0):
