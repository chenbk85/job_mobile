...
// from a DLL simpler. All files within this DLL are compiled with the JC_GAMEENTITYDLL_EXPORTS
// symbol defined on the command line. this symbol should not be defined on any project
// that uses this DLL. This way any other project whose source files include this file see
// JC_GAMEENTITYDLL_API functions as being imported from a DLL, wheras this DLL sees symbols
// defined with this macro as being exported.
#if !defined _XBOX && !defined(LINUX)
#ifdef JC_GAMEENTITYDLL_EXPORTS
#define JC_GAMEENTITYDLL_API __declspec(dllexport)
#else
#define JC_GAMEENTITYDLL_API __declspec(dllimport)
#endif
#else
#define JC_GAMEENTITYDLL_API
#endif
...
enum EScriptEventId
{
	...
	ScriptEvent_AnimationKey_Table		,
...
public:
	...
	nMech::nMath::jVec3	scale;
	nMech::nMath::Matrix44 mtx;

	IStatObj*	object;
//## [kirill] character newer used here
//##	jcICharInstance *m_pjcCharInstance;

	// flags for "spring" objects that are connected to 2 other entity parts (ETY_OBJ_IS_LINKED)
	int ipart0,ipart1;
	nMech::nMath::jVec3 link_start0,link_end0;
};
...
struct SEntityUpdateContext
{
	...
	nMech::nLUA1::IScriptObject *pScriptUpdateParams;
	//! Current rendering frame id.
	int nFrameID;
	//! Current camera.
	nMech::nMath::CCamera *pCamera;
	//! Current system time.
	float fCurrTime;
...
*/
	...
	virtual void GetHelperPosition(const char *helper, nMech::nMath::jVec3 &pos, bool objectspace = false) = 0;
  ...
  //     ClassId - The desired entity type. This must be a valid class id registered previously in the IEntityClassRegistry.
	...
	// See Also:
	//     EScriptEventId, nMech::nLUA1::IScriptObject
	virtual void SendScriptEvent(enum EScriptEventId Event, nMech::nLUA1::IScriptObject *pParamters, bool *pRet=NULL) = 0;
...
*/
  ...
  //     Enables or disables physic calculation for this entity.
	...
	virtual void AddImpulse(int ipart,nMech::nMath::jVec3 pos,nMech::nMath::jVec3 impulse,bool bPos=true,float fAuxScale=1.0f) = 0;
	...
	virtual bool CreateRigidBody(pe_type type, float density,float mass,int surface_id,nMech::nMath::jVec3* pInitialVelocity = NULL, int slot=-1,
...
/*! Load and create vehicle physics. This function actually loads a vehicle from a gof file, and according to some helper data that it finds
 in the gof file creates a physical representation of a vehicle.
	@param objfile	Name of gof file
...
/*! Load and create boat physics. This function actually loads a boat from a gof file, and according to some helper data that it finds
 in the gof file creates a physical representation of a vehicle.
	@param objfile	Name of gof file
...
/*! Add geometry object to entity, which is loaded from a gof file into an available slot in the entity. There is no limit on the number of
	gof files that can be loaded into an entity.
	...
	@param	fileName	The filename of the gof
	@param  scale	The desired scale of the object
	@return True upon successful loading of gof file
*/
	virtual bool	LoadObject( unsigned int slot,const char *fileName,float scale, const char *geomName=NULL) = 0;
	virtual bool GetObjectPos(unsigned int slot,nMech::nMath::jVec3 &pos)=0;
	virtual bool SetObjectPos(unsigned int slot,const nMech::nMath::jVec3 &pos)=0;
	virtual bool GetObjectAngles(unsigned int slot,nMech::nMath::jVec3 &ang)=0;
	virtual bool SetObjectAngles(unsigned int slot,const nMech::nMath::jVec3 &ang)=0;
/*! Load a pre-broken object into the entity, piece by piece from the gof-file. The pre broken pieces will be loaded in a separate slot
...
*/
	virtual void PlaySound(nMech::nJC_SOUND::ISound *pSound, float fSoundScale, nMech::nMath::jVec3 &Offset) = 0;
...
*/
	virtual void	SetBBox(const nMech::nMath::jVec3 &mins,const nMech::nMath::jVec3 &maxs) = 0;
...
/*! Get Axis Aligned bounding box of entity in local space.
	...
	@param maxs The value that contains the Top Right far corner of box after function completion
*/
	virtual void GetLocalBBox( nMech::nMath::jVec3 &min,nMech::nMath::jVec3 &max ) = 0;
...
*/
	virtual bool CreateParticleEntity(float size,float mass, nMech::nMath::jVec3 heading, float acc_thrust=0,float k_air_resistance=0,
...
//@{
	virtual void SetPos(const nMech::nMath::jVec3 &pos, bool bWorldOnly = true) = 0;
	virtual const nMech::nMath::jVec3 & GetPos(bool bWorldOnly = true) const = 0;

	virtual void SetPhysAngles(const nMech::nMath::jVec3 &angl) = 0;
	virtual void SetAngles(const nMech::nMath::jVec3 &pos,bool bNotifyContainer=true,bool bUpdatePhysics=true,bool forceInWorld=false) = 0;
	virtual const nMech::nMath::jVec3 & GetAngles(int realA=0) const = 0;
 ...
 */
	virtual void SetParentLocale(const nMech::nMath::Matrix44 &matParent)= 0;
...
/*! Set the script object that describes this entity in the script.
	...
	@see nMech::nLUA1::IScriptObject
*/
	virtual void SetScriptObject(nMech::nLUA1::IScriptObject *pObject) = 0;

/*! Get the script object that describes this entity in the script.
	@return	Pointer to the script object interface
	@see nMech::nLUA1::IScriptObject
*/
	virtual nMech::nLUA1::IScriptObject *GetScriptObject()= 0 ;

	virtual bool Write(nMech::nJC_stream::CStream&,EntityCloneState *cs=NULL) = 0;
	...
	*/
	virtual bool Read(nMech::nJC_stream::CStream&,bool bNoUpdate=false) = 0;
	...
	*/
	...
	/*!	serialize the object to a bitstream(file persistence)
		...
		@see nMech::nJC_stream::CStream
	*/
	virtual bool Save(nMech::nJC_stream::CStream &stm,nMech::nLUA1::IScriptObject *pStream=NULL) = 0;
	...
	*/
	...
	@see nMech::nJC_stream::CStream
	*/
	virtual bool LoadRELEASE(nMech::nJC_stream::CStream &stm,nMech::nLUA1::IScriptObject *pStream=NULL) = 0;
	...
	@see nMech::nJC_stream::CStream
	*/
	virtual bool LoadPATCH1(nMech::nJC_stream::CStream &stm,nMech::nLUA1::IScriptObject *pStream=NULL) = 0;
...
*/
	...
	//! @pObj structure to describe the hit. - script object, to be passed to script OnDamage
		virtual void OnDamage( nMech::nLUA1::IScriptObject *pObj ) = 0;
	...
	virtual int GetSide(const nMech::nMath::jVec3& direction) = 0;
...
//  virtual EntityRenderState & GetEntityRenderState() = 0;
	...
	*/
	virtual void SetCommonCallbacks(nMech::nLUA1::IScriptSystem *pScriptSystem)=0;

	//! Create particle emitter at specified slot and with specified params
	virtual int	CreateEntityParticleEmitter(int nSlotId, const ParticleParams & PartParams, float fSpawnPeriod,nMech::nMath::jVec3 vOffSet,nMech::nMath::jVec3 vDir,IParticleEffect *pEffect = 0,float fSize=1.0f ) = 0;
...
*/
	virtual void SetHandsIKTarget( const nMech::nMath::jVec3* target=NULL ) = 0;
...
/*! called by the entity when the position is changed
	...
	@see IEntity
*/
	virtual void OnSetAngles( const nMech::nMath::jVec3 &ang ) = 0;
...
/*! Get the interface which describes this container in script. Containers are not mapped to separate tables in script, but rather to a
  table which is a member of the sctipt table of the entity contained within the container. So this script object should not be used as
	...
	@see nMech::nLUA1::IScriptSystem
	@see nMech::nLUA1::IScriptSystem::BeginCall(const char*, const char *)
*/
	virtual bool Write(nMech::nJC_stream::CStream &stm,EntityCloneState *cs=NULL) = 0;
	virtual bool Read(nMech::nJC_stream::CStream &stm) = 0;

/*! Position for sound-sources attached to this entity object.
*/
	virtual nMech::nMath::jVec3 CalcSoundPos() = 0;

	virtual nMech::nLUA1::IScriptObject *GetScriptObject() = 0;
/*! Set the object that describes this container in script.
	@param object The desired script object.
*/
	virtual void SetScriptObject(nMech::nLUA1::IScriptObject *object) = 0;
...
*/
	...
	virtual void PreloadInstanceResources(nMech::nMath::jVec3d vPrevPortalPos, float fPrevPortalDistance, float fTime) = 0;
	...
	virtual void OnEntityNetworkUpdate( const EntityId &idViewerEntity, const nMech::nMath::jVec3d &v3dViewer
		, nMech::uint32 &inoutPriority,
		EntityCloneState &inoutCloneState ) const=0;

	virtual void OnAnimationEvent_XML(nMech::cstr szAni, AnimSinkEventData& ased) {}
};
...
/*! Assign character model to a slot.
	...
	@see jcICharInstance
*/
	virtual void SetCharacter( int pos,jcICharInstance *character ) = 0;
...
/*! Retrieves  character model from specified slot.
	...
	@see jcICharInstance
*/
	virtual jcICharInstance* GetCharacter( int pos ) = 0;
...
#endif
	virtual void	GetEntitiesInRadius( const nMech::nMath::jVec3 &origin, float radius, std::vector<IEntity*> &entities,int physFlags=PHYS_ENTITY_ALL ) const = 0;
...
//! Calls reset for every entity, to reset its state
	...
	//! According to the specifications in interface nMech::jcISizer
	virtual void GetMemoryStatistics(nMech::jcISizer *pSizer) = 0;
...
{
	virtual void Update(nMech::nMath::jVec3 v3Viever) = 0;
	virtual bool WriteEntity(nMech::nJC_stream::CStream &stm) = 0;
	virtual void Release() = 0;
};*/
...
{
	float m_cam_dist;
	nMech::nMath::jVec3 m_cam_dir, m_1pcam_butt_pos,m_1pcam_eye_pos;
	float m_cam_kstiffness,m_cam_kdamping;
	float m_cam_angle_kdamping;
	nMech::jIVar* m_cam_angle_kstiffness;
	...
	nMech::nMath::jVec3 m_cur_cam_rotax;
	nMech::nMath::jVec3 m_camoffset;
	float m_viewoffset;
};

typedef const nMech::nMath::jVec3d jVec3d_const_t;
struct IEntityCamera
{
	struct _STPM
	{
		_STPM(){ memset(this, 0, sizeof(*this));}

		jVec3d_const_t*center, *angles0;
		nMech::nXML::jE cameraMode; //jCamera.xml에성 CameraType에따른 노드
		float frameTime;
		float range;
		int dangleAmmount;
		IPhysicalEntity *physic;
		IPhysicalEntity *physicMore;
		I3DEngine* p3DEngine;
		float safeRange;
	private:
		friend IEntityCamera;
		friend class CEntityCamera;
		// 아래는 내부함수에서 세팅되어 사용되는 변수.
		bool bPiercing;
		float cam_ThirdPersonAngle;
		nMech::nMath::jVec3d arg_cam_angles , arg_cam_pos;
		bool isSkip_arg_cam;
	};

	virtual void Release() = 0;
	virtual void SetPos( const nMech::nMath::jVec3 &p ) = 0;
	virtual nMech::nMath::jVec3 GetPos() const = 0;
	virtual void SetAngles( const nMech::nMath::jVec3 &p ) = 0;
	virtual nMech::nMath::jVec3 GetAngles() const = 0;
	virtual void SetFov( const float &f, const unsigned int iWidth, const unsigned int iHeight ) = 0;
	virtual float GetFov() const = 0;
//	virtual void SetMatrix( const nMech::nMath::Matrix44& m ) = 0;
	virtual nMech::nMath::Matrix44 GetMatrix() const = 0;
	virtual void Update() = 0;
	virtual nMech::nMath::CCamera& GetCamera() = 0;
	virtual void SetCamera( const nMech::nMath::CCamera &cam ) = 0;
	...
	virtual void SetCamOffset(nMech::nMath::jVec3 v) = 0;
	virtual nMech::nMath::jVec3& GetCamOffset() = 0;
	virtual void SetThirdPersonMode( _STPM* pSTPM)= 0;
	virtual void SetCameraMode(const nMech::nMath::jVec3 &lookat,const nMech::nMath::jVec3 &lookat_angles, IPhysicalEntity *physic)=0;
	virtual void SetCameraOffset(const nMech::nMath::jVec3 &offset) = 0;
	virtual void GetCameraOffset(nMech::nMath::jVec3 &offset) = 0;
};

extern "C"
{
	JC_GAMEENTITYDLL_API struct IEntitySystem * CreateEntitySystem(ISystem *);
}

typedef struct IEntitySystem * (* PFNCREATEENTITYSYSTEM) (ISystem *);
...
extern "C"
{
	JC_GAMEENTITYDLL_API struct IEntitySystem * CreateMainEntitySystem(ISystem *);
}

typedef struct IEntitySystem * (* PFNCREATEMAINENTITYSYSTEM) (ISystem *);
