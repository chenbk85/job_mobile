...
struct SInpData
{
  nMech::nMath::jVec3 Org;
  nMech::nMath::jVec3 Normal;
  CFColor Color;
  int UniqLightStyle;
  int OrigLightStyle;
};
...
struct SMRendVert
{
  ...
  nMech::nMath::jVec3 vert;
  union
  {
    nMech::uint m_uiInfo;
    nMech::uchar m_bInfo[4];
  };
};
...
public:
  ...
  nMech::uint mfGetFlags(void) { return m_Flags; }
  void mfSetFlags(nMech::uint fl) { m_Flags = fl; }
  void mfUpdateFlags(nMech::uint fl) { m_Flags |= fl; }
  void mfClearFlags(nMech::uint fl) { m_Flags &= ~fl; }
  ...
  virtual void mfCenter(nMech::nMath::jVec3& centr, CCObject*pObj);
  virtual void mfGetBBox(nMech::nMath::jVec3& vMins, nMech::nMath::jVec3& vMaxs)
  {
    vMins.Set(0,0,0);
    vMaxs.Set(0,0,0);
  }
