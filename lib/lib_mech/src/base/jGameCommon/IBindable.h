...
#ifndef _JCGAME_COMMON_BINDABLE_INTERFACE_HDR_
#define _JCGAME_COMMON_BINDABLE_INTERFACE_HDR_
...
//     member functions of jcICharInstance.
struct IBindable
{
	// Summary:
	...
	// Return Value:
	//     A nMech::nMath::jVec3 object which contains the position.
	// Summary:
	//     Gets the position of a specified helper
	virtual nMech::nMath::jVec3 GetHelperPos(const char * szHelperName) = 0;
	...
	//     szHelperName - A null terminated string holding the name of the helper
	// Return Value:
	//     A nMech::nMath::Matrix44 of the object
	// Summary:
	//     Gets the matrix of a specified helper
	virtual const nMech::nMath::Matrix44 * GetHelperMatrixByName(const char * szHelperName) = 0;
	...
	// Summary:
	//     Renders the object
	virtual void Render(const struct SRendParams & rParams,const nMech::nMath::jVec3& t, int nLodLevel)=0;
	...
	virtual void StartMorph (const char* szMorphTarget, const struct jcCharMorphParams& params) {}
	...
	//     Get the character instance, if valid
	// Return Value:
	//     A pointer to an jcICharInstance object if the IBindable represent 
	//     a character instance, else the NULL value will be returned.
	virtual struct jcICharInstance* GetjcICharInstance() {return NULL;}
};
