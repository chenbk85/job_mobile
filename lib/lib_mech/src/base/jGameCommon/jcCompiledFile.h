...
//
//  File:jcCompiledFile.h
//  Description: jc Compiled File Format declarations: necessary structures,
//               chunk descriptions.
//
//	Notes on jc Compiled File Format.
...
//
//////////////////////////////////////////////////////////////////////
#ifndef __JCGAME_COMMON_JCGAME_COMPILED_FILE_FORMAT_HDR__
#define __JCGAME_COMMON_JCGAME_COMPILED_FILE_FORMAT_HDR__

// the required header for file i/o operations
#include <stdio.h>

// jc Compiled File format chunk header
...
enum CCFChunkTypeEnum
{
	...
	// the Compiled jc Geometry header, contains one structure CCFCCGHeader
	CCF_HEADER_CCG,
	...
	// an array of jcUV's - in external indexation, for immediate copying into the vertex buffer
	CCF_GI_EXT_UVS,
	...
	// the linearized hierarchy of bones: each bone represented by serialized by jcBoneDesc data
	CCF_BONE_DESC_ARRAY,
	...
	// this is the vertex skin, serialized jcSkinFull
	CCF_SKIN_VERTICES,

	// the normal skin, serialized jcSkinFull
	CCF_SKIN_NORMALS,

	// the skin representing tangents,
	CCF_SKIN_TANGENTS,
	
	// the array of materials: it's just followed by an integral number of structures nMech::nJC::MAT_ENTITY
	CCF_MATERIALS,
	...
	// the raw morph target skin (serialized from the jcSkinMorph) and then the name
...
// followed by the raw morph target skin (serialized from the jcSkinMorph) and then the name
// All the serialized skins and the name are padded for 4 byte alignment
...
struct CCFBGBone
{
	...
	// a nMech::nMath::jVec3d array of this size
...
struct CCFIntFace
{
	...
	void operator = (const jcFace& face)
	{
		v[0] = face.v0;
		v[1] = face.v1;
		v[2] = face.v2;
...
{
	// number of bones in the hierarchy; each bone should be read with Serialize function of jcBoneDesc
	unsigned numBones;
};
