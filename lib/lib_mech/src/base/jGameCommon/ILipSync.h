...
struct ILipSync
{
	virtual bool Init(ISystem* pSys, IEntity *pEntity)=0;											// initializes and prepares the character for lip-synching
	...
	virtual bool LoadDialog(const char *pszFilename, int nSoundVolume, float fMinSoundRadius, float fMaxSoundRadius, float fClipDist, int nSoundFlags=0,nMech::nLUA1::IScriptObject *pAITable=NULL)=0;														
	...
	virtual bool DoExpression(const char *pszMorphTarget, jcCharMorphParams &MorphParams, bool bAnim=true)=0;	// do a specific expression
