...
#ifndef JC_GAMEFONT_I_JC_FONT_H
#define JC_GAMEFONT_I_JC_FONT_H
...
#ifdef JC_GAMEFONT_EXPORTS
	__declspec(dllexport)
#else
	__declspec(dllimport)
#endif
#endif
struct jcIFont* CreatejcFontInterface(ISystem* pSys);

typedef jcIFont *(*PFNCREATE_JC_FONTINTERFACE)(ISystem* pSys);

//////////////////////////////////////////////////////////////////////////////////////////////
// Rendering interfaces
enum JC_GAMEFONT_RENDERINGINTERFACE
{
	_JC_FONT_RI_OPENGL = 0,		// pRIData is ignored
	_JC_FONT_RI_LAST
};
...
struct IFFont
{
	...
	virtual void SetColor(const nMech::nMath::color4f& col, int nPass = 0) = 0;
	virtual void UseRealPixels(bool bRealPixels=true)=0;
	virtual bool UsingRealPixels()=0;

	//! Set the characters base size
	virtual void SetSize(const nMech::nMath::vector2f &size) = 0;

	//! Return the seted size
	virtual nMech::nMath::vector2f &GetSize() = 0;
	...
	//! \param bASCIIMultiLine true='\','n' is a valid return, false=it's not
	virtual nMech::nMath::vector2f GetTextSize(const char *szMsg, const bool bASCIIMultiLine=true ) = 0;
	...
	//! Compute the text size
	//! \param bASCIIMultiLine true='\','n' is a valid return, false=it's not
	virtual nMech::nMath::vector2f GetTextSizeW(const wchar_t *swStr, const bool bASCIIMultiLine=true ) = 0;	

	// Compute the text size
	virtual nMech::nMath::vector2f GetWrappedTextSizeW(const wchar_t *swStr, float w, const bool bASCIIMultiLine=true ) = 0;
