...
{
	nMech::nJC::AgentParameters	 m_sParamStruct;
...
typedef struct IAIObject
{
	...
	virtual void SetPos(const nMech::nMath::jVec3 &pos,bool bKeepEyeHeight = true) = 0;
	virtual const nMech::nMath::jVec3 &GetPos() = 0;
	virtual void SetAngles(const nMech::nMath::jVec3 &angles) = 0;
	virtual const nMech::nMath::jVec3 &GetAngles() = 0;
...
//	virtual IAIObject* GetBound( ) = 0;
	virtual	void CreateBoundObject( unsigned short type, const nMech::nMath::jVec3& vBindPos, const nMech::nMath::jVec3& vBindAngl)=0;
	...
	virtual void Save(nMech::nJC_stream::CStream & stm)=0;
	virtual void Load(nMech::nJC_stream::CStream & stm)=0;
	virtual void Load_PATCH_1(nMech::nJC_stream::CStream & stm)=0;
...
{
	virtual nMech::nJC::AgentParameters GetPuppetParameters()=0;
	virtual void SetPuppetParameters(nMech::nJC::AgentParameters &pParams)=0;
} IPuppet;
...
//	bool	bLost;		// proxy would like to regenerate path
	...
	bool aimLook;
	int bodystate;
	nMech::nMath::jVec3 vFireDir;
	nMech::nMath::jVec3 vTargetPos;
	nMech::nMath::jVec3 vMoveDir;

	// jump related
	bool  jump;
	nMech::nMath::jVec3 vJumpDirection;
	float fJumpDuration;
...
typedef struct IUnknownProxy
{
	...
	virtual void Release()=0;
	virtual bool CustomUpdate(nMech::nMath::jVec3& pos, nMech::nMath::jVec3 &angles) = 0;
	...
	virtual void Load(nMech::nJC_stream::CStream &str)=0;
	virtual void Load_PATCH_1(nMech::nJC_stream::CStream &str)=0;
	virtual void Save(nMech::nJC_stream::CStream &str)=0;
...
typedef struct IVehicleProxy : public IUnknownProxy
{
	virtual void GetDimensions(int bodypos, float &eye_height, float &height) = 0;
	virtual void Reset() = 0;
	virtual nMech::nMath::jVec3	UpdateThreat( void* threat ) = 0;
	virtual nMech::nMath::jVec3	HeliAttackAdvance( SOBJECTSTATE &state ) = 0;
...
typedef struct GraphLink
{
	...
	nMech::nMath::jVec3	vEdgeCenter;
	nMech::nMath::jVec3	vWayOut;											// shows the out direction from this edge
...
typedef struct GraphNode
{
	...
	float				GetCross( const nMech::nMath::jVec3 &vCutStart, const nMech::nMath::jVec3 &vDir, const GraphLink &theLink );
	GraphLink*			FindNewLink();

/*	void AddHidePoint(const nMech::nMath::jVec3 &pos, const nMech::nMath::jVec3 &dir)
	{
			ObstacleData od;
			od.vPos = pos;
			od.vDir = dir;
...
typedef struct IGraph {
	...
	virtual GraphNode *GetEnclosing(const nMech::nMath::jVec3 &pos, GraphNode *pStart = 0 ,bool bOutsideOnly = false) = 0;
	virtual void AddHidePoint(GraphNode *pOwner, const nMech::nMath::jVec3 &pos, const nMech::nMath::jVec3 &dir) = 0;
	virtual void RemoveHidePoint(GraphNode *pOwner, const nMech::nMath::jVec3 &pos, const nMech::nMath::jVec3 &dir) = 0;
	virtual void DisableInSphere(const nMech::nMath::jVec3 &pos,float fRadius) = 0;
	virtual void EnableInSphere(const nMech::nMath::jVec3 &pos,float fRadius) = 0;
} IGraph;
