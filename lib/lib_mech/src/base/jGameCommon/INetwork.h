...
#define _INETWORK_H_
	...
	#ifdef JC_GAMENETWORK_EXPORTS
		#define JC_GAMENETWORK_API __declspec(dllexport)
	#else
		#define JC_GAMENETWORK_API __declspec(dllimport)
	#endif
...
#include "jc_Math.h"					// CjVec3
#include "IBitStream.h"				// nMech::nJC_stream::IBitStream
...
//@}
#endif
// jcNet specific errors and messages
#define NET_FACILITY_JC_NETWORK	0x02000000

#define NET_NOIMPL							MAKE_NRESULT(NET_FAIL, NET_FACILITY_JC_NETWORK, 0x01)
#define NET_SOCKET_NOT_CREATED	MAKE_NRESULT(NET_FAIL, NET_FACILITY_JC_NETWORK, 0x02)
...
struct INetwork
{
  ...
  virtual void Release() = 0;
	...
	virtual nMech::nJC_stream::ICompressionHelper *GetCompressionHelper() = 0;
	...
	virtual void OnAfterServerLoadLevel( const char *szServerName, const nMech::uint32 dwPlayerCount, const WORD wPort )=0;
...
struct IClientSink
{
	...
	*/
	virtual void OnXContextSetup(nMech::nJC_stream::CStream &stmContext) = 0;
	/*! called by the client when some data is received
		@param stmContext stream that contain the data
	*/
	virtual void OnXData(nMech::nJC_stream::CStream &stm) = 0;
...
struct IClient
{
	...
	*/
	virtual void SendReliable(nMech::nJC_stream::CStream &stm) = 0;
	/*! send unreliable data to the server
		@param stm the bitstream that store the data
	*/
	virtual void SendUnreliable(nMech::nJC_stream::CStream &stm) = 0;
	...
	*/
	virtual void ContextReady(nMech::nJC_stream::CStream &stm) = 0;
...
{
	virtual void OnReceivingPacket( const unsigned char inPacketID, nMech::nJC_stream::CStream &stmPacket, CIPAddress &ip )=0;
};
...
struct IServerSlotSink
{
	...
	*/
	virtual void OnContextReady(nMech::nJC_stream::CStream &stm) = 0; //<<FIXME>> add some level validation code
	/*! called by the serverslot when some data is received
		@param stm bitstream that store the received data
	*/
	virtual void OnData(nMech::nJC_stream::CStream &stm) = 0;
	//! 
...
struct IServerSlot
{
	...
	*/
	virtual bool ContextSetup(nMech::nJC_stream::CStream &stm) = 0;
	/*! send reliable data to the client
		@param stm the bitstream that store the data
	*/
	virtual void SendReliable(nMech::nJC_stream::CStream &stm,bool bSecondaryChannel=false) = 0;
...
struct IServer
{
	...
	*/
	...
	virtual void SetVariable(enum jcNetworkVarible eVarName,unsigned int nValue) = 0;
	...
	*/
	...
	virtual nMech::uint8 GetMaxClientID() const = 0;
};
...
// exports;
extern "C"{
	JC_GAMENETWORK_API INetwork *CreateNetwork(ISystem* pSys);
	typedef INetwork *(*PFNCREATENETWORK)(ISystem* pSys);
}
#endif //_INETWORK_H_
