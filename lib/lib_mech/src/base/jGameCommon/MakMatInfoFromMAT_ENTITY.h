#ifndef _MAKEMATINFOFROMnMecMAT_ENTITY_H_
#define _MAKEMATINFOFROMnMecMAT_ENTITY_H_

// material loading code is shared between resource compiler and 3dengine

inline int SetTexType(nMech::nJC::TextureMap3 *tm)
{
...
// szFileName - [May be NULL] the file name (without the path) of the file containing the nMech::nJC::MAT_ENTITY chunk
// szFolderName - the folder path where the file containing the nMech::nJC::MAT_ENTITY chunk resides; with the trailing shash.
//                This is used for searching textures.
inline bool CIndexedMesh__LoadMaterial(const char *szFileName, const char *szFolderName, 
                                CMatInfo & newMat, IRenderer * pRenderer, nMech::nJC::MAT_ENTITY * me)
{
  ...
  char normalmap[256]="";
  if(me->map_displ.name[0] && (me->flags & MTLFLAG_JC_SHADER))
  ...
  /*  if (nLM > 0)
  {
  Res.m_Textures[EFTT_LIGHTMAP].m_TU.m_ITexPic = g_pISystem->GetIRenderer()->EF_GetTextureByID(nLM);
  Res.m_Textures[EFTT_LIGHTMAP].m_Name = Res.m_Textures[EFTT_LIGHTMAP].m_TU.m_ITexPic->GetName();
  }
  if (nLM_LD > 0)
  {
  Res.m_Textures[EFTT_LIGHTMAP_DIR].m_TU.m_ITexPic = g_pISystem->GetIRenderer()->EF_GetTextureByID(nLM_LD);
  ...
  Res.m_Textures[EFTT_DIFFUSE].m_TexModificator.m_Rot[0] = nMech::nMath::Degr2Word(me->map_d.urot_val);
  Res.m_Textures[EFTT_DIFFUSE].m_TexModificator.m_Rot[1] = nMech::nMath::Degr2Word(me->map_d.vrot_val);
  ...
  Res.m_Textures[EFTT_BUMP].m_TexModificator.m_Rot[0] = nMech::nMath::Degr2Word(me->map_b.urot_val);
  Res.m_Textures[EFTT_BUMP].m_TexModificator.m_Rot[1] = nMech::nMath::Degr2Word(me->map_b.vrot_val);
  ...
  Res.m_Textures[EFTT_SPECULAR].m_TexModificator.m_Rot[0] = nMech::nMath::Degr2Word(me->map_s.urot_val);
  Res.m_Textures[EFTT_SPECULAR].m_TexModificator.m_Rot[1] = nMech::nMath::Degr2Word(me->map_s.vrot_val);
  ...
  Res.m_Textures[EFTT_GLOSS].m_TexModificator.m_Rot[0] = nMech::nMath::Degr2Word(me->map_g.urot_val);
  Res.m_Textures[EFTT_GLOSS].m_TexModificator.m_Rot[1] = nMech::nMath::Degr2Word(me->map_g.vrot_val);
  ...
  if (!Res.m_Textures[EFTT_DETAIL_OVERLAY].m_Name.empty())
  {
    ...
    Res.m_Textures[EFTT_DETAIL_OVERLAY].m_TexModificator.m_Rot[0] = nMech::nMath::Degr2Word(me->map_detail.urot_val);
    Res.m_Textures[EFTT_DETAIL_OVERLAY].m_TexModificator.m_Rot[1] = nMech::nMath::Degr2Word(me->map_detail.vrot_val);
  ...
  if (!Res.m_Textures[EFTT_OPACITY].m_Name.empty())
  {
    ...
    Res.m_Textures[EFTT_OPACITY].m_TexModificator.m_Rot[0] = nMech::nMath::Degr2Word(me->map_o.urot_val);
    Res.m_Textures[EFTT_OPACITY].m_TexModificator.m_Rot[1] = nMech::nMath::Degr2Word(me->map_o.vrot_val);
  ...
  if (!Res.m_Textures[EFTT_DECAL_OVERLAY].m_Name.empty())
  {
    ...
    Res.m_Textures[EFTT_DECAL_OVERLAY].m_TexModificator.m_Rot[0] = nMech::nMath::Degr2Word(me->map_o.urot_val);
    Res.m_Textures[EFTT_DECAL_OVERLAY].m_TexModificator.m_Rot[1] = nMech::nMath::Degr2Word(me->map_o.vrot_val);
...
// newMat.SetName(mName);		// set material name
  ...
  if(!pRenderer)
  { // in compiler remember also source nMech::nJC::MAT_ENTITY
    assert(newMat.pMatEnt != me);
    newMat.pMatEnt = new nMech::nJC::MAT_ENTITY;
    *newMat.pMatEnt = *me;
  }
...
#endif // _MAKEMATINFOFROMnMecMAT_ENTITY_H_
