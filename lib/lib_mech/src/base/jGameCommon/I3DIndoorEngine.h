//////////////////////////////////////////////////////////////////////
//
//	n_jGame_c Indoor Engine DLL source code
//	
//	File: jcIndoorEngine.h
//
//	History:
...
struct IndoorBaseInterface
{
	...
	nMech::nCONSOLE::ICVar			*m_pCVDrawSoftEdges;
	nMech::nCONSOLE::ICVar			*m_pCVRefreshCubeMap;
	nMech::nCONSOLE::ICVar			*m_pCVRefreshLights;
	nMech::nCONSOLE::ICVar			*m_pCVShadowsEnable;	
	nMech::nCONSOLE::ICVar			*m_pCVDrawBorderEdges;
	nMech::nCONSOLE::ICVar			*m_pCVDrawLight;	
	nMech::nCONSOLE::ICVar			*m_pCVTransparentShadows;
	nMech::nCONSOLE::ICVar			*m_pCVCLipSimple;
	nMech::nCONSOLE::ICVar			*m_pCVStaticLights;	
	nMech::nCONSOLE::ICVar			*m_pCVDrawModels;
	nMech::nCONSOLE::ICVar			*m_pCVCalcLight;
	nMech::nCONSOLE::ICVar			*m_pCVSetFence;
	nMech::nCONSOLE::ICVar			*m_pCVAmbientOnly;
	nMech::nCONSOLE::ICVar			*m_pCVDetectShader;
	nMech::nCONSOLE::ICVar			*m_pCVUpdateBuffers;
	nMech::nCONSOLE::ICVar			*m_pCVAnimatedShadows;
	//! ambient color for the building
	nMech::nCONSOLE::ICVar			*m_pcvRAmbientColor;
	nMech::nCONSOLE::ICVar			*m_pcvGAmbientColor;
	nMech::nCONSOLE::ICVar			*m_pcvBAmbientColor;
	nMech::nCONSOLE::ICVar			*m_pcvAmbientColor;
	//! M.M. for debugging stencil shadows (0 is default, use !=0 to force reacalculation of indoor stencil shadow volumes)
	nMech::nCONSOLE::ICVar			*m_pcvRecalcStencilVolumes;
};
...
struct IndoorRayIntInfo
{
	IStatObj	*pObj;
	int				nFaceIndex;		
	nMech::nMath::jVec3_tpl<float>			vPoint; //point of intersection
	float			fDist2; //squared distance from the ray's starting point
	nMech::nMath::jVec3_tpl<float>			vNormal; //normal of the polygon intersecting the ray
	int				nBuildId;
};
...
struct ItShadowVolume
{
	...
	virtual nMech::nMath::jVec3d	GetPos()=0;
	virtual void	SetPos(const nMech::nMath::jVec3d &vPos)=0;
...
{		
	...
	*/	
	virtual void			Render(const nMech::nMath::CCamera &cam)=0;
	...
	*/	
	virtual	bool			CheckInside(const nMech::nMath::jVec3d &vPos,int nBuilding=-1)=0;	
	...
	/*! Check if a point is inside the building
		...
		@return true if the point is inside the building, false otherwise		
	*/	
	virtual	bool			CheckInside(const nMech::nMath::jVec3d &vPos,int &nBuilding,int &nSectorId)=0;	
	...
	/*! Check if a point is inside the building
		...
		@return true if the point is inside the building, false otherwise		
	*/	
	virtual	bool			CheckInside(const nMech::nMath::jVec3d &vPos,tIndoorSector &tInfo)=0;	
	...
	*/		
	virtual bool	RegisterEntity(IEntityRender *pModel,const nMech::nMath::jVec3d &pos)=0;
	...
	*/		
	...
	/*! Activate or deactivate a light source
		@param szName name of the light source, as exported into the .gof file
	...
	*/		
	...
	/*! For debug purposes, should not be used
	*/		
	virtual void	RenderDebug(const nMech::nMath::CCamera &cam)=0;
	...
	*/
	virtual nMech::nMath::jVec3d			GetPosition(int nBuildId)=0;
  ...
  //! Load the buildings of the level from the xml file
	...
	*/
	...
	virtual void	SetIndoorAmbientColor(const nMech::nMath::jVec3d &Vcolor)=0;
...
//Editor's functions
	...
	*/
	virtual void	SetBuildingPos(const nMech::nMath::jVec3d &vPos,int nBuildId)=0;
	...
	@param nBuildId the building's IdNumber returned by LoadBuilding
	*/
	virtual void	SetBuildingBBox(const nMech::nMath::jVec3d &vMins,const nMech::nMath::jVec3d &vMaxs,int nBuildId)=0;
	...
	*/
	...
	@param	szFilename gof room filename to add to the building
	@param	nBuildId the building's IdNumber returned by LoadBuilding
	...
	*/
	...
	@param	nBuildId the building's IdNumber returned by LoadBuilding	
	*/
	virtual void	DrawBuilding(const nMech::nMath::CCamera &cam,int nBuildId)=0;

	/*! Get building's bbox	
	@param	vBBMin,vBBMax bbox information (filled by the function)
	@param	nBuildId the building's IdNumber returned by LoadBuilding	
	*/
	virtual void	GetBBox(nMech::nMath::jVec3d &vBBMin,nMech::nMath::jVec3d &vBBMax,int nBuilID)=0;
	...
	*/
	virtual const	char *GetHelper(int nHelperId, int nBuildId, nMech::nMath::jVec3d &vPos, nMech::nMath::Matrix44 * pMat = NULL, int * pnHelperType = NULL)=0;
	...
	*/
	...
	/*! Activate or deactivate a main light source
		...
		@param specify if the light should be either activated or deactivated
	*/		
	virtual void	ActivateMainLight(const nMech::nMath::jVec3d &vPos,bool bActivate)=0;
	...
	*/
  virtual	bool IsPointPotentiallyVisible(const nMech::nMath::jVec3d &vPos,tIndoorSector *tInfo=NULL)=0;
	...
	/*! Check visibility between sector containing the bbox and camera sector
		...
		@return	false if sector is not visible from the current camera, true otherwise		
	*/
  virtual	bool IsBBoxPotentiallyVisible(const nMech::nMath::jVec3d &vMins,const nMech::nMath::jVec3d &vMaxs,tIndoorSector *tInfo=NULL)=0;
	...
	*/
  virtual	int GetLightMaskForPoint(const nMech::nMath::jVec3d &vPos,tIndoorSector *tInfo=NULL)=0;
	...
	/*! Return the sorrounding ambient color for this point
		...
		@return	color
	 */
	virtual nMech::nMath::jVec3d GetAmbientColor(const nMech::nMath::jVec3d &vPos,tIndoorSector *tInfo=NULL)=0;
	 ...
	 */
	virtual void	ActivatePortal(const nMech::nMath::jVec3d &vPos,bool bActivate,IEntityRender *pEntity)=0;
	...
	*/
	...
	/*! adds/removes a gof to the outside part of the building (no portals / areas /occlusion)
	 @param nBuldingId	The buildingId
	 @param pObj				The gof to add	 
...
#ifndef _XBOX
#ifdef JC_GAMEINDOORENGINE_EXPORTS
#define JC_GAMEINDOORENGINE_API __declspec(dllexport)
#else
#define JC_GAMEINDOORENGINE_API __declspec(dllimport)
#endif
#else
#define JC_GAMEINDOORENGINE_API
#endif
...
JC_INDOORENGINE_API IIndoorBase* CreateBuildingManager(IndoorBaseInterface *pIndInterface);
