...
//////////////////////////////////////////////////////////////////////
//
//	n_jGame_c Source code 
//	Copyright (c) n_jGame_c 2001-2004
//	
//	File:jc/jc_Camera.h
...
#ifdef WIN64
#include "jc/jc_XOptimise.h" // workaround for Amd64 compiler
#endif
...
#define ROLL	(2)   
		...
		inline Matrix33	jcViewMatrixYPR(const Ang3 &angle);
		inline Matrix33	jcViewMatrix(const Ang3 &angle);
		...
		// All calculation are based on the JC_GAMEENGINE coordinate-system<P><P>       
		//
		...
		// from the viewer.<P><P>
		//
		// To convert from the jcEngine system into D3D9 we are just doing a clockwise rotation of 
		...
		private:		
			...
			jVec3	m_Position,m_OldPosition;	

			jVec3	m_GameAnglesDeg;		
			jVec3	m_CameraAnglesRad;		
			...
			jVec3 m_edge_nlt;		//this is the left/upper vertex of the near-plane
			jVec3 m_edge_plt;		//this is the left/upper vertex of the projection-plane 
			jVec3 m_edge_flt;		//this is the left/upper vertex of the far-clip-plane
			...
			jVec3 cltp,crtp,clbp,crbp;		//this are the 4 vertices of the projection-plane in cam-space
			...
			jVec3	m_OccPosition;		//Position for calculate occlusions (needed for portals rendering)
		...
		public:
			...
			void SetFrustumVertices(jVec3d * arrvVerts)
			{
				clbp = arrvVerts[0];
				cltp = arrvVerts[1];
				crtp = arrvVerts[2];
			...
			jVec3d GetFrustumVertex(int nId) const
			{
				switch(nId)
				{
				case 0:	return clbp;
			...
			void SetFrustumVertex(int nId, const jVec3d & vVert)
			{
				switch(nId)
				{
			...
			jVec3 m_vOffset;
			IVisArea * m_pPortal; // pointer to portal used to create this camera
			struct ScissorInfo{
		...
		public:
			...
			CCamera()
			{
				...
				for (int k=0;k<FRUSTUM_PLANES;k++)	{		m_frustum[k].Set(jVec3(0,0,0),0);	}
			...
			inline void	SetAngle(const jVec3 &newangles)	{ 
			...
			inline void	SetPos(const jVec3 &newpos)	{  m_OldPosition=m_Position;  m_Position=newpos;  m_OccPosition = newpos;	}
			inline const jVec3& GetPos() const { return(m_Position); }
			inline const jVec3& GetPosPrec()	const { return(m_OldPosition); }

			inline void	SetOccPos(const jVec3 &newpos) {   m_OccPosition = newpos;	}
			inline const jVec3& GetOccPos() const { return(m_OccPosition); }
			...
			inline jVec3 GetEdgeP() const { return m_edge_plt; }
			inline jVec3 GetEdgeN() const { return m_edge_nlt; }
			inline jVec3 GetEdgeF() const { return m_edge_flt; }
			...
			inline void	SetScale(const jVec3 &newscale)	{ m_Scale=newscale; }
			...
			bool IsPointVisible(const jVec3 &vPoint) const;
		...
		*/
		inline void CCamera::Update(int nWidth=-1,int nHeight=-1) {
			...
			Matrix33 v					=	jcViewMatrix(ca);  //with this approach its impossible to do a roll
			m_VMat							=	v*diag*t;	
			...
			m_edge_plt.y	= jc_cosf(m_fov*m_ProjectionRatio*0.5f) / jc_sinf(m_fov*m_ProjectionRatio*0.5f) * m_ViewSurfaceZ*0.50f;
			...
			jVec3 ltp,rtp,lbp,rbp;	//this are the 4 vertices of the projection-plane in cam-space
			jVec3 ltn,rtn,lbn,rbn;	//this are the 4 vertices of the near-plane in cam-space
			jVec3 ltf,rtf,lbf,rbf;	//this are the 4 vertices of the far-clip-plane in cam-space


			//-----------------------------------------------------
			//--- calculate frustum-edges of projection-plane   ---
			//-----------------------------------------------------
			ltp=iVMat*jVec3(+m_edge_plt.x,+m_edge_plt.y,+m_edge_plt.z);
			rtp=iVMat*jVec3(-m_edge_plt.x,+m_edge_plt.y,+m_edge_plt.z);
			lbp=iVMat*jVec3(+m_edge_plt.x,+m_edge_plt.y,-m_edge_plt.z);
			rbp=iVMat*jVec3(-m_edge_plt.x,+m_edge_plt.y,-m_edge_plt.z);

			//-----------------------------------------------------
			//--- calculate frustum-edges of projection-plane   ---
			//-----------------------------------------------------
			cltp=jVec3(+m_edge_plt.x,+m_edge_plt.y,+m_edge_plt.z)*Matrix33(m_VMat);
			crtp=jVec3(-m_edge_plt.x,+m_edge_plt.y,+m_edge_plt.z)*Matrix33(m_VMat);
			clbp=jVec3(+m_edge_plt.x,+m_edge_plt.y,-m_edge_plt.z)*Matrix33(m_VMat);
			crbp=jVec3(-m_edge_plt.x,+m_edge_plt.y,-m_edge_plt.z)*Matrix33(m_VMat);

			//-----------------------------------------------------
			//---     calculate frustum-edges of near-plane     ---
			//-----------------------------------------------------
			ltn=iVMat*jVec3(+m_edge_nlt.x,+m_edge_nlt.y,+m_edge_nlt.z);
			rtn=iVMat*jVec3(-m_edge_nlt.x,+m_edge_nlt.y,+m_edge_nlt.z);
			lbn=iVMat*jVec3(+m_edge_nlt.x,+m_edge_nlt.y,-m_edge_nlt.z);
			rbn=iVMat*jVec3(-m_edge_nlt.x,+m_edge_nlt.y,-m_edge_nlt.z);

			//-----------------------------------------------------
			//---  calculate frustum-edges of far-clip-plane    ---
			//-----------------------------------------------------
			ltf=iVMat*jVec3(+m_edge_flt.x,+m_edge_flt.y,+m_edge_flt.z);
			rtf=iVMat*jVec3(-m_edge_flt.x,+m_edge_flt.y,+m_edge_flt.z);
			lbf=iVMat*jVec3(+m_edge_flt.x,+m_edge_flt.y,-m_edge_flt.z);
			rbf=iVMat*jVec3(-m_edge_flt.x,+m_edge_flt.y,-m_edge_flt.z);
		...
		*/
		inline bool	CCamera::IsPointVisible(const jVec3 &p) const {
		...
		*/
		inline bool CCamera::IsAABBVisible_exact(const AABB& aabb) const
		{
			...
			jVec3 p = (aabb.min+aabb.max)*0.5f;
			...
			jVec3 v[8];
			jVec3 s0,s1,s2,s3,s4,s5;	//caabb-side-normals (....but not normalized)

			v[0] =	jVec3(caabb.min.x,caabb.min.y,caabb.min.z);
			v[1] =	jVec3(caabb.max.x,caabb.min.y,caabb.min.z);
			v[2] =	jVec3(caabb.min.x,caabb.max.y,caabb.min.z);
			v[3] =	jVec3(caabb.max.x,caabb.max.y,caabb.min.z);
			v[4] =	jVec3(caabb.min.x,caabb.min.y,caabb.max.z);
			v[5] =	jVec3(caabb.max.x,caabb.min.y,caabb.max.z);
			v[6] =	jVec3(caabb.min.x,caabb.max.y,caabb.max.z);
			v[7] =  jVec3(caabb.max.x,caabb.max.y,caabb.max.z);
			//---------------------------------------------------------------------
		...
		*/
		inline char CCamera::IsAABBVisible_hierarchical(const AABB& aabb, bool *bAllIn=0 ) const
		{
			...
			jVec3 v[8];
			jVec3 s0,s1,s2,s3,s4,s5;	//AABB-side-normals (....but not normalized)

			v[0] =	jVec3(AABB.min.x,AABB.min.y,AABB.min.z);
			v[1] =	jVec3(AABB.max.x,AABB.min.y,AABB.min.z);
			v[2] =	jVec3(AABB.min.x,AABB.max.y,AABB.min.z);
			v[3] =	jVec3(AABB.max.x,AABB.max.y,AABB.min.z);
			v[4] =	jVec3(AABB.min.x,AABB.min.y,AABB.max.z);
			v[5] =	jVec3(AABB.max.x,AABB.min.y,AABB.max.z);
			v[6] =	jVec3(AABB.min.x,AABB.max.y,AABB.max.z);
			v[7] =  jVec3(AABB.max.x,AABB.max.y,AABB.max.z);
			//---------------------------------------------------------------------
			//---            find the silhouette-vertices of the AABB            ---
			//---------------------------------------------------------------------
			unsigned long p0,p1,p2,p3,p4,p5,sideamount;
		...
		////////////////////////////////////////////////////////////////		
		inline jVec3 ConvertToRadAngles( const Ang3& v )	{	

			jVec3 vec=ConvertToRad(v);	
		...
		z is left/right	
		*/
		inline jVec3 ConvertUnitVectorToCameraAngles(const jVec3& vec)	{
			...
			else 
			{
				if (v.x!=0) 
				{
					fYaw=(float)(jc_atan2f((float)(v.y),(float)(v.x))*180.0f/gf_PI);
				}
				else 
					//lokking left/right	
					if (v.y>0) 							
				...
				fForward=(float)jc_sqrtf(v.x*v.x+v.y*v.y);
				fPitch=(float)(jc_atan2f(v.z,fForward)*180.0f/gf_PI);
				if (fPitch<0) 
					fPitch+=360;			
			}
		...
		z is left/right	
		*/
		inline jVec3 ConvertVectorToCameraAngles(const jVec3& v) {
		...
		z is left/right	
		*/
		inline Ang3 ConvertVectorToCameraAnglesSnap180(const jVec3& vec)
		{
