...
//	
//	File:jcHeaders.h
//  Definition of gof, gaf file chunks
//
//	History:
...
namespace nMech
{
	namespace nJC
	{
		enum ChunkTypes
		{
			...
			ChunkType_SourceInfo // describes the source from which the gof was exported: source max file, machine and user
		};
...
#pragma pack(push,4)
		...
		struct jcVertex
		{
			nMech::nMath::jVec3 p;			//position
			nMech::nMath::jVec3 n;			//normal vector
		};
		...
		// structure used for sorting thejcFace arrays by material id
		struct jcFaceOrderByMatID
		{
			bool operator () (const jcFace& faceLeft, const jcFace& faceRight)const
			{
		...
		{
			int			time;		//in ticks
			nMech::nMath::jVec3	abspos;		//absolute position;

			nMech::nMath::jVec3	relpos;		//relative position;
			nMech::nMath::jcQuat		relquat;	//relative quaternion
		...
		struct jcTexFace
		{
			...
			jcTexFace(){}
			jcTexFace (int _t0,int _t1, int _t2):t0(_t0),t1(_t1),t2(_t2) {}
		...
		*/
		...
		{
			int			BoneID;
			nMech::nMath::jVec3	offset;
			float		Blending;
		};

		// structure used to sort thejclink array by the blending factor, descending
		struct jcLinkOrderByBlending
		{
			bool operator () (const jcLink& left, const jcLink& right)
			{
...
#include "jc/jMTL_CHUNK.h"
		...
		struct LIGHT_CHUNK_DESC_0351
		{
			...
			bool 		on;				// light is on
			nMech::nMath::jVec3	pos;			// world position
			nMech::nMath::Matrix44	tm;				// transformation matrix
			jcIRGB		color;			// RGB color
		...
		struct LIGHT_CHUNK_DESC_0744
		{
			...
			nMech::nMath::jVec3		vDirection;			//spot light direction
...
#define SCENEPROPS_CHUNK_DESC_VERSION SCENEPROPS_CHUNK_DESC::VERSION
		...
		struct jcCustomAttribute
		{
			// One of jcCustomAttribFlags.
			int type;
			char name[32];
			char value[64];
		};
...
#define MTLLIST_CHUNK_DESC_VERSION MTLLIST_CHUNK_DESC::VERSION
		...
		struct BaseKey3 : BaseKey { nMech::nMath::jVec3	val; };
		struct BaseKeyQ : BaseKey { nMech::nMath::jcQuat	val; };

		struct jcLin1Key : BaseKey1 {};
		struct jcLin3Key : BaseKey3 {};
		struct jcLinQKey : BaseKeyQ {};
		struct jcTCB1Key : BaseKey1 , BaseTCB {};
		struct jcTCB3Key : BaseKey3 , BaseTCB {};
		struct jcTCBQKey : BaseKeyQ , BaseTCB {};
		struct jcBez1Key : BaseKey1 { float		intan, outtan; };
		struct jcBez3Key : BaseKey3 { nMech::nMath::jVec3	intan, outtan; };
		struct jcBezQKey : BaseKeyQ {};
		struct jcBoneKey : BaseKey
		{
			nMech::nMath::jVec3	abspos;
			nMech::nMath::jVec3	relpos;
			nMech::nMath::jcQuat	relquat;
		};
		...
		struct jcKeyPQLog
		{
			...
			nMech::nMath::jVec3 vRotLog; // logarithm of the rotation
...
#define JC_RENDERMODE_SHADOW   0
#define JC_RENDERMODE_TEXTURED 1
#define JC_RENDERMODE_BUMP     2
#define JC_RENDERMODE_REFRACT  3
		...
		struct NODE_CHUNK_DESC_0823
		{
			...
			nMech::nMath::Matrix44	tm;				// transformation matrix
			nMech::nMath::jVec3	pos;			// pos component of matrix
			nMech::nMath::jcQuat		rot;			// rotation component of matrix
			nMech::nMath::jVec3	scl;			// scale component of matrix
...
#define NODE_CHUNK_DESC_VERSION NODE_CHUNK_DESC::VERSION
		...
		struct HELPER_CHUNK_DESC_0744
		{
			...
			nMech::nMath::jVec3	size;			// size in local x,y,z axises (for dummy only)
		};
...
#define HELPER_CHUNK_DESC_VERSION HELPER_CHUNK_DESC::VERSION
		...
		struct SBoneLightBind
		{
			...
			nMech::nMath::jVec3 vLightOffset;         // light position in the parent bone coordinate system
			nMech::nMath::jVec3 vRotLightOrientation; // logarithm of quaternion that describes the light orientation relative to the parent bone
		};
		...
		struct SMeshMorphTargetVertex
		{
			...
			nMech::nMath::jVec3 ptVertex;     // the target point of the morph target
		};
		...
		struct SBoneInitPosMatrix
		{
			...
			const nMech::nMath::jVec3& getOrt (int nOrt)const {return *(const nMech::nMath::jVec3*)(mx[nOrt]);}
		};
