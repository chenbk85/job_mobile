...
namespace nMech
{
	namespace nMath
	{
		...
		namespace NAABB_SV
		{
			...
			//***************************************************************************************
			void AABB_ReceiverShadowVolume(const jVec3& PointLight, const AABB& Occluder, Shadowvolume &sv);
			...
			//***************************************************************************************
			void AABB_ShadowVolume(const jVec3& PointLight, const AABB& Occluder, Shadowvolume &sv, f32 lightrange);	
		...
		inline void NAABB_SV::AABB_ReceiverShadowVolume(const jVec3& PointLight, const AABB& Occluder, Shadowvolume &sv)
		{
			sv.sideamount=0;
			sv.nplanes=0;
			...
			if (back&0x01) { sv.oplanes[sv.nplanes].SetPlane(jVec3(-1,+0,+0),Occluder.min); sv.nplanes++; } 
			if (back&0x02) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+1,+0,+0),Occluder.max); sv.nplanes++; }
			if (back&0x04) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,-1,+0),Occluder.min); sv.nplanes++; } 
			if (back&0x08) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,+1,+0),Occluder.max); sv.nplanes++; }
			if (back&0x10) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,+0,-1),Occluder.min); sv.nplanes++; } 
			if (back&0x20) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,+0,+1),Occluder.max); sv.nplanes++; }
			...
			jVec3 o[8] = 
			{ 
				jVec3(Occluder.min.x,Occluder.min.y,Occluder.min.z),
					jVec3(Occluder.max.x,Occluder.min.y,Occluder.min.z),
					jVec3(Occluder.min.x,Occluder.max.y,Occluder.min.z),
					jVec3(Occluder.max.x,Occluder.max.y,Occluder.min.z),
					jVec3(Occluder.min.x,Occluder.min.y,Occluder.max.z),
					jVec3(Occluder.max.x,Occluder.min.y,Occluder.max.z),
					jVec3(Occluder.min.x,Occluder.max.y,Occluder.max.z),
					jVec3(Occluder.max.x,Occluder.max.y,Occluder.max.z)
			};
		...
		inline void NAABB_SV::AABB_ShadowVolume(const jVec3& PointLight, const AABB& Occluder, Shadowvolume &sv, f32 lightrange) 
		{
			sv.sideamount=0;
			sv.nplanes=0;
			...
			if (front&0x01) { sv.oplanes[sv.nplanes].SetPlane(jVec3(-1,+0,+0),Occluder.min); sv.nplanes++; } 
			if (front&0x02) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+1,+0,+0),Occluder.max); sv.nplanes++; }
			if (front&0x04) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,-1,+0),Occluder.min); sv.nplanes++; } 
			if (front&0x08) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,+1,+0),Occluder.max); sv.nplanes++; }
			if (front&0x10) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,+0,-1),Occluder.min); sv.nplanes++; } 
			if (front&0x20) { sv.oplanes[sv.nplanes].SetPlane(jVec3(+0,+0,+1),Occluder.max); sv.nplanes++; }
			...
			jVec3 MiddleOfOccluder	=	(Occluder.max+Occluder.min)*0.5f;
		...
		inline bool NAABB_SV::Is_AABB_In_ShadowVolume( const Shadowvolume &sv,  const AABB& Receiver ) 
		{
			...
			f32 d;
			const jVec3* pAABB=&Receiver.min;
