...
#ifndef JC_GAMEOVERLAP_H
#define JC_GAMEOVERLAP_H
...
namespace nMech
{
	namespace nMath
	{
		...
		namespace Overlap {
			...
			inline bool	Point_AABB(const jVec3 &p, const jVec3 &mins,const jVec3 &maxs)
			{
			...
			*
			*/
			ILINE bool	Point_OBB(const jVec3& p, const jVec3& wpos, const OBB& obb) {
				AABB aabb=AABB(obb.c-obb.h,obb.c+obb.h);
				jVec3 t=(p-wpos)*obb.m33;
			...
			*
			*/
			inline bool Lineseg_AABB ( const Lineseg &ls, const jVec3 &pos, const AABB &aabb ) {
				//calculate the half-length-vectors of the AABB
				jVec3 h	=	(aabb.max-aabb.min)*0.5f;
				//"t" is the transfer-vector from one center to the other
				jVec3 t		= ((ls.start+ls.end)*0.5f - ((aabb.max+aabb.min)*0.5f+pos));
				//calculate line-direction
				jVec3 ld  =  (ls.end-ls.start)*0.5f;
			...
			*
			*/
			inline bool Lineseg_OBB ( const Lineseg &ls, const jVec3 &pos, const OBB &obb ) {
				//the new center-position of Lineseg and OBB in world-space
				jVec3 wposobb	=	obb.m33*obb.c + pos;
				jVec3 wposls	=	(ls.start+ls.end)*0.5f;
				//"t" is the transfer-vector from one center to the other
				jVec3 t		= (wposls - wposobb)*obb.m33;
				//calculate line-direction in local obb-space
				jVec3 ld  =  ( (ls.end-ls.start)*obb.m33 )*0.5f;
			...
			* 
			*/
			inline bool Line_Triangle( const Line &line, const jVec3 &v0, const jVec3 &v1, const jVec3 &v2 ) {
				//find vectors for two edges sharing v0
				jVec3 edge_1 = v1-v0;
				jVec3 edge_2 = v2-v0;
				//begin calculating determinant - also used to calculate U parameter
				jVec3 pvec  =  line.direction % edge_1; 
				//if determinat is near zero, line lies in plane of triangel 
				float det = edge_2 | pvec;
				if (det<=0) return 0;
				//calculate distance from v0 to line origin
				jVec3 tvec=line.pointonline-v0;
				float u=tvec | pvec;
				if ( (u<0.0f) || (u>det)) return 0;
				//prepare to test V parameter
				jVec3 qvec=tvec % edge_2;
			...
			*/
			inline bool Ray_Triangle( const Ray &ray, const jVec3 &v0, const jVec3 &v1, const jVec3 &v2 ) {
				//find vectors for two edges sharing v0
				jVec3 edge_1 = v1-v0;
				jVec3 edge_2 = v2-v0;
				//begin calculating determinant - also used to calculate U parameter
				jVec3 pvec  =  ray.direction % edge_1; 
				//if determinat is near zero, ray lies in plane of triangle 
				float det = edge_2 | pvec;
				if (det<=0) return 0;
				//calculate distance from v0 to ray origin
				jVec3 tvec=ray.origin-v0;
				...
				jVec3 output = (ray.direction*t)+ray.origin;
			...
			*/
			inline bool Lineseg_Triangle( const Lineseg &lineseg, const jVec3 &v0, const jVec3 &v1, const jVec3 &v2 ) {
				//find vectors for two edges sharing v0
				jVec3 edge_1 = v1-v0;
				jVec3 edge_2 = v2-v0;
				//direction of lineseg. normalizing is not necessary
				jVec3 direction=(lineseg.end-lineseg.start);
				//begin calculating determinant - also used to calculate U parameter
				jVec3 pvec  =  direction % edge_1; 
				//if determinat is near zero, ray lies in plane of triangle 
				float det = edge_2 | pvec;
				if (det<=0) return 0;
				//calculate distance from v0 to ray origin
				jVec3 tvec=lineseg.start-v0;
				//calculate U parameter and test bounds
				float u=tvec | pvec;
				if (u<0.0f || u>det) return 0;
				//prepare to test V parameter
				...
				jVec3 output = (direction*t)+lineseg.start;
			...
			*/
			ILINE char Sphere_AABB_Inside(  const Sphere &s, const AABB& aabb ) {
				if ( Sphere_AABB(s,aabb) ) {
					jVec3 amin=aabb.min-s.center;
					jVec3 amax=aabb.max-s.center;
			...
			inline bool Sphere_OBB( const Sphere &s, const OBB &obb ) {
				...
				jVec3 SphereInOBBSpace	=	s.center*obb.m33;
			...
			inline bool Sphere_Sphere( const Sphere &s1, const Sphere &s2 ) {
				jVec3 distc	=	s1.center-s2.center;
			...
			template<typename F>
				ILINE bool Sphere_Triangle( const Sphere &s, const Triangle_tpl<F> &t ) {
					...
					jVec3_tpl<F> middle=(t.v0+t.v1+t.v2)*(1/3.0f);
					jVec3_tpl<F> ov0=t.v0-middle;
					jVec3_tpl<F> ov1=t.v1-middle;
					jVec3_tpl<F> ov2=t.v2-middle;
					F SqRad1=(ov0|ov0);
				...
				*
				*/
				inline bool OBB_Plane( const jVec3 &pos, const OBB &obb, const Plane &plane ) {
					//the new center-position in world-space
					jVec3 p	=	obb.m33*obb.c + pos;
					//extract the orientation-vectors from the columns of the 3x3 matrix
					//and scale them by the half-lengths
					jVec3 ax	= jVec3(obb.m33(0,0), obb.m33(1,0), obb.m33(2,0))*obb.h.x;
					jVec3 ay	= jVec3(obb.m33(0,1), obb.m33(1,1), obb.m33(2,1))*obb.h.y;
					jVec3 az	= jVec3(obb.m33(0,2), obb.m33(1,2), obb.m33(2,2))*obb.h.z;
				...
				*
				*/
				inline bool AABB_Triangle ( const AABB &aabb,  const jVec3 &tv0, const jVec3 &tv1, const jVec3 &tv2 ) {

					//------ convert AABB into half-length AABB -----------
					jVec3 h	=	(aabb.max-aabb.min)*0.5f;	//calculate the half-length-vectors
					jVec3 c	=	(aabb.max+aabb.min)*0.5f;	//the center is relative to the PIVOT
				...
				*
				*/
				inline bool OBB_Triangle( const jVec3 &pos, const OBB &obb,  const jVec3 &tv0, const jVec3 &tv1, const jVec3 &tv2 ) {

					jVec3 p	=	obb.m33*obb.c + pos;									//the new center-position in world-space

					//move everything so that the boxcenter is in (0,0,0)
					jVec3 v0	=	(tv0-p)*obb.m33; //pre-transform
					jVec3 v1	=	(tv1-p)*obb.m33; //pre-transform
					jVec3 v2	=	(tv2-p)*obb.m33; //pre-transform
				...
				*
				*/
				ILINE bool AABB_AABB( const jVec3 &pos1,const AABB &aabb1,  const jVec3 &pos2,const AABB &aabb2 ) {
				...
				*
				*/
				inline bool OBB_OBB ( const jVec3 &pos1,const OBB &obb1,  const jVec3 &pos2,const OBB &obb2 ) {

					//tranform obb2 in local space of obb1
					Matrix33 M=obb1.m33.T()*obb2.m33;

					//the new center-position in world-space
					jVec3 p1	=	obb1.m33*obb1.c + pos1;
					jVec3 p2	=	obb2.m33*obb2.c + pos2;

					//"t" is the transfer-vector from one center to the other
					jVec3 t		= (p2-p1)*obb1.m33;
					...
					if( fabsf(t|jVec3(M(0,0),M(1,0),M(2,0))) > (ra+obb2.h.x) ) return 0;
					ra	= fabsf(M(0,1)*obb1.h.x) + fabsf(M(1,1)*obb1.h.y) + fabsf(M(2,1)*obb1.h.z);
					if( fabsf(t|jVec3(M(0,1),M(1,1),M(2,1))) > (ra+obb2.h.y) ) return 0;
					ra	= fabsf(M(0,2)*obb1.h.x) + fabsf(M(1,2)*obb1.h.y) + fabsf(M(2,2)*obb1.h.z);
					if( fabsf(t|jVec3(M(0,2),M(1,2),M(2,2))) > (ra+obb2.h.z) ) return 0;
...
#define	PLANE_NON_AXIAL 3
				...
				inline bool	PointInTriangle(const jVec3& point, const jVec3& v0,const jVec3& v1,const jVec3& v2,const jVec3& normal)
				{
