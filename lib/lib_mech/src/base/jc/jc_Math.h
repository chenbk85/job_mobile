...
#ifndef JC_GAMEMATH_H
#define JC_GAMEMATH_H
...
#define ILINE __forceinline
		...
		ILINE void jc_sincosf (float angle, float* pCosSin) {	fastsincosf(angle,pCosSin);	}
		ILINE void jc_sincos  (double angle, double* pCosSin) {	pCosSin[0] = cos(angle);	pCosSin[1] = sin(angle); }
		ILINE float jc_sinf(float x) {return fastsinf(x); }
		ILINE float jc_cosf(float x) {return fastcosf(x); }

		ILINE float jc_fmod(float x, float y) {return (float)fmod((double)x,(double)y);}

		ILINE float jc_asinf(float x) {return (float)asin((double)x);}
		ILINE float jc_acosf(float x) {return (float)acos((double)x);}
		ILINE float jc_atanf(float x) {return (float)atan((double)x);}
		ILINE float jc_atan2f(float x, float y) {return (float)atan2((double)x,(double)y);}

		ILINE float jc_tanhf(float x) {	double expz = exp(double(x)), exp_z = exp(-double(x));	return (float)((expz-exp_z)/(expz+exp_z)); }
		ILINE float jc_tanf(float x) {return (float)tan((double)x);}

		ILINE float jc_sqrtf(float x) {return (float)sqrt((double)x);}
		ILINE float jc_fabsf(float x) {return (float)fabs((double)x);}
		ILINE float jc_expf(float x) {return (float)exp((double)x);}
		ILINE float jc_logf(float x) {return (float)log((double)x);}
		ILINE float jc_powf(float x, float y) {return (float) pow((double)x,(double)y);}

		ILINE float jc_ceilf(float x) {return (float)ceil((double)x);}
		ILINE float jc_floorf(float x) {return (float)floor((double)x);}

		ILINE double jc_sinh(double z) {return (exp (z) - exp (-z)) * 0.5;}
		ILINE double jc_cosh(double z) {return (exp (z) + exp (-z)) * 0.5;}
#endif
		...
		//-------------------------------------------
#if defined(_CPU_X86) && defined(_MSC_VER) && !defined(_WIN64)
#define ILINE __forceinline

		// calculates the cosine and sine of the given angle in radians 
		ILINE void jc_sincosf (float angle, float* pCosSin) {
			__asm {
				FLD         DWORD PTR       angle
					FSINCOS
					MOV         EAX,pCosSin
		...
		// calculates the cosine and sine of the given angle in radians 
		ILINE void jc_sincos (double angle, double* pCosSin) {
			__asm {
				FLD         QWORD PTR       angle
					FSINCOS
					MOV         EAX,pCosSin
		...
		ILINE float jc_sinf(float x) {return sinf(x);}
		ILINE float jc_cosf(float x) {return cosf(x);}
		ILINE float jc_fmod(float x, float y) {return (float)fmodf(x,y);}
		ILINE float jc_ceilf(float x) {return ceilf(x);}
		ILINE float jc_asinf(float x) {return asinf(x);}
		ILINE float jc_acosf(float x) {return acosf(x);}
		ILINE float jc_atanf(float x) {return atanf(x);}
		ILINE float jc_atan2f(float x, float y) {return atan2f(x,y);}
		ILINE float jc_sqrtf(float x) {return sqrtf(x);}
		ILINE float jc_tanhf(float x) {return tanhf(x);}
		ILINE float jc_fabsf(float x) {return fabsf(x);}
		ILINE float jc_expf(float x) {return expf(x);}
		ILINE float jc_logf(float x) {return logf(x);}
		ILINE float jc_floorf(float x) {return floorf(x);}
		ILINE float jc_tanf(float x) {return tanf(x);}
		ILINE float jc_powf(float x, float y) {return powf(x,y);}
#else

#define ILINE inline

		ILINE void jc_sincosf (float angle, float* pCosSin) { 	pCosSin[0] = (float)cos(angle);	pCosSin[1] = (float)sin(angle); 	}
		ILINE void jc_sincos (double angle, double* pCosSin) {	pCosSin[0] = cos(angle);	pCosSin[1] = sin(angle); 	}
		ILINE float jc_sinf(float x) {return sinf(x);}
		ILINE float jc_cosf(float x) {return cosf(x);}
		ILINE float jc_fmod(float x, float y) {return (float)fmodf(x,y);}
		...
		ILINE void sincos_tpl(double angle, double* pCosSin) { jc_sincos(angle,pCosSin); }
		ILINE void sincos_tpl(float angle, float* pCosSin) { jc_sincosf(angle,pCosSin); }

		ILINE double cos_tpl(double op) { return cos(op); }
		ILINE float  cos_tpl(float op) { return jc_cosf(op); }

		ILINE double sin_tpl(double op) { return sin(op); }
		ILINE float  sin_tpl(float op) { return jc_sinf(op); }

		ILINE double acos_tpl(double op) { return acos(op); }
		ILINE float  acos_tpl(float op) { return jc_acosf(op); }

		ILINE double asin_tpl(double op) { return asin(op); }
		ILINE float  asin_tpl(float op) { return jc_asinf(op); }

		ILINE double atan_tpl(double op) { return atan(op); }
		ILINE float  atan_tpl(float op) { return jc_atanf(op); }

		ILINE double atan2_tpl(double op1,double op2) { return atan2(op1,op2); }
		ILINE float  atan2_tpl(float op1,float op2) { return jc_atan2f(op1,op2); }

		ILINE double exp_tpl(double op) { return exp(op); }
		ILINE float  exp_tpl(float op) { return jc_expf(op); }

		ILINE double log_tpl(double op) { return log(op); }
		ILINE float  log_tpl(float op) { return jc_logf(op); }

		ILINE double sqrt_tpl(double op) { return sqrt(op); }
		ILINE float  sqrt_tpl(float op) { return jc_sqrtf(op); }

		ILINE double fabs_tpl(double op) { return fabs(op); }
		ILINE float  fabs_tpl(float op) { return jc_fabsf(op); }
		ILINE int    fabs_tpl(int op) { int mask=op>>31; return op+mask^mask; }

		ILINE int    floor_tpl(int op) {return op;}
		ILINE float  floor_tpl(float op) {return jc_floorf(op);}
		ILINE double floor_tpl(double op) {return floor(op);}

		ILINE int    ceil_tpl(int op) {return op;}
		ILINE float  ceil_tpl(float op) {return jc_ceilf(op);}
		ILINE double ceil_tpl(double op) {return ceil(op);}

		ILINE float  tan_tpl(float op) {return jc_tanf(op);}
...
namespace nMech
{
	namespace nMath
	{
		...
		public:
			...
			template<class F> unused_marker& operator,(jVec3_tpl<F> &x) { return *this,x.x; }
		...
		template<class F> bool is_unused(const jVec3_tpl<F> &x) { return is_unused(x.x); }
...
#define MARK_UNUSED nMech::nMath::unused_marker(),
...
#if defined(PHYSICS_EXPORTS) || (defined(WIN64) && defined(JC_GAMEENTITYDLL_EXPORTS))  || (defined(LINUX))
#ifdef max
#undef max
#endif
#ifdef min
