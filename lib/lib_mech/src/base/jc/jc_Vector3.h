...
#ifndef jc_vector3_header_232_h
#define jc_vector3_header_232_h
...
#define DEG2RAD( a ) ( (a) * (nMech::nMath::gf_PI/180.0f) )
		#define RAD2DEG( a ) ( (a) * (180.0f/nMech::nMath::gf_PI) )


		#if defined(LINUX)
		template <class F> struct jVec3_tpl;
		template<class F> 
		F GetLengthSquared( const jVec3_tpl<F> &v );
		template<class F> 
		F GetLength( const jVec3_tpl<F>& v );
		#endif
		...
		public:
			...
			jVec3_tpl() {};
			jVec3_tpl(type_zero) { x=y=z=0; }
			jVec3_tpl(type_min) { x=y=z=-100; }
			jVec3_tpl(type_max) { x=y=z=100; }

			jVec3_tpl( const F vx, const F vy, const F vz ) { x=vx; y=vy; z=vz; };
			...
			//f32 farray[3]={1,2,3};
			//jVec3 v=jVec3(farray);
			//
			//f32* fptr;
			//jVec3 newv=jVec3(fptr);
			//
			//Matrix44 tm=GetTranslationMat(jVec3(44,55,66));
			//jVec3 tvec=jVec3(tm[3]); //really dangerous! Should be replaced by "GetTranslation(tm)"
			template <class T>
			explicit jVec3_tpl(const T *src) { x=src[0]; y=src[1]; z=src[2]; }

			//CONSTRUCTOR: implement the copy/casting/assignement constructor:	
			template <class T>
			ILINE jVec3_tpl( const jVec3_tpl<T>& v ) {	x=(F)v.x;	y=(F)v.y;	z=(F)v.z; }

			//overload = operator to copy f64=doube or f32=f32
			//jVec3_tpl<f32>=jVec3_tpl<f32>
			//jVec3_tpl<f64>=jVec3_tpl<f64>
			template <class T>
			ILINE jVec3_tpl& operator=(const jVec3_tpl<T> &v) { x=v.x; y=v.y; z=v.z; return *this; }
			...
			ILINE F Dot (const jVec3_tpl<F> &vec2)	const	{ return x*vec2.x + y*vec2.y + z*vec2.z; }
			//two methods for a "cross-product" operation
			ILINE jVec3_tpl<F> Cross (const jVec3_tpl<F> &vec2) const	{	return jVec3_tpl<F>( y*vec2.z  -  z*vec2.y,     z*vec2.x -    x*vec2.z,   x*vec2.y  -  y*vec2.x); 	}	


			ILINE jVec3_tpl<F> operator*(F k) const { return jVec3_tpl<F>(x*k,y*k,z*k); }
			ILINE jVec3_tpl<F> operator/(F k) const { k=(F)1.0/k; return jVec3_tpl<F>(x*k,y*k,z*k); }
			ILINE friend jVec3_tpl operator * (f32 f, const jVec3_tpl &vec)	{ return jVec3_tpl(f*vec.x, f*vec.y, f*vec.z); }


			ILINE jVec3_tpl<F>& operator *= (F k) { x*=k;y*=k;z*=k; return *this; }
			ILINE jVec3_tpl<F>& operator /= (F k) { k=(F)1.0/k; x*=k;y*=k;z*=k; return *this; }



			//flip vector
			ILINE jVec3_tpl<F> operator - ( void ) const { return jVec3_tpl<F>(-x,-y,-z); }
		//	ILINE jVec3_tpl&	Negate() { x=-x; y=-y; z=-z; return *this; }	
			ILINE jVec3_tpl& Flip()   { x=-x; y=-y; z=-z; return *this; }
			ILINE jVec3_tpl& flip()   { x=-x; y=-y; z=-z; return *this; }



			ILINE friend bool operator ==(const jVec3_tpl<F> &v0, const jVec3_tpl<F> &v1)	{
				return ((v0.x==v1.x) && (v0.y==v1.y) && (v0.z==v1.z));
			}
			ILINE friend bool operator !=(const jVec3_tpl<F> &v0, const jVec3_tpl<F> &v1)	{ 
				return !(v0==v1); 
			}
			ILINE friend bool IsEquivalent(const jVec3_tpl<F>& v0, const jVec3_tpl<F>& v1, f32 epsilon ) {
				return  ((fabs_tpl(v0.x-v1.x) <= epsilon) &&	(fabs_tpl(v0.y-v1.y) <= epsilon)&&	(fabs_tpl(v0.z-v1.z) <= epsilon));	
			}
			ILINE friend bool IsEquivalent(const jVec3_tpl<F>& v0, const jVec3_tpl<F>& v1 ) {
			...
			//common methods
			////////////////////////////////////////////////////////////////
			ILINE jVec3_tpl& zero() { x=y=z=0; return *this; }
			ILINE jVec3_tpl<F>& Set(const F xval,const F yval, const F zval) { x=xval; y=yval; z=zval; return *this; }
			...
			ILINE friend jVec3_tpl<F> GetNormalized( const jVec3_tpl<F> &v ) {
				F vlength = nMath::GetLength(v);	
				assert(vlength>0.0f);
			...
			ILINE jVec3_tpl& normalize() { 
				F rlen=sqrt_tpl(x*x+y*y+z*z); 
				//assert(rlen>0.00001f);
				if (rlen>0) { rlen=(F)1.0/rlen; x*=rlen;y*=rlen;z*=rlen; } 
				else Set(0,0,1); return *this; 
			}
			ILINE jVec3_tpl normalized() const { 
				F rlen=sqrt_tpl(x*x+y*y+z*z);
				//assert(rlen>0.00001f);
				if (rlen>0) { rlen=(F)1.0/rlen; return jVec3_tpl(x*rlen,y*rlen,z*rlen); } 
				else return jVec3_tpl(0,0,1);
			}

			ILINE jVec3_tpl GetNormalized() const { return normalized(); } // vlad: all get functions should begin from Get...
			...
			ILINE friend F	GetSquaredDistance(const jVec3_tpl<F> &vec1, const jVec3_tpl<F> &vec2)	{		
				return (vec2.x-vec1.x)*(vec2.x-vec1.x)+(vec2.y-vec1.y)*(vec2.y-vec1.y)+(vec2.z-vec1.z)*(vec2.z-vec1.z);
			}
			ILINE friend F GetDistance(const jVec3_tpl<F> &vec1, const jVec3_tpl<F> &vec2) { 
				return  sqrt_tpl((vec2.x-vec1.x)*(vec2.x-vec1.x)+(vec2.y-vec1.y)*(vec2.y-vec1.y)+(vec2.z-vec1.z)*(vec2.z-vec1.z)); 
			}	

			ILINE F GetDistance(const jVec3_tpl<F> vec1) const {
			...
			ILINE jVec3_tpl permutated(int new_z) const { return jVec3_tpl(*(&x+inc_mod3[new_z]), *(&x+dec_mod3[new_z]), *(&x+new_z)); }
			...
			ILINE jVec3_tpl orthogonal() const {
			...
			// returns a vector orthogonal to this one
			ILINE void SetOrthogonal( const jVec3_tpl<F>& v ) {
			...
			ILINE jVec3_tpl abs() const { return jVec3_tpl(fabs_tpl(x),fabs_tpl(y),fabs_tpl(z)); }


			//! check for min bounds
			ILINE void	CheckMin(const jVec3_tpl &other)	{ 
			...
			ILINE void	CheckMax(const jVec3_tpl &other)	{
				if (other.x>x) x=other.x;
				if (other.y>y) y=other.y;
				if (other.z>z) z=other.z;
			}
			...
			ILINE jVec3_tpl rotated(const jVec3_tpl &axis, F angle) const { 
				return rotated(axis,cos_tpl(angle),sin_tpl(angle)); 
			}
			ILINE jVec3_tpl rotated(const jVec3_tpl &axis, F cosa,F sina) const {
				jVec3_tpl zax = axis*(*this*axis); 
				jVec3_tpl xax = *this-zax; 
				jVec3_tpl yax = axis^xax;
				return xax*cosa + yax*sina + zax;
			}

			ILINE jVec3_tpl rotated(const jVec3_tpl &center,const jVec3_tpl &axis, F cosa,F sina) const { 
				return center+(*this-center).rotated(axis,cosa,sina); 
			}
			ILINE jVec3_tpl rotated(const jVec3_tpl &center,const jVec3_tpl &axis, F angle) const { 
			...
			* 
			* Example:
			*  jVec3 result=jVec3::CreateProjection( incident, normal );
			*/
			ILINE static jVec3_tpl CreateProjection( const jVec3_tpl& i, const jVec3_tpl& n ) { return i - n*(n|i); }

			/*!
			* Calculate a reflection vector. jVec3 n is assumed to be a unit-vector.
			* 
			* Example:
			*  jVec3 result=jVec3::CreateReflection( incident, normal );
			*/
			ILINE static jVec3_tpl CreateReflection( const jVec3_tpl& i, const jVec3_tpl &n ) {	return (n*(i|n)*2)-i; }
		...
		// Typedefs                                                                  //
		///////////////////////////////////////////////////////////////////////////////
		typedef jVec3_tpl<f32>		jVec3;   //we will use only this throughout the project
		typedef jVec3_tpl<real>	jVec3_f64;


		typedef jVec3 jVec3d;  //obsolete! please use just jVec3
		typedef jVec3 vectorf;
		typedef jVec3_f64 vectorr;
		typedef jVec3_tpl<int>		vectori;


		inline jVec3_tpl<f32>::jVec3_tpl(type_min) { x=y=z=-3.3E38f; }
		inline jVec3_tpl<f32>::jVec3_tpl(type_max) { x=y=z=3.3E38f; }
		inline jVec3_tpl<f64>::jVec3_tpl(type_min) { x=y=z=-1.7E308; }
		inline jVec3_tpl<f64>::jVec3_tpl(type_max) { x=y=z=1.7E308; }

		template<class F> 
		ILINE F GetLengthSquared( const jVec3_tpl<F> &v ) { return v.x*v.x + v.y*v.y + v.z*v.z; }
		template<class F> 
		ILINE F GetLength( const jVec3_tpl<F>& v ) { return sqrt_tpl(v.x*v.x + v.y*v.y + v.z*v.z); }


		// dot product (2 versions)
		template<class F1,class F2> 
		ILINE F1 operator * (const jVec3_tpl<F1> &v0, const jVec3_tpl<F2> &v1) { 
		...
		// cross product
		template<class F1,class F2> 
		ILINE jVec3_tpl<F1> operator ^ (const jVec3_tpl<F1> &v0, const jVec3_tpl<F2> &v1) {
			return jVec3_tpl<F1>(v0.y*v1.z-v0.z*v1.y, v0.z*v1.x-v0.x*v1.z, v0.x*v1.y-v0.y*v1.x); 
		} 
		template<class F1,class F2> 
		ILINE jVec3_tpl<F1> operator % (const jVec3_tpl<F1> &v0, const jVec3_tpl<F2> &v1) {
			return jVec3_tpl<F1>(v0.y*v1.z-v0.z*v1.y, v0.z*v1.x-v0.x*v1.z, v0.x*v1.y-v0.y*v1.x); 
		...
		template<class F1,class F2>
		ILINE jVec3_tpl<F1> operator + (const jVec3_tpl<F1> &v0, const jVec3_tpl<F2> &v1) {
			return jVec3_tpl<F1>(v0.x+v1.x, v0.y+v1.y, v0.z+v1.z);
		}
		//vector subtraction
		template<class F1,class F2>
		ILINE jVec3_tpl<F1> operator - (const jVec3_tpl<F1> &v0, const jVec3_tpl<F2> &v1) {
		...
		template<class F1,class F2>
		ILINE jVec3_tpl<F1>& operator += (jVec3_tpl<F1> &v0, const jVec3_tpl<F2> &v1) {
		...
		ILINE f32 CalcAngleBetween( const jVec3 &invA, const jVec3 &invB )	{
		...
		// returns a vector orthogonal to this one
		template<class F>
		ILINE  jVec3_tpl<F> GetOrthogonal( const jVec3_tpl<F>& v ) {
		...
		template <class F> 
		struct Ang3_tpl : public jVec3_tpl<F>
		{
			Ang3_tpl() {}

			Ang3_tpl(const jVec3 &v) { this->x=v.x;	this->y=v.y; this->z=v.z;	}  
			Ang3_tpl	&operator = (const jVec3 &v)  { this->x=v.x; this->y=v.y; this->z=v.z; return *this; 	}
		...
		ILINE f32 Snap_s360( f32 val ) {
			if( val < 0.0f )
				val =f32( 360.0f + jc_fmod(val,360.0f));
			else
				if(val >= 360.0f)
					val =f32(jc_fmod(val,360.0f));
			return val;
		}
		...
		template <class F> struct AngleAxis_tpl {
			...
			AngleAxis_tpl( const f32 a, const jVec3_tpl<F> &n ) { angle=a; axis=n; }
			void operator () ( const f32 a, const jVec3_tpl<F> &n ) {  angle=a; axis=n; }
			...
			const jVec3_tpl<F> operator * ( const jVec3_tpl<F>& v ) const;
		...
		*/
		...
		ILINE AngleAxis_tpl<F>::AngleAxis_tpl( const Matrix33_tpl<F> &m ) {
			...
			axis		= GetNormalized( jVec3_tpl<F>(m(2,1)-m(1,2), m(0,2)-m(2,0), m(1,0)-m(0,1)) );
		...
		*/
		template<class F> 
		ILINE const jVec3_tpl<F> AngleAxis_tpl<F>::operator * ( const jVec3_tpl<F> &v ) const {
			jVec3_tpl<F> origin 	= axis*(axis|v);
		...
		struct Plane
		{
			...
			ILINE Plane( const jVec3 &normal, const f32 &distance ) {  n=normal; d=distance; }

			//! set normal and dist for this plane and  then calculate plane type
			ILINE void	Set(const jVec3 &vNormal,const f32 fDist)	{	
				n = vNormal; 
				d = fDist;
			}

			ILINE void SetPlane( const jVec3 &normal, const jVec3 &point ) { 
				n=normal; 
				d=-(point | normal); 
			}

			ILINE friend Plane GetPlane(  const jVec3 &normal, const jVec3 &point ) {  
				return Plane( normal,-(point|normal)  );
			}

			/*!
			*
			* Constructs the plane by tree given jVec3s (=triangle) 
			*
			* Example 1:
			* \code
			*  jVec3 v0(1,2,3),v1(4,5,6),v2(6,5,6);
			*  Plane  plane;
			...
			*
			*/
			ILINE void SetPlane( const jVec3 &v0, const jVec3 &v1, const jVec3 &v2 ) {  
				n = GetNormalized((v1-v0)%(v0-v2));	//vector cross-product
				d	=	-(n | v0);				//calculate d-value
			}
			ILINE friend Plane GetPlane( const jVec3 &v0, const jVec3 &v1, const jVec3 &v2 ) {  
				Plane p;
			...
			*
			*/
			ILINE f32 operator | ( const jVec3 &point ) const { return (n | point) + d; }
		...
		//-------------------------------------------------------------------------------------
			...
			void	CalcPlane(const jVec3 &p0, const jVec3 &p1, const jVec3 &p2) {
				n = (p0-p1)^(p2-p1);
				n.Normalize();
				d = (p0*n);
			}
			...
			void Init(const jVec3 & v1, const jVec3 & v2, const jVec3 & v3)
			{
				jVec3 u, v, t;
				u = v2 - v1;
				v = v2 - v3;
			...
			ILINE f32	DistFromPlane(const jVec3 &vPoint) const	{
				return (n*vPoint-d);
			}

			jVec3 MirrorVector(jVec3& i)  {
				return i - n*((n|i)*2);
			}

			jVec3 MirrorPosition(jVec3& i) {
		...
		ILINE void GetOtherBaseVec( const jVec3 &invDirection, jVec3 &outvA, jVec3 &outvB )
		{
		...
		namespace jMath
		{
			inline cstr ToString(vectori& v, fname_t buf)	{	jt_sprintf(buf, TEXT("%d,%d,%d"),v.x,v.y,v.z);return buf;	}
			inline cstr ToString(jVec3& v, fname_t buf)	{	jt_sprintf(buf, TEXT("%f,%f,%f"),v.x,v.y,v.z);return buf;	}
			inline cstr ToString(Plane& v, fname_t buf)	{	jt_sprintf(buf, TEXT("%f,%f,%f(%f)"),v.n.x,v.n.y,v.n.z, v.d);return buf;	}
