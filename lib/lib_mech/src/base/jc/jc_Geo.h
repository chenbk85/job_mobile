...
#ifndef JC_GAMEGEOSTRUCTS_H
#define JC_GAMEGEOSTRUCTS_H
...
#define FINDMINMAX(x0,x1,x2,min,max) \
	...
	if(x2>max) max=x2;
		...
		struct Line {
			...
			jVec3 pointonline;
			jVec3 direction; //caution: the direction is important for any intersection test

			//default Line constructor (without initialisation)
			inline Line( void ) {}
			inline Line( const jVec3 &o, const jVec3 &d ) {  pointonline=o; direction=d; }
			inline void operator () (  const jVec3 &o, const jVec3 &d  ) {  pointonline=o; direction=d; }
		...
		struct Ray {
			...
			inline Ray( void ) {}
			inline Ray( const jVec3 &o, const jVec3 &d ) {  origin=o; direction=d; }
			inline void operator () (  const jVec3 &o, const jVec3 &d  ) {  origin=o; direction=d; }
		...
		struct Lineseg {
			...
			inline Lineseg( void ) {}
			inline Lineseg( const jVec3 &s, const jVec3 &e ) {  start=s; end=e; }
			inline void operator () (  const jVec3 &s, const jVec3 &e  ) {  start=s; end=e; }
		...
		template <typename F> struct Triangle_tpl {
			...
			jVec3_tpl<F> v0,v1,v2;

			//default Lineseg constructor (without initialisation)
			inline Triangle_tpl( void ) {}
			inline Triangle_tpl( const jVec3_tpl<F>& a, const jVec3_tpl<F>& b, const jVec3_tpl<F>& c ) {  v0=a; v1=b; v2=c; }
			inline void operator () (  const jVec3_tpl<F>& a, const jVec3_tpl<F>& b, const jVec3_tpl<F>& c ) { v0=a; v1=b; v2=c; }
		...
		struct AABB {
			...
			inline AABB( void ) {}
			inline AABB( const jVec3 &vmin, const jVec3 &vmax ) {  min=vmin; max=vmax; }
			inline void operator () (  const jVec3 &vmin, const jVec3 &vmax  ) {  min=vmin; max=vmax; }

			~AABB( void ) {};

			//! Reset Bounding box before calculating bounds.
			void Reset()	{	min = jVec3(MAX);	max = jVec3(MIN);	}

			//! Check if bounding box is empty (Zero volume).
			bool IsEmpty() const { return min == max; }

			void Add( const jVec3 &v )	{
			...
			bool IsOverlapSphereBounds( const jVec3 &pos,float radius ) const
			{
			...
			bool IsContainSphere( const jVec3 &pos,float radius ) const
			{
				if (pos.x-radius < min.x) return false;
				if (pos.y-radius < min.y) return false;
				if (pos.z-radius < min.z) return false;
			...
			void Transform( const Matrix44 &tm )	{
				jVec3 m = tm.TransformPointOLD( min );
				jVec3 vx = jVec3(tm(0,0),tm(0,1),tm(0,2))*(max.x-min.x);
				jVec3 vy = jVec3(tm(1,0),tm(1,1),tm(1,2))*(max.y-min.y);
				jVec3 vz = jVec3(tm(2,0),tm(2,1),tm(2,2))*(max.z-min.z);
				min = m;
				max = m;
			...
			*/
			ILINE void SetTransformedAABB( const Matrix34& m34, const AABB& aabb ) {
				jVec3 sz		=	Matrix33(m34).GetFabs()*((aabb.max-aabb.min)*0.5f);
				jVec3 pos	= m34*((aabb.max+aabb.min)*0.5f);
			...
			template<typename F>
				...
				template<typename F>
					...
					*/
					template<typename F>
						ILINE void SetAABBfromOBB( const jVec3& wpos, const OBB_tpl<F>& obb, f32 scaling=1.0f ) {
							jVec3 pos	= obb.m33*obb.c*scaling + wpos;
							jVec3 sz		=	obb.m33.GetFabs()*obb.h*scaling;
							min=pos-sz; max=pos+sz;
						}
						template<typename F>
							ILINE static AABB CreateAABBfromOBB( const jVec3& wpos, const OBB_tpl<F>& obb, f32 scaling=1.0f) { AABB taabb; taabb.SetAABBfromOBB(wpos,obb,scaling); return taabb; 	}
		...
		template <typename F> struct OBB_tpl {
			...
			jVec3 h;				//half-length-vector
			jVec3 c;				//center of obb 

			//default OBB constructor (without initialisation)
			inline OBB_tpl() {}

			ILINE void SetOBB( const Matrix33& m33, const jVec3& hlv, const jVec3& center  ) {  m33=m33; h=hlv; c=center;	}
			ILINE static OBB_tpl<F> CreateOBB( const Matrix33& m33, const jVec3& hlv, const jVec3& center  ) {	OBB_tpl<f32> obb; obb.m33=m33; obb.h=hlv; obb.c=center; return obb;	}
		...
		struct Sphere {
			...
			inline Sphere( void ) {}
			inline Sphere( const jVec3 &c, const float &r ) {  center=c; radius=r; }
			inline void operator () (  const jVec3 &c, const float &r  ) {  center=c; radius=r; }
		...
		struct AAEllipsoid {
			...
			inline AAEllipsoid( const jVec3 &c, const jVec3 &rv  ) {  radius_vec=rv; center=c; }
			inline void operator () ( const jVec3 &c, const jVec3 &rv ) {  radius_vec=rv; center=c; }
...
#include "jc_GeoDistance.h" 
#include "jc_GeoOverlap.h" 
#include "jc_GeoIntersect.h" 
...
#define MIN_BB	-99999.0f
		...
		inline bool IsMinBB( const jVec3& v ) {
		...
		inline bool IsMaxBB( const jVec3& v ) {
		...
		inline jVec3	SetMaxBB( void ) { return jVec3(MAX_BB,MAX_BB,MAX_BB); }
		inline jVec3	SetMinBB( void ) { return jVec3(MIN_BB,MIN_BB,MIN_BB); }

		inline void AddToBounds (const jVec3& v, jVec3& mins, jVec3& maxs) {
		...
		inline float CalcArea(const jVec3 *vertices,int numvertices,const jVec3 &normal)
		{	
			jVec3 csum(0,0,0);
