...
public:
  ...
  void Save(const char * file_name)
  {
	...
	void Load(const char * file_name, nMech::nPAK::jIPak *)
  {
    Clear();
    FILE * f;

    f = nInterface::g_pjIPak->FOpen(file_name, "rb");
    if(!f)
      return;

    int size = 0;
    nInterface::g_pjIPak->FRead(&size, 4, 1, f);

    while(!nInterface::g_pjIPak->FEof(f) && sizeof(T)==size)
    {
      T tmp;
      if(nInterface::g_pjIPak->FRead(&tmp, 1, sizeof(T), f) == sizeof(T))
        Add(tmp);
    }

    nInterface::g_pjIPak->FClose(f);
  }
