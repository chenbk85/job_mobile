	...
	namespace nJC
	{
		...
		*/
		namespace stl
		{
			...
			template <typename Type>
				inline const char* constchar_cast2( const Type &type )
			{
				return (const char*)type;
			}

			/*
			ResourceCompiler.exe에서 에러남.
			...
			*/
			...
			//! Case insensetive less key for any type convertable to const char*.
			template <typename Type>
			struct less_stricmp2 : public std::binary_function<Type,Type,bool> 
			{
				bool operator()( const Type &left,const Type &right ) const
				{
					return stricmp(constchar_cast2(left),constchar_cast2(right)) < 0;
				}
			};
			...
			public:
				enum {	// parameters for hash table
					...
					{
						unsigned int h = 0; 
						const char *s = constchar_cast2(key);
					...
					{
						return strcmp(constchar_cast2(key1),constchar_cast2(key2)) < 0;
					}
			};
			...
			public:
				enum {	// parameters for hash table
					...
					bool operator()( const Key& key1,const Key& key2 ) const
					{
						return stricmp(constchar_cast2(key1),constchar_cast2(key2)) < 0;
					}
			};
#if !defined(LINUX)
