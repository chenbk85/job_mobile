...
namespace nMech
{
	...
	*/
	struct ITcbKey : public IKey
	{
		...
		void SetjVec3( const nMath::jVec3 &val ) { SetValue(val); };
		void SetQuat( const nMath::Quat &val ) { SetValue(val); };

		float GetFloat() const { return *((float*)fval); };
		const nMath::jVec3& GetjVec3() const { return *((nMath::jVec3*)fval); };
		const nMath::Quat& GetQuat() const { return *((nMath::Quat*)fval); };
	};
