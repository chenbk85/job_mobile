/* ========================================================================================== */
/* JC_GAMESOUND Main header file. Copyright (c), Firelight ES_Clientlogies Pty, Ltd 1999-2003.           */
/* ========================================================================================== */
...
#include <basetsd.h>		//AMD Port (We need the basetsd defs sooner for jcSound.h)
...
[
    [DESCRIPTION]   
    On failure of commands in JC_GAMESOUND, use CS_GetError to attain what happened.
...
[
    ...
    It is not nescessary to set the mixer. JC_GAMESOUND will autodetect the best mixer for you.
...
enum CM_TYPES
{
    ...
    CM_TYPE_FSB         /* JC_GAMESOUND Sample Bank file */
};
...
[
    ...
    [DESCRIPTION]   
    These default priorities are used by JC_GAMESOUND internal system DSP units.  They describe the 
...
#define CS_MPEGHALFRATE  0x00200000  /* For JC_GAMESOUNDCE only - decodes mpeg streams using a lower quality decode, but faster execution */
...
[
    ...
    [DESCRIPTION]
    Miscellaneous values for JC_GAMESOUND functions.
...
#define CS_UNMANAGED        -2      /* value to allocate a sample that is NOT managed by JC_GAMESOUND or placed in a sample slot. */
...
#define CS_SYSTEMCHANNEL    -1000   /* special 'channel' ID for all channel based functions that want to alter the global JC_GAMESOUND software mixing output channel */
#define CS_SYSTEMSAMPLE     -1000   /* special 'sample' ID for all sample based functions that want to alter the global JC_GAMESOUND software mixing output sample */
...
[
    [DESCRIPTION]
    Structure defining the properties for a reverb source, related to a JC_GAMESOUND channel.
    ...
    Linux and JC_GAMESOUNDCE do not support the reverb api.
...
[
    ...
    CS_INIT_ENABLESYSTEMCHANNELFX Is an init mode which enables the JC_GAMESOUND mixer buffer to be affected by DirectX 8 effects.
...
*/
#define CS_INIT_USEDEFAULTMIDISYNTH     0x0001    /* Causes MIDI playback to force software decoding. */
#define CS_INIT_GLOBALFOCUS             0x0002    /* For DirectSound output - sound is not muted when window is out of focus. */
#define CS_INIT_ENABLESYSTEMCHANNELFX   0x0004    /* For DirectSound output - Allows CS_FX api to be used on global software mixer output! (use CS_SYSTEMCHANNEL as channel id) */
#define CS_INIT_ACCURATEVULEVELS        0x0008    /* This latency adjusts CS_GetCurrentLevels, but incurs a small cpu and memory hit */
#define CS_INIT_PS2_DISABLECORE0REVERB  0x0010    /* PS2 only - Disable reverb on CORE 0 to regain SRAM */
#define CS_INIT_PS2_DISABLECORE1REVERB  0x0020    /* PS2 only - Disable reverb on CORE 1 to regain SRAM */
#define CS_INIT_PS2_SWAPDMACORES        0x0040    /* PS2 only - By default JC_GAMESOUND uses DMA CH0 for mixing, CH1 for uploads, this flag swaps them around */
...
#define CS_INIT_GC_INITLIBS             0x0100    /* Gamecube only - Initializes GC audio libraries */
#define CS_INIT_STREAM_FROM_MAIN_THREAD 0x0200    /* Turns offjcsound streamer thread, and makes streaming update from CS_Update called by the user */
...
/* 
    ...
    called (they will fail). They set up JC_GAMESOUND system functionality. 
*/
...
/* 
    ...
    Note : Use CS_FREE as the 'channel' variable, to let JC_GAMESOUND pick a free channel for you.
...
/* 
    ...
    - CS_INIT_ENABLESYSTEMCHANNELFX can be used to apply hardware effect processing to the
      global mixed output of JC_GAMESOUND's software channels.
...
/*
    Note : Use CS_LOADMEMORY   flag with CS_Stream_Open to stream from memory.
           ...
           Use CS_FREE as the 'channel' variable, to let JC_GAMESOUND pick a free channel for you.
*/
...
DLL_API signed char        F_API CS_Stream_SetSubStream(CS_STREAM *stream, int index);     /* For JC_GAMESOUND .FSB bank files. */
DLL_API int                F_API CS_Stream_GetNumSubStreams(CS_STREAM *stream);            /* For JC_GAMESOUND .FSB bank files. */
...
/* 
    ...
    These functions allow you access to the mixed stream that JC_GAMESOUND uses to play back sound on.
*/
...
/* 
    Functions to get hold of JC_GAMESOUND 'system DSP unit' handles. 
*/
...
/* ========================================================================================== */
/* JC_GAMEMUSIC API (MOD,S3M,XM,IT,MIDI PLAYBACK)                                                   */
/* ========================================================================================== */
