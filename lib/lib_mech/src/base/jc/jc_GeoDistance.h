...
#ifndef JC_GAMEDISTANCE_H
#define JC_GAMEDISTANCE_H
...
namespace nMech
{
	namespace nMath
	{
		...
		namespace Distance {
			...
			*
			* Example:
			*  jVec3 result = Distance::Origin_Triangle2D( triangle );
			*/
			template<typename F>
				ILINE jVec3_tpl<F> Origin_Triangle2D( const Triangle_tpl<F>& t ) {
					jVec3_tpl<F> a=t.v0;	
					jVec3_tpl<F> b=t.v1;
					jVec3_tpl<F> c=t.v2;
					...
					switch (flag) {
			case 0:	return jVec3_tpl<F>(0,0,a.z); //center is inside of triangle
			...
			case 6:	return c; //vertex C is closed
					...
					switch (flag) {
			case 1: { jVec3_tpl<F> n=(b-a).GetNormalized(); return n*(-a|n)+a; }
			case 2:	{	jVec3_tpl<F> n=(c-b).GetNormalized(); return n*(-b|n)+b; }
			case 3:	return b;
			case 4:	{ jVec3_tpl<F> n=(a-c).GetNormalized(); return n*(-c|n)+c; }
			case 5:	return a;
			case 6:	return c;
					}
					return jVec3_tpl<F>(0,0,0);
				}
				...
				*/
				template<typename F>
					ILINE F Point_Triangle( const jVec3_tpl<F>& p, const Triangle_tpl<F> &t ) {
						//translate triangle into origin
						jVec3_tpl<F> a=t.v0-p;	
						jVec3_tpl<F> b=t.v1-p;	
						jVec3_tpl<F> c=t.v2-p;
						...
						jVec3_tpl<F> h = Origin_Triangle2D(Triangle_tpl<F>(r33*a,r33*b,r33*c));
					...
					*/
					template<typename F>
						ILINE F Point_Triangle( const jVec3_tpl<F>& p, const Triangle_tpl<F> &t, jVec3_tpl<F>& output ) {
							//translate triangle into origin
							jVec3_tpl<F> a=t.v0-p;	
							jVec3_tpl<F> b=t.v1-p;	
							jVec3_tpl<F> c=t.v2-p;
						...
						template<typename F>
							...
							template<typename F>
								ILINE F Sphere_Triangle( const Sphere &s, const Triangle_tpl<F> &t, jVec3_tpl<F>& output ) {
