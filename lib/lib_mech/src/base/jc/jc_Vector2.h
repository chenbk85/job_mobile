...
#ifndef jcGame_JC_VECTOR2_H
#define jcGame_JC_VECTOR2_H
...
namespace nMech
{
	namespace nMath
	{
		...
		template<class F> struct Vec2_tpl {
			...
			template<class F1> explicit Vec2_tpl(const jVec3_tpl<F1> &src) { x=src.x;y=src.y; }
			...
			template<class F1> Vec2_tpl& operator=(const jVec3_tpl<F1>& src) { x=src.x;y=src.y; return *this; }
		...
		namespace jMath
		{
			inline cstr ToString(jVec2& v, fname_t buf)	{	jt_sprintf(buf, _T("%5.3f,%5.3f"),v.x,v.y);return buf;	}
			inline cstr ToString(vector2l& v, fname_t buf)	{	jt_sprintf(buf, _T("%d,%d"),v.x,v.y);return buf;	}
			inline cstr ToString(vector2dui& v, fname_t buf)	{	jt_sprintf(buf, _T("%u,%u"),v.x,v.y);return buf;	}
		};
