...
#undef SWAP_ROWS
		...
		inline float jcISqrtf(float fVal)
		{
...
#if defined _CPU_X86 && !defined(LINUX)
		// ***************************************************************************
		inline void jcPrecacheSSE(const void *src, int nbytes)
		{
			_asm
			{
				mov esi, src
...
endLabel:
		...
		// ***************************************************************************
		inline void jcPrecacheMMX(const void *src, int nbytes)
		{
			_asm
			{
				mov esi, src
...
endLabel:
		...
		inline void jcPrefetchNTSSE(const void *src)
		{
		...
		ILINE void jcPrefetchT0SSE(const void *src)
		{
...
#define CACHEBLOCK 80h // number of 64-byte blocks (cache lines) for block prefetch
		...
		inline void jcMemcpy( void* Dst, const void* Src, int Count )
		{
...
$memcpy_final:
		...
		inline void jcPrefetch(const void* Src, int nCount)
		{
			nCount >>= 6;
			if (nCount > 0)
			{
...
mPr1:
		...
		inline void jcMemcpy (void* inDst, const void* inSrc, int nCount, int nFlags)
		{
			int i;
			int cnt;
			INT_PTR nAlign16;
...
m1:
		...
		// NOTE: On AMD Athlon - much faster then SSE version
		inline void transformjVec3f_3DNow(float *result, float *inV, float *matrix)
		{
			_asm
			{
				femms
		...
		// NOTE: On AMD Athlon - much faster then SSE version
		inline void transformjVec3Nf_3DNow(float *result, float *inV, float *matrix)
		{
			_asm
			{
				femms
...
lEnd:
		...
		inline void transformjVec3f_SSE(float *result, float *inV, float *matrix)
		{
			_asm
			{
				and         esp,0FFFFFFF0h
...
lEnd:
		...
		// IMPORTANT NOTE: much faster if matrix is 16 bytes aligned
		inline void transformjVec3Nf_SSE(float *result, float *inV, float *matrix)
		{
			_asm
			{
				and         esp,0FFFFFFF0h
...
lEnd:
		...
		ILINE void jcMemcpy(void* Dst, const void* Src, int n) {
			char* dst=(char*)Dst;
			char* src=(char*)Src;
		...
		ILINE void jcMemcpy( void* Dst, const void* Src, INT n, int nFlags )
		{
			char* dst=(char*)Dst;
			char* src=(char*)Src;
			while (n > PREFNTA_BLOCK) 
		...
		inline void mathTransformjVec3fN(float *pOut, float *pIn, float *matrix, int nV, int OptFlags)
		{
#if defined _CPU_X86 && !defined(LINUX)
			...
			if (OptFlags & CPUF_3DNOW)
			{
				for (i=0; i<nV; i++)
				{
					transformjVec3Nf_3DNow(pOut, pIn, matrix);
					pIn += 3;
					pOut += 3;
				}
			}
				...
				if (OptFlags & CPUF_SSE)
				{
					for (i=0; i<nV; i++)
					{
						transformjVec3Nf_SSE(pOut, pIn, matrix);
						pIn += 3;
						pOut += 3;
					}
				}
		...
		inline void mathTransformjVec3f(float *pOut, float *pIn, float *matrix, int nV, int OptFlags)
		{
#if defined(_CPU_X86) && !defined(LINUX)
			int i;
			// TODO: AMD64 port: NEED TO IMPLEMENT!!!
			if (OptFlags & CPUF_3DNOW)
			{
				for (i=0; i<nV; i++)
				{
					transformjVec3f_3DNow(pOut, pIn, matrix);
					pIn += 3;
					pOut += 3;
				}
			}
				...
				if (OptFlags & CPUF_SSE)
				{
					for (i=0; i<nV; i++)
					{
						transformjVec3f_SSE(pOut, pIn, matrix);
						pIn += 3;
						pOut += 3;
					}
				}
		...
		inline void mathScale(float *pMatr, jVec3d vScale, int)
		{
		...
		inline void mathCalcMatrix(Matrix44& Matrix, jVec3d vPos, jVec3d vAngs, jVec3d vScale, int OptFlags)
		{
			...
			if (!IsEquivalent(vScale, jVec3d(1.0f, 1.0f, 1.0f)))
