...
#ifndef JC_GAMEINTERSECTION_H
#define JC_GAMEINTERSECTION_H
...
namespace nMech
{
	namespace nMath
	{
		...
		namespace Intersect {
			...
			inline bool Ray_Plane(const Ray &ray, const Plane &plane, jVec3 &output ) {
			...
			inline bool Line_Plane(const Line &line, const Plane &plane, jVec3 &output ) {
			...
			* 
			*/
			inline bool Line_Triangle( const Line &line, const jVec3 &v0, const jVec3 &v1, const jVec3 &v2, jVec3 &output ) {
			...
			* "output" is undefined  
			*/
			inline bool Ray_Triangle( const Ray &ray, const jVec3 &v0, const jVec3 &v1, const jVec3 &v2, jVec3 &output ) {
				//find vectors for two edges sharing v0
				jVec3 edge_1 = v1-v0;
				jVec3 edge_2 = v2-v0;
				//begin calculating determinant - also used to calculate U parameter
			...
			* 
			*/
			inline bool Lineseg_Triangle( const Lineseg &lineseg, const jVec3 &v0, const jVec3 &v1, const jVec3 &v2, jVec3 &output ) {
				//find vectors for two edges sharing v0
				jVec3 edge_1 = v1-v0;
				jVec3 edge_2 = v2-v0;
			...
			inline unsigned char Line_Sphere( const Line &line, const Sphere &s, jVec3 &i0, jVec3 &i1 ) {

				jVec3 end=line.pointonline+line.direction;
				...
				if (desc >= 0.0f)
				{
					float lamba0 = (-b - jc_sqrtf(desc)) / (2.0f * a); 
					...
					float lamba1 = (-b + jc_sqrtf(desc))/(2.0f*a); 
			...
			inline unsigned char Ray_Sphere( const Ray &ray, const Sphere &s, jVec3 &i0, jVec3 &i1 ) {
				jVec3 end=ray.origin+ray.direction;
			...
			inline bool Ray_SphereFirst( const Ray &ray, const Sphere &s, jVec3 &intPoint )
			{
				jVec3 p2;
			...
			//--- 0x03 = two intersection, lineseg has ENTRY and EXIT point  --
			//----------------------------------------------------------------------------------
			inline unsigned char Lineseg_Sphere( const Lineseg &ls, const Sphere &s, jVec3 &i0, jVec3 &i1 ) {

				jVec3 dir = (ls.end - ls.start);
			...
			inline int Lineseg_Sphere(jVec3 p1, jVec3 p2, jVec3 p3, float r, jVec3 &i1, jVec3 i2)
			{
				float dx=p2.x-p1.x;
				float dy=p2.y-p1.y;
				float dz=p2.z-p1.z;
