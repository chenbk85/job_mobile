 ...
 */
#ifndef _JCGAME_COMMON_STREAM_ENGINE_HDR_
#define _JCGAME_COMMON_STREAM_ENGINE_HDR_
	...
	namespace nJC_stream
	{
		...
		public:
			...
			virtual IReadStreamPtr StartRead (const char* szSource, const char* szFile
				, IStreamCallback* pCallback = NULL, StreamReadParams* pParams = NULL) = 0;

			// returns the size of the file; returns 0 if there's no such file.
			// njcPakFlags is the flag set as in jIPak
			virtual unsigned GetFileSize (const char* szFile, unsigned njcPakFlags = 0) = 0;
...
#endif //_JCGAME_COMMON_STREAM_ENGINE_HDR_
