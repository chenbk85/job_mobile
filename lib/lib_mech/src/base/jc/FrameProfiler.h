...
namespace nMech
{
	namespace nJC
	{
		...
		*/
		...
		struct IFrameProfileSystem
		{
			...
			// *_* by icandoit : 2006-05-14 21:20:03
			// ISystem에서 가져왔음 g_pIFrameProfileSystem를 사용하기위해.
	...
	namespace nInterface {extern JBASE_API nJC::IFrameProfileSystem* g_pIFrameProfileSystem;}
	...
	namespace nJC
	{
		...
		public:
			...
			CFrameProfiler( const char *sCollectorName,EProfiledSubsystem subsystem=PROFILE_ANY )
			{
				m_pParent = 0;
				m_pGraph = 0;
				m_bExpended = false;
				...
				nInterface::g_pIFrameProfileSystem->AddFrameProfiler( this );
			}
		};
		...
		public:
			...
			__forceinline CFrameProfilerSection( CFrameProfiler *profiler )
			{
				...
				if (profiler && nInterface::g_pIFrameProfileSystem)
					nInterface::g_pIFrameProfileSystem->StartProfilerSection( this );
			}
			__forceinline ~CFrameProfilerSection()
			{
				if (m_pFrameProfiler && nInterface::g_pIFrameProfileSystem)
					nInterface::g_pIFrameProfileSystem->EndProfilerSection( this );
			}
		};
		...
		public:
			...
			__forceinline CCustomProfilerSection( CFrameProfiler *profiler,int *pValue )
			{
				...
				m_pFrameProfiler = profiler;
				if (profiler && nInterface::g_pIFrameProfileSystem)
					nInterface::g_pIFrameProfileSystem->StartCustomSection( this );
			}
			__forceinline ~CCustomProfilerSection()
			{
				if (m_pFrameProfiler && nInterface::g_pIFrameProfileSystem)
					nInterface::g_pIFrameProfileSystem->EndCustomSection( this );
			}
		};
...
#define FUNCTION_PROFILER( subsystem ) \
	static nJC::CFrameProfiler staticFrameProfiler( __FUNCTION__,subsystem ); \
	nJC::CFrameProfilerSection frameProfilerSection( &staticFrameProfiler );

#define FUNCTION_PROFILER_FAST( subsystem,bProfileEnabled ) \
	static nJC::CFrameProfiler staticFrameProfiler( __FUNCTION__,subsystem ); \
	nJC::CFrameProfilerSection frameProfilerSection( (bProfileEnabled)?&staticFrameProfiler:NULL );
...
#define FRAME_PROFILER( szProfilerName,subsystem ) \
	static nJC::CFrameProfiler staticFrameProfiler( szProfilerName,subsystem ); \
	nJC::CFrameProfilerSection frameProfilerSection( &staticFrameProfiler );
