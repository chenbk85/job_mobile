...
#ifndef jc_matrix_header___
#define jc_matrix_header___ 
...
template<typename F> struct Matrix33diag_tpl {
	...
	Matrix33diag_tpl(const jVec3_tpl<F> &src) { x=src.x; y=src.y; z=src.z; } 
	...
	template<class F1> const Matrix33diag_tpl& operator=(const jVec3_tpl<F1> &src) {
		x=src.x; y=src.y; z = src.z; return *this; 
	}
...
template<class F1,class F2>
jVec3_tpl<F2> operator *(const Matrix33diag_tpl<F1> &mtx, const jVec3_tpl<F2> &vec) {
	return jVec3_tpl<F2>(mtx.x*vec.x, mtx.y*vec.y, mtx.z*vec.z);
}

template<class F1,class F2>
jVec3_tpl<F1> operator *(const jVec3_tpl<F1> &vec, const Matrix33diag_tpl<F2> &mtx) {
	return jVec3_tpl<F1>(mtx.x*vec.x, mtx.y*vec.y, mtx.z*vec.z);
}
...
template<class F,int SI,int SJ> struct Matrix33_tpl {
	...
	template<class F1> 
	Matrix33_tpl& operator = (const jVec3_tpl<F1> &v) {
		M00=v.x;		M01=0;			M02=0; 
		M10=0;			M11=v.y;		M12=0;
		M20=0;			M21=0;			M22=v.z; 
		return *this;
...
//---------------------------------------------------------------------------------------
	...
	ILINE void SetRotationAA(F angle, jVec3_tpl<F> axis) {	F cs[2]; sincos_tpl( angle, cs);	SetRotationAA(cs[0],cs[1], axis);	}
	ILINE void SetRotationAA(F c, F s, jVec3_tpl<F> axis) { 
	...
	ILINE static Matrix33_tpl<F,SI,SJ> CreateRotationAA( const f32 rad, const jVec3_tpl<F>& axis ) {	
  ...
  *	c) If both vectors are exactly diametrical it returns a matrix that rotates 
	...
	*
	*/
	ILINE void SetRotationV0V1( const jVec3_tpl<F>& v0, const jVec3_tpl<F>& v1 ) {
	...
	ILINE static Matrix33_tpl<F,SI,SJ> CreateRotationV0V1( const jVec3_tpl<F>& v0, const jVec3_tpl<F>& v1 ) {	Matrix33_tpl<F,SI,SJ> m33;	m33.SetRotationV0V1(v0,v1); return m33;	}
  ...
  *  Creates a rotation matrix that rotates a vector called "n" into the vector (0,0,1). 
	...
	*/
	ILINE void SetRotationV0( const jVec3_tpl<F>& n ) {
	...
	ILINE static Matrix33_tpl<F,SI,SJ> CreateRotationV0( const jVec3_tpl<F>& n ) {	Matrix33_tpl<F,SI,SJ> m33;	m33.SetRotationV0(n); return m33;	}



	ILINE void SetScale( const jVec3_tpl<F> &s ) {
		M00=s.x;		M01=0;			M02=0;
		M10=0;			M11=s.y;		M12=0;
		M20=0;			M21=0;			M22=s.z;
	}
	ILINE static Matrix33_tpl<F,SI,SJ> CreateScale( const jVec3_tpl<F> &s  ) { Matrix33_tpl<F,SI,SJ> m; m.SetScale(s);	return m;	}


	//NOTE: all vectors are stored in columns
	ILINE void SetMatFromVectors(const jVec3_tpl<F>& vx, const jVec3_tpl<F>& vy, const jVec3_tpl<F>& vz)	{
		M00=vx.x;		M01=vy.x;		M02=vz.x;
		M10=vx.y;		M11=vy.y;		M12=vz.y;
		M20=vx.z;		M21=vy.z;		M22=vz.z;
	}
	ILINE static Matrix33_tpl<F,SI,SJ> CreateMatFromVectors( const jVec3_tpl<F>& vx, const jVec3_tpl<F>& vy, const jVec3_tpl<F>& vz ) {	Matrix33_tpl<F,SI,SJ> dst; dst.SetMatFromVectors(vx,vy,vz); return dst;	}
...
//--------------------------------------------------------------------------------
	...
	ILINE jVec3_tpl<F> GetRow(int iRow) const { return jVec3_tpl<F>(	data[iRow*SI+0*SJ],data[iRow*SI+1*SJ],data[iRow*SI+2*SJ]); 	}
	ILINE void SetRow(int iRow, const jVec3_tpl<F> &row)	{	data[iRow*SI+0*SJ]=row.x; data[iRow*SI+1*SJ]=row.y;		data[iRow*SI+2*SJ]=row.z;	}

	ILINE jVec3_tpl<F> GetColumn(int iCol) const	{	return jVec3_tpl<F>(	data[0*SI+iCol*SJ],	data[1*SI+iCol*SJ], data[2*SI+iCol*SJ] );	}
	ILINE void SetColumn(int iCol, const jVec3_tpl<F> &col)	{	data[0*SI+iCol*SJ]=col.x;	data[1*SI+iCol*SJ]=col.y;	data[2*SI+iCol*SJ]=col.z;	}

	ILINE jVec3_tpl<F> GetOrtX() const	{	return jVec3_tpl<F>( M00, M10, M20 );	}
	ILINE jVec3_tpl<F> GetOrtY() const	{	return jVec3_tpl<F>( M01, M11, M21 );	}
	ILINE jVec3_tpl<F> GetOrtZ() const	{	return jVec3_tpl<F>( M02, M12, M22 );	}
	...
	template<class F1> 
	ILINE void extract_from4x4T(const Matrix44_tpl<F1,4,1>& m, jVec3_tpl<F>& offset, F& scale) {
		*this=Matrix33(m).T();
...
//post-multiply
template<class F1, class F2,int SI,int SJ>
ILINE jVec3_tpl<F1> operator*(const Matrix33_tpl<F2,SI,SJ> &m, const jVec3_tpl<F1> &v) {
	return jVec3_tpl<F1>(v.x*m.M00 + v.y*m.M01 + v.z*m.M02,
...
//pre-multiply
template<class F1, class F2,int SI,int SJ>
ILINE jVec3_tpl<F1> operator*(const jVec3_tpl<F1> &v, const Matrix33_tpl<F2,SI,SJ> &m) {
	return jVec3_tpl<F1>(v.x*m.M00 + v.y*m.M10 + v.z*m.M20,
		                  v.x*m.M01 + v.y*m.M11 + v.z*m.M21,
		                  v.x*m.M02 + v.y*m.M12 + v.z*m.M22);
}

template<class F1,int SI,int SJ> 
ILINE Matrix33_tpl<F1,SI,SJ>& crossproduct_matrix(const jVec3_tpl<F1> &v, Matrix33_tpl<F1,SI,SJ> &m) {
...
template<class F1,int SI,int SJ> 
ILINE Matrix33_tpl<F1,SI,SJ>& dotproduct_matrix(const jVec3_tpl<F1> &v, const jVec3_tpl<F1> &op, Matrix33_tpl<F1,SI,SJ> &m) {
...
template <typename F> struct Matrix34_tpl {
	...
	ILINE explicit Matrix34_tpl(const Matrix44_tpl<F,SI,SJ>& m ) {
		...
		void SetTranslation( const jVec3_tpl<F>& t );
		jVec3_tpl<F> GetTranslation() const;

		void SetRotationAA(const F rad, const jVec3_tpl<F>& axis, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) );
		static Matrix34_tpl<F> CreateRotationAA( const F rad, const jVec3_tpl<F>& axis, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) );


		void SetRotationX(const f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) );
		static Matrix34_tpl<F> CreateRotationX( const f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f)   );
		void SetRotationY(const f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) );
		static Matrix34_tpl<F> CreateRotationY( const f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f)   );
		void SetRotationZ(const f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) );
		static Matrix34_tpl<F> CreateRotationZ( const f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f)   );


		void SetRotationXYZ( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) );
		static Matrix34_tpl<F> CreateRotationXYZ( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f)  );

		void SetScale( const jVec3_tpl<F> &s, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) );
		static Matrix34_tpl<F> CreateScale(  const jVec3_tpl<F> &s, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f)  );


		ILINE void SetTranslationMat(  const jVec3_tpl<F>& v  ) {
			m00=1.0f;	m01=0.0f;	m02=0.0f;	m03=v.x;
			m10=0.0f;	m11=1.0f;	m12=0.0f;	m13=v.y;
			m20=0.0f;	m21=0.0f;	m22=1.0f;	m23=v.z;
		}
		ILINE static Matrix34_tpl<F> CreateTranslationMat(  const jVec3_tpl<F>& v  ) {	Matrix34_tpl<F> m34; m34.SetTranslationMat(v); return m34; 	}
	

		//NOTE: all vectors are stored in columns
		ILINE void SetMatFromVectors(const jVec3& vx, const jVec3& vy, const jVec3& vz, const jVec3& pos)	{
			m00=vx.x;		m01=vy.x;		m02=vz.x;		m03 = pos.x;
			m10=vx.y;		m11=vy.y;		m12=vz.y;		m13 = pos.y;
			m20=vx.z;		m21=vy.z;		m22=vz.z;		m23 = pos.z;
		}
		ILINE static Matrix34_tpl<F> CreateMatFromVectors(const jVec3_tpl<F>& vx, const jVec3_tpl<F>& vy, const jVec3_tpl<F>& vz, const jVec3_tpl<F>& pos) {
    ...
    //this it not the same as in M44
		ILINE jVec3 TransformVector(const jVec3 &p) const  {
			jVec3 v;
		...
		//NOTE: vector p is multiplied by the rows of the matrix
		//this it not the same as in M44
		ILINE jVec3 TransformPoint(const jVec3 &p) const  {
			jVec3 v;
...
/*!
*
*  multiplication of Matrix34 by a (column) jVec3.
*  This function transforms the given input jVec3
*  into the coordinate system defined by this matrix.
*
*  Example:
*   Matrix34 m34;
*   jVec3 vector(44,55,66);
*   jVec3 result = m34*vector;
*
*/
template<class F> 
ILINE jVec3_tpl<F> operator * (const Matrix34_tpl<F>& m, const jVec3_tpl<F> &p) {
	jVec3_tpl<F> tp;
...
*
*  Example:
*   Matrix34 m34=Matrix34::CreateRotationX(1.94192f, jVec3(11,22,33));;
...
*  Example:
*		Matrix34 m34;
*		m34.SetTranslation( jVec3 (0.5f, 1.0f, 2.0f) );
*/
template<class F> 
ILINE void Matrix34_tpl<F>::SetTranslation( const jVec3_tpl<F>& t ) { m03=t.x;	m13=t.y; m23=t.z;	}
...
*  Example 1:
*		Matrix m34;
*		jVec3 v=m34.GetTranslation();
*
*/
template<class F> 
ILINE jVec3_tpl<F> Matrix34_tpl<F>::GetTranslation() const { return jVec3_tpl<F>(m03,m13,m23); }


/*!
*
*  Create a rotation matrix around an arbitrary axis (Eulers Theorem).  
*  The axis is specified as an normalized jVec3. The angle is assumed to be in radians.  
*  This function also assumes a translation-vector and stores it in the right column.  
*
*  Example:
*		Matrix34 m34;
*		jVec3 axis=GetNormalized( jVec3(-1.0f,-0.3f,0.0f) );
*		m34.SetRotationAA( 3.14314f, axis, jVec3(5,5,5) );
*/
template<class F> 
ILINE void Matrix34_tpl<F>::SetRotationAA(const F rad, const jVec3_tpl<F>& axis, const jVec3_tpl<F>& t )	{
	assert((fabs_tpl(1-(axis|axis)))<0.001); //check if unit-vector
	*this=Matrix33::CreateRotationAA(rad,axis); this->SetTranslation(t);
}
template<class F> 
ILINE Matrix34_tpl<F> Matrix34_tpl<F>::CreateRotationAA( const F rad, const jVec3_tpl<F>& axis, const jVec3_tpl<F>& t )	{	Matrix34_tpl<F> m34;  m34.SetRotationAA(rad,axis,t);	return m34;	}
...
*/
template<class F> 
ILINE void Matrix34_tpl<F>::SetRotationX(const f32 rad, const jVec3_tpl<F>& t )	{
	*this=Matrix33::CreateRotationX(rad); this->SetTranslation(t);
}
template<class F> 
ILINE Matrix34_tpl<F> Matrix34_tpl<F>::CreateRotationX( const f32 rad, const jVec3_tpl<F>& t   )	{		Matrix34_tpl<F> m34;  m34.SetRotationX(rad,t);	return m34;	}
...
*/
template<class F> 
ILINE void Matrix34_tpl<F>::SetRotationY(const f32 rad, const jVec3_tpl<F>& t )	{
	*this=Matrix33::CreateRotationY(rad);	this->SetTranslation(t);
}
template<class F> 
ILINE Matrix34_tpl<F> Matrix34_tpl<F>::CreateRotationY( const f32 rad, const jVec3_tpl<F>& t   )	{	Matrix34_tpl<F> m34;  m34.SetRotationY(rad,t);	return m34;	}
...
*/
template<class F> 
ILINE void Matrix34_tpl<F>::SetRotationZ(const f32 rad, const jVec3_tpl<F>& t )	{
	*this=Matrix33::CreateRotationZ(rad);  this->SetTranslation(t);
}
template<class F> 
ILINE Matrix34_tpl<F> Matrix34_tpl<F>::CreateRotationZ( const f32 rad, const jVec3_tpl<F>& t   )	{	Matrix34_tpl<F> m34;  m34.SetRotationZ34(rad,t);	return m34;	}
...
*/
template<class F> 
ILINE void Matrix34_tpl<F>::SetRotationXYZ( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t ) {
	*this=Matrix33::CreateRotationXYZ(rad); this->SetTranslation(t);
}
template<class F> 
ILINE Matrix34_tpl<F> Matrix34_tpl<F>::CreateRotationXYZ( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t  )	{	Matrix34_tpl<F> m34;  m34.SetRotationXYZ(rad,t);	return m34;	}
...
*  Example 1:
*		Matrix m34;
*		m34.SetScale( jVec3(0.5f, 1.0f, 2.0f) );
*
*  Example 2:
*		Matrix34 m34 = Matrix34::CreateScale( jVec3(0.5f, 1.0f, 2.0f) );
*
*/
/*template<class F> 
ILINE void Matrix34_tpl<F>::SetScale( const jVec3_tpl<F> &s, const jVec3_tpl<F>& t ) {
	*this=Matrix33::GetScale(s); this->SetTranslation(t);
}
*/
template<class F> 
ILINE Matrix34_tpl<F> Matrix34_tpl<F>::CreateScale(  const jVec3_tpl<F> &s, const jVec3_tpl<F>& t  )	{ 	Matrix34_tpl<F> m34;  m34.SetScale(s,t);	return m34;	}
...
*
* \code
*		jVec3 normal( 0.0f,-1.0f, 0.0f);
*		jVec3 pos(0,1000,0);
*		Matrix34 m34=CreateReflectionMat( pos, normal );
* \endcode
*/

ILINE Matrix34 CreateReflectionMat ( const jVec3& p, const jVec3& n )
{
	Matrix34 m;	
...
/*		ILINE Matrix44_tpl(const jVec3 &v1, const jVec3 &v2, const jVec3 &v3)	{ 
...
/*		//! build a matrix from 4 vectors
		ILINE Matrix44_tpl(const jVec3 &v1, const jVec3 &v2, const jVec3 &v3, const jVec3 &v4)  { 
		...
		ILINE friend	jVec3 operator * (const Matrix44 &m, const jVec3 &v)	{
			return jVec3(		m.M00*v.x + m.M01*v.y + m.M02*v.z,
		...
		jVec3 GetOrtX() const	{	return jVec3( M00, M10, M20 );	}
		jVec3 GetOrtY() const	{	return jVec3( M01, M11, M21 );	}
		jVec3 GetOrtZ() const	{	return jVec3( M02, M12, M22 );	}


		ILINE jVec3_tpl<F> GetRow(int iRow) const {	return jVec3_tpl<F>(	data[iRow*SI+0*SJ],	data[iRow*SI+1*SJ],	data[iRow*SI+2*SJ] );	}
		ILINE void SetRow(int iRow, const jVec3_tpl<F> &row)	{	data[iRow*SI+0*SJ]=row.x;	data[iRow*SI+1*SJ]=row.y;		data[iRow*SI+2*SJ]=row.z;	}

		jVec3_tpl<F> GetColumn(int iCol) const	{	return jVec3_tpl<F>(	data[0*SI+iCol*SJ],	data[1*SI+iCol*SJ],		data[2*SI+iCol*SJ]	);	}
		void SetColumn(int iCol, const jVec3_tpl<F> &col)	{	data[0*SI+iCol*SJ]=col.x;		data[1*SI+iCol*SJ]=col.y;		data[2*SI+iCol*SJ]=col.z;	}
		...
		*/
		//member function
	/*	ILINE void SetRotationAA44(f32 rad, const jVec3_tpl<F>& axis, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) ) {
			assert((fabs_tpl(1-(axis|axis)))<0.001); //check if unit-vector
			F	c		= cos_tpl(rad);
			F	s		= sin_tpl(rad);
			F	mc	=	(F)1.0-c;
		...
		ILINE friend Matrix44_tpl<F,4,1> CreateRotationAA44(f32 rad, const jVec3_tpl<F>& axis, const jVec3_tpl<F>& t  ) {
			Matrix44_tpl<F,4,1> m44; m44.SetRotationAA44(rad,axis,t); return m44;
		}
		ILINE friend Matrix44_tpl<F,4,1> CreateRotationAA44(f32 rad, const jVec3_tpl<F>& axis ) {
		...
		*/
	/*	ILINE void SetRotationX44(f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f)  ) {
		...
		*/
/*		ILINE void SetRotationY44(f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) ) {
		...
		*
		*/
/*		ILINE void SetRotationZ44(const f32 rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) ) {
		...
		*/
		ILINE void SetRotationXYZ( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) )	{
			F csx[2]; sincos_tpl(rad.x, csx);
			F csy[2]; sincos_tpl(rad.y, csy);
			F csz[2]; sincos_tpl(rad.z, csz);
	...
	/*	ILINE static Matrix44_tpl<F,SI,SJ> CreateRotationXYZ44( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) )	{
			Matrix44_tpl<F,SI,SJ> m; m.SetRotationXYZ44(rad,t); return m;
		}
		ILINE friend Matrix44_tpl<F,4,1> CreateRotationXYZ44( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t )	{	Matrix44_tpl<F,4,1> m; m.SetRotationXYZ44(rad,t); return m;	}
...
*/
		...
		/*ILINE void SetMatrixSRT( const jVec3_tpl<F>& s, const Ang3_tpl<F>& rad,  const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f) )	{
		...
		*/
		ILINE void SetRotationZYX( const Ang3_tpl<F>& rad, const jVec3_tpl<F>& t=jVec3(0.0f,0.0f,0.0f)  )	{
			F csx[2]; sincos_tpl(rad.x, csx);
			F csy[2]; sincos_tpl(rad.y, csy);
			F csz[2]; sincos_tpl(rad.z, csz);
			F sxsy=sx*sy;
		...
		ILINE static Matrix44_tpl<F,4,1> CreateRotationZYX( const jVec3_tpl<F>& rad, const jVec3_tpl<F>& t )	{
			Matrix44_tpl<F,4,1> m44; m44.SetRotationZYX(rad,t);	return m44;
		}
		ILINE static Matrix44_tpl<F,4,1> CreateRotationZYX( const jVec3_tpl<F>& rad )	{
			Matrix44_tpl<F,4,1> m44; m44.SetRotationZYX(rad);	return m44;
		}

		//NOTE: all vectors are stored in columns
		/*ILINE void SetMatFromVectors44(const jVec3& vx, const jVec3& vy, const jVec3& vz, const jVec3& pos)	{
			M00=vx.x;		M01=vy.x;		M02=vz.x;		M03 = pos.x;
			M10=vx.y;		M11=vy.y;		M11=vz.y;		M13 = pos.y;
			M20=vx.z;		M21=vy.z;		M22=vz.z;		M23 = pos.z;
			M30=0.0f;		M31=0.0f;		M32=0.0f;		M33 = 1.0f;
		}
		ILINE friend Matrix44_tpl<F,4,1> GetMatFromVectors44(const jVec3_tpl<F>& vx, const jVec3_tpl<F>& vy, const jVec3_tpl<F>& vz, const jVec3_tpl<F>& pos) {
		...
		*  Example 1:
		*		Matrix44 m44;
		*		m44.SetTransMat44( jVec3(0.5f, 1.0f, 2.0f) );
		*
		*  Example 2:
		*		Matrix44 m44 = GetTransMat44( jVec3(0.5f, 1.0f, 2.0f) );
		*
		*/
/*		ILINE void SetTransMat44(  const jVec3_tpl<F>& v  ) {
			M00=1.0f;	M01=0.0f;	M02=0.0f;	M03=v.x;
			M10=0.0f;	M11=1.0f;	M12=0.0f;	M13=v.y;
			M20=0.0f;	M21=0.0f;	M22=1.0f;	M23=v.z;
			M30=0.0f;	M31=0.0f;	M32=0.0f;	M33=1.0f;
		}
		ILINE static Matrix44_tpl<F,4,1> GetTransMat44( const jVec3_tpl<F>& v  ) {
...
//-------------------------------------------------------------------------------------
		...
		ILINE void BuildFromVectors(const jVec3& vx, const jVec3& vy, const jVec3& vz, const jVec3& pos)
		{
		...
		ILINE void ScaleMatRow( const jVec3_tpl<F>& s)	
		{
		...
		ILINE jVec3 TransformVectorOLD(const jVec3 &b) const  
		{
			jVec3 v;
		...
		//! transform a point by a "transposed" matrix
		//this function expects the matrix to be in transposed format in memory => othonormal base in rows.
		ILINE jVec3 TransformPointOLD(const jVec3 &b) const  
		{
			jVec3 v;
		...
		ILINE jVec3 GetTranslationOLD() const	        {	return jVec3( M30, M31, M32 );	}
		ILINE void SetTranslationOLD( const jVec3& t )	{	M30 =t.x;	M31 =t.y;	M32 =t.z;	}
		ILINE void AddTranslationOLD( const jVec3& t )	{	M30+=t.x;	M31+=t.y;	M32+=t.z;	}
		...
		*  Example 1:
		*		Matrix m34;
		*		m34.translationXYZ34( jVec3(0.5f, 1.0f, 2.0f) );
		*
		*  Example 2:
		*		Matrix34 m34 = translationXYZ34( jVec3(0.5f, 1.0f, 2.0f) );
		*
		*/
	ILINE void SetTranslationMat(  const jVec3_tpl<F>& v  ) {
		M00=1.0f;	M01=0.0f;	M02=0.0f;	M03=0.0f;
	...
	ILINE friend Matrix44_tpl<F,4,1> GetTranslationMat( const jVec3_tpl<F>& v  ) {
...
ILINE jVec3 transform_vector(const Matrix44 &mtx, const jVec3 vec) {
	jVec3 res = mtx*vec;
	res.x += mtx(0,3);
	res.y += mtx(1,3);
	res.z += mtx(2,3);
...
ILINE jVec3 UntransformVector (const Matrix44& m, const jVec3& v) {
	return jVec3 (
