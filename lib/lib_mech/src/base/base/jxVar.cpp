...
#define __JVAR_NAME_BY_TYPE(TAG_STRING , ENUM_VAR) \
			case ENUM_VAR:			_stprintf(szOut, _T(#TAG_STRING) _T("%s"), szTagName);				break;

		switch(m_eType)
		{
			_for_each_data_type_jvar(__JVAR_NAME_BY_TYPE)
		default:	        _tcscpy(szOut, szTagName);						break;
		}
		return szOut;
	}
		...
		//****************************************************************
	{
		...
		_tcscpy(szName , szTag);
		_tcscpy(szType , szTag);
#define __JVAR_TYPE_FINDER_BY_TAGNAME(TAG_STRING,ENUM_VAR) \
	else if( _T(#TAG_STRING)[0]!=_T('0') && !_tcsncmp(szType ,  _T(#TAG_STRING) ,_tcslen(_T(#TAG_STRING)) ) )\
		{	m_eType = ENUM_VAR ; 	_tcscpy( szTag , szName+_tcslen(_T(#TAG_STRING) )); 	}	
...
#define __JVAR_ENUM_STR(x) if(!jt_strncmp(_T(#x), szName, jt_strlen(_T(#x)) )) return x;
	...
	//---------------------------------------------------------------
	void jxVar::SaveXML(jXmlFile* fp)const 
		//----------------------------------------------------------------
	{
		throw _T("jxVar::SaveXML");
		if(fp->fp)
		{
			if(GetType()==ETelement) return;
			fname_t buf;
			cstr sz = ToString(buf);
			jt_fprintf(fp->fp, _T("%s"),sz);
		}
		else
		{
...
#define _InitValue_define(X)		jIVar* 	jxVar::InitValue(X x){ SetType(ED##X); Set_##X(x); return this;}
	...
	cstr jxVar::ToString(TCHAR szBufferTemp[256]) const 
	{
		...
		case EDcstr:			
			{
				cstr sz = Get_cstr();
				_tcsncpy(szBufferTemp, sz, iMAX_LEN );
				return sz;
			}
		case EDargb :
			{
		...
		case EDmatrix33 :// v에는 float v[3*3]
			{
				...
				fname_t sz1;
				_tcscpy(sz1 , szBufferTemp);
				for(int idx = 1 ; idx< itot;++idx)
				{
					jt_sprintf(szBufferTemp,_T(";%5.3f") ,m_float_p[idx]);
					_tcscat(sz1, szBufferTemp);
				}
				_tcscpy(szBufferTemp, sz1);
		...
		case EDargb :
			{
				...
				int aa, rr, gg, bb;
				aa = _tstoi(out[0].c_str() );
				rr = _tstoi(out[1].c_str() );
				gg = _tstoi(out[2].c_str() );
				bb = _tstoi(out[3].c_str() );
		...
		case EDrgb:
			{
				...
				int rr, gg, bb;
				rr = _tstoi(out[0].c_str() );
				gg = _tstoi(out[1].c_str() );
				bb = _tstoi(out[2].c_str() );
				m_int = RGB(rr,gg,bb);
				break;
			}
		case EDint2: 
		...
		case EDint4 :// v에는 int v[4]
			{
				...
				for(uint idx = 0 ; idx< itot;++idx)
				{
					...
					//m_int_p[idx] = _tstoi(out[idx].c_str() );
				}
				break;
			}
	...
	//---------------------------------------------------------------
	void jxVar::writeChunk(nUtil::jMemoryChunk& mc)const 
		//----------------------------------------------------------------
	{
		mc.writeChunk(m_eType);
		writePacket(mc);
	}

	//---------------------------------------------------------------
	void jxVar::writePacket(nUtil::jMemoryChunk& mc)const  // network packet용.
		//----------------------------------------------------------------
	{
		if(IsIntSizeValue())
		{
			if(m_eType==jIVar::EDStringID)
			{
				mc.writeChunk(const_cast<TCHAR*>(Get_cstr() ) );
			}
			else 	
			{
				if(IsOuterMemorySource())
				{
					mc.writeChunk(*(int*)m_int_p);
				}
				else
					mc.writeChunk(m_int); 
			}
		...
		{
			uchar itot=GetArraySize();
			mc._writeChunk(itot*sizeof(int) , m_int_p);
		}
	}

	//---------------------------------------------------------------
	void jxVar::readChunk(nUtil::jMemoryChunk& mc)
		//----------------------------------------------------------------
	{
		uchar eType; // 데이타 타입.
		mc.readChunk(eType);
		SetType((jIVar::EElementType) eType);
		readPacket(mc);
	}

	//---------------------------------------------------------------
	void jxVar::readPacket(nUtil::jMemoryChunk& mc)
		//----------------------------------------------------------------
	{
		if(IsIntSizeValue())
		{
		...
		else if( IsArrayValue())
		{
			jASSERT0(m_int_p);
			uchar itot=GetArraySize();
			mc._readChunk(itot*sizeof(int) , m_int_p);
		}
	}
		...
		//--------------------------------------------------------------------------
	{
		...
		if(m_eType == jxVar::EDcstr || isEle)	
		{
			...
			if(IsOuterMemorySource())
			{
				jt_strncpy((TCHAR*)m_cstr, sz,255);
				m_cstr[255]=0;
			}
			else
			{
				size_t iLen = jt_strlen(sz);
				m_cstr=nString::char_p_resize(&m_cstr , iLen);
				jt_strcpy( m_cstr , sz);
			}
		}
	...
	bool		jxVar::operator <(const  jxVar& var) const 
	{
		...
		if(m_eType!= var.m_eType) return false;
			...
			return ( 0  > _tcscmp( Get_cstr() , var.Get_cstr() ) );
		else
		...
		//--------------------------------------------------------------------------
	{
		...
		{
			isRet = ( !_tcscmp( Get_cstr() , var.Get_cstr() ) );
		}
		else if(IsArrayValue())
		{
	...
	void jxVar::ChangeMemorySource(void* pVout)
	{
		jRAISE(m_eType < EDataType_SIMPLE_END);
		//jASSERT0(m_eType < EDataType_SIMPLE_END);
		if(!pVout)
		{
			jASSERT0(IsOuterMemorySource());
			c8InnerOption &=(!eIS_OUTER_MEMORY);
			m_int_p=0;
			return;
		}
		...
		case EDcstr:
			{
				if(m_cstr)
					jt_strncpy((TCHAR*)pVout , m_cstr , 255);
				m_cstr= (TCHAR*)pVout;
				c8InnerOption |= eIS_OUTER_MEMORY;
				break;
			}
		default:
			assert(0&& _T("void jxVar::ChangeMemorySource() Invalid data type"));
		}
	}
	...
	void jxVar::_Resize_string(size_t i)
	{ 
		assert_jxVar(GetType() == EDcstr);
		assert_jxVar(!IsOuterMemorySource());
		if(nString::char_p_size(m_cstr) < i)
		{
			nString::char_p_delete(m_cstr);
			m_cstr = nString::char_p_new(i);
			memset(m_cstr, 0, i);
		}
	}
