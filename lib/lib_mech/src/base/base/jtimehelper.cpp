...
const TCHAR*  jTimeHelper::Get_diff_day_string(IN time_t tFuture, TCHAR buf[128])
{
	...
	jt_sprintf(buf, _T("%d/%d/%d %d:%d:%d"), st.wYear,st.wMonth,st.wDay,st.wHour,st.wMinute,st.wSecond);
	return buf;
}
...
SYSTEMTIME jTimeHelper::to_day(IN int diff_second)
{
	SYSTEMTIME t;
	memset(&t,0,sizeof(t));
	to_day(diff_second, t.wDay,t.wHour,t.wMinute,t.wSecond);
	return t;
}
