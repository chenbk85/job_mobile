...
namespace nMech
{
	namespace nUNI
	{
		...
		public:
			...
			jStringW(const jStringW& stringSrc)
			{
				jASSERT0(stringSrc.GetData()->nRefs != 0);
				if (stringSrc.GetData()->nRefs >= 0)
				{
					jASSERT0(stringSrc.GetData() != _atltmpDataNilW);
			...
			jStringW(WCHAR ch, int nRepeat = 1)
			{
				Init();
				if (nRepeat >= 1)
				{
			...
			jStringW(LPCSTR lpsz)
			{
				...
				if (lpsz != NULL && HIWORD(lpsz) == NULL)
				{
					...
					if (!LoadString(nID))
						jLOG(jFUNC1 _T("Warning: implicit LoadString(%u) in CString failed\n"), nID);
				}
				else
				{
			...
			jStringW(LPCSTR ch, int nLength)
			{
				Init();
				if (nLength != 0)
				{
			...
			jStringW(LPCWSTR lpch, int nLength)
			{
				Init();
				if (nLength != 0)
				{
			...
			jStringW(const unsigned char* psz) 
			...
			const jStringW& operator=(const jStringW& stringSrc)
			{
			...
			const jStringW& operator=(WCHAR ch)
			{
				AssignCopy(1, &ch);
				return *this;
			}

			const jStringW& operator=(LPCSTR lpsz)
			{
				int nSrcLen = (lpsz != NULL) ? lstrlenA(lpsz) : 0;
			...
			const jStringW& operator=(LPCWSTR lpsz)
			{
			...
			const jStringW& operator=(const unsigned char* psz)
			{ *this = (LPCSTR)psz; return *this; }

			// string concatenation
			const jStringW& jStringW::operator+=(LPCWSTR lpsz)
			{
				jASSERT0(lpsz == NULL || _IsValidString(lpsz));
			...
			const jStringW& operator+=(LPCSTR lpsz)
			{
				jASSERT0(lpsz == NULL || _IsValidString(lpsz));
				int nSrcLen = (lpsz != NULL) ? lstrlenA(lpsz) : 0;
				ConcatInPlace(nSrcLen, jStringW(lpsz));
				return *this;
			}
			...
			friend jStringW __stdcall operator+(const jStringW& string1, const jStringW& string2);
			friend jStringW __stdcall operator+(const jStringW& string, char ch);
			friend jStringW __stdcall operator+(const jStringW& string, WCHAR ch);
			friend jStringW __stdcall operator+(WCHAR ch, const jStringW& string);
			friend jStringW __stdcall operator+(char ch, const jStringW& string);
			...
			jStringW Mid(int nFirst, int nCount) const
			{
			...
			jStringW Mid(int nFirst) const
			{
				return Mid(nFirst, GetData()->nDataLength - nFirst);
			}

			jStringW Left(int nCount) const
			{
				if (nCount < 0)
					nCount = 0;
			...
			jStringW Right(int nCount) const
			{
				if (nCount < 0)
					nCount = 0;
				else if (nCount > GetData()->nDataLength)
			...
			jStringW SpanIncluding(LPCWSTR lpszCharSet) const
			{
				// strspn equivalent
			...
			jStringW SpanExcluding(LPCWSTR lpszCharSet) const
			{
				// strcspn equivalent
				jASSERT0(_IsValidString(lpszCharSet));
		...
		protected:
			...
			{ jASSERT0(m_pchData != NULL); return ((CStringDataW*)m_pchData) - 1; }

			void Init()
			{ m_pchData = _GetEmptyString().m_pchData; }

			void AllocCopy(jStringW& dest, int nCopyLen, int nCopyIndex, int nExtraLen) const
			{
			...
			static int __stdcall _LoadString(UINT nID, LPWSTR lpszBuf, UINT nMaxBuf,HINSTANCE hResourceInst)
			{
#ifdef _DEBUG
...
#endif //_DEBUG
				int nLen = ::LoadStringW(hResourceInst, nID, lpszBuf, nMaxBuf);
				if (nLen == 0)
					lpszBuf[0] = '\0';
				return nLen;
			}

			static const jStringW& __stdcall _GetEmptyString()
			{ return *(jStringW*)&_atltmpPchNilW; }

			// jStringW conversion helpers
			...
			static WCHAR* _cstrchr(const WCHAR* p, WCHAR ch)
			{ return const_cast<WCHAR*>(wcschr(p, ch)); }

			static WCHAR* _cstrrchr(const WCHAR* p, WCHAR ch)
			{ return const_cast<WCHAR*>(wcsrchr(p, ch)); }

			static WCHAR* _cstrrev(WCHAR* pStr)
			{ return _wcsrev(pStr); }

			static WCHAR* _cstrstr(const WCHAR* pStr, const WCHAR* pCharSet)
			{ return const_cast<WCHAR*>(wcsstr(pStr, pCharSet)); }
			...
			static WCHAR* _cstrpbrk(const WCHAR* p, const WCHAR* lpszCharSet)
			{ return const_cast<WCHAR*>(wcspbrk(p, lpszCharSet)); }
		...
		// Compare helpers
		inline bool __stdcall operator==(const jStringW& s1, const jStringW& s2)
		{ return s1.Compare(s2) == 0; }
		inline bool __stdcall operator==(const jStringW& s1, LPCWSTR s2)
		{ return s1.Compare(s2) == 0; }
		inline bool __stdcall operator==(LPCWSTR s1, const jStringW& s2)
		...
		// friend functions
		inline jStringW __stdcall operator+(const jStringW& string1, const jStringW& string2)
		{
			jStringW s;
		...
		inline jStringW __stdcall operator+(const jStringW& string1, char ch)
		{
			jStringW s;
			WCHAR wch(ch);
		...
		inline jStringW __stdcall operator+(const jStringW& string1, WCHAR ch)
		{
			jStringW s;
		...
		inline jStringW __stdcall operator+(WCHAR ch, const jStringW& string1)
		{
			jStringW s;
		...
		inline jStringW __stdcall operator+(char ch, const jStringW& string1)
		{
			jStringW s;
			WCHAR wch(ch);
