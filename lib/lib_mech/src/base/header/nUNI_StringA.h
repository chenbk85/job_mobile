...
http://www.codeproject.com/string/stringwa.asp
*/
...
namespace nMech
{
	namespace nUNI
	{
		...
		public:
			...
			jStringA(const jStringA& stringSrc)
			{
				if(stringSrc.GetData()->nRefs == 0)
					throw _T("jStringA(const jStringA& stringSrc) 1");
				if (stringSrc.GetData()->nRefs >= 0)
				{
					if(stringSrc.GetData() == _atltmpDataNilA)
						throw _T("jStringA(const jStringA& stringSrc) 2");
			...
			jStringA(CHAR ch, int nRepeat = 1)
			{
				Init();
				if (nRepeat >= 1)
				{
			...
			jStringA(LPCSTR lpsz)
			{
				...
				if (lpsz != NULL && HIWORD(lpsz) == NULL)
				{
					...
					if (!LoadString(nID))
						jWARN(_T("implicit LoadString(%u) in CString failed\n"), nID);
				}
				else
				{
			...
			jStringA(LPCSTR ch, int nLength)
			{
				Init();
				if (nLength != 0)
				{
			...
			jStringA(LPCWSTR lpch, int nLength)
			{
				Init();
				if (nLength != 0)
				{
			...
			jStringA(const unsigned char* psz) 
			...
			void Empty()                       // free up the data
			{
				...
				jASSERT0(GetData()->nDataLength == 0);
				jASSERT0(GetData()->nRefs < 0 || GetData()->nAllocLength == 0);
			}

			CHAR GetAt(int nIndex) const       // 0 based
			{
				jASSERT0(nIndex >= 0);
				jASSERT0(nIndex < GetData()->nDataLength);
			...
			const jStringA& operator=(const jStringA& stringSrc)
			{
				if (m_pchData != stringSrc.m_pchData)
				{
					...
					else
					{
						...
						Release();
						jASSERT0(stringSrc.GetData() != _atltmpDataNilA);
						m_pchData = stringSrc.m_pchData;
						InterlockedIncrement(&GetData()->nRefs);
					}
				}
			...
			const jStringA& operator=(WCHAR wch)
			{
				jStringA ch((LPCWSTR)wch, 1);
				AssignCopy(1, ch);
				return *this;
			}

			const jStringA& operator=(CHAR ch)
			{
				AssignCopy(1, &ch);
				return *this;
			}

			const jStringA& operator=(LPCSTR lpsz)
			{
				jASSERT0(lpsz == NULL || _IsValidString(lpsz));
				AssignCopy(SafeStrlen(lpsz), lpsz);
				return *this;
			}

			const jStringA& operator=(LPCWSTR lpsz)
			{
				int nSrcLen = (lpsz != NULL) ? lstrlenW(lpsz) : 0;
			...
			const jStringA& operator=(const unsigned char* psz)
			{ *this = (LPCSTR)psz; return *this; }

			// string concatenation
			const jStringA& jStringA::operator+=(LPCWSTR lpsz)
			{
				jASSERT0(lpsz == NULL || _IsValidString(lpsz));
				int nSrcLen = (lpsz != NULL) ? lstrlenW(lpsz) : 0;
				ConcatInPlace(nSrcLen, jStringA(lpsz));
				return *this;
			}
			...
			friend jStringA __stdcall operator+(const jStringA& string1, const jStringA& string2);
			friend jStringA __stdcall operator+(const jStringA& string, char ch);
			friend jStringA __stdcall operator+(const jStringA& string, WCHAR ch);
			friend jStringA __stdcall operator+(WCHAR ch, const jStringA& string);
			friend jStringA __stdcall operator+(char ch, const jStringA& string);
			...
			jStringA Mid(int nFirst, int nCount) const
			{
			...
			jStringA Mid(int nFirst) const
			{
				return Mid(nFirst, GetData()->nDataLength - nFirst);
			}

			jStringA Left(int nCount) const
			{
				if (nCount < 0)
					nCount = 0;
			...
			jStringA Right(int nCount) const
			{
				if (nCount < 0)
					nCount = 0;
				else if (nCount > GetData()->nDataLength)
			...
			jStringA SpanIncluding(LPCSTR lpszCharSet) const
			{
				// strspn equivalent
				jASSERT0(_IsValidString(lpszCharSet));
				return Left(_cstrspn(m_pchData, lpszCharSet));

			}

			jStringA SpanExcluding(LPCSTR lpszCharSet) const
			{
				// strcspn equivalent
				jASSERT0(_IsValidString(lpszCharSet));
			...
			int Replace(LPCSTR lpszOld, LPCSTR lpszNew)
			{
				...
				if (nCount > 0)
				{
					...
					jASSERT0(m_pchData[nNewLength] == '\0');
			...
			{
				jASSERT0(_IsValidString(lpszSub));
			...
			{
				jASSERT0(_IsValidString(lpszFormat));
				...
				for (LPCSTR lpsz = lpszFormat; *lpsz != '\0'; lpsz = ::CharNextA(lpsz))
				{
					...
					else
					{
						...
						case 'g':
						case 'G':
							jASSERT0(!"Floating point (%%e, %%f, %%g, and %%G) is not supported by the WTL::CString class.");
#ifndef _DEBUG
						...
						default:
							jASSERT0(FALSE);  // unknown formatting option
						}
					}
			...
			// Windows support
			BOOL LoadString(UINT nID,HINSTANCE hResourceInst=0)          // load from string resource (255 chars max)
			{
				// try fixed buffer first (to avoid wasting space in the heap)
				CHAR szTemp[256];
				int nCount =  sizeof(szTemp) / sizeof(szTemp[0]);
				int nLen = _LoadString(nID, szTemp, nCount,hResourceInst);
				if (nCount - nLen > 2)
				{
					*this = szTemp;
					return (nLen > 0);
				...
				do
				{
					...
					nLen = _LoadString(nID, lpstr, nSize,hResourceInst);
				} while (nSize - nLen <= 2);
				ReleaseBuffer();
			...
			LPSTR GetBuffer(int nMinBufLength)
			{
				...
				jASSERT0(GetData()->nRefs <= 1);

				// return a pointer to the character storage for this string
				jASSERT0(m_pchData != NULL);
				return m_pchData;
			}
			...
			void ReleaseBuffer(int nNewLength = -1)
			{
				...
				jASSERT0(nNewLength <= GetData()->nAllocLength);
				GetData()->nDataLength = nNewLength;
			...
			void FreeExtra()
			{
				jASSERT0(GetData()->nDataLength <= GetData()->nAllocLength);
				if (GetData()->nDataLength != GetData()->nAllocLength)
				{
					...
					if(AllocBuffer(GetData()->nDataLength))
					{
						...
						jASSERT0(m_pchData[GetData()->nDataLength] == '\0');
						jStringA::Release(pOldData);
					}
				}
				jASSERT0(GetData() != NULL);
			}
		...
		protected:
			...
			{ jASSERT0(m_pchData != NULL); return ((CStringDataA*)m_pchData) - 1; }

			void Init()
			{ m_pchData = _GetEmptyString().m_pchData; }

			void AllocCopy(jStringA& dest, int nCopyLen, int nCopyIndex, int nExtraLen) const
			{
			...
			BOOL AllocBuffer(int nLen)
			{
				...
				jASSERT0(nLen >= 0);
				jASSERT0(nLen <= INT_MAX - 1);    // max size (enough room for 1 extra)
			...
			static int __stdcall _LoadString(UINT nID, LPSTR lpszBuf, UINT nMaxBuf,HINSTANCE hResourceInst=0)
			{
#ifdef _DEBUG
				...
				if (::FindResource(hResourceInst , MAKEINTRESOURCE((nID>>4) + 1), RT_STRING) == NULL)
				{
					lpszBuf[0] = '\0';
					return 0; // not found
				}
#endif //_DEBUG
				int nLen = ::LoadStringA(hResourceInst , nID, lpszBuf, nMaxBuf);
				if (nLen == 0)
					lpszBuf[0] = '\0';
				return nLen;
			}
			static const jStringA& __stdcall _GetEmptyString()
			{ return *(jStringA*)&_atltmpPchNilA; }

			// jStringA conversion helpers
			static int __cdecl _wcstombsz(char* mbstr, const wchar_t* wcstr, size_t count)
			{
				...
				jASSERT0(mbstr == NULL || result <= (int)count);
				if (result > 0)
					mbstr[result - 1] = 0;
				return result;
			}
			...
			static int __cdecl _mbstowcsz(wchar_t* wcstr, const char* mbstr, size_t count)
			{
				...
				jASSERT0(wcstr == NULL || result <= (int)count);
				if (result > 0)
					wcstr[result - 1] = 0;
				return result;
			}
			...
			static CHAR* _cstrchr(const CHAR* p, CHAR ch)
			{ return const_cast<char*>(strchr(p, ch)); }

			static CHAR* _cstrrchr(const CHAR* p, CHAR ch)
			{ return const_cast<char*>(strrchr(p, ch)); }

			static CHAR* _cstrrev(CHAR* pStr)
			{ return _strrev(pStr); }

			static CHAR* _cstrstr(const CHAR* pStr, const CHAR* pCharSet)
			{ return const_cast<char*>(strstr(pStr, pCharSet)); }
			...
			static CHAR* _cstrpbrk(const CHAR* p, const CHAR* lpszCharSet)
			{ return const_cast<char*>(strpbrk(p, lpszCharSet)); }
		...
		// Compare helpers
		inline bool __stdcall operator==(const jStringA& s1, const jStringA& s2)
		{ return s1.Compare(s2) == 0; }
		inline bool __stdcall operator==(const jStringA& s1, LPCSTR s2)
		{ return s1.Compare(s2) == 0; }
		inline bool __stdcall operator==(LPCSTR s1, const jStringA& s2)
		...
		// friend functions
		inline jStringA __stdcall operator+(const jStringA& string1, const jStringA& string2)
		{
			jStringA s;
		...
		inline jStringA __stdcall operator+(const jStringA& string1, char ch)
		{
			jStringA s;
		...
		inline jStringA __stdcall operator+(const jStringA& string1, WCHAR wch)
		{
			jStringA s;
			jStringA ch((LPCWSTR)&wch, 1);
			s.ConcatCopy(string1.GetData()->nDataLength, string1.m_pchData, 1, ch);
		...
		inline jStringA __stdcall operator+(WCHAR wch, const jStringA& string1)
		{
			jStringA s;
			jStringA ch((LPCWSTR)&wch, 1);
		...
		inline jStringA __stdcall operator+(char ch, const jStringA& string1)
		{
			jStringA s;
			s.ConcatCopy(1, &ch, string1.GetData()->nDataLength, string1.m_pchData);
			return s;
