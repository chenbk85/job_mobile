...
#define gen_ToString_base_type(TYPE,TOK) \
	...
	inline TYPE		FromString(tcstr szBuf,TYPE& v){jt_sscanf(szBuf,jT(TOK),&v);return v;}\
	inline TYPE		FromString(std::tstring &szBuf,TYPE& v){jt_sscanf(szBuf.c_str(),jT(TOK),&v);return v;}\
...
#define for_each_ToString_string_type(X)\
	...
	inline tcstr ToString(TYPE v,tname1024_t szBuf){ jt_sprintf_s(szBuf,1023,_T(TOK),VAR);return szBuf;}\
