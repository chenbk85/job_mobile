...
namespace nMech
{
	...
	// _jScopedLock
	template<typename T>
	class _jScopedLock : private _NonCopyable
	{
	private:
		T &mutex_;
		bool locked_;
	public:
		explicit _jScopedLock(T &mutex, bool initiallyLocked = true):mutex_(mutex),locked_(false){if (!initiallyLocked ) return;Lock();	}
		~_jScopedLock(void){if (locked_ == true){UnLock();}	}

		void Lock(void)
		{
			if (locked_ == true){assert(!"[_jScopedLock::Lock()] locked_ must be false."); throw "[_jScopedLock::Lock()] locked_ must be false.";}
			mutex_.Lock();
			locked_ = true;
		}
		void UnLock(void)
		{
			if (locked_ == false){assert(!"[_jScopedLock::UnLock()] locked_ must be true.");throw "[_jScopedLock::UnLock()] locked_ must be true.";}
			locked_ = false;
			mutex_.UnLock();
		}
	...
	#define jAUTO_LOCK_MU(x) nMech::_jScopedLock<jMutex> UIDEN(_jMutex)(x);
}//namespace nMech
