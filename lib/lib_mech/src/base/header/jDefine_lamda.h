...
#if 0	
		struct __get_tbl_pitcherskill
		{
			vector<s_tbl_pitcherskill*>& out;
			Tbl_PlayerPitcher_id_t pid;
			__get_tbl_pitcherskill( OUT vector<s_tbl_pitcherskill*>& _out,IN Tbl_PlayerPitcher_id_t _pid )
				:out(_out),pid(_pid)
			{

			}
			void operator()(s_tbl_pitcherskill* it)
			{
				if (it && it->pid() == pid) 
					out.push_back(it);
			}
...
#define jLAMDA_CTOR(STRUCT)	struct STRUCT{cstr get_name(){ return jS(NAME);}jLAMDA_##STRUCT(_jLAMDA_MEMBER_,_jLAMDA_MEMBER_); \
	STRUCT(){};\
	STRUCT( jLAMDA_##STRUCT(_jLAMDA_ARG_X,_jLAMDA_ARG_Z)):\
	jLAMDA_##STRUCT(_jLAMDA_SET_ARG_X,_jLAMDA_SET_ARG_Z)\
