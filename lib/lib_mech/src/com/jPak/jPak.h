#ifndef __JIPAK_HEADER__
#define __JIPAK_HEADER__
#ifdef JPAK_EXPORTS
#define JPAK_API  __declspec(dllexport)
#else
#define JPAK_API  __declspec(dllimport)
#endif

#include "ZipDir.h"
#include "MTSafeAllocator.h"
#include "jc/CritSection.h"
...
namespace nMech
{
	namespace nPAK
	{
		...
		struct CCachedFileData: public _reference_target_MT_novtbl<CCachedFileData> //, public jICachedFileData
		{
			CCachedFileData (class jPak* pPak, ZipDir::Cache*pZip, ZipDir::FileEntry* pFileEntry);

			~CCachedFileData();

			virtual unsigned long GetUnCompFileSize(){ return GetFileEntry()->desc.lSizeUncompressed;}
			virtual unsigned short GetFileEntryMethod(){ return GetFileEntry()->nMethod;}
			virtual const char* GetFilePath(){ return GetZip()->GetFilePath();}
		...
		//typedef CCachedFileData_AutoPtr CCachedFileData_AutoPtr;
		...
		struct CZipPseudoFile
		{
			...
			// this object needs to be freed manually when the jcPak shuts down..
			void Destruct()
			{
		...
		protected:
			size_t m_nCurSeek;
			CCachedFileData_AutoPtr m_pFileData;
			// nFlags is a combination of _O_... flags
			unsigned m_nFlags;
			FILE* m_fDirect;
		};
		...
		class CjcPakFindData: public _reference_target_t
		{
		public:
			// the directory wildcard must already be adjusted
			CjcPakFindData (class jPak*pPak, const char* szDir);
			bool	empty() const;
			bool	Fetch(_finddata_t* pfd);
			void	Scan(jPak*pPak, const char* szDir);

			size_t sizeofThis()const;
		protected:
			void ScanFS(jPak*pPak, const char* szDir);
			void ScanZips(jPak*pPak, const char* szDir);
		...
		TYPEDEF_AUTOPTR(CjcPakFindData);



		//////////////////////////////////////////////////////////////////////
		class jPak : public jIPak
		{
		public:
			jIVar* m_pvPriority;
			jIVar* m_pvReadSlice;
			jIVar* m_pvLogMissingFiles;
			int GetPriority(){ return m_pvPriority->intValue();}
			int GetReadSlice(){ return m_pvReadSlice->intValue();}
			int GetLogMissingFiles(){ return m_pvLogMissingFiles->intValue();}

			virtual bool WriteZipFile(char *filename, void *data, unsigned int bitlen);
			virtual unsigned int GetZipFileSize(char *filename);
			virtual unsigned int ReadZipFile(char *filename, void *data, unsigned int maxbitlen);
			virtual nMech::nJC_stream::IStreamEngine *GetStreamEngine(nLOG::jILog* pLog);
			virtual void SetStreamEngine(nMech::nJC_stream::IStreamEngine * pSE);
			virtual void GetMemoryStatistics(jcISizer *pSizer);
		...
		private:
			...
			typedef std::vector<jcIArchive*> ArchiveArray;
			...
			ZipArray m_arrZips;
			friend class CjcPakFindData;

			typedef std::set<CjcPakFindData_AutoPtr> jcPakFindDataSet;
			jcPakFindDataSet m_setFindData;
			...
			typedef std::set<std::string,nJC::stl::less_stricmp<string> > RecordedFilesSet;
		...
		public:
			...
			virtual jcIArchive* OpenArchive (const char* szPath, unsigned nFlags = 0);
			...
			CCachedFileData_AutoPtr GetFileData(const char* szName);

			//jICachedFileData GetFileDataI(const char* szName);
			...
			virtual long FileLen(FILE *handle);
			...
			void Register (jcIArchive* pArchive);
			void Unregister (jcIArchive* pArchive);
			jcIArchive* FindArchive (const char* szFullPath);
			...
			int Compress( void *dest,unsigned int &destLen,const void *source, unsigned int sourceLen,int level );
			int Uncompress( void *dest,unsigned int &destLen,const void *source, unsigned int sourceLen );

		};

		struct jcArchiveSortByName
		{
			bool operator () (const jcIArchive* left, const jcIArchive* right)const
			{
				return stricmp(left->GetFullPath(), right->GetFullPath()) < 0;
			}
			bool operator () (const char* left, const jcIArchive* right)const
			{
				return stricmp(left, right->GetFullPath()) < 0;
			}
			bool operator () (const jcIArchive* left, const char* right)const
			{
		...
		class TjcArchive: public jcIArchive
		{			
		public:
			TjcArchive (jPak*pPak, const string& strBindRoot, Cache* pCache, unsigned nFlags = 0):
					m_pCache(pCache),
		...
		protected:
			...
			const char* AdjustPath (const char* szRelativePath, char szFullPathBuf[jPak::g_nMaxPath])
			{
				if (!szRelativePath[0])
					return NULL;
				...
				if (szRelativePath[1] == ':' || (m_nFlags & FLAGS_ABSOLUTE_PATHS))
				{
					...
					const char* szFullPath = g_pPak->AdjustFileName (szRelativePath, szFullPathBuf, m_nFlags & FLAGS_IGNORE_MODS ? jIPak::FLAGS_PATH_REAL : 0);
		...
		class jcArchiveRW: public TjcArchive<ZipDir::CacheRW>
		{
		public:

			jcArchiveRW (jPak*pPak, const string& strBindRoot, ZipDir::CacheRW* pCache, unsigned nFlags = 0):
					TjcArchive<ZipDir::CacheRW>(pPak, strBindRoot, pCache, nFlags)
					{
					}
		...
		class jcArchive: public TjcArchive<ZipDir::Cache>
		{
		public:
			jcArchive (jPak* pPak, const string& strBindRoot, ZipDir::Cache* pCache, unsigned nFlags):
					TjcArchive<ZipDir::Cache>(pPak, strBindRoot, pCache, nFlags)
					{	}
