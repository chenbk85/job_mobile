...
namespace nMech
{
	namespace nPAK
	{
		...
		CRefReadStreamProxy::CRefReadStreamProxy (const char* szSource, CRefReadStream* pStream, nMech::nJC_stream::IStreamCallback* pCallback, nMech::nJC_stream::StreamReadParams* pParams):
...
#if LOG_IO
			jLOG_T ("\006io:CRefReadStreamProxy %p(%s, %p)", this, szSource, pCallback);
#endif
		...
		CRefReadStreamProxy::~CRefReadStreamProxy ()
		{
			if (!m_bFinished && !m_bError)
				OnFinishRead(nMech::nJC_stream::ERROR_UNEXPECTED_DESTRUCTION);
			if (m_bFreeBuffer && m_pBuffer)
				g_pBigHeap->Free (m_pBuffer);
#if LOG_IO
			jLOG_T ("\006io:~CRefReadStreamProxy %p(%s, %p)", this, m_strClient.c_str(), m_pCallback);
#endif
		...
		void CRefReadStreamProxy::Abort()
		{
			...
			else
			{
				m_pCallback = NULL;
				m_bError = true;
				m_nIOError = nMech::nJC_stream::ERROR_USER_ABORT;
			}
		...
		void CRefReadStreamProxy::Wait()
		{
			...
			{
				m_pStream->GetEngine()->UpdateAndWait(100, nMech::nJC_stream::IStreamEngine::FLAGS_DISABLE_CALLBACK_TIME_QUOTA);
			}
		}
		...
		bool CRefReadStreamProxy::StartRead (unsigned nMemQuota)
		{
			if (m_bError || m_bFinished || m_bPending)
			{
				...
				assert (m_nIOError == nMech::nJC_stream::ERROR_USER_ABORT);
				return true; // invalid call, no need to retry
			}

			if (!m_pStream->Activate())
			{
				OnFinishRead(nMech::nJC_stream::ERROR_CANT_OPEN_FILE); // can't open file
				return true; // no need to retry
			}

			if (m_pStream->IsError())
			{
				OnFinishRead(nMech::nJC_stream::ERROR_REFSTREAM_ERROR); // file is invalid
				return true;
			}
			...
			{
				// offset out of range
				OnFinishRead(nMech::nJC_stream::ERROR_OFFSET_OUT_OF_RANGE);
				return true;
			}

			if (m_Params.nSize > m_pStream->GetFileSize())
			{
				OnFinishRead(nMech::nJC_stream::ERROR_SIZE_OUT_OF_RANGE);
				return true;
			}
			...
			if (m_Params.nSize == 0)
			{
				...
				if (m_Params.nOffset + m_Params.nSize > m_pStream->GetFileSize())
				{
					...
					OnFinishRead(nMech::nJC_stream::ERROR_REGION_OUT_OF_RANGE);
					return true;
				}
				...
				if (!m_pBuffer)
				{
					if (nMemQuota < m_Params.nSize)
					{
						...
						{
							OnFinishRead(nMech::nJC_stream::ERROR_OUT_OF_MEMORY_QUOTA);
							return false;
						}
					}
					...
					if (!m_pBuffer)
					{
						OnFinishRead(nMech::nJC_stream::ERROR_OUT_OF_MEMORY);
						return true;
					}
					m_bFreeBuffer = true;
				}
				...
				unsigned nMaxBlockLength = g_pPak->GetReadSlice() * 1024;
				if (!nMaxBlockLength)
				...
				if ((m_Params.nFlags & nMech::nJC_stream::SRP_FLAGS_ASYNC_PROGRESS) && m_pCallback)
		...
		DWORD CRefReadStreamProxy::CallReadFileEx ()
		{
			...
			if (hFile == INVALID_HANDLE_VALUE)
			{
				...
				if (!pSource)
				{
					OnIOComplete(nMech::nJC_stream::ERROR_ZIP_CACHE_FAILURE,0);
					return 0;
				}
			...
			if (m_pStream->isOverlapped())
			{
				...
				if (!ReadFileEx (hFile, ((char*)m_pBuffer) + m_nPieceOffset, m_nPieceLength, &m_Overlapped, FileIOCompletionRoutine))
				{
					...
					if (!dwError)
						dwError = nMech::nJC_stream::ERROR_CANT_START_READING;
		...
		void CRefReadStreamProxy::FinalizeIO()
		{
			if (m_pCallback)
			{
				...
				nMech::nJC_stream::IStreamCallback* pCallback = m_pCallback;
				m_pCallback = NULL;	// don't call this callback any more as it may have been deallocated
#if LOG_IO
				jLOG_T ("\006io(%s) err %d%s%s%s%s piece(%d:%d) read %d %s userdata %d, pri %d, flags %x, offs %d, size %d", m_strClient.c_str(), m_nIOError, 
...
#if LOG_IO
				jLOG_T ("\006io callback %p returned", pCallback);
#endif
			}
		}
