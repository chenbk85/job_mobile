...
namespace nMech
{
	namespace nPAK
	{
		...
		CRefStreamEngine::CRefStreamEngine (unsigned useWorkerThreads, bool bOverlappedIO):
		m_nMaxReadDepth (16),
		...
		CRefStreamEngine::~CRefStreamEngine()
		{
			...
			while (!m_queIOJobs.empty())
			{
				...
				pJob->OnFinishRead(nMech::nJC_stream::ERROR_ABORTED_ON_SHUTDOWN); // aborted
			}
			...
			if (nInterface::g_pjILog)
				for (NameStreamMap::iterator it = m_mapFilesByName.begin(); it != m_mapFilesByName.end(); ++it)
					GetjILog()->Log(jLogL,_T("%s: %s"), jT(it->first.c_str()), jT(it->second->Dump().c_str()) );
		...
		// MT: Main thread only
		nMech::nJC_stream::IReadStream_AutoPtr CRefStreamEngine::StartRead (const char* szSource, const char* szFilePathPC, nMech::nJC_stream::IStreamCallback* pCallback, nMech::nJC_stream::StreamReadParams* pParams)
		{
			unsigned nFlags = 0;
			if (pParams)
				nFlags = pParams->nFlags;

			g_pPak->RecordFile( szFilePathPC );

			// get rid of some jobs if there are too many in the queue
			if (!(nFlags & nMech::nJC_stream::SRP_QUICK_STARTREAD))
				while (numIOJobs(eWaiting) >= m_nMaxQueueLength)
				{
					GetjILog()->Warning(jLogL
						,jT("StreamEngine: The number of jobs waiting %d >= max queue length %d, waiting to free up the queue")
						, numIOJobs(eWaiting), m_nMaxQueueLength);
					UpdateAndWait(20, FLAGS_DISABLE_CALLBACK_TIME_QUOTA);
				}

				char szFilePathBuf[jIPak::g_nMaxPath];
				const char* szFilePath = g_pPak->AdjustFileName (szFilePathPC, szFilePathBuf
					, pParams && (pParams->nFlags & nMech::nJC_stream::SRP_FLAGS_PATH_REAL) ? jIPak::FLAGS_PATH_REAL: 0);
				...
				if (nFlags & nMech::nJC_stream::SRP_FLAGS_MAKE_PERMANENT)
					m_setLockedStreams.insert (pStream);
				else
					if (nFlags & nMech::nJC_stream::SRP_FLAGS_MAKE_TRANSIENT)
				...
				return (nMech::nJC_stream::IReadStream*)pProxy;
		}
		...
		unsigned CRefStreamEngine::GetFileSize (const char* szFilePathPC, unsigned njcPakFlags)
		{
			char szFilePathBuf[g_pPak->g_nMaxPath];
			const char *szFilePath = g_pPak->AdjustFileName (szFilePathPC, szFilePathBuf, njcPakFlags);
			NameStreamMap::iterator it = m_mapFilesByName.find (szFilePath);
			if (it != m_mapFilesByName.end())
			{
...
#else
			HANDLE hFile = CreateFileA (szFilePath, GENERIC_READ, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
#endif
			...
			{
				extern jPak *g_pPak;
				CCachedFileData_AutoPtr pFileData = g_pPak->GetFileData(szFilePath);
				if (pFileData)
					return pFileData->GetUnCompFileSize();
				else
				{
					g_pPak->OnMissingFile(szFilePathPC);
					return 0;
				}
			}
		}
		...
		void CRefStreamEngine::SetCallbackTimeQuota (int nMicroseconds)
		{
			...
			if (m_nCallbackTimeQuota < 0)
			{
				GetjILog()->Warning(jLogL,_T("\004io: overdraft of callback time quota in the last frame: %I64d mcs"), m_nCallbackTimeQuota);
		...
		{
			if (m_hIOWorker && (pJobProxy->GetParams().nFlags & nMech::nJC_stream::SRP_FLAGS_ASYNC_CALLBACK))
		...
		unsigned CRefStreamEngine::GetSectorSize(const char* szPath)
		{
			...
			if (szPath[0] == '\\' && szPath[1] == '\\')
			{
				...
				nJC::TElementaryArray<char> pShareName;
				pShareName.reinit (pEnd - szPath+1);
				memcpy (&pShareName[0], szPath, pEnd - szPath);
				pShareName[pEnd - szPath] = '\0';

				if (!GetDiskFreeSpaceA(&pShareName[0], &dwSectorsPerCluster, &dwBytesPerSector, &dwNumberOfFreeClusters, &dwTotalNumberOfClusters))
				...
				else
				{
					...
					nJC::TElementaryArray<char> pDir;
					DWORD dwLen = GetCurrentDirectory(0, NULL);
					pDir.reinit (dwLen);
					if (dwLen != GetCurrentDirectoryA(dwLen, &pDir[0]))
					{
						DWORD dwSectorsPerCluster, dwBytesPerSector, dwNumberOfFreeClusters, dwTotalNumberOfClusters;
		...
		unsigned CRefStreamEngine::GetDriveSectorSize (char cDrive)
		{
			...
			{
				DWORD dwSectorsPerCluster, dwBytesPerSector, dwNumberOfFreeClusters, dwTotalNumberOfClusters;
				if (!GetDiskFreeSpaceA(szBuf, &dwSectorsPerCluster, &dwBytesPerSector, &dwNumberOfFreeClusters, &dwTotalNumberOfClusters))
		...
		void CRefStreamEngine::CheckOSCaps()
		{
			...
			{
				GetjILog()->Warning(jLogL,_T("StreamEngine: OS (platform %d) doesn't support streaming, turning overlapped IO off"),os.dwPlatformId );
...
#error // if your OS doesn't support it, you should disable Overlapped IO here
		...
		void CRefStreamEngine::GetMemoryStatistics(jcISizer *pSizer)
		{
			/* *_* by icandoit : 2005-07-09 21:22:51
			...
			*/ //*_* by icandoit : 2005-07-09 21:22:55
		...
		void CRefStreamEngine::ResumeCallbackTimeQuota ()
		{
			if (m_nPerfFreq > 0)
			{
				...
				{
					GetjILog()->Warning(jLogL,_T("\005io: overdraft of callback time quota after resume: %I64d"), m_nCallbackTimeQuota);
					m_nCallbackTimeQuota = 0;
				}
			}
		}
