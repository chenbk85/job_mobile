...
#ifndef _JCGAME_SYSTEM_REFERENCE_STREAM_ENGINE_HDR_
#define _JCGAME_SYSTEM_REFERENCE_STREAM_ENGINE_HDR_
#include <deque>
#include "interface/IMiniLog.h"
...
namespace nMech
{
	namespace nPAK
	{
		...
		class CRefStreamEngine:public nMech::nJC_stream::IStreamEngine
		{
		public:
			...
			CRefStreamEngine(unsigned useWorkerThreads = 1, bool bOverlappedIO = true);

			//! destructor
			~CRefStreamEngine();

			// interface nMech::nJC_stream::IStreamEngine --------------------------------

			virtual DWORD GetStreamCompressionMask() const;

			virtual nMech::nJC_stream::IReadStream_AutoPtr StartRead (const char* szSource
				, const char* szFile
				, nMech::nJC_stream::IStreamCallback* pCallback
				, nMech::nJC_stream::StreamReadParams* pParams = NULL);

			virtual unsigned GetFileSize (const char* szFile, unsigned njcPakFlags);
