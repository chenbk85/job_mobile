...
namespace nMech
{
	...
	bool CRefReadStream::Activate()
	{
		...
		if (m_pZipEntry == NULL && m_hFile == INVALID_HANDLE_VALUE)
			m_hFile = CreateFileA (m_strFileName.c_str(), GENERIC_READ, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
		...
		if (m_pZipEntry == NULL && m_hFile == INVALID_HANDLE_VALUE)
		{
			...
			m_pZipEntry = g_pPak->GetFileData(m_strFileName.c_str());
			if (m_pZipEntry)
			{
				m_nFileSize = m_pZipEntry->GetFileEntry()->desc.lSizeUncompressed;
				//m_nFileSize = m_pZipEntry->GetUnCompFileSize();
				m_bError = false;
				...
				if (!m_pZipEntry->GetData(false)
					// *_* by icandoit : 2005-07-04 11:13:42
					//&& m_pZipEntry->GetFileEntry()->nMethod == ZipFile::METHOD_STORE
					&& m_pZipEntry->GetFileEntryMethod() == ZipFile::METHOD_STORE
					)
				{
					// try to open the file - this should really be not often the case
					const char* szPakFile = m_pZipEntry->GetFilePath();
					// even if we can't open it, it doesn't matter: we automatically resort to using the cache
					m_hFile = CreateFileA (szPakFile, GENERIC_READ, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
						m_bOverlapped?FILE_FLAG_OVERLAPPED:0,
						NULL);
				}
			...
			else
			{
				...
				m_bError = true;
				g_pPak->OnMissingFile(m_strFileName.c_str());
				return false;
			}
		}
		else
