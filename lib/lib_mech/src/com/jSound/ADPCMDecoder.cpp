...
namespace nMech
{
	namespace nJC_SOUND
	{
		...
		{
			FILE *pNewFile = nInterface::g_pjIPak->FOpen(m_szFilename.c_str(), "rb");
			if (!pNewFile)
				return NULL;
			return pNewFile;
		}
		...
		inline void CADPCMDecoder::CloseFileHandle(FILE*& pFile)
		{
			if (pFile) 
			{
				nInterface::g_pjIPak->FClose(pFile);
				pFile=NULL;
			}
		}
		...
		//////////////////////////////////////////////////////////////////////////
		inline void CADPCMDecoder::GetMemoryUsage(nMech::jcISizer* pSizer) 
		{
			if (!pSizer->Add(*this))
				return;
		}
		...
		inline bool CADPCMDecoderInstance::Seek0(int nDelay)
		{
			...
			if (!m_pFile || m_uiDataStartPos == 0)
				return false;
			nInterface::g_pjIPak->FSeek(m_pFile,m_uiDataStartPos,SEEK_SET);
			m_iFilePos = 0;
			return true;
		}
		...
		const bool CADPCMDecoderInstance::InitStreamWAV()
		{
			...
			nInterface::g_pjIPak->FRead(&adpcmHeader,1, sizeof(SADPCMWaveHdr),m_pFile);
			nInterface::g_pjIPak->FRead(&m_sCoefs,	1, sizeof(short),		m_pFile);
			if(adpcmHeader.FormatTag == WAVE_FORMAT_PCM)
			{
#ifdef LOG_MUSICFILES
				nInterface::g_pIMusicSystem->LogMsg( "Uncompressed pcm data, use pcm decoder %s\n", m_pDecoder->FileName());
#else
			...
			{
#ifdef LOG_MUSICFILES
				nInterface::g_pIMusicSystem->LogMsg("No stereo adpcm file:  %s\n", m_pDecoder->FileName());
#else		
			...
			{
#ifdef LOG_MUSICFILES
				nInterface::g_pIMusicSystem->LogMsg("Unsupported adpcm format %s\n", m_pDecoder->FileName());
#else		
				TRACE("unsupported adpcm format %s\n", m_pDecoder->FileName());
#endif
				return false;
			}
			nInterface::g_pjIPak->FRead(m_aCoefs,	1,	m_sCoefs/*7*/ * 2/*stereo->2 channels*/ * sizeof(short), m_pFile);
			//now seek to start position (next to "data")
			unsigned int uiDataLeft = 0;
			for(;;)
			{	
				char pcMagic[ 4 ];
				nInterface::g_pjIPak->FRead( pcMagic, 1, sizeof( unsigned char ) * 4, m_pFile );

				if(0 != nInterface::g_pjIPak->FEof( m_pFile ))
					return  false;
				nInterface::g_pjIPak->FRead( &uiDataLeft, 1, sizeof( unsigned int ), m_pFile );
				if( 0 == strncmp( "data", pcMagic, 4 ) )
					break;		
				nInterface::g_pjIPak->FSeek( m_pFile, uiDataLeft, SEEK_CUR);
			}
			m_uiDataStartPos = nInterface::g_pjIPak->FTell(m_pFile);//to make seek0 work properly, data start position
		...
		{
			//read block
			unsigned short bytesRead( (unsigned short) nInterface::g_pjIPak->FRead( m_aEncodedBlock, 1, m_uiCurrentBlockSize, m_pFile ) );
