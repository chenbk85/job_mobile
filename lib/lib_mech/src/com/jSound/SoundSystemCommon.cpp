...
namespace nMech
{
	namespace nJC_SOUND
	{
		...
		{
			m_pCVARDummySound= g_pIConsole->CreateVariable("s_DummySound","0",VF_DUMPTODISK,
				"Toggles dummy(NULL) sound system.\n");	
			m_pCVARSoundEnable= g_pIConsole->CreateVariable("s_SoundEnable","1",VF_DUMPTODISK,
				"Toggles sound on and off.\n"
				"Usage: s_SoundEnable [0/1]\n"
				"Default is 1 (on). Set to 0 to disable sound.");	
			m_pCVarMaxSoundDist= g_pIConsole->CreateVariable("s_MaxSoundDist","500",VF_DUMPTODISK,
				"Sets the maximum sound distance, in metres. Default is 500 metres.\n"
				"Usage: s_MaxSoundDist 500");	
			m_pCVarDopplerEnable = g_pIConsole->CreateVariable("s_DopplerEnable","0",VF_DUMPTODISK,
				"Toggles Doppler effect on and off.\n"
				"Usage: s_DopplerEnable [0/1]\n"
				"Default is 0 (off).");	
			m_pCVarDopplerValue = g_pIConsole->CreateVariable("s_DopplerValue","1.0",VF_DUMPTODISK,
				"Sets the strength of the Doppler effect.\n"
				"Usage: s_DopplerValue 1.0"
				"Default is 1.0. This multiplier affects the sound velocity.");	
			m_pCVarSFXVolume = g_pIConsole->CreateVariable("s_SFXVolume","1",VF_DUMPTODISK,
				"Sets the percentile volume of the sound effects.\n"
				"Usage: s_SFXVolume 0.5\n"
				"Default is 1, which is full volume.");	
			m_pCVarMusicVolume = g_pIConsole->CreateVariable("s_MusicVolume","0.5",VF_DUMPTODISK,
				"Sets the music volume from 0 to 1.\n"
				"Usage: s_MusicVolume 7\n"
				"Default is 1, which is full volume.");	
			m_pCVarSampleRate = g_pIConsole->CreateVariable("s_SampleRate","44100",VF_DUMPTODISK,
			...
			m_pCVarSpeakerConfig = g_pIConsole->CreateVariable("s_SpeakerConfig","5",VF_DUMPTODISK,
				...
				"	6: Surround\n"
				"Default is 5 (Stereo).");
			m_pCVarEnableSoundFX = g_pIConsole->CreateVariable("s_EnableSoundFX","1",VF_DUMPTODISK,
				"Toggles sound effects on and off.\n"
				"Usage: s_EnableSoundFX [0/1]\n"
				"Default is 1 (on).");
			m_pCVarDebugSound = g_pIConsole->CreateVariable("s_DebugSound","0",VF_CHEAT,
				"Toggles sound debugging mode.\n"
				"Usage: s_DebugSound [0/1]\n" 
				"Default is 0 (off).");
			m_pCVarSoundInfo = g_pIConsole->CreateVariable("s_SoundInfo","0",VF_CHEAT,
				"Toggles onscreen sound statistics.\n"
				"Usage: s_SoundInfo [0/1]"
				"Default is 0 (off). Set to 1 to display sound information.");
			m_pCVarInactiveSoundIterationTimeout = g_pIConsole->CreateVariable("s_InactiveSoundIterationTimeout","1",VF_DUMPTODISK,
				"This variable is for internal use only.");
			m_pCVarMinHWChannels = g_pIConsole->CreateVariable("s_MinHWChannels","16",VF_DUMPTODISK,
			...
			// 28 = hw mixing
			m_pCVarMaxHWChannels= g_pIConsole->CreateVariable("s_MaxHWChannels","28",VF_DUMPTODISK,
				"Sets the maximum number of sound hardware channels used./n"
				"0=software mixing.");
			m_pCVarVisAreaProp=g_pIConsole->CreateVariable("s_VisAreasPropagation","10",VF_DUMPTODISK,
			...
			m_pCVarMaxSoundSpots=g_pIConsole->CreateVariable("s_MaxActiveSoundSpots","100",VF_DUMPTODISK,
				"Sets the maximum number of active soundspots.\n");
			m_pCVarMinRepeatSoundTimeout = g_pIConsole->CreateVariable("s_MinRepeatSoundTimeout","200",VF_DUMPTODISK,
				"Timeout in ms,play of same sounds cannot be repeated more often then that.\n");		
			m_pCVarCompatibleMode = g_pIConsole->CreateVariable("s_CompatibleMode","0",VF_DUMPTODISK,
				"Toggles sound system compatible mode ON/OFF. Use only in case of playback issues.\n");		
			m_pCVarCapsCheck= g_pIConsole->CreateVariable("s_CapsCheck","0",VF_DUMPTODISK,
