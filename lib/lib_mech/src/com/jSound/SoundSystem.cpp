...
#include "interface/jIPak.h" //needed to check if the listener is in indoor or outdoor
...
void* _DSPUnit_SFXFilter_Callback(void *pOriginalBuffer, void *pNewBuffer, int nLength, INT_PTR nParam)
{
	if (!nParam)
		return pNewBuffer;
	nMech::nJC_SOUND::CSoundSystem *pOwner=(nMech::nJC_SOUND::CSoundSystem*)nParam;
...
#endif
static void * F_CALLBACKAPI jcSound_Alloc (unsigned int size)
{
#if CS_SAFEBORDER
...
static void F_CALLBACKAPI jcSound_Free (void* ptr)
{
#if CS_SAFEBORDER
...
static void * F_CALLBACKAPI jcSound_Realloc (void *ptr, unsigned int size)
{
#if CS_SAFEBORDER
	char* pOld = ((char*)ptr) - CS_SAFEBORDER;
	unsigned* pNOld = ((unsigned*)pOld) - 1;

	char* pRet = (char*)jcSound_Alloc(size);
...
static void * F_CALLBACKAPI jcSound_fopen( const char *name )
{
	// File is opened for streaming.
	FILE *file = nInterface::g_pjIPak->FOpen( name,"rb",nMech::nPAK::jIPak::FOPEN_HINT_DIRECT_OPERATION );
	//FILE *file = nInterface::g_pjIPak->FOpen( name,"rb" );
	if (!file)
	{
		if(nInterface::g_pjILog&&nInterface::g_pjILog->LogLevelTest(g_Current_LogLevel_jStringID,jx(MISS) ) )
		GetjILog()->Warning( _T("Sound %s failed to load"), name );
	}
	return (void *)file;
}

static void F_CALLBACKAPI jcSound_fclose( void * nFile )
{
	FILE *file = (FILE*)nFile;
	nInterface::g_pjIPak->FClose( file );
}

static int F_CALLBACKAPI jcSound_fread( void *buffer, int size, void * nFile )
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FRead( buffer,1,size,file );
}

static int F_CALLBACKAPI jcSound_fseek( void * nFile, int pos, signed char mode)
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FSeek( file,pos,mode );
}

static int F_CALLBACKAPI jcSound_ftell( void * nFile )
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FTell( file );
}
...
static unsigned int F_CALLBACKAPI jcSound_fopen( const char *name )
{
	// File is opened for streaming.
	FILE *file = nInterface::g_pjIPak->FOpen( name,"rb",nMech::nPAK::jIPak::FOPEN_HINT_DIRECT_OPERATION );
	//FILE *file = nInterface::g_pjIPak->FOpen( name,"rb" );
...
static void F_CALLBACKAPI jcSound_fclose( unsigned int nFile )
{
	FILE *file = (FILE*)nFile;
	nInterface::g_pjIPak->FClose( file );
}

static int F_CALLBACKAPI jcSound_fread( void *buffer, int size, unsigned int nFile )
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FRead( buffer,1,size,file );
}

static int F_CALLBACKAPI jcSound_fseek( unsigned int nFile, int pos, signed char mode)
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FSeek( file,pos,mode );
}

static int F_CALLBACKAPI jcSound_ftell( unsigned int nFile )
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FTell( file );
}
...
static UINT_PTR F_CALLBACKAPI jcSound_fopen( const char *name )
{
	// File is opened for streaming.
	FILE *file = nInterface::g_pjIPak->FOpen( name,"rb",nMech::nPAK::jIPak::FOPEN_HINT_DIRECT_OPERATION );
	//FILE *file = nInterface::g_pjIPak->FOpen( name,"rb" );
...
static void F_CALLBACKAPI jcSound_fclose( UINT_PTR nFile )
{
	FILE *file = (FILE*)nFile;
	nInterface::g_pjIPak->FClose( file );
}

static int F_CALLBACKAPI jcSound_fread( void *buffer, int size, UINT_PTR nFile )
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FRead( buffer,1,size,file );
}

static int F_CALLBACKAPI jcSound_fseek( UINT_PTR nFile, int pos, signed char mode)
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FSeek( file,pos,mode );
}

static int F_CALLBACKAPI jcSound_ftell( UINT_PTR nFile )
{
	FILE *file = (FILE*)nFile;
	return nInterface::g_pjIPak->FTell( file );
}
...
namespace nMech
{
	namespace nJC_SOUND
	{
		...
		//////////////////////////////////////////////////////////////////////
		CSoundSystem::CSoundSystem(HWND hWnd) : CSoundSystemCommon()
		{
			GUARD_HEAP;
			...
			m_bPause=false;
			m_pStreamEngine=g_pISystem->GetStreamEngine();
			...
			{
				GetjILog()->Log(TEXT("Music:Init:Incorrect DLL version for JC_GAMESOUND: Need %.2f\n"),CS_VERSION);
				return;
			}

#if !defined(_DEBUG)// || defined(WIN64)
			CS_SetMemorySystem(NULL,NULL,jcSound_Alloc,jcSound_Realloc,jcSound_Free);
#endif

			GetjILog()->Log(TEXT("------------------------------------------JC_SOUND VERSION=%f\n"),CS_GetVersion());
			...
			CS_File_SetCallbacks( jcSound_fopen,jcSound_fclose,jcSound_fread,jcSound_fseek,jcSound_ftell );

			for (int i=0; i < CS_GetNumDrivers(); i++) 
			{
				GetjILog()->Log(TEXT("%d - %s\n"), i+1, CS_GetDriverName(i));	// print driver names
				...
				{		
					...
					if (caps & CS_CAPS_HARDWARE)
						GetjILog()->Log(TEXT(" Driver supports hardware 3D sound"));
					if (caps & CS_CAPS_EAX2)
						GetjILog()->Log(TEXT(" Driver supports EAX 2.0 reverb"));
					if (caps & CS_CAPS_EAX3)
						GetjILog()->Log(TEXT(" Driver supports EAX 3.0 reverb"));
					//if (caps & CS_CAPS_GEOMETRY_OCCLUSIONS)
					//	GetjILog()->Log(TEXT(" Driver supports hardware 3d geometry processing with occlusions"));
					//if (caps & CS_CAPS_GEOMETRY_REFLECTIONS)
					//	GetjILog()->Log(TEXT(" Driver supports hardware 3d geometry processing with reflections"));
					//if (caps & CS_CAPS_EAX2)
					//	GetjILog()->Log(TEXT(" Driver supports EAX 2.0 reverb!"));
				}
			}
			...
			{
				GetjILog()->Log(TEXT("Cannot init JC_GAMESOUND\n"));
				CS_Close();
				return;
			}
			//CS_3D_SetRolloffFactor(0.0);

			GetjILog()->Log(TEXT("JC_SOUND Driver: %s"), CS_GetDriverName(CS_GetDriver()));		
			GetjILog()->Log(TEXT("Total number of channels available: %d\n"),CS_GetMaxChannels());

			// WARNING!!! Wat
			//GetjILog()->Log(TEXT("Total number of hardware channels available: %d\n"),CS_GetNumHardwareChannels());	
		...
		{
			GUARD_HEAP;
			// originalbuffer =jcsounds original mixbuffer.
		...
		{
			FUNCTION_PROFILER( PROFILE_SOUND );
		...
		void CSoundSystem::Update()
		{
			...
			if (g_pISystem && m_pCVarSoundInfo->GetIVal())
			{ 
				IRenderer *pRenderer=g_pISystem->GetIRenderer();
				if (pRenderer)
				{
			...
			jVec3_tpl<float> vPos=m_cCam.GetPos();
			...
			*/
			...
			{		
				FRAME_PROFILER( "CSoundSystem::Update:Inactive",PROFILE_SOUND );
				...
				int nInactiveSoundItCount=(int)jc_ceilf((nTIME::Get_ITimer()->GetFrameTime()/m_pCVarInactiveSoundIterationTimeout->GetFVal())*(float)m_nInactiveSoundSpotsSize);	
			...
			{
				FRAME_PROFILER( "CSoundSystem::Update:Active",PROFILE_SOUND );
				...
				{
					FRAME_PROFILER( "CSoundSystem::Update:Active-Deactivate",PROFILE_SOUND );
					//++It;
				...
				if (!bIsSoundPotentiallyHearable)
				{
					FRAME_PROFILER( "CSoundSystem::Update:Active-FadeOut",PROFILE_SOUND );
				...
				if (cs->m_nFlags & FLAG_SOUND_RADIUS)
				{
					...
					{					
						...
						{					
							...
							//fRatio=0;
							fRatio=1.0f;
							//GetjILog()->Log(TEXT("fratio=%f"),fRatio);
						}
						else
				...
				{
					FRAME_PROFILER( "CSoundSystem::Update:Active-SetAttrib",PROFILE_SOUND );
			...
			*/
			...
			{
				FRAME_PROFILER( "CSoundSystem::CS_Update",PROFILE_SOUND );
				CS_Update();
			}
		}
		...
		{
			nInterface::g_pIMusicSystem = new CMusicSystem();
			return nInterface::g_pIMusicSystem;
		}
		...
		//////////////////////////////////////////////////////////////////////
		nMech::nJC_SOUND::ISound* CSoundSystem::LoadSound(const char *szFile, int flags)
		{
			GUARD_HEAP;
			FUNCTION_PROFILER( PROFILE_SOUND );
			...
			if ((!(flags & FLAG_SOUND_STREAM)) && (nit!=m_soundBuffers.end()))	// we cannot share streams !
			{
				...
				{			
					...
					{
						GetjILog()->Log(TEXT("\001 [ERROR] trying to load the same sound buffer file as 2d and 3d sound (%s)"),szFile);
						return (NULL);
					}
				}
				else
			...
			if (!(flags & FLAG_SOUND_LOAD_SYNCHRONOUSLY))
			{
				...
				{
					GetjILog()->Log(TEXT("\001 WARNING - THE FILE %s is a streaming sound but there is an attempt to decompress it into memory-it will not be loaded"),szFile);
					return (NULL);
				}
			}
			...
			//GetjILog()->Log(TEXT("sound %s loaded, sound size=%d"),szFile,m_setSounds.size());
		...
		void	CSoundSystem::RecomputeSoundOcclusion(bool bRecomputeListener,bool bForceRecompute,bool bReset)
		{
			...
			I3DEngine	*p3dEngine=g_pI3DEngine;
			if (p3dEngine)	
			{			
		...
		//////////////////////////////////////////////////////////////////////
		nMech::nJC_SOUND::ISound* CSoundSystem::GetSound(int soundId)
		{
			return (NULL);
		}
		...
		//////////////////////////////////////////////////////////////////////
		void CSoundSystem::SetListener(const nMech::nMath::CCamera &cCam,const jVec3d &vVel)
		{
			GUARD_HEAP;
			FUNCTION_PROFILER( PROFILE_SOUND );
			...
			jVec3d vPos=m_cCam.GetPos();
			if (GetLengthSquared(vPos)<1)
				return; // hasnt been set yet, but the update3d function has been called
			jVec3d vAngles=m_cCam.GetAngles();
			...
			{ 
				jVec3d vTempVel=(vPos-m_vPos);
				float fTimeDelta=nTIME::Get_ITimer()->GetFrameTime();
		...
		void	CSoundSystem::Pause(bool bPause,bool bResetVolume)
		{
			...
			while (It!=m_vecSounds.end())
			{
				...
				{			
					...
					{
						pSound->m_fChannelPlayTime=nTIME::Get_ITimer()->GetCurrTime();
		...
		bool CSoundSystem::SetEaxListenerEnvironment(int nPreset, CS_REVERB_PROPERTIES *tpProps, int nFlags)
		{
			...
			if (nFlags==FLAG_SOUND_OUTDOOR)
			{
				...
				GetjILog()->Log(TEXT("Setting global EAX outdoor"));
				if (m_bInside)
		...
		bool CSoundSystem::SetGroupScale(int nGroup, float fScale)
		{
			...
			//GetjILog()->Log(TEXT("scaling group for %s"),cs->GetName());
		...
		void CSoundSystem::RemoveBuffer(SSoundBufferProps &sn)
		{
			...
			{
				GetjILog()->Log(TEXT("Removing sound [%s]"),itor->first.sName.c_str());
				m_soundBuffers.erase(itor);
			}
			else
			{
				GetjILog()->Log(TEXT("Warning Trying to remove sound [%s] NOT FOUND!!!"),sn.sName.c_str());
			}
		}
		...
		//////////////////////////////////////////////////////////////////////////
		void CSoundSystem::CalcDirectionalAttenuation(jVec3d &Pos, jVec3d &Dir, float fConeInRadians)
		{
			GUARD_HEAP;
			float fCosCone=(float)jc_cosf(fConeInRadians);
		...
		//////////////////////////////////////////////////////////////////////////
		void CSoundSystem::GetMemoryUsage(nMech::jcISizer* pSizer)
		{
			GUARD_HEAP;
