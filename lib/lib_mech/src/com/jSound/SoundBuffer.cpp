	...
	for (TBufferLoadReqVecIt It=m_vecLoadReq.begin();It!=m_vecLoadReq.end();++It) \
		...
		void CSoundBuffer::SoundLoaded()
		{
			...
			if (m_pSoundSystem->m_pCVarDebugSound->GetIVal()==3)
				GetjILog()->Log(TEXT("file %s, loaded \n"), m_Props.sName.c_str());
		...
		{
			IF_jILogT(MISS)
				GetjILog()->Warning( _T("Sound %s failed to load"), m_Props.sName.c_str() );
			m_pReadStream=NULL;
		...
		bool CSoundBuffer::Load(bool bLooping, CSound *pSound)
		{
			...
			{		 
...
#ifndef _ISNOTJGAME
				IGameMods *pMods=g_pISystem->GetIGame()->GetModsInterface();
				if (pMods)		
				...
				{
					//cannot be loaded
					GetjILog()->Log(TEXT("Warning: Cannot load streaming sound %s\n"), m_Props.sName.c_str());
					m_bLoadFailure=true;
					return false;
				}
		...
		//////////////////////////////////////////////////////////////////////////
		void CSoundBuffer::StreamOnComplete(nMech::nJC_stream::IReadStream *pStream, unsigned nError)
		{
			GUARD_HEAP;
			FUNCTION_PROFILER( PROFILE_SOUND );
			if (nError)
			...
			if (!pSample)
			{
				GetjILog()->Log(TEXT("Warning: Cannot load sample sound %s\n"), m_Props.sName.c_str());
				m_bLoadFailure=true;
				LoadFailed();
				return;
			}		
