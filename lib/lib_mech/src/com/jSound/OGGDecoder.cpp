	...
	namespace nJC_SOUND
	{
		...
		bool COGGDecoder::Open(const char *pszFilename)
		{
			if (m_FileAccess.pFile)
				return false;
			m_FileAccess.pFile=nInterface::g_pjIPak->FOpen(pszFilename, "rb");
			if (!m_FileAccess.pFile)
				return false;
		...
		bool COGGDecoder::Close()
		{
			if (!m_FileAccess.pFile)
				return false;
			nInterface::g_pjIPak->FClose(m_FileAccess.pFile);
		...
		void COGGDecoder::GetMemoryUsage(nMech::jcISizer* pSizer)
		{
			if (!pSizer->Add(*this))
				return;
		}
		...
		size_t COGGDecoderInstance::ReadFile(void *ptr, size_t size, size_t nmemb, void *datasource)
		{
			...
			return nInterface::g_pjIPak->FRead(ptr, size, nmemb, pFileAccess->pFile);
		}

		int COGGDecoderInstance::SeekFile(void *datasource, ogg_int64_t offset, int whence)
		{
			SFileAccess *pFileAccess=(SFileAccess*)datasource;
			return nInterface::g_pjIPak->FSeek(pFileAccess->pFile, (long)offset, whence);
		}
		...
		{
			SFileAccess *pFileAccess=(SFileAccess*)datasource;
			return nInterface::g_pjIPak->FTell(pFileAccess->pFile);
		}
		...
		COGGDecoderInstance::COGGDecoderInstance(COGGDecoder *pDecoder)
		{
			...
			ASSERT(pDecoder);
			m_pDecoder=pDecoder;
			nInterface::g_pjIPak->FSeek(m_pDecoder->m_FileAccess.pFile, 0, SEEK_SET);
			ov_callbacks Callbacks;
			...
			m_nPos=0;
			m_nPosBytes=nInterface::g_pjIPak->FTell(m_pDecoder->m_FileAccess.pFile);	// store current file-cursor
			//	getTicks(&nEndTime);
		...
		bool COGGDecoderInstance::GetPCMData(signed long *pDataOut, int nSamples, bool bLoop)
		{
			...
			// prepare file-cursor
			nInterface::g_pjIPak->FSeek(m_pDecoder->m_FileAccess.pFile, m_nPosBytes, SEEK_SET);
			int nSamplesToRead;
			for (;;)
			{
			...
			// store file-cursor
			m_nPosBytes=nInterface::g_pjIPak->FTell(m_pDecoder->m_FileAccess.pFile);
			return true;
		}

		bool COGGDecoderInstance::FillPCMBuffer(signed long *pBuffer, int nSamples)
		{
			int nBytesToRead=nSamples*nInterface::g_pIMusicSystem->GetBytesPerSample();
			int nBytesRead=0;
