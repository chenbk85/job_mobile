						...
						case MUSICLOAD_MODE_BASE:
							{
								...
								{
									CMusicLoadSink Sink(m_pMusicData, nInterface::g_pIScriptSystem, &pObj, MUSICLOAD_MODE_PATTERNS);
									pObj->Dump(&Sink);
								}else
									if (strcmp("Themes", sName)==0)
									{
										CMusicLoadSink Sink(m_pMusicData, nInterface::g_pIScriptSystem, &pObj, MUSICLOAD_MODE_THEMES);
										pObj->Dump(&Sink);
									}
									break;
							}
						case MUSICLOAD_MODE_PATTERNS:
							{
								...
								if (!pObj->GetValue("File", pszFilename))
									break;
								_SmartScriptObject pSubObj(nInterface::g_pIScriptSystem, true);
								...
								CMusicLoadSink FadePosSink(m_pMusicData, nInterface::g_pIScriptSystem, &pSubObj, MUSICLOAD_MODE_FADEPOS, pPatternDef);
						...
						case MUSICLOAD_MODE_THEMES:
							{
								...
								{
									CMusicLoadSink MoodsSink(m_pMusicData, nInterface::g_pIScriptSystem, &pSubObj, MUSICLOAD_MODE_MOODS, NULL, pTheme);
									pSubObj->Dump(&MoodsSink);
								}
								if (pObj->GetValue("Bridges", pSubObj))
								{
									CMusicLoadSink BridgesSink(m_pMusicData, nInterface::g_pIScriptSystem, &pSubObj, MUSICLOAD_MODE_BRIDGES, NULL, pTheme);
						...
						case MUSICLOAD_MODE_MOODS:
							{
								...
								{
									CMusicLoadSink Sink(m_pMusicData, nInterface::g_pIScriptSystem, &pSubObj, MUSICLOAD_MODE_PATTERNSET, NULL, m_pTheme, pMood);
									pSubObj->Dump(&Sink);
								}
						...
						case MUSICLOAD_MODE_PATTERNSET:
							{
								...
								{
									_SmartScriptObject pPatternsSubObj(nInterface::g_pIScriptSystem, true);
									if (pSubObj->GetValue("Patterns", pPatternsSubObj))
									{
										CMusicLoadSink Sink(m_pMusicData, nInterface::g_pIScriptSystem, &pPatternsSubObj, MUSICLOAD_MODE_MOODLAYERS, NULL, m_pTheme, m_pMood, pPatternSet, MUSICLAYER_MAIN);
								...
								if (pObj->GetValue("RhythmicLayer", pSubObj))
								{
									...
									if (pSubObj->GetValue("Patterns", pPatternsSubObj))
									{
										CMusicLoadSink Sink(m_pMusicData, nInterface::g_pIScriptSystem, &pPatternsSubObj, MUSICLOAD_MODE_MOODLAYERS, NULL, m_pTheme, m_pMood, pPatternSet, MUSICLAYER_RHYTHMIC);
										pPatternsSubObj->Dump(&Sink);
									}
								}
								...
								if (pObj->GetValue("IncidentalLayer", pSubObj))
								{
									...
									if (pSubObj->GetValue("Patterns", pPatternsSubObj))
									{
										CMusicLoadSink Sink(m_pMusicData, nInterface::g_pIScriptSystem, &pPatternsSubObj, MUSICLOAD_MODE_MOODLAYERS, NULL, m_pTheme, m_pMood, pPatternSet, MUSICLAYER_INCIDENTAL);
										pPatternsSubObj->Dump(&Sink);
									}
								}
