...
// 
//  File: Sound.cpp
//  Description: nMech::nJC_SOUND::ISound interface implementation.
// 
//  History: 
...
#define JC_GAMESOUND_MAXDIST	10000.0f
...
namespace nMech
{
	namespace nJC_SOUND
	{
		...
		CSound::~CSound()
		{
			...
			if (m_pSoundSystem->m_pCVarDebugSound->GetIVal()==1)
			{
				GetjILog()->Log(TEXT("<Sound> Destroying sound %s \n"),GetName());		
			}

			m_pSoundSystem->RemoveReference(this);
		}
		...
		int CSound::Release()
		{
			...
			if((ref=--m_refCount)<=0)
			{
				...
				nMech::nCONSOLE::ICVar* pVar = g_pIConsole->GetCVar("s_EnableRelease");
		...
		{
			//FRAME_PROFILER( "CSound:CalcSoundVolume",PROFILE_SOUND );
			...
			for (int i=0;i<MAX_SOUNDSCALE_GROUPS;i++)
			{
				if (m_nSoundScaleGroups & SETSCALEBIT(i))
					fRatio*=m_pSoundSystem->m_fSoundScale[i];
			}

			/*	if (m_pSound->m_btType==SoundBuffer::btSTREAM)
			{
			//straming sounds use music volume
			nVolume=(int)((float)nSoundValue*fRatio*m_pSoundSystem->m_fMusicVolume);
			}
			else 
			{*/
			//if (m_nFlags & FLAG_SOUND_MUSIC)
			//	nVolume=(int)((((float)nSoundValue*m_pSoundSystem->m_fMusicVolume)*fRatio));
			//else			
			nVolume=(int)((((float)nSoundValue*m_pSoundSystem->m_fSFXVolume)*fRatio));
			//	}
		...
		bool CSound::IsPlayingOnChannel()
		{
			...
			if (bRes==TRUE)
			{
				//GetjILog()->Log(TEXT("channel %d IS playing!!"),m_nChannel);
				return (true);
			}

			//GetjILog()->Log(TEXT("channel %d,not playing!!"),m_nChannel);
			return (false);	
		}
		...
		{
			m_pSoundSystem->m_lstFadeUnderwaterSounds.insert(this);
		...
		void CSound::Play(float fRatio, bool bForceActiveState, bool bSetRatio)
		{
			...
			float currTime = nTIME::Get_ITimer()->GetCurrTime();
			...
			if (IsPlayingOnChannel())
			{
				if (!m_bLoop)
				{
					...
					if (m_pSoundSystem->m_pCVarMinRepeatSoundTimeout->GetIVal() > timeSinceLastPlay*1000.0f)
					{
						return;
					}
				}
			...
			if (m_nSoundPriority < m_pSoundSystem->GetMinSoundPriority())
				return;
			//////////////////////////////////////////////////////////////////////////
			...
			bool bCanPlay = ((int)(m_pSoundSystem->m_lstSoundSpotsActive.size())<m_pSoundSystem->m_pCVarMaxSoundSpots->GetIVal());
			...
			if (!m_bLoop)
			{
				...
				{
					if (!m_pSoundSystem->m_bValidPos)
					{
						return; //too far
					}
					//check the distance from the listener
					float fDist2 = GetSquaredDistance(m_pSoundSystem->m_cCam.GetPos(),m_RealPos);//position);
			...
			if (m_pSound->Loading())
			{
				...
				if (m_pSoundSystem->m_pCVarDebugSound->GetIVal()==3)
					GetjILog()->Log(TEXT("<Sound> Warning: %s, Play() called before loading was finished"), GetName());
				...
				/*
				float fTime;
				if (m_pSoundSystem->m_pCVarDebugSound->GetIVal())
				fTime=nTIME::Get_ITimer()->GetAsyncCurTime();
				TRACE("INFO: Synchroneous reading of %s due to Play() call before loading was finished.", GetName());
				if (!m_pSound->WaitForLoad())
				return;
				if (m_pSoundSystem->m_pCVarDebugSound->GetIVal())
				GetjILog()->Log(TEXT("Warning: Synchroneous reading of %s due to Play() call before loading was finished stalled for %1.3f milliseconds !"), GetName(), (nTIME::Get_ITimer()->GetAsyncCurTime()-fTime)*1000.0f);
				*/
			} 
			//CHECK_LOADED(Play);
			//UpdatePosition();
				...
				case btSAMPLE:
					{										
						if (m_nFlags & FLAG_SOUND_3D)
						{
							...
							//check the distance from the listener
							float fDist=GetSquaredDistance(m_pSoundSystem->m_cCam.GetPos(),m_RealPos);//position);
							...
							FRAME_PROFILER( "CSound:CS_PlaySoundEX",PROFILE_SOUND );						
							if ((!m_bLoop) || (!IsPlayingOnChannel()))
							{						
								if (bCanPlay)
								{
									m_nChannel=CS_PlaySoundEx(CS_FREE, m_pSound->GetSample(), NULL, true);//,m_pSoundSystem->GetDSPUnitFilter(),FALSE);
									m_PlayingChannels++;
									m_fChannelPlayTime = currTime;
									m_fSoundLengthSec = GetLengthMs()/1000.0f;
									bAllocatedChannel = true;
			...
			if (m_bLoop && bForceActiveState && (!m_pSound->LoadFailure()))
			{
				...
				{		
					if (bCanPlay)
					{
						m_pSoundSystem->SetSoundActiveState(this, eSoundState_Active);			
						bAddedToList = true;
					}
					else
					{
						...
						// right now
						m_pSoundSystem->SetSoundActiveState(this, eSoundState_Inactive);
						bAddedToList = true;
					}
				}
			}
			...
			{
				m_bAutoStop = true;
				m_pSoundSystem->RegisterAutoStopSound( this );
			}

			m_fLastPlayTime = currTime;

			if (bAllocatedChannel && m_pSoundSystem->m_pCVarDebugSound->GetIVal()==1)
			{
				GetjILog()->Log(TEXT("<Sound> Playing sound: %s,channel %d,volume=%d,pan=%d, priority=%d Snd:%X, TimeDiff:%.2f"),GetName(),m_nChannel,m_nPlayingVolume,m_nPan,m_nSoundPriority,this,timeSinceLastPlay);
			}
		...
		void CSound::Stop()
		{
			...
			m_bPlaying = false;
			m_nPlayingVolume = -1;
			if (bUsedChannel && (m_pSoundSystem->m_pCVarDebugSound->GetIVal()==1))
				GetjILog()->Log(TEXT("<Sound> Stop sound: %s"),GetName());

			if (m_nFlags & FLAG_SOUND_FADE_OUT_UNDERWATER)
			{
				SoundListItor It=m_pSoundSystem->m_lstFadeUnderwaterSounds.find(this);
				if (It!=m_pSoundSystem->m_lstFadeUnderwaterSounds.end())
					m_pSoundSystem->m_lstFadeUnderwaterSounds.erase(It);
			}
			if (bUsedChannel)
				OnEvent( SOUND_EVENT_ON_STOP );
			...
			if (m_bAutoStop)
			{
				...
				m_pSoundSystem->UnregisterAutoStopSound( this );
			}
		}
		...
		*/
		...
		const char *CSound::GetName()
		{
			...
			return (m_pSoundSystem->m_szEmptyName);
		}
		...
		{
			GUARD_HEAP;
			//FRAME_PROFILER( "CSound:SetLoopMode",PROFILE_SOUND );
		...
		{
			//FRAME_PROFILER( "CSound:SetFrequency",PROFILE_SOUND );
		...
		{
			//FRAME_PROFILER( "CSound:SetMinMaxDistance",PROFILE_SOUND );
			...
			if(fMinDist<0.1f)
			{
				GetjILog()->Log(TEXT("SOUND[%s] min out of range %f"),GetName(),fMinDist);
			}
			if(fMaxDist>1000000000.0f)
			{
				GetjILog()->Log(TEXT("SOUND[%s] max out of range %f"),GetName(),fMaxDist);
			}
#endif
			if (m_pSound->GetSample())
			{
				GUARD_HEAP;
				CS_Sample_SetMinMaxDistance(m_pSound->GetSample(), fMinDist, JC_GAMESOUND_MAXDIST);//fMaxDist);
			}
		}
		...
		//////////////////////////////////////////////////////////////////////
		void CSound::SetAttrib(const jVec3d &pos, const jVec3d &speed)
		{
			m_position=pos;
			m_speed=speed;
		}
		...
		//////////////////////////////////////////////////////////////////////
		void CSound::SetPosition(const jVec3d &pos)
		{
			...
			if (m_pSoundSystem->m_pCVarDebugSound->GetIVal()==1)
			{
			GetjILog()->Log(TEXT("Sound %s, pos+%f,%f,%f"),m_strName.c_str(),m_position.x,m_position.y,m_position.z);
			}
			*/
			...
			if ((m_pSoundSystem->m_fDirAttCone>0.0f) && (m_pSoundSystem->m_fDirAttCone<1.0f))
			{
				jVec3d DirToPlayer=m_position-m_pSoundSystem->m_DirAttPos;
				DirToPlayer.Normalize();
				float fScale=((1.0f-(DirToPlayer*m_pSoundSystem->m_DirAttDir))/(1.0f-m_pSoundSystem->m_fDirAttCone));
				if ((fScale>0.0f) && (fScale<=1.0f))	// inside cone
				{
					if ((1.0f-fScale)>m_pSoundSystem->m_fDirAttMaxScale)
						m_pSoundSystem->m_fDirAttMaxScale=1.0f-fScale;
					m_RealPos=m_position*fScale+(m_pSoundSystem->m_DirAttPos+(m_pSoundSystem->m_DirAttDir*0.1f))*(1.0f-fScale);
					fPos[0]=m_RealPos.x;
					fPos[1]=m_RealPos.z;
					fPos[2]=m_RealPos.y;   	
					//TRACE("Sound-Proj: %s, %1.3f, %1.3f, %1.3f, (%5.3f, %5.3f, %5.3f)", m_strName.c_str(), fScale, DirToPlayer*m_pSoundSystem->m_DirAttDir, m_pSoundSystem->m_fDirAttCone, m_RealPos.x, m_RealPos.y, m_RealPos.z);
				}
			}

			if (m_pSoundSystem->m_pCVarDopplerEnable->GetIVal() && (m_nFlags & FLAG_SOUND_DOPPLER))
			{
				jVec3d vVel=(pos-m_position);
				float fTimeDelta=nTIME::Get_ITimer()->GetFrameTime();

				vVel=vVel*(m_pSoundSystem->m_pCVarDopplerEnable->GetFVal()/fTimeDelta);
			...
			{
				GUARD_HEAP;
				//GetjILog()->Log(TEXT("Setting sound pos for channel %d"),m_nChannel);
		...
		//////////////////////////////////////////////////////////////////////
		const bool CSound::GetPosition(jVec3d &vPos)
		{
			if (m_nFlags & FLAG_SOUND_3D)
			{
				vPos=m_position;
		...
		//////////////////////////////////////////////////////////////////////
		void CSound::SetVelocity(const jVec3d &speed)
		{  
			m_speed=speed;  
		}

		//////////////////////////////////////////////////////////////////////
		void CSound::SetDirection(const jVec3d &vDir)
		{
			m_orient=vDir;
		}
		...
		void CSound::SetVolume(int nVolume)
		{
			...
			//if (m_pSoundSystem->m_pCVarDebugSound->GetIVal()==5)
			//	GetjILog()->Log(TEXT("VOL:%s,ch=%d,Set vol=%d"),GetName(),m_nChannel,nVolume);
			ChangeVolume(nVolume);
		}
		...
		{
			if (!m_pSoundSystem->m_pCVarEnableSoundFX->GetIVal())
				return;
		...
		bool CSound::FadeIn()
		{
			...
			m_fCurrentFade += m_fFadingValue*nTIME::Get_ITimer()->GetFrameTime(); 

			//GetjILog()->Log(TEXT("Fade IN, current fade=%f"),m_fCurrentFade);
			if (m_fCurrentFade<0)
				m_fCurrentFade=0; 
			else
			{
		...
		{	
			...
			//GetjILog()->Log(TEXT("Fade OUT, current fade=%f"),m_fCurrentFade);
		...
		//////////////////////////////////////////////////////////////////////////
		void CSound::GetMemoryUsage(nMech::jcISizer* pSizer)
		{
			if (!pSizer->Add(*this))
				return;
		...
		{
			if (m_fChannelPlayTime < 0 || m_bLoop || m_pSoundSystem->m_bPause)
				return false;
