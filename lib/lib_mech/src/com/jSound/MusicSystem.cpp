...
/*
 *	INFO: You should read the document jcMusicSystem.doc first to know about the terminology used.
 */
...
namespace nMech
{
	namespace nJC_SOUND
	{
...
#endif
		{
			if (!nParam)
				return FALSE;
			nMech::nJC_SOUND::CMusicSystem *pMusicSystem=(nMech::nJC_SOUND::CMusicSystem*)nParam;
		...
		*/
		CMusicSystem::CMusicSystem()
		{
			...
			if (g_pISoundSystem)
				m_fMasterVolume=g_pISoundSystem->GetMusicVolume();
			else
			...
			m_pCVarDebugMusic=g_pIConsole->CreateVariable("s_DebugMusic","0",0,
				"Toggles music-debugging on and off.\n"
				"Usage: s_DebugMusic [0/1]\n"
				"Default is 0 (off). Set to 1 to debug music.");	
			m_pCVarMusicEnable=g_pIConsole->CreateVariable("s_MusicEnable","1",VF_DUMPTODISK,"enable/disable music");
			m_pCVarMusicMaxSimultaniousPatterns = g_pIConsole->CreateVariable("s_MusicMaxPatterns","12",VF_DUMPTODISK,
				"Max simultaniously playing music patterns.\n"
				);
			m_pCVarMusicStreamedData = 
				g_pIConsole->CreateVariable( "s_MusicStreamedData", "0", VF_DUMPTODISK,
		...
		*/
		CMusicPattern* CMusicSystem::AddPattern(const char *pszName, const char *pszFilename)
		{
			...
			// Check if file exist.
			/*
			FILE *pFile = nInterface::g_pjIPak->FOpen(pszFilename, "rb");
		...
		*/
		bool CMusicSystem::SetData(SMusicData *pMusicData,bool bNoRelease)
		{
			...
			for (TPatternDefVecIt It=m_pDataPtr->vecPatternDef.begin();It!=m_pDataPtr->vecPatternDef.end();++It)
			{
				...
				{
					IF_jILogT(MISS)
						GetjILog()->Warning( jFUNC1 _T("Music file %s failed to load"),pPatternDef->sFilename.c_str() );
					continue;
		...
		*/
		void CMusicSystem::Update()
		{
			...
			// get deltatime
			float fDeltaTime=nTIME::Get_ITimer()->GetFrameTime();
			// check if the volume has changed...
			if (g_pISoundSystem)
			{
				if (m_fMasterVolume!=g_pISoundSystem->GetMusicVolume())
				{
					GUARD_HEAP;
					m_fMasterVolume=g_pISoundSystem->GetMusicVolume();
		...
		*/
		signed char CMusicSystem::StreamingCallback(CS_STREAM *pStream, void *pBuffer, int nLength)
		{
			...
			if (m_pStream!=pStream)
			{
				...
				GetjILog()->Error(TEXT("<jcSound> (CMusicSystem::StreamingCallback) Wrong Stream") );
				return FALSE;
			}
		...
		*/
		...
		//////////////////////////////////////////////////////////////////////////
		void CMusicSystem::GetMemoryUsage(nMech::jcISizer* pSizer)
		{
			if (!pSizer->Add(*this))
				return;
			if (m_pMixBuffer)
		...
		bool CMusicSystem::LoadFromXML( const char *sFilename,bool bAddData )
		{
			...
			XmlNodeRef root = g_pISystem->LoadXmlFile( sFilename );
			if (!root)
			{
				GetjILog()->Warning( _T("Music description XML file %s failed to load"),sFilename );
				return false;
			}
