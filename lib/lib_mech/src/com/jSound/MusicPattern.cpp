...
namespace nMech
{
	namespace nJC_SOUND
	{
		...
		static bool inline 
			FindFile( nMech::nPAK::jIPak* pPak, const char* pszFilename )
		{
			bool bRet( false );
		...
		bool CMusicPattern::Open( const char* pszFilename )
		{
			...
			bool bWaveFallback( false != m_pMusicSystem->StreamOGG() && false == FindFile( nInterface::g_pjIPak, strFilename.c_str() ) );
			if( false == m_pMusicSystem->StreamOGG() || false != bWaveFallback )
			{
				...
				FILE* pFile( nInterface::g_pjIPak->FOpen( strFilename.c_str(), "rb" ) );
				if( 0 != pFile )
				{
					// check encoding and spawn PCM or ADPCM decoder accordingly
					SWaveHdr header;
					nInterface::g_pjIPak->FRead( &header, sizeof( SWaveHdr ), 1, pFile );
		...
		void CMusicPattern::GetMemoryUsage(nMech::jcISizer* pSizer)
		{
			if (!pSizer->Add(*this))
				return;
			if (m_pDecoder)
