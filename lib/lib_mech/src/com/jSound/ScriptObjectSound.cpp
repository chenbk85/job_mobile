...
#include <jcSound.h>
#endif
#include "jc/jc_Camera.h"
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////


_DECLARE_SCRIPTABLEEX_AUTO_SINGLETON(CScriptObjectSound)
...
namespace nMech
{
	namespace nJC_SOUND
	{
		...
		@param g_pISystem Pointer to the System-interface
		@see IScriptSystem
		@see CXGame
		@see ISystem
		*/
		void CScriptObjectSound::Init(IScriptSystem *pScriptSystem)
		{
			m_pMusicSystem=g_pISystem->GetIMusicSystem();
			InitGlobal(pScriptSystem,"Sound",this);

			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_RELATIVE",FLAG_SOUND_RELATIVE);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_RADIUS",FLAG_SOUND_RADIUS);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_DOPPLER",FLAG_SOUND_DOPPLER);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_NO_SW_ATTENUATION",FLAG_SOUND_NO_SW_ATTENUATION);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_MUSIC",FLAG_SOUND_MUSIC);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_OUTDOOR",FLAG_SOUND_OUTDOOR);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_INDOOR",FLAG_SOUND_INDOOR);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_UNSCALABLE",FLAG_SOUND_UNSCALABLE);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_LOOP",FLAG_SOUND_LOOP);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_OCCLUSION",FLAG_SOUND_OCCLUSION);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_STREAM",FLAG_SOUND_STREAM);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_STEREO",FLAG_SOUND_STEREO);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_FADE_OUT_UNDERWATER",FLAG_SOUND_FADE_OUT_UNDERWATER);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_LOAD_SYNCHRONOUSLY",FLAG_SOUND_LOAD_SYNCHRONOUSLY);	

			nInterface::g_pIScriptSystem->SetGlobalValue("SOUNDSCALE_MASTER", SOUNDSCALE_MASTER);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUNDSCALE_SCALEABLE", SOUNDSCALE_SCALEABLE);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUNDSCALE_DEAFNESS", SOUNDSCALE_DEAFNESS);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUNDSCALE_UNDERWATER", SOUNDSCALE_UNDERWATER);
			nInterface::g_pIScriptSystem->SetGlobalValue("SOUNDSCALE_MISSIONHINT", SOUNDSCALE_MISSIONHINT);

			nInterface::g_pIScriptSystem->SetGlobalValue("SOUND_VOLUMESCALEMISSIONHINT", 0.45f);
		}
		...
		*/
		int CScriptObjectSound::IsPlaying(IFunctionHandler *pH)
		{
			...
			int nCookie=0;
			bool bRes=false;
			nMech::nJC_SOUND::ISound *pSound=NULL;
		...
		*/
		int CScriptObjectSound::LoadSound(IFunctionHandler *pH)
		{
			...
			{
				nInterface::g_pIScriptSystem->RaiseError("Sound.LoadSound wrong number of arguments");
				return pH->EndFunctionNull();
			};

			const char *sSound;
			nMech::nJC_SOUND::ISound *pSound;
			int nFlags=0;
			...
			if (g_pISoundSystem)
			{
				...
				if(pSound)
				{
					...
					pSound->AddRef();
					USER_DATA ud=nInterface::g_pIScriptSystem->CreateUserData((INT_PTR)pSound,USER_DATA_SOUND);		//AMD Port
					return pH->EndFunction(ud);
				}
				else{
					nInterface::g_pIScriptSystem->RaiseError("Sound.LoadSound error loading %s",sSound);
				}
			}
			return pH->EndFunctionNull();
		}
		...
		*/
		int CScriptObjectSound::Load3DSound(IFunctionHandler *pH)
		{
			...
			if(nParamCount<1)
			{
				nInterface::g_pIScriptSystem->RaiseError("Sound.Load3DSound wrong number of arguments");
				return pH->EndFunctionNull();
			};
			...
			{
				nInterface::g_pIScriptSystem->RaiseError("Load3dSound: First parameter not a string");
			...
			if (g_pISoundSystem)
			{			
				pSound=g_pISoundSystem->LoadSound(sSound, FLAG_SOUND_3D | nFlags);
				...
				{
					//nInterface::g_pIScriptSystem->RaiseError("Sound.Load3DSound error loading %s",sSound);
					IF_jILogT(MISS)
						GetjILog()->Warning( _T("Sound %s Failed to Load"),sSound );
				}
			}
		...
		*/
		...
		int CScriptObjectSound::SetEaxEnvironment(IFunctionHandler *pH)
		{
			...
			_SmartScriptObject pObj(nInterface::g_pIScriptSystem,true);
			...
			*/
			...
			if (pH->GetParam(1,pObj))
			{
				...
				if (g_pISoundSystem)		
					g_pISoundSystem->SetEaxListenerEnvironment(0,&pProps,nFlags);		
			}
			else
			{
				...
				{
					if (g_pISoundSystem)		
						g_pISoundSystem->SetEaxListenerEnvironment(nEaxEnvironment,NULL,nFlags);		
				}
			}
		...
		*/
		...
		{	
			...
			{  
				nInterface::g_pIScriptSystem->RaiseError("System.LoadStreamSound wrong number of arguments"); 
				return pH->EndFunctionNull(); 
			};

			/*
			nMech::nCONSOLE::ICVar *pMusic=g_pIConsole->GetCVar("g_MusicEnable");
			...
			*/
			...
			if (g_pISoundSystem)
			{
				pSound=g_pISoundSystem->LoadSound(sSound,FLAG_SOUND_STREAM | nFlags);
				if (pSound)
				{
					pSound->AddRef();
					USER_DATA ud=nInterface::g_pIScriptSystem->CreateUserData((INT_PTR)pSound,USER_DATA_SOUND);	//AMD Port
			...
			{
				nInterface::g_pIScriptSystem->RaiseError("Sound.LoadStreamSound error loading %s",sSound);
			}
			return pH->EndFunctionNull();
		...
		*/
		int CScriptObjectSound::PlaySound(IFunctionHandler *pH)
		{
			...
			{
				if(g_pISoundSystem)
					nInterface::g_pIScriptSystem->RaiseError("PlaySound NULL SOUND!!");
			}
			return pH->EndFunction();
		}
		...
		*/
		...
		int CScriptObjectSound::GetListener(IFunctionHandler *pH)
		{
			...
			CScriptObjectVector oVecParam1(nInterface::g_pIScriptSystem, true);
			CScriptObjectVector oVecParam2(nInterface::g_pIScriptSystem, true);
			CScriptObjectVector oVecParam3(nInterface::g_pIScriptSystem, true);
		...
		*/
		int CScriptObjectSound::SetMinMaxDistance(IFunctionHandler *pH)
		{
			...
			float fMinDist;
			float fMaxDist;
			nMech::nJC_SOUND::ISound *pISound = NULL;
		...
		*/
		...
		int CScriptObjectSound::SetMasterVolumeScale(IFunctionHandler *pH)
		{
			...
			float fScale=1;
			if(pH->GetParam(1, fScale) && g_pISoundSystem)
				g_pISoundSystem->SetMasterVolumeScale(fScale);
			return pH->EndFunction();
		}
		...
		int CScriptObjectSound::SetGroupScale(IFunctionHandler *pH)
		{
			...
			return pH->EndFunction(g_pISoundSystem->SetGroupScale(nGroup, fScale));
		}
		...
		int	CScriptObjectSound::SetDirectionalAttenuation(IFunctionHandler *pH)
		{
			...
			g_pISoundSystem->CalcDirectionalAttenuation(Pos, Dir, fConeInRadians);
			return pH->EndFunction();
		}
		...
		int	CScriptObjectSound::GetDirectionalAttenuationMaxScale(IFunctionHandler *pH)
		{
			...
			CHECK_PARAMETERS(0);
			return pH->EndFunction(g_pISoundSystem->GetDirectionalAttenuationMaxScale());
		}
		...
		int	CScriptObjectSound::LoadMusic(IFunctionHandler *pH)
		{
			...
			bRes=g_pISystem->GetIMusicSystem()->LoadFromXML(pszFilename,true);
			if (!bRes)
			GetjILog()->Log(TEXT("Unable to load music from %s !"), pszFilename);
			*/
			return pH->EndFunction(bRes);
		}
		...
		int	CScriptObjectSound::SetMusicTheme(IFunctionHandler *pH)
		{
			...
			if (!bRes)
				GetjILog()->Log(TEXT("Unable to set music-theme \")%s\" !"), pszTheme);
			return pH->EndFunction(bRes);
		}
		...
		int	CScriptObjectSound::ResetMusicThemeOverride(IFunctionHandler *pH)
		{
			...
			if (!bRes)
				GetjILog()->Log(TEXT("Unable to reset music-theme-override !"));
			return pH->EndFunction(bRes);
		}
		...
		int	CScriptObjectSound::SetMusicMood(IFunctionHandler *pH)
		{
			...
			if (!bRes)
				GetjILog()->Log(TEXT("Unable to set music-mood \")%s\" !"), pszMood);
			return pH->EndFunction(bRes);
		}
		...
		int	CScriptObjectSound::SetDefaultMusicMood(IFunctionHandler *pH)
		{
			...
			if (!bRes)
				GetjILog()->Log(TEXT("Unable to set default music-mood \")%s\" !"), pszMood);
			return pH->EndFunction(bRes);
		}
		...
		int	CScriptObjectSound::GetMusicThemes(IFunctionHandler *pH)
		{
			...
			if (!pVec)
				return pH->EndFunctionNull();
			_SmartScriptObject pTable(nInterface::g_pIScriptSystem);
			int i=1;
			while (!pVec->IsEnd())
			{
		...
		int CScriptObjectSound::GetMusicStatus(IFunctionHandler *pH)
		{
			...
			CHECK_PARAMETERS(0);
			SMusicSystemStatus *pStatus=m_pMusicSystem->GetStatus();
			GetjILog()->Log(TEXT("--- MusicSystem Status Info ---"));
			GetjILog()->Log(TEXT("  Streaming: %s"), pStatus->bPlaying ? "Yes" : "No");
			GetjILog()->Log(TEXT("  Theme: %s"), pStatus->sTheme.c_str());
			GetjILog()->Log(TEXT("  Mood: %s"), pStatus->sMood.c_str());
			GetjILog()->Log(TEXT("  Playing patterns:"));
			for (TPatternStatusVecIt It=pStatus->m_vecPlayingPatterns.begin();It!=pStatus->m_vecPlayingPatterns.end();++It)
			{
				...
				GetjILog()->Log(TEXT("    %s (Layer: %s; Volume: %s)"), PatternStatus.sName.c_str(), (PatternStatus.nLayer==MUSICLAYER_MAIN) ? "Main" : ((PatternStatus.nLayer==MUSICLAYER_RHYTHMIC) ? "Rhythmic" : "Incidental"), PatternStatus.nVolume);
			}
			return pH->EndFunction();
		}
