...
end_fpu_type:
		...
		void CCpuFeatures::Detect(void)
		{
			...
			jLOG(_T("\n--- CPU detection ---\n" ));
			jLOG(_T("Number of system processors: %d\n"), m_NumSystemProcessors );
			jLOG(_T("Number of available processors: %d\n"), m_NumAvailProcessors );
			...
			for(int i=0; i<num; i++)
			{
				...
				jLOG(_T("Processor %d:\n"), i );
				jLOG(_T("CPU: %s %s\n"), nUNI::scb256_t(p->mVendor).getT(), nUNI::scb256_t(p->mCpuType).getT() );
				jLOG(_T("Family: %d, Model: %d, Stepping: %d\n"), p->mFamily, p->mModel, p->mStepping );
				jLOG(_T("FPU: %s\n"), nUNI::scb256_t(p->mFpuType).getT() );
				jLOG(_T("CPU Speed (estimated): %f MHz\n"), 1.0e-6/p->m_SecondsPerCycle );
				if (p->mFeatures & CFI_MMX)
				{	jLOG(_T("MMX: present\n"));}
				else
				{jLOG(_T("MMX: not present\n"));}
				if (p->mFeatures & CFI_SSE)
				{jLOG(_T("SSE: present\n"));}
				else
				{jLOG(_T("SSE: not present\n"));}
				if (p->mFeatures & CFI_3DNOW)
				{jLOG(_T("3DNow!: present\n"));}
				else
				{jLOG(_T("3DNow!: not present\n"));}
				if( p->mbSerialPresent )
				{jLOG(_T("Serial number: %s\n\n"), nUNI::scb256_t(p->mSerialNumber).getT() );}
				else
				{jLOG(_T("Serial number not present or disabled\n\n") );}
			}
			jLOG(_T("---------------------") );
		...
		jCTOR_INTERFACE(nMech::nOS::jICpuInfo,CCpuFeatures)
		{
			static CCpuFeatures _a;
			return &_a;
		}
		jDTOR_INTERFACE(nMech::nOS::jICpuInfo,CCpuFeatures )
		{
