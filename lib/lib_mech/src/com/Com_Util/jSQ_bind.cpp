...
DECLARE_INSTANCE_TYPE(jIConsoleCmd);


jSQ_gfn_DEF(jIConsoleCmd*,jIConsoleCmd, ParseCmd)
{
	jSQ_init_pThis(jIConsoleCmd);// paramCount ,arg_This ,pThis , arg_Start=2
	cstr szCmd = sa.GetString(2);
	pThis->ParseCmd(szCmd);
	return pThis;
}

jSQ_REGIST_BIND(jIConsoleCmd)
{
	jSQ_Interface(jIConsoleCmd)
		jSQ_gfn(jIConsoleCmd,ParseCmd,"szCmd","콘솔명령수행")
		jSQ_end();

	jIConsoleCmd* g_pjIConsoleCmd= nCONSOLE::Get_jIConsoleCmd();
	jSQ_g_var((jIConsoleCmd*)g_pjIConsoleCmd ,g_pjIConsoleCmd);
}
