...
#ifndef OrbHost___H_CHANGE_XML_NODE__ 
#define OrbHost___H_CHANGE_XML_NODE__
...
pA의 컨텐츠 수정사항만 적용함 	
(자식의 내용은 적용안함) ]]*/
	const int pk_H_CHANGE_XML_NODE = /*nMech::nNET::EProtocal_jINet_OGS_END+*/1000+17;
	/* tname256_t szDoc;*/
	/* tname1024_t szXPath;*/
	/* int8 idx;*/
	/* bool isAttr;*/
	...
	struct S_H_CHANGE_XML_NODE;
	bool READ_H_CHANGE_XML_NODE(jIPacketSocket_IOCP* pS,uint16 data_size,BYTE *buffer, S_H_CHANGE_XML_NODE &param);
	struct S_H_CHANGE_XML_NODE
	{
		tname256_t szDoc;
		...
		tname256_t szData;
		#ifndef jNOT_USE_PACKET_SET_GET_FUNC_OrbHost
					void set_szData(tcstr sz){jt_strncpy(szData,nUNI::scb1024_t(sz).getT(), 256);	szData[256-1]=0;}
					tcstr get_szData(fname_t _buf){jt_strncpy(_buf, nUNI::scb256_t(szData).getT() , 256);_buf[256-1]=0;return _buf;}
		#endif //jNOT_USE_PACKET_SET_GET_FUNC_OrbHost
		...
		void DebugPrint(bool isPrintAll)
		{
			...
			jDebugPrint(_T("iType"),iType);
			jDebugPrint(_T("szData"),szData);
		}
		#endif //jNOT_USE_DebugPrint_
		void ReadPacket(jIPacketSocket_IOCP* pS,jPacket_Base* pk)
		{
			READ_H_CHANGE_XML_NODE(pS,pk->GetLen(),pk->buf,*this);
		}
	};
	#ifndef jNOT_USE_PACKET_READ_FUNC_OrbHost
	
	inline bool READ_H_CHANGE_XML_NODE(jIPacketSocket_IOCP* pS,uint16 data_size , BYTE *buffer, S_H_CHANGE_XML_NODE &param)
	{
		try{
		...
		nStream::Read(st,param.iType);
		nStream::Read(st,param.szData, 256);
		if(st.GetCurrPos()>data_size) throw _T("st.GetCurrPos()>data_size");
		}catch(tcstr sz){jWARN_T("H_CHANGE_XML_NODE : %s (data_size=%u " , sz,data_size); return false;}
		return true;
	}
	#endif //jNOT_USE_PACKET_READ_FUNC_OrbHost

	#ifndef jNOT_USE_PACKET_WRITE_FUNC_OrbHost
	inline jPacket_Base WRITE_H_CHANGE_XML_NODE(nNET::nStream::_jNetStreamWriteBufferBase &buffer, S_H_CHANGE_XML_NODE &param)
	{
		try{
			...
			nStream::Write(st,param.iType);
			nStream::Write(st,param.szData);
			if(st.size()>st.capacity() ) throw _T("st.size()>st.capacity");
			
			jPacket_Base pk; pk.num = pk_H_CHANGE_XML_NODE;
			pk.buf=st.GetBuffer();	 pk.len=(jPacketNum_t)st.size();	return pk;
		}catch(tcstr sz){ jERROR(_T(" H_CHANGE_XML_NODE :data overflow: %s"),sz); }
		catch(nStream::jNetStreamWrite_error i){ jERROR(_T(" H_CHANGE_XML_NODE :overflow: (curr=%d,capa=%d,size=%d)"),i.currLen,i.capa , i.isize); }
		jPacket_Base pk;pk.buf=0;pk.len=0;pk.num=0;return pk;
	};
	
	inline jPacket_Base WRITE_H_CHANGE_XML_NODE(nNET::nStream::_jNetStreamWriteBufferBase &buffer ,  tcstr _szDoc,  tcstr _szXPath, const int8 &_idx, const bool &_isAttr,  tcstr _szTag, const int32 &_iType,  tcstr _szData)
	{
		try{
			nMech::nNET::nStream::jNetStreamWrite st(buffer);
			...
			nStream::Write(st,_iType);
			nStream::Write(st,_szData);
		if(st.size()>st.capacity() ) throw _T("st.size()>st.capacity");
...
#endif //OrbHost___H_CHANGE_XML_NODE__