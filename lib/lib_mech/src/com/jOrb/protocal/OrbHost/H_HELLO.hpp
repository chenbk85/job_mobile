...
#ifndef OrbHost___H_HELLO__ 
#define OrbHost___H_HELLO__
...
{
	const int pk_H_HELLO = /*nMech::nNET::EProtocal_jINet_OGS_END+*/1000+4;
	/* tname32_t orbServiceName;*/
	/* tname32_t myName;*/
	/* bool isServerMode;*/
	
	struct S_H_HELLO;
	bool READ_H_HELLO(jIPacketSocket_IOCP* pS,uint16 data_size,BYTE *buffer, S_H_HELLO &param);
	struct S_H_HELLO
	{
		tname32_t orbServiceName;
		...
		{
			READ_H_HELLO(pS,pk->GetLen(),pk->buf,*this);
		}
	};
	#ifndef jNOT_USE_PACKET_READ_FUNC_OrbHost
	
	inline bool READ_H_HELLO(jIPacketSocket_IOCP* pS,uint16 data_size , BYTE *buffer, S_H_HELLO &param)
	{
		try{
		...
		}catch(tcstr sz){jWARN_T("H_HELLO : %s (data_size=%u " , sz,data_size); return false;}
		return true;
	}
	#endif //jNOT_USE_PACKET_READ_FUNC_OrbHost

	#ifndef jNOT_USE_PACKET_WRITE_FUNC_OrbHost
	inline jPacket_Base WRITE_H_HELLO(nNET::nStream::_jNetStreamWriteBufferBase &buffer, S_H_HELLO &param)
	{
		try{
			...
			jPacket_Base pk; pk.num = pk_H_HELLO;
			pk.buf=st.GetBuffer();	 pk.len=(jPacketNum_t)st.size();	return pk;
		}catch(tcstr sz){ jERROR(_T(" H_HELLO :data overflow: %s"),sz); }
		catch(nStream::jNetStreamWrite_error i){ jERROR(_T(" H_HELLO :overflow: (curr=%d,capa=%d,size=%d)"),i.currLen,i.capa , i.isize); }
		jPacket_Base pk;pk.buf=0;pk.len=0;pk.num=0;return pk;
	};
	
	inline jPacket_Base WRITE_H_HELLO(nNET::nStream::_jNetStreamWriteBufferBase &buffer ,  tcstr _orbServiceName,  tcstr _myName, const bool &_isServerMode)
	{
		try{
			nMech::nNET::nStream::jNetStreamWrite st(buffer);
