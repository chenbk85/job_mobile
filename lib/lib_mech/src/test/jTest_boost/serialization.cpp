...
std::stringstream g_Save;
void save_schedule(const bus_schedule &s, const char * filename){
    // make an archive
	//boost::archive::binary_oarchive oa(g_SaveO);
		fast_binary_oarchive oa(g_Save);

    //std::ofstream ofs(filename);
    //boost::archive::text_oarchive oa(ofs);
    oa << s;
		//char* a = "asdfasdf asdf";		oa << a;
		//g_Save.rdbuf()->sputn();
		//g_Save.rdbuf()->put
	//string str = 	g_Save.str();
	//cout <<str;
}
...
#else
	//std::istringstream  g_SaveI(g_SaveO.str());
	#if 0
		boost::archive::binary_iarchive ia(g_SaveI);
	#else
		fast_binary_iarchive ia(g_Save);
	#endif
#endif
