...
PBYTE FindMemory(PBYTE pbBuf, DWORD cbBuf, 
   PBYTE pbSearchBytes, DWORD cbSearchBytes) {
	   ...
	   DWORD n ;
	   DWORD x ;
   for (n = 0; n < (cbBuf - cbSearchBytes); n++) {
      if (pbBuf[n] == pbSearchBytes[0]) {
         for (x = 1; x < cbSearchBytes; x++) {
...
DWORD CPrfData::ActivatePrfData() {
   ...
   OBJORD ObjOrd ;
   for (ObjOrd = 0; ObjOrd < (OBJORD) m_nNumObjects; ObjOrd++) {
      PRFMETRICS pm;
      PPRFITM pPrfObj;
   ...
   m_hfm = CreateFileMappingW((HANDLE) 0xffffffff, NULL, PAGE_READWRITE, 
