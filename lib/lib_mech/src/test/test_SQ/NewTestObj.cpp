...
// GetjISQ()->VM_GetVMPtr() could also be used with PostConstruct(): no HSQUIRRELVM argument would be required.
//--------------------------------------------------------------------------
...
{
	SquirrelObject testReg1 = GetjISQ()->VM_CompileBuffer(_T(" \
			local t = NewTestObj(); \
			t.newtestR1(\"Hello\"); \
			t.val = 789; \
			print(t.val); \
	...
	*/
	...
	SquirrelObject testReg2 = GetjISQ()->VM_CompileBuffer(_T(" \
			local t = NewTestObj(); \
			t.val = 789; \
			print(t.val); \
			t.val = 876; \
