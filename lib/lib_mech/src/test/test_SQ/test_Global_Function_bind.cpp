...
//--------------------------------------------------------------------------
{
	...
	// Implemented as asdfasdf_SquirrelVM::CreateFunction(rootTable,func,name,typeMask). CreateFunctionGlobal() binds a standard SQFUNCTION (stack args).
	GetjISQ()->VM_CreateFunctionGlobal(testFunc,_T("testFunc0"));
	GetjISQ()->VM_CreateFunctionGlobal(testFunc,_T("testFuncN"),_T("n"));
	GetjISQ()->VM_CreateFunctionGlobal(testFunc,_T("testFuncS"),_T("s"));
#if 0
	SquirrelObject testStandardFuncs = GetjISQ()->VM_CompileBuffer(_T(" testFunc0(); testFuncN(1.); testFuncS(\"Hello\"); "));
	GetjISQ()->VM_RunScript(testStandardFuncs);
#endif
	...
	SquirrelObject testReg3 = GetjISQ()->VM_CompileBuffer(_T(" \
					test(); \
					local rv = newtestR1_global(\"Hello\"); \
					print(rv); "));
	GetjISQ()->VM_RunScript(testReg3);     
	/* 출력
	...
	*/
	...
	SquirrelObject tr = GetjISQ()->VM_GetRootTable(); // Can be any object supporting closures (functions).
	...
	SquirrelObject testReg4 = GetjISQ()->VM_CompileBuffer(_T(" \
					print(\"\\nMembers:\"); \
					testObj_newtest1(); \
					testObj_newtest2(); \
					print(testObj_newtestR1(\"Hello Again\")); "));
	GetjISQ()->VM_RunScript(testReg4);
	/* 출력
