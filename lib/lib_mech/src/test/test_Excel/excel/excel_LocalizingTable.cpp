...
#include "excel_LocalizingTable.h"
...
namespace nMech
{
	...
	#if defined(jEXCEL_TABLE_COMMON_CODE_CPP_SQ_GLOBAL_Sys) && defined(for_each_nEXCEL_excel_LocalizingTable_Sys_STRUCT_LIST)
		for_each_nEXCEL_excel_LocalizingTable_Sys_STRUCT_LIST(jEXCEL_TABLE_COMMON_CODE_CPP_SQ_GLOBAL_Sys);
	#endif //jEXCEL_TABLE_COMMON_CODE_CPP_SQ_GLOBAL_Sys


	#ifdef jEXCEL_TABLE_COMMON_CODE_CPP_SQ_GLOBAL_Tbl
		for_each_nEXCEL_excel_LocalizingTable_Tbl_STRUCT_LIST(jEXCEL_TABLE_COMMON_CODE_CPP_SQ_GLOBAL_Tbl);
	#endif //jEXCEL_TABLE_COMMON_CODE_CPP_SQ_GLOBAL_Tbl

#endif //jNOT_USE_SQ_BIND_CODE


for_each_nEXCEL_excel_LocalizingTable_ENUM(jEXCEL_SQ_DECLARE_ENUM_TYPE);
namespace nMech {
	namespace nEXCEL {
...
jEXCEL_LIB_API nExcelUtil::jEnumString g_ES_ETextCategory[] = 
{
	{ 5 , 0, 0},
		...
		for_each_nEXCEL_ETextCategory_ENUM_FIELD(jEXCEL_ENUM_STRING_CPP_CODE)
	...
	for_each_nEXCEL_excel_LocalizingTable_Sys_STRUCT_LIST(jEXCEL_TABLE_COMMON_CODE_CPP_Sys);
...
{	tname1024_t szBuf;
	...
	for_each_nEXCEL_Sys_LocalizingText_STRUCT_FIELD_all_member(jEXCEL_STRUCT_FIELD_DebugPrint);
}

void Sys_LocalizingWord::DebugPrint()
{	tname1024_t szBuf;

	for_each_nEXCEL_Sys_LocalizingWord_STRUCT_FIELD_all_member(jEXCEL_STRUCT_FIELD_DebugPrint);
}
	...
	{
		jSQ_REGIST_BIND(nMech_nEXCEL_excel_LocalizingTable)
		{
			{
				SquirrelObject enumRoot= nSQ::jSQ_GetEnumTable();
				for_each_nEXCEL_excel_LocalizingTable_ENUM(jEXCEL_SQ_bind_EnumField_BEGIN);
				for_each_nEXCEL_ETextCategory_ENUM_FIELD(jEXCEL_SQ_bind_EnumField);
			}
			...
			for_each_nEXCEL_Sys_LocalizingText_STRUCT_FIELD(jEXCEL_SQ_TABLE_FILED_BIND);
			...
			for_each_nEXCEL_Sys_LocalizingWord_STRUCT_FIELD(jEXCEL_SQ_TABLE_FILED_BIND);
