...
jNET_EVENT_BEGIN(PlugIn_Connect)
{
	namespace _from_TestServer
	{
		...
		jNET_EVENT_RECEIVER_P2P(TS_HELLO,TestServer)
		{
			...
			jPC_READ_PACKET_PB_Data(TS_HELLO,pPC->m_DT_HELLO);
		...
		jNET_EVENT_RECEIVER_P2P(TS_ONLINE,TestServer)
		{
			...
			pPC->Logout_Dummy_AllSocket(1000,0);
		}
		...
		{
			// Data, PlugIn_ToServer*pPC , B YTE buf_[1024] jPlayerObj* pPlayer
			jPC_READ_PACKET_PB(TM_SET_TEST_ENV);
		...
		{
			jPC_ret(0);
			pPC->Start_Login_AllDummy(g_TM_TEST_LOGIN_START.per_user,g_TM_TEST_LOGIN_START.delay);
			return 0;
		}
		...
		{
			// Data, PlugIn_ToServer*pPC , B YTE buf_[1024] jPlayerObj* pPlayer
			jPC_READ_PACKET_PB(TM_TEST_LOG_OUT);
			extern bool g_isCancel_StartSession_AllDummy;
			g_isCancel_StartSession_AllDummy=true;

			pPC->Logout_Dummy_AllSocket(Data.per_user,Data.delay);
		}
