...
// AQUSER_API 함수를 DLL에서 가져오는 것으로 보고, 이 DLL은
// 이 DLL은 해당 매크로로 정의된 기호가 내보내지는 것으로 봅니다.
#ifdef AQUSER_EXPORTS
#define AQUSER_API __declspec(dllexport)
#else
#define AQUSER_API __declspec(dllimport)
#endif
...
#define var_enum(var_func)\
	...
	var_func(jSERVER_NAME,U2W_USER_DISCONNECT, 3.0f, 0 , "time of disconnected user notify to world(sec)")\
