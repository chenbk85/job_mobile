...
#ifndef LoginServer___L2X_USER_LOGIN_OK__ 
#define LoginServer___L2X_USER_LOGIN_OK__
...
{
	const int pk_L2X_USER_LOGIN_OK = /*nMech::nNET::EProtocal_jINet_OGS_END+*/2000+6;
	/* player_id_t pid;*/
	/* serverid_t channel[-1];*/
	/* astring channel_name[-1];*/
	/* uint8 buzy_level[-1];*/
	
	struct S_L2X_USER_LOGIN_OK;
	bool READ_L2X_USER_LOGIN_OK(jIPacketSocket_IOCP* pS,uint16 data_size,BYTE *buffer, S_L2X_USER_LOGIN_OK &param);
	struct S_L2X_USER_LOGIN_OK
	{
		player_id_t pid;
		...
		{
			READ_L2X_USER_LOGIN_OK(pS,pk->GetLen(),pk->buf,*this);
		}
	};
	#ifndef jNOT_USE_PACKET_READ_FUNC_LoginServer
	
	inline bool READ_L2X_USER_LOGIN_OK(jIPacketSocket_IOCP* pS,uint16 data_size , BYTE *buffer, S_L2X_USER_LOGIN_OK &param)
	{
		try{
		...
		}catch(tcstr sz){jWARN_T("L2X_USER_LOGIN_OK : %s (data_size=%u " , sz,data_size); return false;}
		return true;
	}
	#endif //jNOT_USE_PACKET_READ_FUNC_LoginServer

	#ifndef jNOT_USE_PACKET_WRITE_FUNC_LoginServer
	inline jPacket_Base WRITE_L2X_USER_LOGIN_OK(nNET::nStream::_jNetStreamWriteBufferBase &buffer, S_L2X_USER_LOGIN_OK &param)
	{
		try{
			...
			jPacket_Base pk; pk.num = pk_L2X_USER_LOGIN_OK;
			pk.buf=st.GetBuffer();	 pk.len=(jPacketNum_t)st.size();	return pk;
		}catch(tcstr sz){ jERROR(_T(" L2X_USER_LOGIN_OK :data overflow: %s"),sz); }
		catch(nStream::jNetStreamWrite_error i){ jERROR(_T(" L2X_USER_LOGIN_OK :overflow: (curr=%d,capa=%d,size=%d)"),i.currLen,i.capa , i.isize); }
		jPacket_Base pk;pk.buf=0;pk.len=0;pk.num=0;return pk;
	};
	
	inline jPacket_Base WRITE_L2X_USER_LOGIN_OK(nNET::nStream::_jNetStreamWriteBufferBase &buffer , const player_id_t &_pid, const std::vector<serverid_t> &_channel, const std::vector<astring> &_channel_name, const std::vector<uint8> &_buzy_level)
	{
		try{
			nMech::nNET::nStream::jNetStreamWrite st(buffer);
...
#endif //LoginServer___L2X_USER_LOGIN_OK__