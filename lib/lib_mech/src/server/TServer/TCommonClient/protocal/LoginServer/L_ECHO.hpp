...
#ifndef LoginServer___L_ECHO__ 
#define LoginServer___L_ECHO__
...
{
	const int pk_L_ECHO = /*nMech::nNET::EProtocal_jINet_OGS_END+*/2000+2;
	/* tname256_t msg;*/
	/* int8 type;*/
	
	struct S_L_ECHO;
	bool READ_L_ECHO(jIPacketSocket_IOCP* pS,uint16 data_size,BYTE *buffer, S_L_ECHO &param);
	struct S_L_ECHO
	{
		tname256_t msg;
		...
		{
			READ_L_ECHO(pS,pk->GetLen(),pk->buf,*this);
		}
	};
	#ifndef jNOT_USE_PACKET_READ_FUNC_LoginServer
	
	inline bool READ_L_ECHO(jIPacketSocket_IOCP* pS,uint16 data_size , BYTE *buffer, S_L_ECHO &param)
	{
		try{
		...
		}catch(tcstr sz){jWARN_T("L_ECHO : %s (data_size=%u " , sz,data_size); return false;}
		return true;
	}
	#endif //jNOT_USE_PACKET_READ_FUNC_LoginServer

	#ifndef jNOT_USE_PACKET_WRITE_FUNC_LoginServer
	inline jPacket_Base WRITE_L_ECHO(nNET::nStream::_jNetStreamWriteBufferBase &buffer, S_L_ECHO &param)
	{
		try{
			...
			jPacket_Base pk; pk.num = pk_L_ECHO;
			pk.buf=st.GetBuffer();	 pk.len=(jPacketNum_t)st.size();	return pk;
		}catch(tcstr sz){ jERROR(_T(" L_ECHO :data overflow: %s"),sz); }
		catch(nStream::jNetStreamWrite_error i){ jERROR(_T(" L_ECHO :overflow: (curr=%d,capa=%d,size=%d)"),i.currLen,i.capa , i.isize); }
		jPacket_Base pk;pk.buf=0;pk.len=0;pk.num=0;return pk;
	};
	
	inline jPacket_Base WRITE_L_ECHO(nNET::nStream::_jNetStreamWriteBufferBase &buffer ,  tcstr _msg, const int8 &_type)
	{
		try{
			nMech::nNET::nStream::jNetStreamWrite st(buffer);
