...
#ifndef WorldServer___W2_USER_DISCONNECT__ 
#define WorldServer___W2_USER_DISCONNECT__
...
{
	const int pk_W2_USER_DISCONNECT = /*nMech::nNET::EProtocal_jINet_OGS_END+*/5000+19;
	/* wstring id[-1];*/
	
	struct S_W2_USER_DISCONNECT;
	bool READ_W2_USER_DISCONNECT(jIPacketSocket_IOCP* pS,uint16 data_size,BYTE *buffer, S_W2_USER_DISCONNECT &param);
	struct S_W2_USER_DISCONNECT
	{
		...
		{
			READ_W2_USER_DISCONNECT(pS,pk->GetLen(),pk->buf,*this);
		}
	};
	#ifndef jNOT_USE_PACKET_READ_FUNC_WorldServer
	
	inline bool READ_W2_USER_DISCONNECT(jIPacketSocket_IOCP* pS,uint16 data_size , BYTE *buffer, S_W2_USER_DISCONNECT &param)
	{
		try{
		...
		}catch(tcstr sz){jWARN_T("W2_USER_DISCONNECT : %s (data_size=%u " , sz,data_size); return false;}
		return true;
	}
	#endif //jNOT_USE_PACKET_READ_FUNC_WorldServer

	#ifndef jNOT_USE_PACKET_WRITE_FUNC_WorldServer
	inline jPacket_Base WRITE_W2_USER_DISCONNECT(nNET::nStream::_jNetStreamWriteBufferBase &buffer, S_W2_USER_DISCONNECT &param)
	{
		try{
			...
			jPacket_Base pk; pk.num = pk_W2_USER_DISCONNECT;
			pk.buf=st.GetBuffer();	 pk.len=(jPacketNum_t)st.size();	return pk;
		}catch(tcstr sz){ jERROR(_T(" W2_USER_DISCONNECT :data overflow: %s"),sz); }
		catch(nStream::jNetStreamWrite_error i){ jERROR(_T(" W2_USER_DISCONNECT :overflow: (curr=%d,capa=%d,size=%d)"),i.currLen,i.capa , i.isize); }
		jPacket_Base pk;pk.buf=0;pk.len=0;pk.num=0;return pk;
	};
	
	inline jPacket_Base WRITE_W2_USER_DISCONNECT(nNET::nStream::_jNetStreamWriteBufferBase &buffer , const std::vector<wstring> &_id)
	{
		try{
			nMech::nNET::nStream::jNetStreamWrite st(buffer);
...
#endif //WorldServer___W2_USER_DISCONNECT__