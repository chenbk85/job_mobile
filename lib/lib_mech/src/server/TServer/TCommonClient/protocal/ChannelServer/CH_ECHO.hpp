...
#ifndef ChannelServer___CH_ECHO__ 
#define ChannelServer___CH_ECHO__
...
{
	const int pk_CH_ECHO = /*nMech::nNET::EProtocal_jINet_OGS_END+*/2500+1;
	/* tname256_t msg;*/
	/* uint8 type;*/
	
	struct S_CH_ECHO;
	bool READ_CH_ECHO(jIPacketSocket_IOCP* pS,uint16 data_size,BYTE *buffer, S_CH_ECHO &param);
	struct S_CH_ECHO
	{
		tname256_t msg;
		...
		{
			READ_CH_ECHO(pS,pk->GetLen(),pk->buf,*this);
		}
	};
	#ifndef jNOT_USE_PACKET_READ_FUNC_ChannelServer
	
	inline bool READ_CH_ECHO(jIPacketSocket_IOCP* pS,uint16 data_size , BYTE *buffer, S_CH_ECHO &param)
	{
		try{
		...
		}catch(tcstr sz){jWARN_T("CH_ECHO : %s (data_size=%u " , sz,data_size); return false;}
		return true;
	}
	#endif //jNOT_USE_PACKET_READ_FUNC_ChannelServer

	#ifndef jNOT_USE_PACKET_WRITE_FUNC_ChannelServer
	inline jPacket_Base WRITE_CH_ECHO(nNET::nStream::_jNetStreamWriteBufferBase &buffer, S_CH_ECHO &param)
	{
		try{
			...
			jPacket_Base pk; pk.num = pk_CH_ECHO;
			pk.buf=st.GetBuffer();	 pk.len=(jPacketNum_t)st.size();	return pk;
		}catch(tcstr sz){ jERROR(_T(" CH_ECHO :data overflow: %s"),sz); }
		catch(nStream::jNetStreamWrite_error i){ jERROR(_T(" CH_ECHO :overflow: (curr=%d,capa=%d,size=%d)"),i.currLen,i.capa , i.isize); }
		jPacket_Base pk;pk.buf=0;pk.len=0;pk.num=0;return pk;
	};
	
	inline jPacket_Base WRITE_CH_ECHO(nNET::nStream::_jNetStreamWriteBufferBase &buffer ,  tcstr _msg, const uint8 &_type)
	{
		try{
			nMech::nNET::nStream::jNetStreamWrite st(buffer);
...
#endif //ChannelServer___CH_ECHO__