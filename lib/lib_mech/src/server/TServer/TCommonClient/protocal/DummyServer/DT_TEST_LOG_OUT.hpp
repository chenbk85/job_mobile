...
#ifndef DummyServer___DT_TEST_LOG_OUT__ 
#define DummyServer___DT_TEST_LOG_OUT__
...
{
	const int pk_DT_TEST_LOG_OUT = /*nMech::nNET::EProtocal_jINet_OGS_END+*/8000+7;
	/* int16 per_user;*/
	/* uint32 delay;*/
	/* tname32_t testClientName;*/
	
	struct S_DT_TEST_LOG_OUT;
	bool READ_DT_TEST_LOG_OUT(jIPacketSocket_IOCP* pS,uint16 data_size,BYTE *buffer, S_DT_TEST_LOG_OUT &param);
	struct S_DT_TEST_LOG_OUT
	{
		int16 per_user;
		...
		{
			READ_DT_TEST_LOG_OUT(pS,pk->GetLen(),pk->buf,*this);
		}
	};
	#ifndef jNOT_USE_PACKET_READ_FUNC_DummyServer
	
	inline bool READ_DT_TEST_LOG_OUT(jIPacketSocket_IOCP* pS,uint16 data_size , BYTE *buffer, S_DT_TEST_LOG_OUT &param)
	{
		try{
		...
		}catch(tcstr sz){jWARN_T("DT_TEST_LOG_OUT : %s (data_size=%u " , sz,data_size); return false;}
		return true;
	}
	#endif //jNOT_USE_PACKET_READ_FUNC_DummyServer

	#ifndef jNOT_USE_PACKET_WRITE_FUNC_DummyServer
	inline jPacket_Base WRITE_DT_TEST_LOG_OUT(nNET::nStream::_jNetStreamWriteBufferBase &buffer, S_DT_TEST_LOG_OUT &param)
	{
		try{
			...
			jPacket_Base pk; pk.num = pk_DT_TEST_LOG_OUT;
			pk.buf=st.GetBuffer();	 pk.len=(jPacketNum_t)st.size();	return pk;
		}catch(tcstr sz){ jERROR(_T(" DT_TEST_LOG_OUT :data overflow: %s"),sz); }
		catch(nStream::jNetStreamWrite_error i){ jERROR(_T(" DT_TEST_LOG_OUT :overflow: (curr=%d,capa=%d,size=%d)"),i.currLen,i.capa , i.isize); }
		jPacket_Base pk;pk.buf=0;pk.len=0;pk.num=0;return pk;
	};
	
	inline jPacket_Base WRITE_DT_TEST_LOG_OUT(nNET::nStream::_jNetStreamWriteBufferBase &buffer , const int16 &_per_user, const uint32 &_delay,  tcstr _testClientName)
	{
		try{
			nMech::nNET::nStream::jNetStreamWrite st(buffer);
...
#endif //DummyServer___DT_TEST_LOG_OUT__