// TChannel.cpp : DLL 응용 프로그램을 위해 내보낸 함수를 정의합니다.
//
...
namespace nMech
{
	StringID g_Current_LogLevel_jStringID = jx(TChannel);
}
...
{
	size_t len = jw_strlen(szName);
	if( len<2 || len>jOBJECT_NAME) return false;
	return true;
}