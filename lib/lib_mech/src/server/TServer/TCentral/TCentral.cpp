// TCentral.cpp : DLL 응용 프로그램을 위해 내보낸 함수를 정의합니다.
//
...
namespace nMech
{
	StringID g_Current_LogLevel_jStringID = jx(TCentral);
}
...
void Central_OnPacketSize_UsageInfo(tcstr szPlugIn,uint64 packet_size,tname1024_t info);
...
extern "C"
{
	__declspec(dllexport) void* jCreateInterface(jIE* pE)
	{
		...
		jAssert0(nInterface::g_pjINet_OGS);
		nInterface::g_pjINet_OGS->RunMessageThread();
		nInterface::g_pjINet_OGS->SetCallback_PacketSize_UsageInfo(Central_OnPacketSize_UsageInfo);
