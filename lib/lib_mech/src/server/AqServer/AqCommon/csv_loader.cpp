...
AQ_SERVER_LIB_API void Load_Common_CSV(tfname_t szDir)
{
	...
	#define jLOAD_CSV(Name,Table) jt_sprintf(buf , _T("%s\\file_%s.csv") ,pv_CSV_PATH->Get_cstr(), jS(Table) ); g_##Table.Load(buf);
	...
	for(int i=1; i <= jCSV(Sys_CastlePos).size() ; ++i)
	{
		...
		jt_sprintf(buf , _T("%s\\file_Sys_TownPos_%s.csv") ,szDir, fn.fname);
		if(!nFileSystem::jFolder::Is_Exist(buf))
		{
			GetjILog()->Warning(jFUNC1 _T("NOT EXIST : %s"),buf);
			continue;
		}
...
AQ_SERVER_LIB_API nAQ::Sys_TownPos* jCSV_Sys_TownPos(Sys_Castle_id_t csid,Sys_TownPos_id_t tpsid)
{
