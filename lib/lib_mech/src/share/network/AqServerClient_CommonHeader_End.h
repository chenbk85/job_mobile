/* file : AqServerClient_CommonHeader_End.h
...
namespace nMech
{
	...
	inline void jDebugPrint(tcstr szName, vector<jHeroItem>& msg)	{		GetjILog()->Log(_T("%s.size()=%d") , szName,msg.size());	}
	inline void jDebugPrint(tcstr szName, jHeroSkill& msg){}
	inline void jDebugPrint(tcstr szName, vector<jHeroSkill>& msg)	{		GetjILog()->Log(_T("%s.size()=%d") , szName,msg.size());	}
