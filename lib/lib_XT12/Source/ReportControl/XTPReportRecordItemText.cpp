...
void CXTPReportRecordItemPreview::SetPreviewText(LPCTSTR strPreviewText)
{
	...
	{
		m_pMarkupUIElement = XTPMarkupParseText(m_pRecord->GetRecords()->GetMarkupContext(), m_strPreviewText);
	}
}
...
CString CXTPReportRecordItemVariant::GetCaption(CXTPReportColumn* pColumn)
{
	...
	CXTPReportRecordItemEditOptions* pEditOptions = m_pEditOptions ? m_pEditOptions :
			pColumn ? pColumn->GetEditOptions() : NULL;
	BOOL bConstraintEdit = pEditOptions ? pEditOptions->m_bConstraintEdit : FALSE;
