...
namespace nMech
{
	...
	namespace nMFC
	{
		typedef int __func_t__(CXTPPropertyGridItem*);

		enum _jGET_CUSTOM_COLOR
		{
			ejGCC_BG // 백그라운드 그릴때 호출됨
			,ejGCC_FG // 글자 그릴때 호출됨
		};
		typedef COLORREF _jGetCustomColor_t(CXTPPropertyGridItem* ,_jGET_CUSTOM_COLOR  type, COLORREF org);
		enum jMY_EVENT
		{
			ejXTP_LB,// 왼쪽 마우스버튼 킬릭
			ejXTP_VAL, // 값이 바낄때 
			ejXTP_COMBO, // 속성 컨트롤의 내부 INPLACE BUTTON 선택시
			ejXTP_RB, // 오른쪽 마우스버튼 클릭, 컨텍스트메뉴용.
			jMY_EVENT_MAX
		};
	}
}
using namespace nMech;
using namespace nMech::nMFC;
...
public:
	__func_t__* m_pFunc[jMY_EVENT_MAX];
	static __func_t__* ms_pGlobalFunc[jMY_EVENT_MAX];
	static _jGetCustomColor_t * m_jGetCustomColor;
private:
	friend class CXTPPropertyGridView;
public:
	static void jSetGlobalFunc(jMY_EVENT e, __func_t__* pf);
	void jSetCallback(jMY_EVENT idx, __func_t__* pf)	{m_pFunc[idx] = pf;	}

	nMech::nXML::jIElement* m_pE;
	nMech::nXML::jIAttribute*		m_pA;
...
AFX_INLINE CXTPMarkupUIElement* CXTPPropertyGridItem::GetMarkupUIElement() const {
