...
__func_t__* CXTPPropertyGridItem::ms_pGlobalFunc[jMY_EVENT_MAX];
_jGetCustomColor_t *CXTPPropertyGridItem::m_jGetCustomColor=0;
void CXTPPropertyGridItem::jSetGlobalFunc(jMY_EVENT e, __func_t__* pf)
{
	ms_pGlobalFunc[e] = pf;
}
namespace 
{
	struct _1234
	{
		_1234()
		{
			memset(CXTPPropertyGridItem::ms_pGlobalFunc, 0, jMY_EVENT_MAX* sizeof(__func_t__*));
		}
	};
	static _1234 __1234;
}
...
void CXTPPropertyGridItem::Init()
{
	...
	memset(m_pFunc , 0 , sizeof(m_pFunc));
}
...
void CXTPPropertyGridItem::OnValueChanged(CString strValue)
{
	if (!m_pGrid)
		return;
	if(ms_pGlobalFunc[ejXTP_VAL] )	
		if(ms_pGlobalFunc[ejXTP_VAL](this))
			return;

	if(m_pFunc[ejXTP_VAL] )	
		if(m_pFunc[ejXTP_VAL](this))
			return;
...
void CXTPPropertyGridItem::OnInplaceButtonDown(CXTPPropertyGridInplaceButton* pButton)
{
	...
	// *_* by icandoit : 2006-12-02 05:52:58
	//
	if(ms_pGlobalFunc[ejXTP_COMBO] )	
		if( ms_pGlobalFunc[ejXTP_COMBO](this) )
			return;

	if(m_pFunc[ejXTP_COMBO] )	
		if( m_pFunc[ejXTP_COMBO](this) )
			return;
...
BOOL CXTPPropertyGridItem::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if(ms_pGlobalFunc[ejXTP_LB] )	
		ms_pGlobalFunc[ejXTP_LB](this);

	if(m_pFunc[ejXTP_LB] )	
		m_pFunc[ejXTP_LB](this);
...
void CXTPPropertyGridItem::OnCaptionChanged()
{
	...
	{
		m_pMarkupUIElement = XTPMarkupParseText(m_pGrid->m_pGrid->GetMarkupContext(), GetCaption());
	}
...
void CXTPPropertyGridItems::RemoveAt(jIE* pE,jIA* pA)
{
	bool is=false;
	for(int nIndex = 0 ; nIndex < GetCount() ; ++nIndex)
	{
		CXTPPropertyGridItem* pItem  = GetAt(nIndex);
		is = pA ? (pItem->m_pA==pA) : (pItem->m_pE==pE);
		if(is)
		{
			m_arrItems.RemoveAt(nIndex);
			if (pItem->GetGrid())
