...
BOOL CXTPStatusBar::SetPaneText(int nIndex, const CString& lpszNewText)
{
	ASSERT_VALID(this);
	ASSERT(::IsWindow(m_hWnd));
...
void CXTPStatusBar::OnWindowPosChanged(WINDOWPOS* lpwndpos)
{
	...
	if (pPaintManager && pPaintManager->BaseTheme() == xtpThemeRibbon)
	{
		...
		{
			GetParentFrame()->SetWindowPos(0, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_FRAMECHANGED);
		}
	}
}
...
{
	pPane;
	GetParentFrame()->SendMessage(WM_COMMAND, Id);
}
...
void CXTPStatusBarSwitchPane::OnLButtonDown(CPoint point)
{
	...
	while (::GetCapture() == m_pStatusBar->GetSafeHwnd())
	{
		BOOL bPressed = rcItem.PtInRect(point);
