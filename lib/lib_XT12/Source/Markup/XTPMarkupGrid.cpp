...
CXTPMarkupObject* CXTPMarkupGridLength::ConvertFrom(CXTPMarkupObject* pObject) const
{
	if (IsStringObject(pObject))
	{
		...
		float fValue = 0;
		WSCANF_S(lpszValue, L"%f", &fValue);

		if (lpszValue[nLength - 1] == '*')
		{
			return new CXTPMarkupGridLength(nLength == 1 ? 1 : fValue, unitTypeStar);
		}

		return new CXTPMarkupGridLength(fValue);
	}
	return NULL;
}
...
IMPLEMENT_MARKUPCLASS(L"ColumnDefinition", CXTPMarkupColumnDefinition, CXTPMarkupObject);
...
void CXTPMarkupColumnDefinition::RegisterMarkupClass()
{
	m_pWidthProperty = CXTPMarkupDependencyProperty::Register(L"Width", MARKUP_TYPE(CXTPMarkupGridLength), MARKUP_TYPE(CXTPMarkupColumnDefinition),
		new CXTPMarkupPropertyMetadata(new CXTPMarkupGridLength(1, CXTPMarkupGridLength::unitTypeStar)));
...
IMPLEMENT_MARKUPCLASS(L"RowDefinition", CXTPMarkupRowDefinition, CXTPMarkupObject);
