...
void CXTPMarkupVisual::Render(CXTPMarkupDrawingContext* drawingContext)
{
	...
	POINT ptViewortOrg = drawingContext->OffsetViewport(m_ptVisualOffset);
	...
	drawingContext->SetViewport(ptViewortOrg);
}
...
void CXTPMarkupUIElement::Arrange(CRect rcFinalRect)
{
	...
	/*if (m_bMeasureDirty || m_bNeverMeasured)
	{
		CXTPMarkupDrawingContext dc;
		...
		{
			Measure(&dc, m_szPreviousAvailableSize);
		}
	}*/
