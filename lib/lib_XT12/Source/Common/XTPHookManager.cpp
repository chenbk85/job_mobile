...
void CXTPShadowManager::SetShadow(CWnd* pShadowOwner, const CRect& rcExclude)
{
	...
	CreateShadow(TRUE, rc, rcExclude, pShadowOwner, FALSE);
	CreateShadow(FALSE, rc, rcExclude, pShadowOwner, FALSE);

	HRGN hRgn = CreateRectRgn(0, 0, 0, 0);
	if (pShadowOwner->GetWindowRgn(hRgn) == COMPLEXREGION)
	{
		CreateShadow(2, rc, rcExclude, pShadowOwner, FALSE);
	}
	DeleteObject(hRgn);
...
void CXTPShadowManager::SetShadow(CRect rcWindow, CWnd* pShadowOwner)
{
	...
	{
		CreateShadow(TRUE, rcWindow, CXTPEmptyRect(), pShadowOwner, TRUE);
		CreateShadow(FALSE, rcWindow, CXTPEmptyRect(), pShadowOwner, TRUE);
	}
}
...
CXTPShadowManager::CShadowWnd* CXTPShadowManager::CreateShadow(BOOL bHoriz, CRect rc, CRect rcExclude, CWnd* pShadowOwner, BOOL bControlPopup)
{
	...
	while (pos)
	{
		...
		if (pShadowPrev->m_pShadowsOwner && (bHoriz != 2 || pShadowPrev->m_pShadowsOwner != pShadowOwner))
		{
