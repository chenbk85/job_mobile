...
void CXTPSyntaxEditAutoCompleteWnd::_AdjustWndRect(CRect& rrcWndRect)
{
	CRect rcWorkArea = XTPMultiMonitor()->GetWorkArea(this);

	m_bFixedBottom = rrcWndRect.bottom > rcWorkArea.bottom;

	if (m_bFixedBottom)
	{
		int nNewTop = rrcWndRect.top - m_nWndHeight -
					  m_pParentWnd->GetDrawTextProcessor().GetRowHeight();
		rrcWndRect.top = max(rcWorkArea.top, nNewTop);
		rrcWndRect.bottom = min(rcWorkArea.bottom, rrcWndRect.top + m_nWndHeight);

		m_bFixedBottom = TRUE;
	}

	if (rrcWndRect.right > rcWorkArea.right)
	{
		int nOffsetX = rrcWndRect.right - rcWorkArea.right;
		rrcWndRect.left = max(rcWorkArea.left, rrcWndRect.left - nOffsetX);
		rrcWndRect.right = rcWorkArea.right;
	}
